...
/// A success notice is double-encoded: the rich check glyph carries the level on
/// color terminals and degrades to an ASCII `v` under `NO_COLOR`, so the level
/// never rides on color alone.
#[test]
fn system_block_success_double_encodes_check_glyph() {
    let rich = render(
        &system_block(SystemLevel::Success),
        50,
        2,
        &Theme::default_dark(),
    );
    assert!(buffer_contains(&rich, "\u{2714}"));
    ...
    assert!(buffer_contains(&plain, "v"));
}
...
/// The rendered-markdown counterpart of the modal wrap fix: an assistant answer
/// in Korean must reach the screen with every syllable intact, at every width.
///
/// ratatui's `WordWrapper` — which the transcript used to wrap with — decides a
/// row is full before adding the grapheme it is holding, so a double-width glyph
/// on the boundary pushes the row one cell past the rect and the paint drops it.
/// The loss is width-dependent and lands mid-sentence, which is what "마크다운이
/// 깨진다" looked like. The transcript now wraps through `blocks::wrap_rows`.
#[test]
fn a_korean_answer_keeps_every_syllable_at_every_width() {
    let theme = Theme::default_dark();
    let body = "모달 본문과 트랜스크립트를 같은 랩 엔진으로 통일했습니다. \
                한글은 한 글자가 두 칸이라 경계에서 글리프가 통째로 사라지는 \
                결함이 있었고, 이제 폭에 상관없이 무손실입니다.";
    let block = text_block(body, true);
    let expected: Vec<char> = body.chars().filter(|c| !c.is_whitespace()).collect();

    for width in 20u16..=80 {
        let buffer = render(&block, width, 40, &theme);
        let painted: String = buffer_rows(&buffer).join("");
        for syllable in &expected {
            assert!(
                painted.contains(*syllable),
                "width {width} lost {syllable:?} out of the rendered answer"
            );
        }
    }
}

/// Same guarantee for the reasoning block, which wraps its own body column
/// beside the gutter rail rather than through the shared body painter.
#[test]
fn korean_reasoning_keeps_every_syllable_when_expanded() {
    let theme = Theme::default_dark();
    let body = "먼저 랩 엔진을 바꾸고, 측정 함수도 같은 엔진으로 옮겨서 \
                measure==paint 불변식을 유지해야 한다.";
    let block = reasoning_block(body, true);
    let expected: Vec<char> = body.chars().filter(|c| !c.is_whitespace()).collect();

    for width in 24u16..=72 {
        let buffer = render_with(&block, width, 40, &theme, false, true);
        let painted: String = buffer_rows(&buffer).join("");
        for syllable in &expected {
            assert!(
                painted.contains(*syllable),
                "width {width} lost {syllable:?} out of the expanded reasoning"
            );
        }
    }
}
