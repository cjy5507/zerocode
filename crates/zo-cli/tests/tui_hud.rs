...
fn hud_zero_tokens_show_pending_usage() {
    ...
    let out = render_buffer(80, &state, &theme);
    assert!(
        // An em-dash, not the word `pending`. This sits where a number goes and
        // the row already has a shape (label + empty gauge) saying "not measured
        // yet"; a word there read as a *status*, as though something were stuck
        // waiting, and it was the widest thing on an otherwise numeric row.
        out.contains('\u{2014}'),
        "fresh usage should read as an em-dash, not a broken-looking zero: {out}"
    );
    assert!(
