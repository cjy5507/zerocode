...
fn input_clears_stale_glyphs_when_text_shrinks() {
    ...
    // The first content row must show "hi" with no trailing ghost from the long
    // line. That is row 1: the composer has no top border, but it does keep one
    // blank row of air above the text.
...
fn input_no_color_theme_draws_without_panic() {
    ...
    terminal
        ...
        .unwrap();
    // The composer's only rule is its bottom edge — the last row of the region.
    let rule = &terminal.backend().buffer()[(0, 5)];
    assert_eq!(rule.symbol(), "─");
    assert_eq!(rule.fg, ratatui::style::Color::Reset);
}

#[test]
fn input_reasoning_modes_color_the_horizontal_rule() {
    use zo_cli::tui::modals::Effort;

    let input = InputWidget::new();
    let theme = Theme::zo();
    let modes = [
        ("Eco", Effort::Low, theme.palette.thinking_low),
        ("Normal", Effort::Medium, theme.palette.thinking_medium),
        ("Sport", Effort::High, theme.palette.thinking_max),
        ("Smart Sport", Effort::Smart, theme.palette.thinking_max),
    ];
    assert_ne!(modes[0].2, modes[1].2);
    assert_ne!(modes[1].2, modes[2].2);
    assert_ne!(modes[0].2, modes[2].2);

    for (label, effort, expected) in modes {
        ...
        terminal
            ...
            .unwrap();
        let buffer = terminal.backend().buffer();
        // One rule, not two: the composer's bottom edge. Its top border is gone
        // (the frame's `rule_top` row above it carries the same effort colour —
        // see `App::draw`), so row 0 here is content, not chrome. The last cell
        // closes the right rail rather than extending the horizontal stroke.
        let y = 3;
        for x in 0..23 {
            assert_eq!(buffer[(x, y)].symbol(), "─", "{label} rule x={x}, y={y}");
            assert_eq!(buffer[(x, y)].fg, expected, "{label} rule x={x}, y={y}");
        }
        assert_eq!(buffer[(23, y)].symbol(), "┘", "{label} closing corner");
        assert_eq!(buffer[(23, y)].fg, expected, "{label} closing corner");
    }
}

/// Heat now rides the prompt glyph alone.
///
/// The composer lost its left rail with the side borders, and with it the
/// bottom-up `rail_fade` gradient — a one-cell glyph cannot carry a gradient.
/// What survives is the distinction that matters: a running turn reads Hot
/// (ember) and a settled one reads Cold (bright). The composer's bottom rule and
/// the `rule_top` row above it stay reserved for reasoning effort, which is a
/// separate signal.
#[test]
fn input_heat_rides_the_prompt_glyph_not_a_rail() {
    let input = InputWidget::new();
    let theme = Theme::default_dark();
    let prompt_cell = |heat_state| {
        let mut terminal = Terminal::new(TestBackend::new(30, 4)).unwrap();
        terminal
            .draw(|frame| {
                let area = frame.area();
                input.draw_with_heat(frame, area, &theme, &AppMode::Normal, heat_state, None);
            })
            .unwrap();
        // Row 1: no top border, but one blank row of air above the text.
        terminal.backend().buffer()[(0, 1)].clone()
    };

    let hot = prompt_cell(HeatState::Hot);
    let cold = prompt_cell(HeatState::Cold);
    assert_eq!(hot.symbol(), "❯");
    assert_eq!(cold.symbol(), "❯");
    assert_eq!(hot.fg, theme.heat().ember);
    assert_eq!(cold.fg, theme.palette.bright);
    assert_ne!(hot.fg, cold.fg, "a running turn must read differently");
}
