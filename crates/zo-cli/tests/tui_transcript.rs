...
#[test]
fn transcript_user_message_shows_its_author_rail() {
    let theme = Theme::no_color();
    let ids = ids();
    let mut t = Transcript::new();
    ...
    // The author rail rides the prompt's own first row. It used to be a `|  You`
    // label row above the text; that row is gone, and the rail plus the block's
    // background band carry the attribution instead.
    assert!(
        rendered.contains("|  test message"),
        "the user rail should make the prompt boundary clear: {rendered}"
    );
    assert!(
        !rendered.contains("You"),
        "the `You` label row must not come back: {rendered}"
    );
    assert!(
        rendered.contains("test message"),
