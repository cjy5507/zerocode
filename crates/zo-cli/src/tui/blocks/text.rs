...
pub fn draw_with_mark(
    ...
    // Single source of truth (folds in the large-block demotion) so the direct
    // draw path can't disagree with the cache flag / `estimate_rows`.
    let preserves = done && preserves_layout_pub(text);
    match prose {
...
pub(crate) fn estimate_rows(text: &str, done: bool, theme: &Theme, width: u16) -> u16 {
    ...
    // `preserves_layout_pub` already excludes demoted large blocks (they are
    // drawn wrapped), so its no-wrap fast path can't undercount their height.
    if done && preserves_layout_pub(text) {
...
pub(crate) fn rendered_lines_for_width(
    ...
    let _ = tick;
    if done {
        // A very large done block (a giant paste, a huge pasted log) would run
        // the full markdown + syntect pass synchronously on the TUI loop —
        // syntect alone is ~1.5ms/KB, so a multi-hundred-KB block freezes the
        // UI for hundreds of ms. Above `PLAIN_RENDER_BYTE_LIMIT` skip markdown
        // entirely and emit plain per-source-line `Line`s; the shared wrap
        // infrastructure (`draw_paragraph`'s `Wrap`) still wraps them at width,
        // and splitting on `\n` keeps every UTF-8 / CJK / emoji boundary intact
        // (no byte slicing). Smaller blocks keep the full markdown path
        // unchanged so existing rendering (and goldens) is preserved.
        if demote_to_plain(text) {
            return plain_demoted_lines(text);
        }
...
/// A settled block larger than this renders as plain wrapped text instead of
/// running the full markdown (+ syntect) pass. syntect is ~1.5ms/KB, so a
/// multi-hundred-KB paste would freeze the TUI loop for hundreds of ms; above
/// this size the styling cost isn't worth a freeze. 256KB is well past a normal
/// answer or code block yet catches the giant-paste pathology. This is the
/// `done`-block analogue of `layout.rs`'s `FINAL_DONE_*` streaming-cache caps,
/// which never applied to `UserMessage`.
const PLAIN_RENDER_BYTE_LIMIT: usize = 256 * 1024;

/// A settled block too large for markdown/syntect (see [`PLAIN_RENDER_BYTE_LIMIT`]),
/// rendered as plain per-source-line [`Line`]s with a one-line notice on top.
/// Reuses [`plain_streaming_lines`] so the wrap path and char-boundary safety
/// are identical to the streaming fallback — no byte slicing, so CJK / emoji
/// stay intact.
fn plain_demoted_lines(text: &str) -> Vec<Line<'static>> {
    let mut lines = plain_streaming_lines(text);
    lines.insert(
        0,
        Line::from(Span::styled(
            "… (large text: markdown rendering skipped)",
...
///
/// A demoted large block (see [`demote_to_plain`]) is drawn *wrapped*, so it is
/// never layout-preserving — the no-wrap window logic in [`draw_cached`] would
/// otherwise mis-slice its lines. This is the single source of truth the cache
/// flag, [`estimate_rows`], and the draw paths all consult, so they can never
/// disagree about whether a block wraps.
pub(crate) fn preserves_layout_pub(text: &str) -> bool {
    !demote_to_plain(text) && markdown::preserves_layout(text)
}

/// Whether a *settled* block is large enough to skip markdown/syntect and
/// render as plain wrapped text (see [`PLAIN_RENDER_BYTE_LIMIT`]).
fn demote_to_plain(text: &str) -> bool {
    text.len() > PLAIN_RENDER_BYTE_LIMIT
}
...
fn draw_paragraph(
    ...
    let para = if preserves_layout {
        ...
        Paragraph::new(super::wrap_rows(&lines, area.width))
            .style(theme.typography.body)
            .scroll((scroll_offset, 0))
    };
...
mod tests {
    ...
    #[test]
    fn large_done_block_demotes_to_plain_and_skips_markdown() {
        // A settled block over `PLAIN_RENDER_BYTE_LIMIT` must render as plain
        // wrapped text: markdown markers stay raw (no parse), a CJK/emoji body
        // survives intact (no byte slicing / panic), and the demotion notice
        // leads. This is the giant-paste freeze guard.
        let theme = Theme::default_dark();
        let width = 80u16;
        let heading = "# 제목 헤더 🚀\n\n";
        // Repeat a markdown-ish CJK line until safely past the limit.
        let body = "**굵게** 그리고 `코드` 그리고 리스트:\n- 항목 하나\n- 항목 둘\n"
            .repeat(8_000);
        let text = format!("{heading}{body}");
        assert!(
            text.len() > super::PLAIN_RENDER_BYTE_LIMIT,
            "test fixture must exceed the demotion limit (len {})",
            text.len()
        );

        let lines = rendered_lines_for_width(&text, true, &theme, 0, width);
        let rendered = flatten(&lines);
        assert!(
            rendered.starts_with("… (large text: markdown rendering skipped)"),
            "demotion notice must lead the plain render"
        );
        // Markdown was NOT parsed: the raw `#`, `**`, and `` ` `` markers survive
        // verbatim instead of being consumed into styled spans.
        assert!(rendered.contains("# 제목 헤더 🚀"), "raw heading marker preserved");
        assert!(rendered.contains("**굵게**"), "raw bold markers preserved");
        assert!(rendered.contains("`코드`"), "raw code markers preserved");
        // One source line per body line (+1 for the notice); no byte-slicing panic.
        assert!(lines.len() > 1, "plain path produced per-source-line output");
        assert!(
            super::demote_to_plain(&text) && !super::preserves_layout_pub(&text),
            "a demoted block is never reported layout-preserving (always wrapped)"
        );
    }

    #[test]
    fn small_done_block_keeps_full_markdown_path() {
        // A normal-sized block stays on the markdown path: markers are consumed
        // (styled), not left raw, and no demotion notice is added — protecting
        // existing rendering and goldens.
        let theme = Theme::default_dark();
        let width = 80u16;
        let text = "# 제목\n\n**굵게** 텍스트와 `코드`.\n";
        assert!(text.len() <= super::PLAIN_RENDER_BYTE_LIMIT);

        let lines = rendered_lines_for_width(text, true, &theme, 0, width);
        let rendered = flatten(&lines);
        assert!(
            !rendered.contains("markdown rendering skipped"),
            "small blocks must not carry the demotion notice"
        );
        assert!(!rendered.contains("**"), "bold markers consumed by markdown parse");
        assert!(!super::demote_to_plain(text), "small block is not demoted");
    }
