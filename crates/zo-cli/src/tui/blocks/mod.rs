...
pub fn draw_user_message(
    ...
    // Route through `blocks::text` so a giant paste is demoted to plain wrapped
    // text (no markdown/syntect freeze) identically to the cached path — this
    // uncached fallback must not disagree with `text_block_height`'s cache flag.
    let body = if text.is_empty() {
        vec![Line::from("")]
    } else {
        text::rendered_lines_for_width(text, true, theme, 0, inner_width)
    };
    let preserves = text::preserves_layout_pub(text);
...
fn draw_user_message_lines(
    ...
    // No `You` label row: the `┃` rail plus the block's background band already
    // say whose turn this is, and a third marker cost a row on every user turn.
    // The body therefore starts at the block's own first row and the scroll
    // offset maps straight onto body rows with no header to subtract.
    let body_y = area.y;
    let body_h = area.height;
    let body_scroll = scroll_offset;
    ...
    let para = if preserves {
        ...
        Paragraph::new(wrap_rows(&body, body_width))
            .style(theme.typography.body)
            .scroll((body_scroll, 0))
    };
...
/// Wrap `lines` to `width` — the transcript's wrap engine.
///
/// Every painter feeds its rows through here and renders them with a `Paragraph`
/// that has **no** `Wrap`, so the rows on screen are exactly the rows this
/// returned. Measurement ([`wrapped_rows`], [`wrapped_row_prefix`]) counts the
/// same rows, which is what keeps the cached layout height and the draw in
/// lockstep — one wrap engine, one source of truth.
///
/// It is not ratatui's wrapper because that one loses text: `WordWrapper` decides
/// a row is full before adding the grapheme it is holding, so a double-width
/// glyph on the boundary makes the row one cell too wide and the paint drops it
/// (see [`crate::tui::text_metrics::wrap_line_to_cells`] for the reproduction —
/// a Hangul syllable disappearing out of the middle of a rendered sentence).
pub(crate) fn wrap_rows(lines: &[ratatui::text::Line<'_>], width: u16) -> Vec<Line<'static>> {
    let budget = usize::from(width.max(1));
    lines
        .iter()
        .flat_map(|line| crate::tui::text_metrics::wrap_line_to_cells(line, budget, false))
        .collect()
}

/// Rows these `lines` occupy once wrapped to `width`, counted with the same
/// engine that paints them ([`wrap_rows`]).
///
/// A hand-rolled `ceil(line_width / width)` counts naive cell-fills, but the
/// renderer breaks on word boundaries, so the two disagreed by a row on lines
/// that wrap mid-word — the cached layout then reserved the wrong height and a
/// block gained a phantom row until the next re-layout (the "extra line until
/// you scroll" glitch). Never reintroduce a separate height formula.
pub(crate) fn wrapped_rows(lines: &[ratatui::text::Line<'_>], width: u16) -> u16 {
    let budget = usize::from(width.max(1));
    let rows: usize = lines
        .iter()
        .map(|line| {
            crate::tui::text_metrics::wrap_line_to_cells(line, budget, false)
                .len()
                .max(1)
        })
        .sum();
...
/// [`wrapped_rows`] (same [`wrap_rows`] engine, line by line — wrapping never
/// carries state across hard line breaks, so the per-line sum and the whole-text
/// count agree; a test pins this).
///
...
pub(crate) fn line_wrapped_rows(line: &ratatui::text::Line<'_>, width: u16) -> u16 {
    let rows = crate::tui::text_metrics::wrap_line_to_cells(line, usize::from(width.max(1)), false)
        .len()
        .max(1);
    u16::try_from(rows).unwrap_or(u16::MAX)
}

/// Resolve the logical-line range covering viewport rows
/// `[scroll, scroll + max_rows)` of a wrapped block.
///
/// Unlike [`visible_line_window`], this works without retaining rendered
/// `Line`s. Reasoning uses it with a compact row-prefix + source-offset index,
/// so a large expanded thought does not duplicate its full text in the render
/// cache merely to support viewport clipping.
pub(crate) fn visible_line_range(
    line_count: usize,
    row_prefix: &[u32],
    scroll: u16,
    max_rows: u16,
) -> (std::ops::Range<usize>, u16, bool) {
    debug_assert_eq!(row_prefix.len(), line_count + 1, "prefix must cover lines");
    if line_count == 0 || row_prefix.len() != line_count + 1 {
        // Defensive: malformed metadata falls back to every logical line. The
        // caller remains correct, only slower.
        return (0..line_count, scroll, true);
    }
    let scroll = u32::from(scroll);
    let end_row = scroll.saturating_add(u32::from(max_rows));
    // First line whose wrapped row range extends past `scroll`.
...
pub(crate) fn visible_line_window<'a>(
    ...
    let (range, line_scroll, reaches_end) =
        visible_line_range(lines.len(), row_prefix, scroll, max_rows);
    (&lines[range], line_scroll, reaches_end)
}
...
mod tests {
    ...
    /// Count rows actually painted for `lines` at `width`, by rendering into a
    /// tall `TestBackend` and counting non-blank rows. This is the ground truth
    /// `wrapped_rows` must match.
    ///
    /// Rendered exactly the way the painters do it — pre-wrapped through
    /// `wrap_rows`, `Paragraph` with no `Wrap` — so the measurement is checked
    /// against the real draw path and not against a wrapper production no longer
    /// uses.
    fn rendered_row_count(lines: &[Line<'static>], width: u16) -> u16 {
        ...
        terminal
            .draw(|frame| {
                let para = Paragraph::new(wrap_rows(lines, width));
    ...
    /// `wrapped_rows` reports must equal the rows actually painted. The old
    /// `ceil(line_width / width)` measure diverged on mid-word wraps; measuring
    /// and painting with the one `wrap_rows` engine keeps them identical.
    #[test]
    ...
    fn visible_line_window_paints_identically_to_full_scroll() {
        ...
        let render = |to_draw: Vec<Line<'static>>, scroll: u16| -> Vec<String> {
            ...
            terminal
                .draw(|frame| {
                    let para =
                        Paragraph::new(wrap_rows(&to_draw, width)).scroll((scroll, 0));
