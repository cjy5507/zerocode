...
/// Shared shape of the two settled headers (`… finished`, `… still running`),
/// which differ only in the verb that follows the batch label.
fn settled_header(tree: &AgentTree, color: bool, verb: &str) -> String {
    let batch = agent_type_count_label(tree);
    match tree.batch_label.as_deref() {
        Some(label) if !label.is_empty() => {
            let label = batch_label_with_glyph(label, color);
            format!("{label} {batch} {verb} ({AGENT_TREE_DETAIL_HINT})")
        }
        _ => format!("{batch} {verb} ({AGENT_TREE_DETAIL_HINT})"),
    }
}

/// The header for a finished batch: `3 Explore agents finished (ctrl+g for details)`.
/// Only an all-terminal tree may use it — see [`detached_header`].
fn finished_header(tree: &AgentTree, color: bool) -> String {
    settled_header(tree, color, "finished")
}

/// The header for a batch whose call returned while its agents kept running:
/// `1 general-purpose agent still running (ctrl+g for details)`.
///
/// `AgentTree::finished` means the owning *tool call* returned, not that the
/// agents did. A `run_in_background` spawn returns the instant it detaches, so
/// treating that flag as completion printed a green `✓ … agents finished`
/// header directly above its own `1 unfinished` child line — the card claimed
/// both at once, and a user reading it had no way to tell the agent was alive.
fn detached_header(tree: &AgentTree, color: bool) -> String {
    settled_header(tree, color, "still running")
}

/// The inline two-line agent card — status marker + header, then the aggregate
/// child line. Shared by the finished, detached, and live batch headers so the
/// three differ only in the marker and wording they pass in.
fn agent_batch_card(
    marker: &str,
    marker_color: Color,
    header: String,
...
fn compact_agent_batch_line(tree: &AgentTree, theme: &Theme) -> Line<'static> {
    ...
    if active > 0 {
        // These rows are what the manifest last reported as live, so they read
        // as `running` whether or not the owning call has returned. Calling them
        // `unfinished` once the batch sealed contradicted the header on the very
        // next line for every backgrounded spawn; the header alone carries
...
pub fn draw(
    ...
    let para = Paragraph::new(super::wrap_rows(&lines, area.width))
...
fn rendered_lines_with_state(
    ...
    if let Some(tree) = agent_tree.filter(|tree| !tree.rows.is_empty()) {
        let live_marker = || {
            (
                running_marker(
                    tick,
        ...
        // Gated on the ROWS, never on `tree.finished`: that flag only says the
        // owning tool call returned. A backgrounded spawn returns immediately,
        // so keying the outcome header off it printed `✓ N agents finished`
        // over agents that were still working (and contradicted its own
        // `N unfinished` child line).
        if tree.terminal_count() == tree.rows.len() {
        ...
        // The call returned while its agents kept running: a `run_in_background`
        // spawn detaches on the spot, and a straggler can outlive the collection
        // window. Both stay on the live marker so the card reads as alive.
        if tree.finished {
            let (marker, color) = live_marker();
            return agent_batch_card(
                marker,
                color,
                detached_header(tree, !theme.no_color),
                tree,
                theme,
            );
        }
