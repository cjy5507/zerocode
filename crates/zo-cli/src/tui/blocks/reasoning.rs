...
#[must_use]
pub(crate) const fn gutter_width() -> u16 {
    GUTTER_WIDTH
}
...
pub fn draw(
    ...
    if !expanded {
        frame.render_widget(
            Paragraph::new(super::wrap_rows(
                &body_lines(text, done, theme, focused, expanded, tick, elapsed, seed),
                area.width,
            ))
    ...
    // The rail is viewport-local. Building `scroll_offset + height` rows made
    // deep scrolling progressively slower even though every skipped row was
    // discarded immediately by Paragraph.
    draw_rail(frame, gutter_area, theme);
    ...
    frame.render_widget(
        Paragraph::new(super::wrap_rows(&lines, body_area.width)).scroll((scroll_offset, 0)),
        body_area,
    );
}

fn draw_rail(frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
    ...
    for y in area.y..area.y.saturating_add(area.height) {
        ...
        cell.set_symbol(rail_glyph);
        cell.set_style(rail_style);
    }
}

fn expanded_header_line<'a>(
    done: bool,
    theme: &Theme,
    tick: u64,
    elapsed: Option<Duration>,
    ...
    let disclosure = if theme.no_color { "v " } else { "▾ " };
    let mut spans = Vec::with_capacity(if elapsed.is_some() || !done { 3 } else { 2 });
    spans.push(Span::styled(disclosure, header_style));
    spans.push(Span::styled("work steps", header_style));
    if let Some(duration) = elapsed {
        spans.push(Span::styled(
            format!(" · {}", format_reasoning_elapsed(duration)),
            meta_style,
        ));
    } else if !done {
        // The placeholder keeps the header width stable across animation ticks.
        ...
        spans.push(Span::styled(dots, header_style));
    }
    Line::from(spans)
}

fn expanded_source_line<'a>(body: &'a str, source_idx: usize, theme: &Theme) -> Line<'a> {
    let body_style = Style::new()
        .fg(theme.palette.dim)
        .add_modifier(Modifier::DIM)
        .add_modifier(Modifier::ITALIC);
    let mut spans = Vec::with_capacity(if theme.no_color && source_idx == 0 {
        2
    } else {
        1
    });
    if theme.no_color && source_idx == 0 {
...
/// Byte starts for every hard line produced by `text.split('\n')`.
///
/// The cache stores these offsets instead of a second copy of the reasoning
/// text. Newlines are ASCII byte boundaries, so every generated offset is also
/// a valid UTF-8 boundary for later zero-copy slices.
pub(crate) fn source_line_starts(text: &str) -> Vec<usize> {
...
fn source_line<'a>(text: &'a str, starts: &[usize], source_idx: usize) -> &'a str {
    let Some(&start) = starts.get(source_idx) else {
        return "";
    };
    let end = starts
        .get(source_idx.saturating_add(1))
        .copied()
        .map_or(text.len(), |next| next.saturating_sub(1));
    text.get(start..end).unwrap_or_default()
}

/// Wrapped-row prefix for an expanded reasoning block without materializing its
/// full styled body.
///
/// Most provider reasoning already arrives as short hard lines. Their display
/// width proves they occupy exactly one row, so cache fill can record that row
/// directly. Only a line that can actually wrap is converted to a styled
/// [`Line`] and measured by Ratatui's authoritative word wrapper. This keeps
/// first expansion and width changes linear in source bytes without paying for
/// one `Paragraph` allocation per hard line, while preserving exact layout for
/// long words, whitespace, Unicode, and the NO_COLOR prefix.
pub(crate) fn expanded_row_prefix(
    text: &str,
    source_starts: &[usize],
    done: bool,
    theme: &Theme,
    ...
    let mut prefix = Vec::with_capacity(source_starts.len().saturating_add(2));
    prefix.push(0);

    let header = expanded_header_line(done, theme, 0, elapsed);
    let mut rows = u32::from(super::line_wrapped_rows(&header, width));
    prefix.push(rows);

    for source_idx in 0..source_starts.len() {
        let body = source_line(text, source_starts, source_idx);
        let prefix_width = if theme.no_color && source_idx == 0 {
            UnicodeWidthStr::width(NO_COLOR_PREFIX)
        } else {
            0
        };
        let display_width = prefix_width.saturating_add(UnicodeWidthStr::width(body));
        let line_rows = if display_width <= usize::from(width) {
            1
        } else {
            super::line_wrapped_rows(&expanded_source_line(body, source_idx, theme), width)
        };
        rows = rows.saturating_add(u32::from(line_rows));
        prefix.push(rows);
    }
    prefix
}

/// Build only the logical lines intersecting a wrapped viewport.
///
/// `logical_range` indexes `[header, source line 0, source line 1, ...]`.
/// Every body span borrows the original reasoning `String`; the per-frame
/// allocation therefore scales with visible rows, not total reasoning length.
pub(crate) fn expanded_window_lines<'a>(
    text: &'a str,
    source_starts: &[usize],
    ...
    let mut lines = Vec::with_capacity(logical_range.len());
    for logical_idx in logical_range {
        if logical_idx == 0 {
            lines.push(expanded_header_line(done, theme, tick, elapsed));
            continue;
        }
        let source_idx = logical_idx.saturating_sub(1);
        lines.push(expanded_source_line(
            source_line(text, source_starts, source_idx),
            source_idx,
            theme,
        ));
    }
...
/// Draw an expanded reasoning block from compact layout metadata.
///
/// The row prefix finds the first visible hard line in O(log n); only that
/// viewport slice is converted to spans and wrapped. A malformed cache safely
/// falls back to the full renderer rather than leaving stale terminal cells.
#[allow(clippy::too_many_arguments)]
pub(crate) fn draw_windowed(
    frame: &mut Frame<'_>,
    area: Rect,
    text: &str,
    done: bool,
    ...
    let logical_count = source_starts.len().saturating_add(1);
    if source_starts.is_empty() || row_prefix.len() != logical_count.saturating_add(1) {
        draw(
            frame,
            area,
            text,
    ...
    let [gutter_area, body_area] =
        Layout::horizontal([Constraint::Length(GUTTER_WIDTH), Constraint::Min(0)]).areas(area);
    draw_rail(frame, gutter_area, theme);
    let (range, line_scroll, _) = super::visible_line_range(
        logical_count,
        row_prefix,
        scroll_offset,
        body_area.height,
    );
    let lines = expanded_window_lines(text, source_starts, range, done, theme, tick, elapsed);
    frame.render_widget(
        Paragraph::new(super::wrap_rows(&lines, body_area.width)).scroll((line_scroll, 0)),
        body_area,
    );
}
...
pub(crate) fn body_lines<'a>(
    ...
    let meta_style = Style::new()
        .fg(theme.palette.dim)
        .add_modifier(Modifier::DIM);
    let disclosure = if theme.no_color { "> " } else { "▸ " };
    let disclosure_style = if focused {
        Style::new()
            .fg(theme.palette.violet)
            .add_modifier(Modifier::BOLD)
    } else {
    ...
    // Collapsed by default (components.md §4). The disclosure marker advertises
    // that mouse click or keyboard activation reveals the complete reasoning.
    if !expanded {
        ...
        if !done {
            ...
            let mut spans: Vec<Span<'a>> =
                Vec::with_capacity(if elapsed.is_some() { 6 } else { 5 });
            spans.push(Span::styled(disclosure, disclosure_style));
            spans.push(Span::styled(spark, label_style));
            spans.push(Span::styled(" ", label_style));
            spans.push(Span::styled(verb, label_style));
            spans.push(Span::styled(dots, label_style));
        ...
        let summary_style = Style::new()
            ...
            .add_modifier(Modifier::ITALIC);
        let mut spans: Vec<Span<'a>> =
            Vec::with_capacity(if elapsed.is_some() { 3 } else { 2 });
        spans.push(Span::styled(disclosure, disclosure_style));
    ...
    // The expanded path borrows each source slice directly into its Span. The
    // same line constructors power the windowed renderer so cache measurement
    // and visible drawing cannot drift stylistically.
    let source_line_count = text.split('\n').count();
    let mut lines: Vec<Line<'a>> = Vec::with_capacity(source_line_count.saturating_add(1));
    lines.push(expanded_header_line(done, theme, tick, elapsed));
    lines.extend(
        text.split('\n')
            .enumerate()
            .map(|(idx, body)| expanded_source_line(body, idx, theme)),
    );
    lines
}
...
mod tests {
    ...
    fn collapsed_streaming_shows_animated_reasoning_line() {
        ...
        assert_eq!(
            text, "\u{25b8} \u{2726} Thinking.  ",
            "streaming reasoning advertises click-to-expand"
        );
    }
    ...
    fn streaming_zo_verb_is_stable_per_seed_and_varies_across_blocks() {
        ...
        // Every seed maps onto one of the SSOT verbs (drawn after `▸ ✦ `).
    ...
    fn collapsed_streaming_zo_carries_elapsed_when_timed() {
        ...
        let text = flat(&lines);
        assert!(
            text.starts_with("\u{25b8} \u{2726} Thinking") && text.contains("· 1.5s"),
            "streaming reasoning line carries disclosure and elapsed: {text:?}"
        );
        assert!(
    ...
    fn collapsed_done_without_timing_is_quiet() {
        let theme = Theme::default_dark();
        // done && elapsed == None → 시간은 없지만 disclosure 는 유지한다.
        let text = flat(&body_lines(
            "done thought",
            true,
            &theme,
        ...
        assert_eq!(
            text, "\u{25b8} done thought",
            "untimed settled reasoning keeps only disclosure and summary"
        );
    }

    #[test]
    fn expanded_reasoning_borrows_source_lines_and_shows_open_disclosure() {
        let theme = Theme::default_dark();
        let source = String::from("first detail\nsecond detail");
        let lines = body_lines(&source, true, &theme, false, true, 0, None, 0);
        let text = flat(&lines);

        assert!(
            text.starts_with("\u{25be} work steps"),
            "expanded reasoning flips to the open disclosure: {text:?}"
        );
        assert!(
            text.contains("first detail") && text.contains("second detail"),
            "expanded reasoning retains the complete body: {text:?}"
        );
        assert!(
            matches!(
                &lines[1].spans[0].content,
                std::borrow::Cow::Borrowed("first detail")
            ),
            "expanded source slices stay borrowed instead of cloning every line"
        );
    }

    #[test]
    fn expanded_row_prefix_matches_full_ratatui_wrap() {
        ...
        for theme in [&color, &mono] {
            let rendered = body_lines(&source, true, theme, false, true, 0, None, 0);
            for width in [1, 4, 9, 18, 40, 120] {
                let actual = expanded_row_prefix(&source, &starts, true, theme, width, None);
                let expected = super::super::wrapped_row_prefix(&rendered, width);
                assert_eq!(
                    actual, expected,
                    "compact prefix diverged at width {width}, no_color={}",
                    theme.no_color
                );
            }
        }
    ...
    #[test]
    fn deep_reasoning_window_builds_only_viewport_lines() {
        let theme = Theme::default_dark();
        let source = (0..10_000)
            ...
            .join("\n");
        let starts = source_line_starts(&source);
        let logical_count = starts.len().saturating_add(1); // expanded header + source
        let row_prefix = expanded_row_prefix(&source, &starts, true, &theme, 120, None);

        let (range, line_scroll, _) =
            super::super::visible_line_range(logical_count, &row_prefix, 9_900, 24);
        assert_eq!(range.len(), 24, "only viewport-sized metadata is selected");
        assert_eq!(line_scroll, 0);

        let lines = expanded_window_lines(&source, &starts, range, true, &theme, 0, None);
        assert_eq!(lines.len(), 24, "frame allocation must stay O(viewport)");
        assert_eq!(
            lines[0].spans[0].content.as_ref(),
            "reasoning row 09899",
            "deep row lookup must preserve logical ordering"
        );
        assert!(
            lines.iter().all(|line| line.spans.iter().all(|span| matches!(
                &span.content,
                std::borrow::Cow::Borrowed(_)
            ))),
            "viewport body spans must borrow the source instead of cloning it"
        );
    }

    #[test]
    fn source_offsets_preserve_unicode_and_trailing_empty_line() {
        let source = "첫 줄\nsecond\n";
        let starts = source_line_starts(source);
        assert_eq!(starts.len(), 3);
        assert_eq!(source_line(source, &starts, 0), "첫 줄");
        assert_eq!(source_line(source, &starts, 1), "second");
        assert_eq!(source_line(source, &starts, 2), "");
    }
}
