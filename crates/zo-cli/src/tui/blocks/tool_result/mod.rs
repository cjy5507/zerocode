...
pub fn draw(
    ...
    let para = Paragraph::new(super::wrap_rows(
        &rendered_lines_for_width(is_error, body, theme, focused, expanded, area.width),
        area.width,
    ))
...
pub(crate) fn rendered_lines<'a>(
    ...
    // Listing bodies (grep/glob results) are always explicitly openable, even
    // when short. Keeping them collapsed by default preserves scan density;
    // the chevron makes the retained entries discoverable by mouse or keyboard.
    let is_listing = matches!(body, ToolResultBody::Listing { entries, .. } if !entries.is_empty());
    ...
    // Show the full body when explicitly expanded, for short non-listing
    // toggleable results, or for a diff that fits inline. Listings stay compact
    // until their own disclosure row is activated.
    let show_body = expanded
        || (toggleable && !is_listing && !is_long && !diff_overflows_inline_cap)
...
mod tests {
    ...
    #[test]
    fn listing_result_discloses_retained_entries_on_expand() {
        let theme = Theme::default_dark();
        let body = ToolResultBody::Listing {
            entries: vec![
                "src/routes.rs:12:match route".to_string(),
                "src/query.rs:44:match query".to_string(),
            ],
            truncated: false,
        };

        let collapsed = rendered_lines(false, &body, &theme, false, false, 80);
        let collapsed_text = flatten(&collapsed);
        assert_eq!(collapsed.len(), 1, "listing stays compact before click");
        assert!(
            collapsed_text.contains(CHEVRON_COLLAPSED),
            "a short grep/listing still advertises click-to-expand: {collapsed_text:?}"
        );
        assert!(
            !collapsed_text.contains("src/query.rs:44:match query"),
            "the second retained match stays behind the disclosure: {collapsed_text:?}"
        );

        let expanded = rendered_lines(false, &body, &theme, false, true, 80);
        let expanded_text = flatten(&expanded);
        assert!(
            expanded_text.contains(CHEVRON_EXPANDED),
            "expanded listing flips its disclosure: {expanded_text:?}"
        );
        assert!(
            expanded_text.contains("src/routes.rs:12:match route")
                && expanded_text.contains("src/query.rs:44:match query"),
            "expanding reveals every retained grep match: {expanded_text:?}"
        );
    }
