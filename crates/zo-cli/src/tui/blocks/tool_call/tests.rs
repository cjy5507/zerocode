...
/// 백그라운드 스폰: 도구 호출은 즉시 반환(`finished: true`)하지만 에이전트는
/// 계속 돈다. `finished` 는 *호출* 이 돌아왔다는 뜻이므로 이 배치는 완료를
/// 주장할 수 없고, 한 카드 안에서 `finished` + `unfinished` 로 자기모순을
/// 일으켜서도 안 된다.
#[test]
fn sealed_batch_over_live_rows_reads_as_still_running_not_finished() {
    let tree = AgentTree {
        rows: vec![tree_row("blocker-repair", "running", 4, 0)],
        batch_label: None,
        finished: true,
    };
    let lines = rendered_lines_with_tree(
    ...
    let text = collect_text(&lines);
    assert!(
        text.contains("1 Explore agent still running (ctrl+g for details)"),
        "a returned call over a live agent must not claim completion: {text}"
    );
    assert!(
        !text.contains("finished"),
        "the finished header belongs to all-terminal trees only: {text}"
    );
    assert!(
        text.contains("1 running") && !text.contains("unfinished"),
        "child line must agree with the header, not contradict it: {text}"
    );
}

/// 모든 행이 종결되면 (배치 봉인 여부와 무관하게) 완료 헤더가 그대로 나온다 —
/// 위 수정이 정상 완료 표시를 잡아먹지 않았음을 고정한다.
#[test]
fn all_terminal_rows_still_render_the_finished_header() {
    let tree = AgentTree {
        rows: vec![tree_row("wiring", "completed", 7, 1_000)],
        batch_label: None,
        finished: true,
    };
    let lines = rendered_lines_with_tree(
    ...
    let text = collect_text(&lines);
    assert!(
        text.contains("1 Explore agent finished (ctrl+g for details)"),
        "completed agents keep the finished header: {text}"
    );
    assert!(!text.contains("still running"), "no live claim: {text}");
}
...
fn agent_tree_rows_clip_to_one_row_each_at_narrow_width() {
    ...
    term.draw(|f| {
        f.render_widget(
            Paragraph::new(crate::tui::blocks::wrap_rows(&lines, width)),
            Rect::new(0, 0, width, height),
        );
    })
    .expect("draw");
