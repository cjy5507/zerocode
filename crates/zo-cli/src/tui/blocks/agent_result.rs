...
pub(crate) fn draw(
    ...
    // Wrapped to the *block's* inner width, which is what ratatui would have
    // wrapped to and what `estimate_rows` measures — see `super::wrap_rows`.
    let block = card_block(view.focused, theme);
    let wrap_width = block.inner(area).width;
    let para = Paragraph::new(super::wrap_rows(&lines, wrap_width))
        .block(block)
