...
pub struct ActivityContext {
    /// Resolved model id already normalized for display (a catalog canonical,
    /// not a generic family or provider-prefixed raw string).
    ...
    /// The active plan step as `{index}/{total} {sentence}`, from the shared
    /// [`crate::tui::hud::now_step`] source. When present it becomes the waved
    /// primary phrase: the user's own words for the current step outrank a
    /// generic verb or a tool label, which demote into the dim detail slot.
...
/// Fit the NOW phrase into `limit` terminal cells, keeping the leading `n/m `
/// coordinate whole: it is the cheapest part of the line to read and the first
/// thing lost to a tail cut. Only the sentence after it is truncated.
fn truncate_now_phrase(now: &str, limit: usize) -> String {
    if display_width(now) <= limit {
        return now.to_string();
    }
    let Some((coordinate, sentence)) = now.split_once(' ') else {
        return truncate_status(now, limit);
    };
    let head_width = display_width(coordinate) + 1;
    if head_width >= limit {
        return truncate_status(now, limit);
    }
    format!(
        "{coordinate} {}",
        truncate_status(sentence, limit - head_width)
    )
}
...
fn activity_context_label(context: Option<&ActivityContext>, width: u16) -> Option<String> {
    ...
    // itself still has room. A NOW phrase already carries the step coordinate,
    // so the badge would restate it — skip it rather than say it twice.
    if width >= WORKFLOW_BADGE_MIN_COLS && context.now.is_none() {
...
pub fn draw_with_context(
    ...
    // generic verb. The verb / tool label demotes ahead of any existing detail so
    // nothing is lost. Pre-truncated here because `truncate_activity_parts` cuts
    // from the tail and would eat the `n/m ` coordinate at tight widths.
    let now_phrase = context
        .and_then(|context| context.now.as_deref())
        .map(|now| truncate_now_phrase(now, activity_limit));
    let demoted_detail = now_phrase.as_ref().map(|_| match action.detail {
        Some(detail) => format!("{} · {detail}", action.phase),
        None => action.phase.to_string(),
    });
    let (phase_source, detail_source) = match (now_phrase.as_deref(), demoted_detail.as_deref()) {
        (Some(now), detail) => (now, detail),
        (None, _) => (action.phase, action.detail),
    };
    let (action_phase, action_detail) =
        truncate_activity_parts(phase_source, detail_source, activity_limit);
...
mod tests {
    ...
    #[test]
    fn now_phrase_truncation_preserves_the_step_coordinate() {
        let full = truncate_now_phrase("2/5 Verifying the parser", 80);
        assert_eq!(full, "2/5 Verifying the parser", "no cut when it fits");

        let tight = truncate_now_phrase("2/5 아주 긴 한국어 단계 문장이 계속 이어진다", 14);
        assert!(tight.starts_with("2/5 "), "coordinate survives: {tight:?}");
        assert!(tight.ends_with('\u{2026}'), "sentence is visibly clipped: {tight:?}");
        assert!(display_width(&tight) <= 14, "cell budget holds: {tight:?}");
    }

    #[test]
    fn workflow_badge_is_suppressed_while_a_now_phrase_shows() {
        let context = ActivityContext {
            workflow: Some("phase 2/4 read-code running \u{2192} synthesize".to_string()),
            now: Some("2/4 Reading the code".to_string()),
            ..Default::default()
        };
        assert_eq!(
            activity_context_label(Some(&context), 132),
            None,
            "the NOW phrase already carries the step coordinate — no duplicate badge"
        );
    }
