...
fn synchronized_output_supported(
    ...
    if program.contains("ghostty") // Ghostty / cmux (libghostty) — native 2026
        ...
        // Zed embeds `alacritty_terminal` but identifies itself as
        // TERM_PROGRAM=zed + TERM=xterm-256color, so the `$TERM` fallback below
        // can never see the implementation it already uses.
        || program == "zed"
    {
        return true;
    }
...
mod tests {
    ...
    fn sync_output_allowlists_native_terminals() {
        ...
        // Exact environment exposed by Zed's embedded terminal. `$TERM` alone
        // looks generic, so TERM_PROGRAM must carry the capability decision.
        assert!(synchronized_output_supported(
            Some("zed"),
            Some("xterm-256color"),
            None
