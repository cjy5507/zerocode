...
#[test]
fn inline_code_preserves_source_geometry_without_a_chip() {
    let theme = dark();
    ...
    let text = first
        ...
        .map(|span| span.content.as_ref())
        .collect::<String>();
    assert_eq!(text, "xcy", "Markdown must not inject layout spaces");
    ...
    assert_eq!(code.style.bg, None, "inline code must not look selected");
}
...
#[test]
fn inline_code_preserves_source_adjacency_without_a_background_chip() {
    let theme = dark();
    let lines = rendered_lines_for_width("x`code`y · `1.26.3`로", &theme, 80);
    let flat = lines
        .iter()
        .flat_map(|line| line.spans.iter())
        .map(|span| span.content.as_ref())
        .collect::<String>();
    assert!(flat.contains("xcodey · 1.26.3로"), "source geometry: {flat:?}");
...
fn list_rows_at_width(md: &str, width: u16) -> (Vec<String>, u16) {
    ...
    let rows: Vec<String> = lines
        ...
        .collect();
    // Measure via the same `wrap_rows` engine used by `text_block_height`
    // so we can assert height==draw.
...
fn bullet_list_height_equals_draw_line_count() {
    ...
    // `wrapped_rows` (which uses `wrap_rows`) must match
...
/// Every code-card row must terminate in a visible right glyph at the same
/// display column, including the ultra-wide shape reported by Zed users. A
/// language label consumes opening-rule cells but must not move the corner.
#[test]
fn code_card_rows_close_at_the_same_width_with_and_without_a_language_label() {
    use crate::tui::text_metrics::display_width;
    use crate::tui::theme::Theme;

    let theme = Theme::default_dark();
    let row = vec![ratatui::text::Span::raw("let x = 1;".to_string())];

    let line_cells = |spans: &Vec<ratatui::text::Span<'static>>| -> usize {
        spans.iter().map(|s| display_width(s.content.as_ref())).sum()
    };

    for width in [40u16, 60, 76, 120, 320] {
        for lang in [None, Some("csharp"), Some("rust")] {
            let out = super::code_card_frame_lines(
                vec![row.clone()],
                lang,
                false,
                width,
            ...
            for framed_row in &out {
                assert_eq!(
                    line_cells(framed_row),
                    usize::from(width),
                    "width {width}, lang {lang:?}: every card row must end at the frame edge"
                );
            }
            assert_eq!(out[0].last().expect("top corner").content, "╮");
            assert_eq!(out[1].last().expect("right rail").content, "│");
            assert!(
                out.last()
                    .expect("bottom border")
                    .last()
                    .expect("bottom corner")
                    .content
                    .ends_with('╯'),
                "width {width}, lang {lang:?}: bottom border must close"
            );
        }
    }
}
