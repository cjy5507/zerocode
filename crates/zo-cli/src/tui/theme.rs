...
pub struct Palette {
    ...
    /// `semantic.info`.
    pub info: Color,
    /// Normal container/composer border.
    pub border: Color,
    /// Highlighted or focused border.
    pub border_accent: Color,
    /// Background behind a user message block.
    pub user_message_bg: Color,
    /// Background behind an in-flight tool call.
    pub tool_pending_bg: Color,
    /// Background behind a tool call that succeeded.
    pub tool_success_bg: Color,
    /// Background behind a tool call that failed.
    pub tool_error_bg: Color,
    /// Composer border by reasoning effort, lowest step.
...
/// Colors a theme may pin for a specific *use* rather than a semantic role.
///
/// The palette answers "what is this theme's accent"; these answer "what colour
/// is a markdown heading". Every field is optional and `None` keeps the derived
/// behaviour that existed before, so a theme opts in one token at a time
/// instead of every theme having to enumerate all of them.
///
/// This exists because a role palette cannot express a design that assigns a
/// heading its own hue: `heading_color` used to hand back `bright`, which meant
/// a theme could only recolour headings by recolouring everything bright.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub struct UsageTokens {
    /// Markdown heading hue. `None` → `bright` for h1–h3, `dim` below.
    pub heading: Option<Color>,
    /// Selected-row wash. `None` → a tint of `accent` over `code_bg`.
    pub selected_bg: Option<Color>,
}

impl UsageTokens {
    /// Requantize each pinned token into `tier`, matching [`Palette::to_tier`].
    #[must_use]
    fn to_tier(self, tier: ColorTier) -> Self {
        match tier {
            ColorTier::TrueColor => self,
            ColorTier::NoColor => Self::default(),
            ColorTier::Ansi256 | ColorTier::Ansi16 => Self {
                heading: self.heading.map(|color| quantize_color(color, tier)),
                selected_bg: self.selected_bg.map(|color| quantize_color(color, tier)),
            },
        }
    }
}
...
impl Palette {
    ...
    fn to_tier(self, tier: ColorTier) -> Self {
        match tier {
            ...
            ColorTier::Ansi256 | ColorTier::Ansi16 => Self {
                ...
                border: quantize_color(self.border, tier),
                border_accent: quantize_color(self.border_accent, tier),
                user_message_bg: quantize_color(self.user_message_bg, tier),
                tool_pending_bg: quantize_color(self.tool_pending_bg, tier),
                tool_success_bg: quantize_color(self.tool_success_bg, tier),
                tool_error_bg: quantize_color(self.tool_error_bg, tier),
                thinking_off: quantize_color(self.thinking_off, tier),
                thinking_minimal: quantize_color(self.thinking_minimal, tier),
                thinking_low: quantize_color(self.thinking_low, tier),
                thinking_medium: quantize_color(self.thinking_medium, tier),
                thinking_high: quantize_color(self.thinking_high, tier),
                thinking_xhigh: quantize_color(self.thinking_xhigh, tier),
                thinking_max: quantize_color(self.thinking_max, tier),
            },
        }
    }
    ...
    pub const fn neutral() -> Self {
        Self {
            ...
            thinking_off: Color::Reset,
            thinking_minimal: Color::Reset,
            thinking_low: Color::Reset,
            thinking_medium: Color::Reset,
            thinking_high: Color::Reset,
            thinking_xhigh: Color::Reset,
            thinking_max: Color::Reset,
        }
    }
}
...
pub struct Theme {
    ...
    /// into `tier`.
    heat: HeatTokens,
    /// Use-specific colors this theme pinned. See [`UsageTokens`].
    usage: UsageTokens,
}
...
/// Eight session-identity hues for the `●` session badge (bottom HUD + sidebar
/// SESSION row), keyed off a stable hash of the session id by
/// [`Theme::session_badge_color`].
///
/// Curated 24-bit hues from the Tomorrow-Night / Pi family so a badge sits
/// beside the rest of the chrome instead of shouting in raw ANSI — the badge
/// used to draw from the eight base ANSI colors, which meant a session could be
/// tinted the exact `Color::Red` a failure row uses. Like [`AGENT_COLORS`],
/// none of these may equal any built-in theme's status colors
/// (success/warn/error/teal); `session_badge_hues_avoid_every_builtin_status_color`
/// enforces that, and the brand `accent` is avoided too so a badge never reads
/// as focus.
pub(crate) const SESSION_BADGE_HUES: [Color; 8] = [
    Color::Rgb(0x81, 0xa2, 0xbe), // Tomorrow-Night blue
    Color::Rgb(0xb2, 0x94, 0xbb), // Tomorrow-Night purple
    Color::Rgb(0xde, 0x93, 0x5f), // Tomorrow-Night orange
    Color::Rgb(0xf0, 0xc6, 0x74), // Tomorrow-Night yellow
    Color::Rgb(0x5f, 0x87, 0xff), // Pi resting-border azure
    Color::Rgb(0x00, 0xd7, 0xff), // Pi focus cyan
    Color::Rgb(0xd1, 0x83, 0xe8), // orchid (Pi thinking ramp)
    Color::Rgb(0xa3, 0xbe, 0x8c), // sage
];

/// FNV-1a hash of `key`. Deterministic across runs (unlike `DefaultHasher`), so
/// an identity's hue is stable for the whole life of the id it is keyed on.
...
/// Bucket a session id into `0..SESSION_BADGE_HUES.len()`. Exposed so the HUD
/// can assert hash stability/spread without owning the palette.
#[must_use]
pub fn session_badge_index(session_id: &str) -> usize {
    usize::try_from(fnv1a(session_id) % SESSION_BADGE_HUES.len() as u64).unwrap_or(0)
}
...
impl Theme {
    ...
    fn load_canonical(path: &Path) -> Result<Self, TuiError> {
        ...
        let palette = Palette {
            ...
            // The tokens schema does not carry these newer roles, so they
            // inherit the `zo()` base palette — the same fallback every
            // omitted role above uses.
            border: base.border,
            border_accent: base.border_accent,
            user_message_bg: base.user_message_bg,
            tool_pending_bg: base.tool_pending_bg,
            tool_success_bg: base.tool_success_bg,
            tool_error_bg: base.tool_error_bg,
            thinking_off: base.thinking_off,
            thinking_minimal: base.thinking_minimal,
            thinking_low: base.thinking_low,
            thinking_medium: base.thinking_medium,
            thinking_high: base.thinking_high,
            thinking_xhigh: base.thinking_xhigh,
            thinking_max: base.thinking_max,
        };
        ...
        Ok(Self::from_parts_default_usage(
            "custom",
            palette,
            spacing,
            borders,
    ...
    pub fn default_dark() -> Self {
        let palette = Palette {
            ...
            border_accent: Color::Indexed(111),
            user_message_bg: Color::Indexed(236),
            tool_pending_bg: Color::Indexed(235),
            tool_success_bg: Color::Indexed(22),
            tool_error_bg: Color::Indexed(52),
            thinking_off: Color::Indexed(237),
            thinking_minimal: Color::Indexed(240),
            thinking_low: Color::Indexed(243),
            thinking_medium: Color::Indexed(103),
            thinking_high: Color::Indexed(111),
            thinking_xhigh: Color::Indexed(75),
            thinking_max: Color::Indexed(117),
        };
    ...
    pub fn default_light() -> Self {
        let palette = Palette {
            ...
            tool_pending_bg: Color::Indexed(253),
            tool_success_bg: Color::Indexed(194),
            tool_error_bg: Color::Indexed(224),
            thinking_off: Color::Indexed(252),
            thinking_minimal: Color::Indexed(248),
            thinking_low: Color::Indexed(245),
            thinking_medium: Color::Indexed(67),
            thinking_high: Color::Indexed(31),
            thinking_xhigh: Color::Indexed(94),
            thinking_max: Color::Indexed(130),
        };
    ...
    pub fn opencode() -> Self {
        let palette = Palette {
            ...
            border_accent: Color::Indexed(215),
            user_message_bg: Color::Indexed(235),
            tool_pending_bg: Color::Indexed(234),
            tool_success_bg: Color::Indexed(22),
            tool_error_bg: Color::Indexed(52),
            thinking_off: Color::Indexed(236),
            thinking_minimal: Color::Indexed(238),
            thinking_low: Color::Indexed(242),
            thinking_medium: Color::Indexed(179),
            thinking_high: Color::Indexed(215),
            thinking_xhigh: Color::Indexed(214),
            thinking_max: Color::Indexed(208),
        };
    ...
    pub fn zo() -> Self {
        Self::from_named_palette(
            ...
            Palette {
                ...
                // Muted-derived hairline border at rest.
                border: Color::Rgb(0x73, 0x73, 0x73),
                // Ember accent for focused borders.
                border_accent: Color::Rgb(0xF5, 0xA5, 0x24),
                // Code backdrop nudged a touch warmer for a user block.
                user_message_bg: Color::Rgb(0x24, 0x20, 0x17),
                // Neutral pending wash over the near-black code backdrop.
                tool_pending_bg: Color::Rgb(0x1F, 0x1F, 0x22),
                // Backdrop nudged toward success green.
                tool_success_bg: Color::Rgb(0x14, 0x20, 0x18),
                // Backdrop nudged toward error red.
                tool_error_bg: Color::Rgb(0x24, 0x16, 0x16),
                // Off stays a faint neutral hairline; active tiers below are
                // grouped into the composer's three driving-mode colors.
                thinking_off: Color::Rgb(0x3F, 0x3F, 0x46),
                thinking_minimal: Color::Rgb(0x5A, 0x54, 0x48),
                // Composer driving modes: Eco / Normal / Sport. Low, medium,
                // and max are the three renderer inputs; high/xhigh remain
                // compatibility aliases of Sport for palette consumers. These
                // dedicated tokens are not resolved through semantic overrides,
                // so custom success/info/warn colors cannot collapse the modes.
                thinking_low: Color::Rgb(0x86, 0xEF, 0xAC),
                thinking_medium: Color::Rgb(0x93, 0xC5, 0xFD),
                thinking_high: Color::Rgb(0xF5, 0xA5, 0x24),
                thinking_xhigh: Color::Rgb(0xF5, 0xA5, 0x24),
                thinking_max: Color::Rgb(0xF5, 0xA5, 0x24),
            },
        )
    }

    /// **Pi** — the "Pi" coding agent's published color system in 24-bit
    /// true color.
    ///
    /// A cool teal accent (`#8abeb7`) over a neutral charcoal chrome, with a
    /// blue resting border and a bright cyan focus border. The thinking ramp
    /// is Pi's own reasoning-effort gradient, stepping from a quiet grey
    /// through blue and violet to a hot magenta at maximum effort.
    #[must_use]
    pub fn pi() -> Self {
        Self::from_named_palette_with_usage(
            "pi",
            Palette {
                // Pi's cool teal brand accent.
                accent: Color::Rgb(0x8a, 0xbe, 0xb7),
                // Desaturated teal for inactive focus.
                accent_dim: Color::Rgb(0x5f, 0x82, 0x7d),
                // Info/link blue, sibling of the resting border.
                cyan: Color::Rgb(0x81, 0xa2, 0xbe),
                // Reasoning violet, borrowed from the thinking ramp.
                violet: Color::Rgb(0xb2, 0x94, 0xbb),
                // Branch/tool teal, a step off the accent.
                teal: Color::Rgb(0x70, 0xa8, 0xa1),
                // Neutral body text.
                fg: Color::Rgb(0xd4, 0xd4, 0xd4),
                // Brightest emphasis.
                bright: Color::Rgb(0xf5, 0xf5, 0xf5),
                ...
                // Muted dividers / collapsed previews.
                muted: Color::Rgb(0x80, 0x80, 0x80),
                // Hairline rules.
                faint: Color::Rgb(0x3a, 0x3a, 0x40),
                // Near-black code backdrop.
                code_bg: Color::Rgb(0x1e, 0x1e, 0x24),
                // Pi status green.
                ...
                // Info stays with the resting-border blue family.
                info: Color::Rgb(0x81, 0xa2, 0xbe),
                // Pi's resting border blue.
                border: Color::Rgb(0x5f, 0x87, 0xff),
                // Pi's focus border cyan.
                border_accent: Color::Rgb(0x00, 0xd7, 0xff),
                // Pi's user-message backdrop.
                user_message_bg: Color::Rgb(0x34, 0x35, 0x41),
                // Pi's in-flight tool backdrop.
                tool_pending_bg: Color::Rgb(0x28, 0x28, 0x32),
                // Pi's succeeded-tool backdrop.
                tool_success_bg: Color::Rgb(0x28, 0x32, 0x28),
                // Pi's failed-tool backdrop.
                tool_error_bg: Color::Rgb(0x3c, 0x28, 0x28),
                // Pi's reasoning-effort ramp.
                thinking_off: Color::Rgb(0x50, 0x50, 0x50),
                thinking_minimal: Color::Rgb(0x6e, 0x6e, 0x6e),
                thinking_low: Color::Rgb(0x5f, 0x87, 0xaf),
                thinking_medium: Color::Rgb(0x81, 0xa2, 0xbe),
                thinking_high: Color::Rgb(0xb2, 0x94, 0xbb),
                thinking_xhigh: Color::Rgb(0xd1, 0x83, 0xe8),
                thinking_max: Color::Rgb(0xff, 0x5f, 0xff),
            },
            UsageTokens {
                // Pi's `mdHeading` — a warm gold that belongs to headings alone.
                // Without a usage token this rendered as `bright` white, losing
                // the most recognisable colour in Pi's markdown.
                heading: Some(Color::Rgb(0xf0, 0xc6, 0x74)),
                // Pi's `selectedBg` is a fixed slate, not a tint of the accent,
                // so the derived blend could never reproduce it.
                selected_bg: Some(Color::Rgb(0x3a, 0x3a, 0x4a)),
            },
        )
    }

    fn from_named_palette(name: &str, palette: Palette) -> Self {
        Self::from_named_palette_with_usage(name, palette, UsageTokens::default())
    }

    /// [`Self::from_named_palette`] for a theme that pins [`UsageTokens`].
    fn from_named_palette_with_usage(
        name: &str,
        palette: Palette,
        usage: UsageTokens,
    ) -> Self {
    ...
    /// [`Self::from_parts`] for a theme that pins no usage tokens.
    fn from_parts_default_usage(
        name: &str,
        palette: Palette,
        spacing: Spacing,
        borders: BorderMap,
    ...
    #[allow(clippy::too_many_arguments)] // mirrors `with_tier`; see its note
    fn from_parts(
        name: &str,
        palette: Palette,
        spacing: Spacing,
    ...
    #[allow(clippy::too_many_arguments)] // one call per theme axis; grouping them would only move the list
    fn with_tier(
        name: &str,
        palette: Palette,
        spacing: Spacing,
    ...
    pub fn tokyonight() -> Self {
        Self::from_named_palette(
            ...
            Palette {
                ...
                tool_success_bg: Color::Indexed(22),
                tool_error_bg: Color::Indexed(52),
                thinking_off: Color::Indexed(238),
                thinking_minimal: Color::Indexed(59),
                thinking_low: Color::Indexed(103),
                thinking_medium: Color::Indexed(68),
                thinking_high: Color::Indexed(111),
                thinking_xhigh: Color::Indexed(117),
                thinking_max: Color::Indexed(147),
            },
        )
    }
    ...
    pub fn catppuccin() -> Self {
        Self::from_named_palette(
            ...
            Palette {
                ...
                border_accent: Color::Indexed(183),
                user_message_bg: Color::Indexed(237),
                tool_pending_bg: Color::Indexed(235),
                tool_success_bg: Color::Indexed(22),
                tool_error_bg: Color::Indexed(52),
                thinking_off: Color::Indexed(238),
                thinking_minimal: Color::Indexed(243),
                thinking_low: Color::Indexed(103),
                thinking_medium: Color::Indexed(146),
                thinking_high: Color::Indexed(147),
                thinking_xhigh: Color::Indexed(183),
                thinking_max: Color::Indexed(219),
            },
        )
    }
    ...
    pub fn gruvbox() -> Self {
        Self::from_named_palette(
            ...
            Palette {
                ...
                border_accent: Color::Indexed(214),
                user_message_bg: Color::Indexed(237),
                tool_pending_bg: Color::Indexed(236),
                tool_success_bg: Color::Indexed(22),
                tool_error_bg: Color::Indexed(52),
                thinking_off: Color::Indexed(239),
                thinking_minimal: Color::Indexed(245),
                thinking_low: Color::Indexed(101),
                thinking_medium: Color::Indexed(172),
                thinking_high: Color::Indexed(179),
                thinking_xhigh: Color::Indexed(214),
                thinking_max: Color::Indexed(208),
            },
        )
    ...
    pub fn nord() -> Self {
        Self::from_named_palette(
            ...
            Palette {
                ...
                border_accent: Color::Indexed(110),
                user_message_bg: Color::Indexed(237),
                tool_pending_bg: Color::Indexed(235),
                tool_success_bg: Color::Indexed(22),
                tool_error_bg: Color::Indexed(52),
                thinking_off: Color::Indexed(238),
                thinking_minimal: Color::Indexed(60),
                thinking_low: Color::Indexed(66),
                thinking_medium: Color::Indexed(67),
                thinking_high: Color::Indexed(110),
                thinking_xhigh: Color::Indexed(111),
                thinking_max: Color::Indexed(153),
            },
        )
    }
    ...
    pub fn everforest() -> Self {
        Self::from_named_palette(
            ...
            Palette {
                ...
                info: Color::Indexed(109),
                border: Color::Indexed(245),
                border_accent: Color::Indexed(108),
                user_message_bg: Color::Indexed(237),
                tool_pending_bg: Color::Indexed(235),
                tool_success_bg: Color::Indexed(22),
                tool_error_bg: Color::Indexed(52),
                ...
                thinking_minimal: Color::Indexed(245),
                thinking_low: Color::Indexed(101),
                thinking_medium: Color::Indexed(107),
                thinking_high: Color::Indexed(108),
                thinking_xhigh: Color::Indexed(151),
                thinking_max: Color::Indexed(157),
            },
        )
    }
    ...
    pub fn kanagawa() -> Self {
        Self::from_named_palette(
            ...
            Palette {
                ...
                border_accent: Color::Indexed(173),
                user_message_bg: Color::Indexed(237),
                tool_pending_bg: Color::Indexed(235),
                tool_success_bg: Color::Indexed(22),
                tool_error_bg: Color::Indexed(52),
                thinking_off: Color::Indexed(238),
                thinking_minimal: Color::Indexed(102),
                thinking_low: Color::Indexed(137),
                thinking_medium: Color::Indexed(173),
                thinking_high: Color::Indexed(179),
                thinking_xhigh: Color::Indexed(215),
                thinking_max: Color::Indexed(216),
            },
        )
    }
    ...
    pub fn dracula() -> Self {
        Self::from_named_palette(
            ...
            Palette {
                ...
                info: Color::Indexed(117),
                border: Color::Indexed(60),
                border_accent: Color::Indexed(141),
                user_message_bg: Color::Indexed(237),
                tool_pending_bg: Color::Indexed(235),
                tool_success_bg: Color::Indexed(22),
                tool_error_bg: Color::Indexed(52),
                ...
                thinking_minimal: Color::Indexed(60),
                thinking_low: Color::Indexed(103),
                thinking_medium: Color::Indexed(98),
                thinking_high: Color::Indexed(141),
                thinking_xhigh: Color::Indexed(177),
                thinking_max: Color::Indexed(213),
            },
        )
    }
    ...
    pub fn darcula() -> Self {
        Self::from_named_palette(
            ...
            Palette {
                ...
                // Panel divider grey — the resting border.
                border: Color::Rgb(0x60, 0x63, 0x66),
                // Keyword orange for a focused border.
                border_accent: Color::Rgb(0xCC, 0x78, 0x32),
                // Warm charcoal nudged for a user block.
                user_message_bg: Color::Rgb(0x33, 0x30, 0x2B),
                // Neutral pending wash over the editor background.
                tool_pending_bg: Color::Rgb(0x2E, 0x2E, 0x30),
                // Editor background nudged toward the olive success green.
                tool_success_bg: Color::Rgb(0x2A, 0x30, 0x24),
                // Editor background nudged toward the dusty error red.
                tool_error_bg: Color::Rgb(0x34, 0x28, 0x28),
                // Thinking ramp: hairline grey stepping up to the keyword orange.
                thinking_off: Color::Rgb(0x3C, 0x3F, 0x41),
                thinking_minimal: Color::Rgb(0x55, 0x54, 0x47),
                thinking_low: Color::Rgb(0x71, 0x67, 0x40),
                thinking_medium: Color::Rgb(0x93, 0x6D, 0x39),
                thinking_high: Color::Rgb(0xB4, 0x72, 0x35),
                thinking_xhigh: Color::Rgb(0xC0, 0x75, 0x33),
                thinking_max: Color::Rgb(0xCC, 0x78, 0x32),
            },
        )
    }
    ...
    pub fn one_dark() -> Self {
        Self::from_named_palette(
            ...
            Palette {
                ...
                border: Color::Indexed(59),
                border_accent: Color::Indexed(75),
                user_message_bg: Color::Indexed(237),
                tool_pending_bg: Color::Indexed(235),
                tool_success_bg: Color::Indexed(22),
                tool_error_bg: Color::Indexed(52),
                ...
                thinking_low: Color::Indexed(66),
                thinking_medium: Color::Indexed(67),
                thinking_high: Color::Indexed(69),
                thinking_xhigh: Color::Indexed(75),
                thinking_max: Color::Indexed(81),
            },
        )
    }
    ...
    pub fn matrix() -> Self {
        Self::from_named_palette(
            ...
            Palette {
                ...
                border_accent: Color::Indexed(46),
                user_message_bg: Color::Indexed(233),
                tool_pending_bg: Color::Indexed(232),
                tool_success_bg: Color::Indexed(22),
                tool_error_bg: Color::Indexed(52),
                thinking_off: Color::Indexed(236),
                thinking_minimal: Color::Indexed(22),
                thinking_low: Color::Indexed(28),
                thinking_medium: Color::Indexed(34),
                thinking_high: Color::Indexed(40),
                thinking_xhigh: Color::Indexed(46),
                thinking_max: Color::Indexed(82),
            },
        )
    }
    ...
    pub fn agent_color(&self, agent_id: &str) -> Color {
        ...
        let bucket = usize::try_from(fnv1a(agent_id) % AGENT_COLORS.len() as u64).unwrap_or(0);
    ...
    /// An identity hue toned down for a row that *already* carries the selection
    /// wash: blended 45% toward the body `fg`.
    ///
    /// [`Self::agent_color`] spreads names across eight saturated hues, two of
    /// which are a hot pink and a magenta. On an unselected row that is exactly
    /// the point — the hue *is* the agent's identity. On the selected row the
    /// slate wash is already saying "this one", so the raw hue on top of it
    /// reads as noise rather than as identity. Pulling it toward `fg` keeps the
    /// name recognisably *its* colour while letting the wash do the selecting.
    /// Identity on a palette that cannot blend (`NO_COLOR` / named colors).
    #[must_use]
    pub fn calm_identity(&self, hue: Color) -> Color {
        blend_toward(hue, self.palette.fg, 0.45).unwrap_or(hue)
    }

    /// Identity color for the `●` session badge, keyed off a stable FNV-1a hash
    /// of the session id (same idiom as [`Self::agent_color`]) over the curated
    /// [`SESSION_BADGE_HUES`]. Requantized into the applied color tier so a
    /// 256-color or 16-color terminal receives an in-tier value; `NO_COLOR`
    /// drops to `Reset` and the badge survives on its glyph alone.
    ///
    /// This replaces the HUD's hardcoded eight-entry ANSI palette: the badge is
    /// theme chrome, so its hues belong to the theme, not to the widget.
    #[must_use]
    pub fn session_badge_color(&self, session_id: &str) -> Color {
        if self.no_color {
            return Color::Reset;
        }
        quantize_color(SESSION_BADGE_HUES[session_badge_index(session_id)], self.tier)
    }
    ...
    pub fn selection_bg(&self) -> Option<Color> {
        // A theme that pinned this wins outright: a fixed slate cannot be
        // reproduced by tinting the accent, however the blend is tuned.
        if self.no_color {
            return None;
        }
        self.usage
            .selected_bg
            .or_else(|| tint(self.palette.accent, self.palette.code_bg))
    }
    ...
    /// the sidebar, and the HUD strip. A 4% fg blend over the surface
    ...
    pub fn surface1(&self) -> Option<Color> {
        let res = tint_strength(self.palette.fg, self.palette.code_bg, 0.04)?;
        // Prevent collapsing to the exact same index as code_bg on 256-color terminals.
        if let (Color::Indexed(s1_idx), Color::Indexed(bg_idx)) = (res, self.palette.code_bg) {
            if s1_idx == bg_idx && (232..255).contains(&s1_idx) {
                if self.has_light_background() {
                    if s1_idx > 232 {
                        return Some(Color::Indexed(s1_idx - 1));
                    }
                } else if s1_idx < 254 {
                    return Some(Color::Indexed(s1_idx + 1));
                }
    ...
    pub fn surface2(&self) -> Option<Color> {
        let res = tint_strength(self.palette.fg, self.palette.code_bg, 0.11)?;
        // Ensure elevation-2 is strictly ordered relative to surface1 on 256-color terminals.
        if let (Some(Color::Indexed(s1_idx)), Color::Indexed(s2_idx)) = (self.surface1(), res) {
            if (232..255).contains(&s2_idx) {
                if self.has_light_background() {
                    if s2_idx >= s1_idx && s1_idx > 232 {
                        return Some(Color::Indexed(s1_idx - 1));
                    }
                } else if s2_idx <= s1_idx && s1_idx < 254 {
                    return Some(Color::Indexed(s1_idx + 1));
                }
            }
        }
    ...
    pub fn heading_color(&self, level: u8) -> Color {
        match level {
            // A pinned heading hue applies to the levels that read as titles;
            // deeper levels stay quiet so a document still has a hierarchy.
            1..=3 => self.usage.heading.unwrap_or(self.palette.bright),
            _ => self.palette.dim,
        }
    }
...
mod tests {
    ...
    // ── Pi theme + new palette-role coverage ──

    #[test]
    fn pi_theme_pins_published_accent_border_and_thinking_max() {
        let palette = Theme::pi().palette;
        assert_eq!(palette.accent, Color::Rgb(0x8a, 0xbe, 0xb7), "Pi accent #8abeb7");
        assert_eq!(palette.border, Color::Rgb(0x5f, 0x87, 0xff), "Pi border #5f87ff");
        assert_eq!(
            palette.border_accent,
            Color::Rgb(0x00, 0xd7, 0xff),
            "Pi focus border #00d7ff"
        );
        assert_eq!(
            palette.thinking_max,
            Color::Rgb(0xff, 0x5f, 0xff),
            "Pi thinking_max #ff5fff"
        );
    }

    #[test]
    fn neutral_palette_resets_every_new_role() {
        let palette = super::Palette::neutral();
        for (name, color) in [
            ("border", palette.border),
            ("border_accent", palette.border_accent),
            ("user_message_bg", palette.user_message_bg),
            ("tool_pending_bg", palette.tool_pending_bg),
            ("tool_success_bg", palette.tool_success_bg),
            ("tool_error_bg", palette.tool_error_bg),
            ("thinking_off", palette.thinking_off),
            ("thinking_minimal", palette.thinking_minimal),
            ("thinking_low", palette.thinking_low),
            ("thinking_medium", palette.thinking_medium),
            ("thinking_high", palette.thinking_high),
            ("thinking_xhigh", palette.thinking_xhigh),
            ("thinking_max", palette.thinking_max),
        ] {
            assert_eq!(color, Color::Reset, "neutral() must reset `{name}`");
        }
    }

    #[test]
    fn no_color_tier_resets_every_new_role() {
        let palette = Theme::pi().palette.to_tier(ColorTier::NoColor);
        for (name, color) in [
            ("border", palette.border),
            ("border_accent", palette.border_accent),
            ("user_message_bg", palette.user_message_bg),
            ...
            assert_eq!(color, Color::Reset, "to_tier(NoColor) must reset `{name}`");
        }
    }
    ...
    #[test]
    fn session_badge_hues_avoid_every_builtin_status_color() {
        let unique: std::collections::HashSet<_> = super::SESSION_BADGE_HUES.iter().collect();
        assert_eq!(unique.len(), 8, "the eight session badge hues must be distinct");
        // Same invariant the agent hues carry: a session tint must never be
        // byte-identical to a status color, or "session deploy-watch" reads as
        // "deploy failed". The brand accent is excluded too — a badge must not
        // masquerade as focus.
        for name in Theme::builtin_names() {
            let t = Theme::builtin(name).expect("builtin theme resolves");
            for reserved in [
                ...
                assert!(
                    !super::SESSION_BADGE_HUES.contains(&reserved),
                    "session hue {reserved:?} collides with a reserved color in theme {name}"
                );
            }
        }
    }
    ...
    #[test]
    fn session_badge_color_is_stable_spreads_and_degrades() {
        let t = Theme::default_dark();
        assert_eq!(t.session_badge_color("session-9"), t.session_badge_color("session-9"));
        // Every bucket is reachable, so the eight hues are all in play.
        let buckets: std::collections::BTreeSet<_> = (0..64)
            .map(|n| super::session_badge_index(&format!("session-{n}")))
            .collect();
        assert_eq!(buckets.len(), super::SESSION_BADGE_HUES.len());
        // NO_COLOR keeps the badge glyph but drops the hue.
        assert_eq!(Theme::no_color().session_badge_color("session-9"), Color::Reset);
    }
    ...
    /// Pi pins colours a role palette cannot express, and pinning them must not
    /// disturb any other theme.
    ///
    /// Regression: `heading_color` handed back `bright`, so Pi's `mdHeading`
    /// gold (`#f0c674`) rendered as white — the most recognisable colour in
    /// Pi's markdown was simply absent. `selection_bg` had the same problem
    /// from the other direction: it *computed* a tint of the accent, which can
    /// never land on Pi's fixed `#3a3a4a` slate.
    #[test]
    fn pinned_usage_tokens_reach_the_render_without_touching_other_themes() {
        let pi = Theme::pi();
        assert_eq!(pi.heading_color(1), Color::Rgb(0xf0, 0xc6, 0x74));
        assert_eq!(pi.heading_color(2), Color::Rgb(0xf0, 0xc6, 0x74));
        assert_eq!(pi.selection_bg(), Some(Color::Rgb(0x3a, 0x3a, 0x4a)));

        // Deep levels stay quiet so a document keeps a visible hierarchy.
        assert_eq!(pi.heading_color(4), pi.palette.dim);

        // A theme that pins nothing keeps the derived behaviour verbatim.
        let dark = Theme::default_dark();
        assert_eq!(dark.heading_color(1), dark.palette.bright);
        assert!(dark.selection_bg().is_some());
        assert_ne!(dark.selection_bg(), pi.selection_bg());

        // NO_COLOR drops the wash entirely rather than quantizing to a grey.
        assert_eq!(Theme::no_color().selection_bg(), None);
    }

    /// Pinned tokens follow the palette down into a lower colour tier instead
    /// of leaking a true-colour value onto a 256-colour terminal.
    #[test]
    fn pinned_usage_tokens_are_requantized_with_the_palette() {
        let pi = Theme::pi().apply_color_tier(ColorTier::Ansi256);
        assert!(
            matches!(pi.heading_color(1), Color::Indexed(_)),
            "heading must quantize with everything else: {:?}",
            pi.heading_color(1)
        );
        assert!(
            matches!(pi.selection_bg(), Some(Color::Indexed(_))),
            "selection wash must quantize too: {:?}",
            pi.selection_bg()
        );
    }
}
