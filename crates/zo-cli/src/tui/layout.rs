...
/// Rows the live plan dock occupies at full extent: its bounded item window
/// plus the title and frontier rows that frame it (`render::TODO_PANEL_MAX_ROWS`
/// plus two). Kept here because the layout must reason about the dock's
/// footprint before `render` gets a chance to lay it out.
pub const PLAN_DOCK_FULL_ROWS: u16 = 7;

/// Transcript rows that must remain before the footer may spend height on its
/// live-activity row.
///
/// Deliberately higher than [`MIN_READABLE_TRANSCRIPT_ROWS`]: that floor keeps
/// the transcript *legible*, while this one keeps it *useful* — the plan dock
/// and its active frontier live inside the transcript and need the room in
/// [`PLAN_DOCK_FULL_ROWS`]. The activity row only restates what a live turn is
/// doing, so on a short terminal the conversation wins and the phase falls
/// back to the anchor row's inline badge.
pub const ACTIVITY_ROW_TRANSCRIPT_FLOOR: u16 = PLAN_DOCK_FULL_ROWS;
...
pub(crate) const MIN_USABLE_WIDTH: u16 = 40;
pub(crate) const MIN_USABLE_HEIGHT: u16 = MIN_READABLE_TRANSCRIPT_ROWS + MIN_COMPOSER_ROWS;
...
/// Default HUD height: the location row plus the anchor row. Callers that know
/// the session is live ask for a third (see `hud::desired_rows`).
pub const HUD_ROWS: u16 = 2;
/// Maximum HUD height — location, anchor, and the live-activity row.
pub const HUD_MAX_ROWS: u16 = 3;
...
/// Maximum width of the sidebar panel. High enough that the 20% share keeps
/// scaling on very wide terminals (a 36-cap left the panel a cramped corner
/// card on 240+ column windows) while still bounding line length for
/// readability.
...
impl LayoutRegions {
    ...
    pub fn compute_with_sidebar(
        ...
        // The footer's rows past [`HUD_ROWS`] are a luxury: they restate what a
        // live turn is doing. The transcript is the conversation itself. So a
        // requested extra row is granted only while the transcript still clears
        // its readable floor — otherwise the height goes to the conversation.
        // Without this, a short terminal spent the footer's growth on the very
        // rows the plan dock and frontier need.
        let clamped_hud = {
            let requested = hud_rows.clamp(HUD_MIN_ROWS, HUD_MAX_ROWS);
            let needed = clamped_input + requested + rules_total + ACTIVITY_ROW_TRANSCRIPT_FLOOR;
            if requested > HUD_ROWS && area.height < needed {
                HUD_ROWS
            } else {
                requested
            }
