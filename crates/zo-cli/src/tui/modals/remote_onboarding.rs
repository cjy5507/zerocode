...
impl RemoteOnboardingModal {
    ...
    pub fn draw(&self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        let inner = CardFrame::new(SurfaceKind::Modal, theme)
            .title(super::modal_title(theme, "Zo Remote"))
            .render(frame, area);
        ...
        self.ensure_selection_visible();
        let lines = self.render_lines(theme, content.width);
        ...
        frame.render_widget(
            Paragraph::new(super::fit_body_rows(visible, content.width)),
            content,
        );
        draw_scrollbar(
            frame,
    ...
    /// `width` is the content rect's width; the hint row is fitted to it because
    /// `draw` renders without `Wrap` and would otherwise have it cut mid-word.
    fn render_lines<'a>(&'a self, theme: &Theme, width: u16) -> Vec<Line<'a>> {
