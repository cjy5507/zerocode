...
impl SmartSettingsModal {
    ...
    pub fn draw(&self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        let inner = CardFrame::new(SurfaceKind::Modal, theme)
            .title(Line::from(vec![
                Span::styled("Smart Model Router", super::modal_title_style(theme)),
        ...
        if inner.width < 78 || inner.height < 18 {
            frame.render_widget(
                Paragraph::new(super::wrap_body_rows(
                    &[Line::from("Smart settings dashboard needs a larger terminal")],
                    inner.width,
                    true,
                ))
    ...
    fn render_targets(&self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ...
        for (idx, target) in targets.iter().enumerate().skip(start).take(end.saturating_sub(start)) {
            ...
            // Pi SettingsList row: `→ label   value   description`.
            ...
            let fit = self.recommendation_for(target).map_or_else(
                || {
                    glyphs::pick(
                        !theme.no_color,
                        glyphs::GAUGE_EMPTY,
                        glyphs::GAUGE_EMPTY_NC,
                    )
                    .repeat(3)
                },
                |rec| confidence_meter(&rec.confidence, theme),
            );
            ...
            let value_style = if selected {
                ...
                badge_style(&target.current, theme)
            };
            let row = Line::from(vec![
                Span::styled(format!("{marker}{changed} "), style),
                Span::styled(format!("{:<16}", truncate(&target.label, 16)), style),
                Span::styled(format!(" {:<6}", target.current.short_label()), value_style),
                Span::styled(format!(" {fit}"), super::row_detail_style(theme, selected)),
            ]);
            lines.push(if selected {
                super::wash_row(row, list_area.width, theme)
            } else {
                row
            });
        }
        frame.render_widget(
            Paragraph::new(super::wrap_body_rows(&lines, list_area.width, true)),
            list_area,
        );

        if let Some(chart_area) = chart_area {
            frame.render_widget(
                Paragraph::new(super::fit_body_rows(overview, chart_area.width)),
                chart_area,
            );
        }
    }
    ...
    fn overview_lines(&self, width: u16, theme: &Theme) -> Vec<Line<'static>> {
        ...
        if total > 0 {
            ...
            for (provider, count) in mix.iter().take(MIX_CHART_MAX_BARS) {
                ...
                // Brand amber is the scarce accent (`styles.md` "Color"), so it
                // marks the one provider that actually owns the session; the
                // rest of the distribution stays neutral instead of every bar
                // competing for the same emphasis. Bold is dropped from the bar
                // itself — weight on a glyph run only muddies the fill.
                ...
                lines.push(Line::from(vec![
                    Span::styled(format!("{:<9}", truncate(provider, 8)), label_style),
                    Span::styled(
                        glyphs::pick(color, glyphs::GAUGE_FILL, glyphs::GAUGE_FILL_NC)
                            .repeat(filled),
                        bar_style,
                    ),
                    Span::styled(
    ...
    fn render_detail(&self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ...
        frame.render_widget(
            Paragraph::new(super::wrap_body_rows(&lines, inner.width, true)),
            inner,
        );
    }
    ...
    fn render_pin_editor(
        ...
        for (idx, candidate) in matches.iter().enumerate().skip(start).take(end.saturating_sub(start)) {
            ...
            let marker = if selected {
                super::cursor_marker(!theme.no_color).to_string()
            } else if is_current {
                format!("{} ", super::current_marker(!theme.no_color))
            } else {
                super::blank_marker().to_string()
            };
            let marker_style = if selected {
                selected_style(theme)
            } else if is_current {
                super::current_style(theme)
            } else {
                theme.typography.body
            };
            let style = if selected {
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(
                    format!(" {:<9}", truncate(&candidate.provider, 9)),
                    super::row_detail_style(theme, selected),
                ),
                Span::styled(
                    format!(" {}/{}", candidate.family, candidate.class),
                    super::row_detail_style(theme, selected),
                ),
            ]));
        }
    ...
    fn render_family_editor(
        ...
        .enumerate()
        {
            ...
            // Pi SettingsList: cursor accent, label plain, value column muted
            // (accent while selected), key hint dim.
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(
                    format!("[ {:<18} ]", truncate(&value, 18)),
                    super::row_detail_style(theme, cursor),
                ),
    ...
    fn render_footer(&self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ...
        let lines = vec![
            ...
            Line::default(),
            key_hint_footer_reflowing(theme, &hints),
        ];
        frame.render_widget(
            Paragraph::new(super::wrap_body_rows(&lines, area.width, true)),
            area,
...
fn pane_block<'a>(title: &'static str, active: bool, theme: &'a Theme) -> Block<'a> {
    // Pi nested pane: quiet dim rules, focus cyan while the pane holds focus.
    // The title rides the rule, taking the accent only on the focused pane.
    let title_style = if active {
        super::modal_title_style(theme)
    } else {
        theme.typography.dim
    };
...
/// Three-cell confidence meter drawn on the shared gauge glyphs.
///
/// The old meter mixed a block `█` with an ASCII `-` track, so its unfilled
/// cells read as stray hyphens rather than an empty bar. The block is also
/// East-Asian Ambiguous and doubles under a `ko_KR` wide-ambiguous tmux, which
/// pushed this column past its budget.
fn confidence_meter(confidence: &str, theme: &Theme) -> String {
    ...
    format!(
        ...
        glyphs::pick(color, glyphs::GAUGE_EMPTY, glyphs::GAUGE_EMPTY_NC).repeat(3 - filled),
    )
}
...
/// Truncate to at most `max_chars` slots, spending the last one on the ellipsis
/// instead of adding a slot for it.
///
/// Appending `…` on top of a full take returned `max_chars + 1`, which overran
/// every `{:<N}` column this feeds — that is how a nine-cell `anthropi…` ended
/// up touching the provider bar drawn immediately after it.
...
mod tests {
    ...
    fn dashboard_renders_provider_mix_chart() {
        ...
        // The mix bar is a fixed-width gauge, so it rides the EAW-Neutral gauge
        // glyphs rather than the block `█`, which doubles under a `ko_KR`
        // wide-ambiguous tmux and overruns the chart's width budget.
        assert!(
            dump.contains(glyphs::GAUGE_FILL),
            "chart bar missing:\n{dump}"
        );
