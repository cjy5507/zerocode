...
use super::{ModalResult, ModalSelection, blank_marker, cursor_marker, key_hint_footer_reflowing};
use crate::tui::fuzzy;
...
impl ModelPickerModal {
    ...
    pub fn render_lines<'a>(&'a self, theme: &Theme) -> Vec<Line<'a>> {
        ...
        for (pos, &idx) in self.filtered.iter().enumerate() {
            ...
            // Pi select-list row: `→ model-id` in the accent, `[provider]` and
            // the context window in muted, `✓` in success for the bound model.
            ...
            // Cursor marker (`→ ` / two blank cells), in the accent.
            ...
            // Model name — the row's primary column.
            ...
            let pad1 = (max_name_len + 2).saturating_sub(name_chars);
            spans.push(Span::styled(" ".repeat(pad1), wash));

            // Provider badge, bracketed and muted the way Pi labels a source.
            let provider_label = clean_provider_label(provider);
            spans.push(Span::styled(
                format!("[{provider_label}]"),
                super::row_detail_style(theme, selected),
            ));
            let badge_width = provider_label.chars().count() + 2;
            spans.push(Span::styled(
                " ".repeat((16usize).saturating_sub(badge_width)),
                wash,
            ));

            // Context window limit — a secondary column, muted.
            ...
            // The bound model carries the `✓` current-item marker.
            if provider == "suggested" {
                spans.push(Span::styled(" ", wash));
                spans.push(Span::styled(
                    super::current_marker(!theme.no_color).to_string(),
        ...
        lines.push(key_hint_footer_reflowing(theme, &hints));
        lines
    }
    ...
    pub fn draw(&self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ...
        // Render the visible slice of list lines. The cursor row's selection
        // wash is carried out to the column edge so it reads as an edge-to-edge
        // band rather than a highlight hugging the glyphs.
        ...
        let list_para = Paragraph::new(super::wrap_body_rows(&list_slice, inner.width, false))
        ...
        if footer_height > 0 {
            ...
            let footer_para =
                Paragraph::new(super::wrap_body_rows(&footer_lines, inner.width, false))
                    .style(theme.typography.body);
...
impl ModelManager {
    ...
    fn draw(&self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ...
        frame.render_widget(
            Paragraph::new(super::wrap_body_rows(&lines, inner.width, false))
                .style(theme.typography.body),
            inner,
        );
    }
...
mod tests {
    ...
    /// Pi select-list row: `→ model-id` in the accent over the selection wash,
    /// a muted `[provider]` badge, and `✓` in success on the bound model.
    #[test]
    fn rows_use_the_pi_select_list_idiom() {
        let theme = Theme::pi();
        let mut entries = registry();
        entries.insert(0, {
            let mut current = entry("anthropic", "opus");
            current.provider = "suggested".to_string();
            current
        });
        let modal = ModelPickerModal::new(entries);
        let lines = modal.render_lines(&theme);

        // Group headers precede rows once more than one group is visible, so
        // find the first row that carries a cursor marker.
        let cursor_row = lines
            .iter()
            .find(|line| line.spans[0].content.as_ref() == "\u{2192} ")
            .expect("the cursor row carries the Pi arrow");
        assert_eq!(cursor_row.spans[0].style.fg, Some(theme.palette.accent));
        assert_eq!(
            cursor_row.spans[0].style.bg,
            theme.selection_bg(),
            "the cursor row is washed"
        );
        // `[provider]` badge, muted (accent while the row is selected).
        let badge = cursor_row
            .spans
            .iter()
            .find(|span| span.content.starts_with('['))
            .expect("bracketed provider badge");
        assert_eq!(badge.style.fg, Some(theme.palette.accent));
        // The bound (suggested) model carries the success `✓`.
        let check = cursor_row
            .spans
            .last()
            .expect("the bound model closes with a marker");
        assert_eq!(check.content.as_ref(), "\u{2713}");
        assert_eq!(check.style.fg, Some(theme.palette.success));

        // An unselected row indents to the same column and stays muted.
        let plain = lines
            .iter()
            .find(|line| line.spans[0].content.as_ref() == "  ")
            .expect("unselected rows keep a two-cell indent");
        assert_eq!(plain.spans[1].style.fg, Some(theme.palette.fg));
    }
    ...
    fn manager_duplicate_alias_error_is_inline() {
        ...
        assert_eq!(
            ...
            Some("Already provided by \"Gemini 3.6 Flash\" (id `gemini-3.6-flash`) — edit that row instead")
        );
        let _ = std::fs::remove_file(path);
    }
