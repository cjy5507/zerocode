...
pub enum ModalSelection {
    ...
    /// A register / edit / delete request from [`ProviderManagerModal`]. The
    /// manager stays open for mutations and re-lists, so the host decides
    /// closing per action rather than the modal closing itself.
    ProviderManage(ProviderManagerAction),
...
impl Modal for UserQuestionModal {
    ...
    fn desired_size(&self, area: Rect, theme: &Theme) -> (u16, u16) {
        // Sizes from its real rendered rows, measured at the width those rows
        // are painted into (the narrow option column when the preview layout
        // splits). Lives next to the paint so the two cannot drift.
        UserQuestionModal::desired_size(self, area, theme)
    }
...
impl Modal for ProviderManagerModal {
    fn as_any_mut(&mut self) -> &mut dyn std::any::Any {
        self
    }

    fn handle_key(&mut self, key: KeyEvent) -> Option<ModalResult> {
        ProviderManagerModal::handle_key(self, key)
    }

    fn draw(&self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ProviderManagerModal::draw(self, frame, area, theme);
    }

    fn desired_size(&self, area: Rect, _theme: &Theme) -> (u16, u16) {
        ProviderManagerModal::size(self, area)
    }

    /// Centered, not fullscreen: the tree is content-sized, so it stays usable
    /// in the small inline viewport instead of degrading to a notice.
    fn placement(&self) -> ModalPlacement {
        ModalPlacement::Centered
    }

    fn scroll(&mut self, up: bool, rows: usize) {
        self.scroll_by(up, rows);
    }
}
...
// ============================================================================
// Shared visual language for selection modals (Pi select-list idiom)
//
// Every list-style modal (question / choice / model / permission / provider /
// tier / …) draws its cursor row the same way, because they all route through
// the three helpers below:
//
//     → selected label            ← `cursor_marker` + `selected_style`, washed
//       unselected label            with `selection_bg()` across the full row
//       another one   description  ← description columns in `muted`
//
// and closes with the same Pi key-hint footer (`key` dim, `label` muted, joined
// with a muted ` · `). Changing the idiom here changes every modal at once.
// ============================================================================

/// Cursor marker for the highlighted row: Pi's `→ ` arrow.
///
/// Degrades to a one-cell ASCII `> ` under `NO_COLOR`/plain mode (callers pass
/// `!theme.no_color`). Always two display cells so selected and blank rows stay
/// column-aligned; pair with [`blank_marker`].
#[must_use]
pub(super) fn cursor_marker(color: bool) -> &'static str {
    glyphs::pick(color, "\u{2192} ", "> ")
}
...
/// The current/active-item marker (`✓`), e.g. the model actually bound to the
/// session. Rendered in [`Theme::palette`]`.success`; degrades to ASCII `*`.
#[must_use]
pub(super) fn current_marker(color: bool) -> &'static str {
    glyphs::pick(color, "\u{2713}", "*")
}

/// Style for the current/active-item marker.
#[must_use]
pub(super) fn current_style(theme: &Theme) -> Style {
    Style::new().fg(theme.palette.success)
}

/// Emphasis style for the selected row: the accent, bold, over the shared
/// selection wash. Under `NO_COLOR` the accent resolves to `Reset`, so the bold
/// weight still distinguishes it.
#[must_use]
...
/// The selection wash on its own — for the padding that carries a selected row's
/// background out to the full width of the list column.
#[must_use]
pub(super) fn selection_wash(theme: &Theme) -> Style {
    match theme.selection_bg() {
...
/// Secondary column style for a list row (provider badges, descriptions,
/// counts): `muted`, or the accent over the wash while the row is selected.
#[must_use]
pub(super) fn row_detail_style(theme: &Theme, selected: bool) -> Style {
    if selected {
        Style::new()
            .fg(theme.palette.accent)
            .patch(selection_wash(theme))
    } else {
        Style::new().fg(theme.palette.muted)
    }
}

/// Extend a selected row's wash to the full width of `width` cells.
///
/// Pi paints selection as an edge-to-edge band, not a highlight around the
/// glyphs, so a selected row is padded with spaces carrying the wash. A row that
/// already fills (or overflows) the column is returned untouched.
#[must_use]
pub(super) fn wash_row<'a>(mut line: Line<'a>, width: u16, theme: &Theme) -> Line<'a> {
    let Some(bg) = theme.selection_bg() else {
        return line;
    };
...
pub(super) fn modal_footer(
    ...
    // Pi key-hint idiom: the key is `dim`, its label `muted`, and the segments
    // are joined by a `muted` separator.
    let key_default = theme.typography.key_hint;
    let label_style = Style::new().fg(theme.palette.muted);
    let mut spans = Vec::with_capacity(segments.len() * 3);
    for (idx, segment) in segments.iter().enumerate() {
        if idx > 0 {
            spans.push(Span::styled(separator.to_string(), label_style));
        }
        match *segment {
            ...
            FooterSegment::Hint {
                ...
                let key_style = key_style.unwrap_or(key_default);
...
/// Pre-wrap `lines` to `width`, so the caller renders the result *without*
/// `Paragraph::wrap`.
///
/// ratatui's own word wrapper can emit a row one cell wider than the width it was
/// given when a double-width grapheme lands on the boundary, and the paint then
/// drops that grapheme — a Hangul syllable vanishing out of the middle of a
/// sentence. [`crate::tui::text_metrics::wrap_line_to_cells`] carries the
/// reproduction and the mechanism. Every modal body that can hold CJK — a
/// question, an agent's task line, an inbox message — wraps through here instead.
///
/// The row count is now the caller's to see (`len()`), which is also what lets a
/// scrolled body clamp against what was actually painted.
#[must_use]
pub(super) fn wrap_body_rows(lines: &[Line<'_>], width: u16, trim: bool) -> Vec<Line<'static>> {
    let budget = usize::from(width);
    lines
        .iter()
        .flat_map(|line| crate::tui::text_metrics::wrap_line_to_cells(line, budget, trim))
        .collect()
}

/// Fit every body row to `width`, truncating with `…` rather than letting the
/// renderer cut one mid-glyph.
///
/// Modal bodies render through `Paragraph` without `.wrap(...)`, where ratatui
/// composes lines via `LineTruncator`: it stops at the rect edge and adds no
/// ellipsis (verified in `ratatui-widgets-0.3.0/src/reflow.rs`). So a row one cell
/// too wide silently loses its tail — a path, model id, or agent name that reads as
/// complete but is not. No modal scrolls horizontally (every `scroll` is
/// `(y, 0)`), so nothing depends on content continuing past the right edge and
/// cutting here loses no capability.
///
/// Truncation is measured in terminal cells, so Hangul and CJK rows — two columns
/// per glyph — are budgeted correctly instead of overrunning by one column each.
#[must_use]
pub(super) fn fit_body_rows(lines: Vec<Line<'_>>, width: u16) -> Vec<Line<'_>> {
    let budget = usize::from(width);
    lines
        .into_iter()
        .map(|line| crate::tui::text_metrics::truncate_line_to_cells(line, budget))
        .collect()
}

/// Build a modal footer that fits `width`, dropping whole segments off the end
/// rather than letting the renderer cut one in half.
///
/// Modal bodies render through `Paragraph` **without** `Wrap`, so a `Line` wider
/// than its rect is truncated at the edge mid-grapheme: a footer reading
/// `… · r restore · Esc back` becomes `… · r restore · Esc ba`. A clipped hint is
/// worse than an absent one — the reader cannot act on half a key name, and the
/// dangling text reads as corruption rather than as an omission. Segments are
/// therefore measured with their separator and dropped whole once the budget is
/// gone.
///
/// The first segment is always kept even if it alone exceeds `width`: something
/// recognizable beats an empty footer, and at that size the modal has bigger
/// problems than its hints.
#[must_use]
pub(super) fn modal_footer_fitted(
    theme: &Theme,
    segments: &[FooterSegment<'_>],
    separator: &str,
    width: u16,
    ...
    for (idx, segment) in segments.iter().enumerate() {
        ...
        let own_width = match *segment {
            FooterSegment::Label(label) => display_width(label),
            // `modal_footer` renders a hint as `"{key} "` + label.
            FooterSegment::Hint { key, label, .. } => {
                display_width(key) + 1 + display_width(label)
            }
        };
        let next = used + separator_width + own_width;
        if idx > 0 && next > budget {
            break;
        }
        used = next;
        kept = idx + 1;
    }
    modal_footer(theme, &segments[..kept.max(1).min(segments.len())], separator)
}

/// Build the shared key-hint footer line, fitted to `width`. See
/// [`modal_footer_fitted`] for why whole hints are dropped instead of clipped.
#[must_use]
pub(super) fn key_hint_footer_fitted(
    theme: &Theme,
    hints: &[(&str, &str)],
    width: u16,
) -> Line<'static> {
    let segments = hints
        ...
        .collect::<Vec<_>>();
    modal_footer_fitted(theme, &segments, " \u{b7} ", width)
}

/// Build the shared key-hint footer line for a body that **reflows**. Each
/// `(key, label)` pair renders the key `dim` and the label `muted`, joined by a
/// muted ` · ` — the Pi hint idiom every modal footer shares.
///
/// Unbounded, and the name says when that is safe: this is valid **only** when the
/// host `Paragraph` sets `.wrap(...)`, which reflows an over-wide row onto a second
/// line. Without `wrap`, ratatui composes lines through `LineTruncator`, which cuts
/// at the rect edge and adds no ellipsis — the row becomes `Esc canc` or
/// `Space/Enter 토`, a hint the reader cannot act on.
///
/// If the body renders without `wrap`, use [`key_hint_footer_fitted`], which drops
/// whole hints to fit. The rename from the old neutral `key_hint_footer_reflowing` is
/// deliberate: eleven modals had silently been clipping, and a name that states its
/// precondition makes the next caller choose rather than inherit the bug.
#[must_use]
pub(super) fn key_hint_footer_reflowing(
    theme: &Theme,
    hints: &[(&str, &str)],
) -> Line<'static> {
    let segments = hints
        ...
        .map(|(key, label)| FooterSegment::hint(key, label))
        .collect::<Vec<_>>();
    modal_footer(theme, &segments, " \u{b7} ")
}
...
/// The Pi dialog title style: bold accent, riding the top rule.
///
/// Every modal titles itself through this so the whole family reads as one
/// surface; `CardFrame` supplies the `─ … ─────` rule inlay around it.
#[must_use]
pub(super) fn modal_title_style(theme: &Theme) -> Style {
    Style::new()
        .fg(theme.palette.accent)
        .add_modifier(Modifier::BOLD)
}

/// Build a Pi dialog title line for `title`.
#[must_use]
pub(super) fn modal_title(theme: &Theme, title: impl Into<String>) -> Line<'static> {
    Line::styled(title.into(), modal_title_style(theme))
}

/// Render the standard modal frame and return its inner content rectangle.
///
/// Every simple modal uses the same Pi dialog chrome: a top rule carrying a
/// bold-accent title, a bottom rule, no sides, and the elevation-2 glass fill.
/// Centralizing that keeps the visual contract in one place; callers remain
/// responsible only for their body layout (including any extra inner margin
/// they intentionally apply).
#[must_use]
pub(super) fn modal_frame(
    ...
    CardFrame::new(SurfaceKind::Modal, theme)
        .title(modal_title(theme, title))
        .render(frame, area)
}

/// Blank `area` and paint the elevation-2 glass fill across every cell of it.
///
/// For a modal that draws its own rules instead of going through
/// [`modal_frame`], this is the equivalent of the fill `CardFrame` bakes into
/// its block. A bare `Clear` is not enough and is in fact actively wrong: the
/// frosted backdrop fills the pane with `surface2` *before* the modal draws, and
/// `Clear` resets every cell it touches — including that fill. The rows a short
/// body never covers were then left on the terminal's own background, reading
/// as a hole punched through the dialog. Clear (for the glyphs) *and* fill (for
/// the surface), so the interior is painted edge to edge whatever the content
/// does.
pub(super) fn dialog_surface(frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
    frame.render_widget(ratatui::widgets::Clear, area);
    let fill = theme
        .surface2()
        .map_or_else(Style::new, |surface| Style::new().bg(surface));
...
mod scrollbar_tests {
    ...
    fn modal_frame_returns_standard_inner_rect() {
        ...
        // The Pi dialog has no side borders, so the body keeps the full width
        // and only the two rules cost rows.
        assert_eq!(inner, Rect::new(0, 1, 10, 3));
    }

    #[test]
    fn modal_frame_titles_ride_the_rule_in_bold_accent() {
        let theme = Theme::zo();
        let mut term = Terminal::new(TestBackend::new(16, 4)).unwrap();
        term.draw(|frame| {
            let _ = modal_frame(frame, Rect::new(0, 0, 16, 4), "Models", &theme);
        })
        .unwrap();
        let buf = term.backend().buffer();
        let top: String = (0..16u16)
            .map(|x| buf.cell((x, 0)).unwrap().symbol())
            .collect();
        assert_eq!(top, "\u{2500} Models \u{2500}\u{2500}\u{2500}\u{2500}\u{2500}\u{2500}\u{2500}");
        let title = buf.cell((2, 0)).unwrap();
        assert_eq!(title.fg, theme.palette.accent);
        assert!(title.modifier.contains(Modifier::BOLD));
        assert_eq!(buf.cell((0, 0)).unwrap().fg, theme.palette.border);
    }

    #[test]
    fn key_hints_use_the_pi_dim_key_muted_label_idiom() {
        let theme = Theme::zo();
        let line = key_hint_footer_reflowing(&theme, &[("enter", "select"), ("esc", "cancel")]);
        let styles: Vec<_> = line
            .spans
            .iter()
            .map(|s| (s.content.to_string(), s.style))
            .collect();
        assert_eq!(styles[0].0, "enter ");
        assert_eq!(styles[0].1, theme.typography.key_hint);
        assert_eq!(styles[1].0, "select");
        assert_eq!(styles[1].1.fg, Some(theme.palette.muted));
        assert_eq!(styles[2].0, " \u{b7} ");
        assert_eq!(styles[2].1.fg, Some(theme.palette.muted));
    }

    #[test]
    fn a_selected_row_is_accent_over_a_full_width_wash() {
        let theme = Theme::pi();
        let wash = theme.selection_bg().expect("pi pins a selection wash");
        assert_eq!(cursor_marker(true), "\u{2192} ");
        assert_eq!(cursor_marker(false), "> ");

        let selected = selected_style(&theme);
        assert_eq!(selected.fg, Some(theme.palette.accent));
        assert_eq!(selected.bg, Some(wash));

        let line = wash_row(Line::from(Span::styled("abc", selected)), 10, &theme);
        assert_eq!(line.width(), 10, "the wash runs to the column edge");
        assert_eq!(line.spans.last().unwrap().style.bg, Some(wash));
    }
...
#[cfg(test)]
mod footer_fitting_tests {
    use super::{key_hint_footer_fitted, FooterSegment, modal_footer_fitted};
    use crate::tui::theme::Theme;
    ...
    /// Hints are dropped whole, never cut mid-word.
    ///
    /// Modal bodies render through `Paragraph` with no `Wrap`, so a `Line` wider
    /// than its rect is truncated at the edge — the reader is left with half a key
    /// name, which cannot be acted on and reads as corruption rather than as an
    /// omission. Dropping the trailing hint instead keeps everything shown usable.
    #[test]
    fn a_footer_over_budget_drops_trailing_hints_whole() {
        let theme = Theme::no_color();
        let hints = [("↑↓", "move"), ("Enter", "confirm"), ("Esc", "cancel")];

        // `↑↓ move · Enter confirm · Esc cancel` is 36 cells.
        assert_eq!(
            text(&key_hint_footer_fitted(&theme, &hints, 36)),
            "↑↓ move · Enter confirm · Esc cancel",
            "everything fits at its exact width"
        );
        // One cell short: the last hint goes entirely, separator included — no
        // dangling ` · ` and no `Esc canc`.
        assert_eq!(
            text(&key_hint_footer_fitted(&theme, &hints, 35)),
            "↑↓ move · Enter confirm"
        );
        assert_eq!(
            text(&key_hint_footer_fitted(&theme, &hints, 23)),
            "↑↓ move · Enter confirm"
        );
        assert_eq!(text(&key_hint_footer_fitted(&theme, &hints, 22)), "↑↓ move");
    }

    /// The first hint survives a budget it cannot fit. An empty footer says
    /// nothing at all; a recognizable one still names the primary key, and at that
    /// width the modal has bigger problems than its hints.
    #[test]
    fn the_first_hint_is_never_dropped() {
        let theme = Theme::no_color();
        let hints = [("Space/Enter", "토글"), ("Esc", "닫기")];
        assert_eq!(text(&key_hint_footer_fitted(&theme, &hints, 1)), "Space/Enter 토글");
    }

    /// Budgeting is in terminal *cells*, not chars: each of `토글`/`닫기` is two
    /// columns wide, so measuring by `chars().count()` would under-count by 4 and
    /// let the footer overrun exactly the way it did before.
    #[test]
    fn cjk_labels_are_measured_in_cells() {
        let theme = Theme::no_color();
        let hints = [("Space/Enter", "토글"), ("Esc", "닫기")];
        // 11+1+4 = 16, ` · ` = 3, 3+1+4 = 8 → 27 cells for both.
        assert_eq!(
            text(&key_hint_footer_fitted(&theme, &hints, 27)),
            "Space/Enter 토글 · Esc 닫기"
        );
        assert_eq!(
            text(&key_hint_footer_fitted(&theme, &hints, 26)),
            "Space/Enter 토글",
            "a char-based budget would have thought this still fit"
        );
    }

    /// Plain labels are measured too, not just key/label hints.
    #[test]
    fn label_segments_share_the_budget() {
        let theme = Theme::no_color();
        let segments = [
            FooterSegment::Label("3 of 9"),
            FooterSegment::hint("Esc", "close"),
        ];
        assert_eq!(
            text(&modal_footer_fitted(&theme, &segments, " · ", 19)),
            "3 of 9 · Esc close"
        );
        assert_eq!(
            text(&modal_footer_fitted(&theme, &segments, " · ", 17)),
            "3 of 9"
        );
    }
}
