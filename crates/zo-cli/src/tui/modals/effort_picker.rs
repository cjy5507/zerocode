//! `/effort` level picker modal.
//!
//! A Pi select list with one row per exact level — `low → medium → high →
//! xhigh → max → ultra → smart` — while its swatches and the composer frame
//! group those levels into three glanceable driving modes: Eco, Normal, and
//! Sport. Controlled by left/right arrows (or `1`-`7`), confirmed with Enter,
//! dismissed with Esc.
//!
...
pub struct EffortStep {
    /// Exact level selected; its swatch is grouped into Eco, Normal, or Sport.
    pub effort: Effort,
    /// Canonical label rendered in the step's row.
...
impl EffortPickerModal {
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> Option<ModalResult> {
        ...
        match key.code {
            ...
            // The ladder renders vertically, so `↑/k` and `↓/j` are aliases of
            // the historical `←/h` and `→/l` bindings: the axis the eye reads
            // and the axis the hand presses must agree.
    ...
    /// unit tests.
    ///
    /// The layout is Pi's select list with one row per exact level. Its leading
    /// swatches group those levels into the same Eco / Normal / Sport colors as
    /// the composer border, while the label and token budget preserve the exact
    /// selection:
    ///
    /// ```text
    ///     ██  low        4 000 tokens   quick, shallow reasoning
    ///     ██  medium     8 000 tokens   …
    ///   → ██  high      16 000 tokens   …          ← selected: wash + `→`
    ///     ██  xhigh     …
    ///
    ///   ↑/↓ move · 1-7 jump · Enter confirm · Esc cancel
    /// ```
    #[must_use]
    pub fn render_lines<'a>(&'a self, theme: &Theme, width: u16) -> Vec<Line<'a>> {
        let label_width = EFFORT_STEPS
            .iter()
            .map(|step| step.label.len())
            .max()
            .unwrap_or(6);
        let mut lines: Vec<Line<'a>> = EFFORT_STEPS
            .iter()
            .enumerate()
            .map(|(idx, step)| {
                effort_row_line(*step, idx == self.cursor, label_width, width, theme)
            })
            .collect();
        lines.push(Line::from(""));
        lines.push(footer_line(theme, width));
        lines
    }

    /// Draw the modal into `area`.
    ///
    /// Pi dialog chrome (top/bottom rules, bold-accent title on the top rule)
    /// with 2-col / 1-row interior padding, hosting the ladder list.
    pub fn draw(&self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        let inner = CardFrame::new(SurfaceKind::Modal, theme)
            .title(super::modal_title(theme, "/effort"))
...
/// Shared color contract for the exact effort picker and the composer frame.
///
/// Labels retain the precise effort, while color groups choices into three
/// glanceable driving modes: Low=Eco, Medium=Normal, and every high-power tier
/// (including Smart's adaptive top band)=Sport. Off stays neutral, so hue is
/// never the only signal and `NO_COLOR` still collapses through the palette.
pub(crate) fn effort_mode_color(effort: Effort, theme: &Theme) -> ratatui::style::Color {
    match effort {
        Effort::Off => theme.palette.thinking_off,
        Effort::Low => theme.palette.thinking_low,
        Effort::Medium => theme.palette.thinking_medium,
        Effort::High | Effort::Xhigh | Effort::Max | Effort::Ultra | Effort::Smart => {
            theme.palette.thinking_max
        }
    }
}

/// One Pi select-list row: `→ ██  label   N tokens   hint`.
fn effort_row_line<'a>(
    step: EffortStep,
    selected: bool,
    label_width: usize,
    width: u16,
    ...
    let rung = effort_mode_color(step.effort, theme);
    let swatch = if theme.no_color { "##" } else { "\u{2588}\u{2588}" };
    ...
    let mut spans = vec![
        ...
        Span::styled(swatch.to_string(), Style::default().fg(rung).patch(wash)),
        Span::styled("  ".to_string(), wash),
        Span::styled(format!("{:<label_width$}", step.label), label_style),
        Span::styled(
            format!("  {budget:>7} tokens  "),
    ...
    if room > 8 {
        let hint: String = step.hint.chars().take(room).collect();
...
/// Fitted to `width`: the banner renders through a `Paragraph` with no `Wrap`,
/// so an over-wide hint row is cut mid-word at the rect edge.
...
#[cfg(test)]
mod tests {
    use super::{EFFORT_STEPS, EffortPickerModal, step_for_budget};
    use crate::tui::Theme;
    ...
    #[test]
    fn effort_rows_share_eco_normal_and_sport_colors() {
        let theme = Theme::pi();
        let modal = EffortPickerModal::new();
        let lines = modal.render_lines(&theme, 80);
        // One row per level, then a blank spacer and the key-hint footer.
        assert_eq!(lines.len(), EFFORT_STEPS.len() + 2);

        // The exact labels stay distinct while their swatches group into three
        // quick-read modes shared with the composer border.
        let swatch_fg = |row: usize| lines[row].spans[1].style.fg.expect("swatch color");
        assert_eq!(swatch_fg(0), theme.palette.thinking_low, "Low = Eco");
        assert_eq!(swatch_fg(1), theme.palette.thinking_medium, "Medium = Normal");
        for row in 2..=6 {
            assert_eq!(swatch_fg(row), theme.palette.thinking_max, "row {row} = Sport");
        }
        assert_ne!(swatch_fg(0), swatch_fg(1));
        assert_ne!(swatch_fg(1), swatch_fg(2));
        assert_ne!(swatch_fg(0), swatch_fg(2));

        // The cursor row carries the `→` marker and a full-width wash.
        assert_eq!(lines[0].spans[0].content.as_ref(), "\u{2192} ");
        assert_eq!(lines[1].spans[0].content.as_ref(), "  ");
        let wash = theme.selection_bg().expect("pi pins a selection wash");
        assert_eq!(lines[0].style.bg, Some(wash));
    ...
    #[test]
    fn vertical_keys_walk_the_ladder_like_the_horizontal_ones() {
        // The ladder renders top-to-bottom, so Down/j must advance exactly
        // like Right/l and Up/k must retreat exactly like Left/h.
        let mut modal = EffortPickerModal::new();
        modal.cursor = 0;
        let _ = modal.handle_key(press(KeyCode::Down));
        assert_eq!(modal.cursor(), 1);
        let _ = modal.handle_key(press(KeyCode::Char('j')));
        assert_eq!(modal.cursor(), 2);
        let _ = modal.handle_key(press(KeyCode::Up));
        assert_eq!(modal.cursor(), 1);
        let _ = modal.handle_key(press(KeyCode::Char('k')));
        assert_eq!(modal.cursor(), 0);

        let mut horizontal = EffortPickerModal::new();
        horizontal.cursor = 0;
        let _ = horizontal.handle_key(press(KeyCode::Right));
        let mut vertical = EffortPickerModal::new();
        vertical.cursor = 0;
        let _ = vertical.handle_key(press(KeyCode::Down));
        assert_eq!(horizontal.cursor(), vertical.cursor());
    }
    ...
    #[test]
    fn the_cursor_marks_exactly_the_active_level_row() {
        let mut modal = EffortPickerModal::new();
        modal.cursor = 6; // smart (last step)
        let theme = Theme::zo();
        ...
        let marked: Vec<usize> = lines
            .iter()
            .take(EFFORT_STEPS.len())
            .enumerate()
            .filter(|(_, line)| line.spans[0].content.as_ref() == "\u{2192} ")
            .map(|(idx, _)| idx)
            .collect();
        assert_eq!(marked, vec![6], "only the active level row is marked");
        // Every row still names its level and its budget.
        let smart: String = lines[6]
            .spans
            .iter()
            .flat_map(|span| span.content.chars())
            .collect();
        assert!(smart.contains("smart"), "row text: {smart:?}");
    }
    ...
    fn level_maps_each_effort_to_its_wire_tier() {
        ...
        // plus a ceiling of Max — the wire backends resolve the concrete
        // per-request rung via `api::resolve_effort_band`. The ceiling is Max,
        // not Ultra: the top tier is reachable only through the explicit
        // `Effort::Ultra` pin, never by automatic escalation.
        assert_eq!(Effort::Smart.level(), Some(L::Xhigh));
        assert_eq!(Effort::Smart.band_ceiling(), Some(L::Max));
    ...
    fn band_labels_for_model_ranges_per_provider_ceiling() {
        use super::Effort;
        // Sol/terra: `max`, not `ultra`. The model's own ceiling IS Ultra, but
        // Smart's band ceiling stops a rung short on purpose — automatic
        // escalation must not spend the top tier, which stays behind the
        // explicit `Effort::Ultra` pin.
        assert_eq!(
            Effort::Smart.band_labels_for_model("gpt-5.6-sol"),
            Some(("xhigh", "max"))
        );
        // Fable/Luna: the model's own selection ceiling is already Max, so the
        // band ceiling is not what binds here — the range is the same either way.
        assert_eq!(
        ...
        // Sonnet 4.6 has no `xhigh` wire tier, but `max` IS in its supported
        // set — so the floor clamps to `high` while a heavy-signal turn still
        // reaches `max`, a genuine (not degenerate) [high..max] band. Sonnet 5
        // gained `xhigh`, so it keeps the full floor.
        assert_eq!(
            Effort::Smart.band_labels_for_model("claude-sonnet-4-6"),
            Some(("high", "max"))
        );
        assert_eq!(
