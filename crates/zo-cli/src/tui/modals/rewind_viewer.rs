...
impl RewindViewerModal {
    ...
    pub fn draw(&self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        let inner = CardFrame::new(SurfaceKind::Modal, theme)
            .title(super::modal_title(theme, "rewind"))
        ...
        if self.show_diff && body_area.height > 0 {
            let body_lines = self
                ...
                .unwrap_or_default();
            frame.render_widget(
                Paragraph::new(super::fit_body_rows(body_lines, body_area.width))
        ...
        frame.render_widget(
            Paragraph::new(super::fit_body_rows(
                vec![footer_line(theme, footer_area.width)],
                footer_area.width,
            )),
            footer_area,
        );
    ...
    fn timeline_row_line(&self, row_index: usize, row: &RewindRow, theme: &Theme) -> Line<'static> {
        ...
        // Pi select-list row: `→ ` marker in the accent over the selection wash.
...
/// Fitted to `width`: the footer renders through a `Paragraph` with no `Wrap`,
...
mod tests {
    ...
    fn timeline_dump_shows_turns_and_caret() {
        ...
        assert!(
            ...
            "Pi selection arrow present: {dump}"
        );
    }
}
