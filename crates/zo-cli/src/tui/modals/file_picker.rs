...
use super::{ModalResult, ModalSelection, key_hint_footer_fitted};
use crate::tui::fuzzy;
...
impl FilePickerModal {
    ...
    pub fn render_lines<'a>(&'a self, theme: &Theme) -> Vec<Line<'a>> {
            ...
            .enumerate()
        {
            ...
            let selected = actual_pos == self.cursor;
            // Pi select-list row: `→ path` in the accent over the selection
            // wash, unselected rows indented to the same column.
    ...
    pub fn draw(&self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        ...
        .areas(inner);
        // Row 0 is the search field, so the cursor row sits one line lower.
        let start = self.cursor.saturating_sub(MAX_VISIBLE / 2);
        let selected_row = self.cursor - start + 1;
        let lines = self
            .render_lines(theme)
            .into_iter()
            .enumerate()
            .map(|(idx, line)| {
                if idx == selected_row {
                    super::wash_row(line, list_area.width, theme)
                } else {
                    line
                }
            })
        ...
        // Fitted to the footer rect: this `Paragraph` has no `Wrap`, so an
        // over-wide hint row would be cut mid-word at the edge.
        frame.render_widget(
            Paragraph::new(key_hint_footer_fitted(
                theme,
                &[
                    ("↑↓", "move"),
                    ("PgUp/PgDn", "page"),
