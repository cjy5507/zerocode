...
/// `▸ ✓ 7 tools · read ×7` — a calm settled-history row whose disclosure
/// marker advertises click-to-expand. Targets and per-tool digests remain
/// available when the group is expanded.
fn settled_group_summary_line(
    ...
    let fixed = 4 + UnicodeWidthStr::width(label.as_str()) + 3;
    ...
    let success = if theme.no_color { "v " } else { "\u{2713} " };
    let mut spans = vec![
        Span::styled(disclosure, dim),
        Span::styled(success, Style::new().fg(theme.palette.success)),
        Span::styled(
            label,
            Style::new()
                .fg(theme.palette.fg)
...
mod tests {
    ...
    fn large_settled_group_compacts_to_one_action_digest() {
        ...
        assert!(
            summary.starts_with("▸ ✓")
                && summary.contains("7 tools")
                && summary.contains("read ×7"),
            "the compact row advertises click and keeps count/action: {summary:?}"
        );
