...
#[test]
fn transcript_byte_cap_front_prunes_and_preserves_table_invariants() {
    // A handful of large blocks can blow the RAM budget while the block *count*
    // stays far under `MAX_TRANSCRIPT_BLOCKS`. The byte cap must front-prune
    // those, keep the four parallel tables in lockstep, and never drop below
    // `MIN_RETAINED_BLOCKS` (so the current screen is never blanked).
    ...
    let mut transcript = Transcript::new();
    // Each block body is a comfortable fraction of the (test-profile) byte cap,
    // so a few dozen blocks blow past `MAX_TRANSCRIPT_BYTES` while staying well
    // under the block-count cap. 200 pushes cross several scan intervals.
    let big = "x".repeat(16 * 1024);
    for index in 0..200u64 {
        transcript.push(RenderBlock::UserMessage {
            id: BlockId(index),
            text: big.clone(),
        });
        assert!(aligned(&transcript), "byte-cap prune must keep tables aligned");
    }

    // The byte cap (not the count cap — we never approached it) drove pruning.
    assert!(
        transcript.len() < 200,
        "byte cap must have front-pruned (len {})",
        transcript.len()
    );
    assert!(
        transcript.len() >= super::MIN_RETAINED_BLOCKS,
        "the minimum-retained floor must be honored even over the byte cap (len {})",
        transcript.len()
    );
    // The retained blocks are the most recent ones (front-prune drops oldest).
    let RenderBlock::UserMessage { id, .. } = &transcript.blocks()[transcript.len() - 1] else {
        panic!("expected user message block");
    };
    assert_eq!(id.0, 199, "the newest block survives the byte-cap prune");
}

#[test]
fn transcript_byte_cap_leaves_small_sessions_untouched() {
    // A short session of tiny blocks stays well under the byte cap: no prune,
    // and the running byte total tracks the pushed bodies after a scan.
    let mut transcript = Transcript::new();
    ...
    assert_eq!(transcript.len(), 10, "small sessions are never byte-pruned");
    let RenderBlock::System { text, .. } = &transcript.blocks()[0] else {
        panic!("expected system block");
    };
    assert_eq!(text, "system 0", "oldest block survives under the cap");
}
...
fn assert_block_background(block: RenderBlock, theme: &Theme, expected: Color) {
    // One-row viewport forces the transcript to reserve its scrollbar gutter.
    // The block's draw rect must fill all content cells but leave that gutter
    // untouched.
    let (width, height) = (60u16, 1u16);
    let mut transcript = Transcript::new();
    ...
    for x in 0..width - 1 {
        assert_eq!(
            buffer[(x, 0)].bg,
            expected,
            "content cell {x} must carry the block background"
        );
    }
    assert_ne!(
        buffer[(width - 1, 0)].bg,
        expected,
        "the scrollbar gutter must not receive the block background"
    );
}

#[test]
fn tool_call_statuses_leave_unused_cells_unpainted() {
    let theme = Theme::default_dark();
    for (status, marker_fg) in [
        (ToolCallStatus::Pending, theme.palette.dim),
        (ToolCallStatus::Running, theme.palette.success),
        (ToolCallStatus::Ok, theme.palette.success),
        (ToolCallStatus::Errored, theme.palette.error),
        (ToolCallStatus::Cancelled, theme.palette.muted),
    ] {
        let status_name = format!("{status:?}");
        let mut transcript = Transcript::new();
        transcript.push(RenderBlock::ToolCall {
            id: id(),
            tool_call_id: ToolCallId(format!("background-{status_name}")),
            name: "Bash".to_string(),
        ...
        let buffer = terminal.backend().buffer();
        let marker_x = (0..width - 1)
            .find(|&x| !buffer[(x, 0)].symbol().trim().is_empty())
            .expect("tool status marker must remain visible");
        assert_eq!(
            buffer[(marker_x, 0)].fg,
            marker_fg,
            "{status_name} marker must retain its semantic foreground"
        );
        for x in 0..width {
            assert_eq!(
                buffer[(x, 0)].bg,
                Color::Reset,
                "{status_name} painted an empty background cell at x={x}"
            );
        }
    }
}
...
#[test]
fn user_message_paints_its_background() {
    let theme = Theme::default_dark();
    assert_block_background(
        RenderBlock::UserMessage {
            id: id(),
            text: "add a background".to_string(),
        },
        &theme,
        theme.palette.user_message_bg,
    );
}
...
/// Leaving the tail already parks the offset, but a reader who scrolled back
/// had no way to see how much arrived behind them. The backlog count must be
/// zero while following, equal the distance after stepping away, and grow with
/// content that lands while parked.
#[test]
fn backlog_below_viewport_is_zero_at_tail_and_tracks_growth_while_parked() {
    let theme = Theme::no_color();
    let (w, h) = (70u16, 8u16);
    let draw = |t: &mut Transcript| {
        let backend = TestBackend::new(w, h);
    ...
    draw(&mut t);
    assert_eq!(
        t.lines_below_viewport(h, &theme, w, ImageProtocol::None),
        0,
        "following the tail hides nothing below the viewport"
    );
    ...
    assert_eq!(
        t.lines_below_viewport(h, &theme, w, ImageProtocol::None),
        3,
        "stepping 3 rows off the tail leaves exactly 3 rows below"
    );
    ...
    draw(&mut t);
    assert_eq!(t.scroll(), parked, "precondition: still parked off the tail");
    assert!(
        t.lines_below_viewport(h, &theme, w, ImageProtocol::None) > 3,
        "content landing while parked must deepen the backlog (was 3, now {})",
        t.lines_below_viewport(h, &theme, w, ImageProtocol::None)
    );
}

/// The badge is the visible half of the backlog count: it must stay off screen
/// while the reader follows the tail, and appear once they park above it.
#[test]
fn backlog_badge_paints_only_while_parked_off_the_tail() {
    let theme = Theme::no_color();
    let (w, h) = (70u16, 8u16);
    let bottom_row = |t: &mut Transcript| -> String {
        let backend = TestBackend::new(w, h);
        let mut term = Terminal::new(backend).expect("backend");
        term.draw(|f| t.draw(f, Rect::new(0, 0, w, h), &theme, 0, ImageProtocol::None))
            .expect("draw");
    ...
    // The scrollbar paints its own bare `v` end-arrow in the gutter, so a badge
    // is only a `v` that carries a count.
    let has_badge = |row: &str| {
        row.split("v ")
            .skip(1)
    ...
    for i in 0..4u64 {
        t.push(RenderBlock::TextDelta {
            id: BlockId(300 + i),
            text: format!("late arrival {i} with body text long enough to fill a row"),
            done: true,
        });
    }
    assert!(
        has_badge(&bottom_row(&mut t)),
        "content landing while parked is exactly what the badge must announce"
    );
}
...
#[test]
fn expanded_reasoning_cache_is_compact_reused_and_width_keyed() {
    let theme = Theme::default_dark();
    let mut t = Transcript::new();
    let bid = BlockId(90_010);
    let source = (0..2_000)
        .map(|idx| format!("reasoning row {idx:04} {}", "x".repeat(96)))
        .collect::<Vec<_>>()
        .join("\n");
    ...
    assert!(t.toggle_expand_for_click(bid));

    draw_for_cache_probe(&mut t, 80, 16, &theme);
    match t.rendered_cache.first().and_then(Option::as_ref) {
        Some(RenderCache::Reasoning {
            width,
            row_prefix,
            source_starts,
            ..
            ...
            assert_eq!(
                *width, 79,
                "an overflowing 80-column viewport reserves one scrollbar column"
            );
            assert_eq!(source_starts.len(), 2_000);
            assert_eq!(row_prefix.len(), source_starts.len() + 2);
        }
        other => panic!("expected reasoning viewport cache, got {other:?}"),
    }
    assert!(
        t.styled_cache_bytes() < source_bytes / 2,
        "cache must retain only indexes, not a second body copy: cache={} source={source_bytes}",
        t.styled_cache_bytes()
    );

    let scroll_misses_before = t.render_cache_misses();
    t.scroll_down(500);
    draw_for_cache_probe(&mut t, 80, 16, &theme);
    assert_eq!(
        t.render_cache_misses(),
        scroll_misses_before,
        "deep scrolling must reuse wrap metadata without rebuilding the body"
    );

    let misses_before = t.render_cache_misses();
    draw_for_cache_probe(&mut t, 48, 16, &theme);
    assert!(
        t.render_cache_misses() > misses_before,
        "resize must rebuild width-dependent reasoning wrap metadata"
    );
    assert!(matches!(
        t.rendered_cache.first().and_then(Option::as_ref),
        Some(RenderCache::Reasoning { width: 47, .. })
    ));
}
...
#[test]
fn collapsing_deep_reasoning_clears_previous_viewport_cells() {
    let theme = Theme::no_color();
    let backend = TestBackend::new(72, 12);
    let mut terminal = Terminal::new(backend).expect("backend");
    let mut transcript = Transcript::new();
    let bid = BlockId(90_011);
    let source = (0..1_500)
        .map(|idx| format!("reasoning row {idx:05}"))
        .collect::<Vec<_>>()
        .join("\n");
    transcript.push(RenderBlock::Reasoning {
        id: bid,
        text: source,
        signature: None,
        done: true,
    });
    assert!(transcript.toggle_expand_for_click(bid));
    ...
    terminal
        ...
        .expect("initial expanded draw");
    transcript.scroll_down(500);
    terminal
        .draw(|frame| {
            transcript.draw(
                frame,
        ...
        .expect("deep expanded draw");
    let expanded = dump_terminal(&terminal, 72, 12);
    assert!(
        expanded.contains("reasoning row 00499"),
        "deep scroll must land on the indexed source row: {expanded}"
    );
    ...
    terminal
        ...
        .expect("collapsed redraw");
    let collapsed = dump_terminal(&terminal, 72, 12);
    assert!(
        !collapsed.contains("reasoning row 00499") && !collapsed.contains("work steps"),
        "collapsing must clear every stale expanded cell: {collapsed}"
    );
}
...
fn sample_conversation_shows_codex_style_events_without_global_tool_rail() {
    ...
    // The user rail and assistant author bullets attribute adjacent prompt/answer
    // rows so a user request and the model's first sentence do not read as one
    // author. The user side spends no row on a `You` label: its `┃` rail and
    // background band carry the attribution, so the mark has to be on the prose
    // row itself.
    assert!(
        !dump.contains("You"),
        "the `You` label row must not come back — the rail attributes the turn:\n{dump}"
    );
    ...
    // The rail rides the prompt's own first row, not a label row above it.
    assert!(
        dump.contains("\u{2503}  아"),
        "the user rail must start on the prompt's first body row:\n{dump}"
    );
    assert_eq!(
...
#[test]
fn styled_cache_retains_a_multiple_of_the_body_bytes_it_renders() {
    // The eviction work needs a measured "before": MAX_TRANSCRIPT_BYTES counts
    // only raw body bytes, so the styled cache is the unaccounted half. Measure
    // it on a real transcript rather than deriving it from struct sizes, which
    // is what the render-performance workflow asks for before optimizing.
    let theme = Theme::default_dark();
    let width = 80u16;
    let mut t = Transcript::new();
    let body = "fn main() { let value = compute(&input); println!(\"{value}\"); }\n".repeat(40);
    ...
    // Height is what builds the styled lines, so this fills the cache.
    let _ = t.cached_block_height(0, width, &theme, ImageProtocol::None);

    let cached = t.styled_cache_bytes();
    // A band, not a point value: the multiple depends on how many spans the
    // highlighter emits per line, and pinning it exactly would break on any
    // styling change. The floor catches an accounting regression that stops
    // seeing the cache at all; the ceiling catches pathological fragmentation.
    assert!(
        cached > body_bytes + body_bytes / 2 && cached < body_bytes * 20,
        "retained multiple outside the measured band: cached={cached} body={body_bytes}"
    );
}

#[test]
fn shedding_reclaims_offscreen_lines_without_moving_any_block() {
    // The correctness property that makes eviction possible at all: heights come
    // from `body_rows`/`row_prefix`, never from `lines`, so dropping the styled
    // output must leave every block at exactly the same row. If that ever stops
    // holding, the transcript scrolls under the user.
    let theme = Theme::default_dark();
    let width = 80u16;
    ...
    for _ in 0..40 {
        t.push(RenderBlock::TextDelta {
            id: id(),
            text: "prose that occupies a few rows once wrapped at eighty columns\n".repeat(4),
            done: true,
        });
    }
    let group_leader = t.len() + SETTLED_GROUP_LEADER;
    for block in settled_group_transcript().blocks {
        t.push(block);
    }
    t.ensure_layout(width, &theme, ImageProtocol::None);
    assert!(
        matches!(
            &t.rendered_cache[group_leader],
            Some(RenderCache::Group { lines, .. }) if !lines.is_empty()
        ),
        "the offscreen settled group must warm its styled cache before shedding"
    );

    let layout_before = t.cached_layout.clone();
    let group_height_before =
        t.cached_block_height(group_leader, width, &theme, ImageProtocol::None);
    let bytes_before = t.styled_cache_bytes();
    assert!(
        bytes_before > 0,
        "the styled cache must be warm before shedding means anything"
    );

    let shed = t.shed_offscreen_styled_lines(viewport_h);
    let bytes_after = t.styled_cache_bytes();

    assert!(
        shed > 0,
        "blocks past the keep margin must be shed (layout rows: {})",
        layout_before.len()
    );
    // A ratio, not just "smaller": with three screens kept out of ~40 blocks the
    // bulk of the styled cache is off-margin, so a reclaim that only shaves a few
    // percent would mean the walk is skipping entries it should shed. The
    // assertion message carries the measured pair either way.
    assert!(
        bytes_after * 2 < bytes_before,
        "shedding must reclaim over half the styled cache: {bytes_before} -> {bytes_after}"
    );
    assert_eq!(
        t.cached_layout, layout_before,
        "shedding must not move or resize any block"
    );
    assert!(
        matches!(
            &t.rendered_cache[group_leader],
        ...
        "an offscreen Group cache must shed its styled lines without losing its height"
    );
    // A visible draw rewarms the empty cache through `cached_block_height`;
    // calling that same layout primitive here catches a permanent cache miss
    // without depending on terminal-frame setup.
    let group_height_after =
        t.cached_block_height(group_leader, width, &theme, ImageProtocol::None);
    assert_eq!(
        group_height_after, group_height_before,
        "rewarming a shed Group must preserve its cached height"
    );
    assert_eq!(
        t.cached_layout, layout_before,
        "rewarming a shed Group must preserve its cached layout"
    );
    assert!(
        matches!(
            &t.rendered_cache[group_leader],
        ...
        "a Group must rewarm after its styled lines were shed"
    );
    // Scroll is 0 and the margin is three screens, so the first block is inside
    // the keep window and must still be drawable from cache.
    assert!(
        matches!(
            &t.rendered_cache[0],
            Some(super::RenderCache::Text { lines, .. }) if !lines.is_empty()
        ),
        "a block inside the keep margin must retain its styled lines"
    );
}
...
fn ultrawide_user_message_paints_every_measured_row() {
    ...
    assert_eq!(
        height, 2,
        "two 180-cell body rows and nothing else — the `You` header row is gone, \
         the rail and background band attribute the turn instead"
    );
    match &t.rendered_cache[0] {
        Some(RenderCache::Text { width, .. }) => assert_eq!(*width, 180),
    ...
    let row_text = |y: u16| -> String { (0..width).map(|x| buf[(x, y)].symbol()).collect() };
    let dump: Vec<String> = (0..view_h).map(row_text).collect();
    // Every measured row is a body row now — there is no header row to discount.
    let painted_body_rows = dump.iter().filter(|row| row.contains('x')).count();
    assert_eq!(painted_body_rows, usize::from(height), "{dump:#?}");
    // The rail starts on row 0 for the same reason.
    for y in 0..height {
        assert_eq!(
            buf[(0, y)].symbol(),
            "|",
...
#[test]
fn click_on_reasoning_toggles_full_body() {
    let theme = Theme::no_color();
    let mut t = Transcript::new();
    t.push(RenderBlock::Reasoning {
        id: BlockId(21),
        text: "inspect matching route\nverify query filters".to_string(),
        signature: None,
        done: true,
    });

    assert!(t.toggle_expand_for_click(BlockId(21)));
    assert!(t.is_expanded(0), "reasoning click opens the full body");
    ...
    terminal
        ...
        .expect("draw");
    let rendered = dump_terminal(&terminal, 72, 8);
    assert!(
        rendered.contains("v work steps")
            && rendered.contains("inspect matching route")
            && rendered.contains("verify query filters"),
        "expanded reasoning exposes its header and complete body: {rendered}"
    );

    assert!(t.toggle_expand_for_click(BlockId(21)));
    assert!(!t.is_expanded(0), "second click collapses reasoning");
}

/// Distinct 2-cell graphemes, one per index — see the pager tests for why the
/// fixture is a run of wide glyphs rather than prose: the defect keys on
/// display width, not on any language.
...
/// The cached tool-result paint is safe only because its lines already fit.
///
/// That arm windows cached lines and paints them. It is *not* allowed to lean
/// on `Paragraph::wrap(...)` to do the folding: ratatui's `WordWrapper` decides
/// a row is full before adding the current glyph's width, so a 2-cell grapheme
/// landing on the break is discarded outright while the row count still looks
/// right — characters vanish mid-sentence.
///
/// Today the arm is safe for a second reason: `rendered_lines_for_width` hands
/// the cache lines that already fit the render width, so any wrap is a no-op.
/// That is the invariant asserted here. If it ever stops holding, the paint
/// must still be lossless — so the glyph check is asserted alongside it rather
/// than assumed from it.
#[test]
fn cached_tool_result_lines_fit_their_width_and_paint_every_glyph() {
    const WORDS: usize = 8;
    ...
    for width in crate::tui::layout::MIN_USABLE_WIDTH..=widest {
        ...
        t.push(RenderBlock::ToolCall {
            id: id(),
            tool_call_id: ToolCallId("call_wide".to_string()),
            name: "Bash".to_string(),
            summary: "echo".to_string(),
            preview: ToolPreview::Generic {
                name: "Bash".to_string(),
        ...
        let mut terminal = Terminal::new(TestBackend::new(width, height)).expect("backend");
        // Draw twice: the first pass fills `rendered_cache`, the second is the
        // cache-hit arm this test is about.
        for _ in 0..2 {
            terminal
                .draw(|f| {
                    t.draw(
        ...
        for entry in t.rendered_cache.iter().flatten() {
            let RenderCache::Tool { lines, .. } = entry else {
                continue;
            };
            for line in lines {
                ...
                assert!(
                    ...
                    "width {width}: cached tool line is {cells} cells — the paint arm \
                     would have to fold it, which its wrapper must do losslessly"
                );
            }
        }

        let painted = dump_terminal(&terminal, width, height);
        for ch in &expected {
            assert!(
                painted.contains(*ch),
                "width {width}: `{ch}` was dropped by the cached tool-result paint"
            );
        }
    }
}
