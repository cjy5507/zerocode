...
impl Transcript {
    ...
    /// Mirror the app's follow-output intent: `true` once the reader chose to
    /// stop following the tail. Gates the backlog badge, which must not fire
    /// for a viewport that merely sits above the tail without the reader ever
    /// having left it.
    pub fn set_user_parked(&mut self, parked: bool) {
        self.user_parked = parked;
    }
    ...
    /// Scroll offset the last layout pass resolved. Paired with the block count
    /// it tells a mouse gesture whether the rows moved under the pointer
    /// between press and release — mid-turn they usually do, because the tail
    /// keeps growing and the view stays pinned to it.
    #[must_use]
    pub fn resolved_scroll(&self) -> u16 {
        self.resolved_scroll
    }
    ...
    /// Rows of content sitting below the viewport, or 0 when the tail is on
    /// screen.
    ///
    /// Leaving the tail already stops follow-output from snapping, but nothing
    /// told the reader how much arrived while they read back — the transcript
    /// grew silently off-screen. This is the count behind that indicator.
    #[must_use]
    pub fn lines_below_viewport(
        &self,
        viewport_h: u16,
        _theme: &Theme,
        width: u16,
        ...
        // Same cache-validity rule as `is_at_bottom`: the draw pass lays out at
        // `width` or `width - 1` (scrollbar gutter). Without a valid layout the
        // totals are stale, and reporting a wrong backlog is worse than none.
        if self.cached_layout.is_empty()
            || (self.cached_layout_width != width
                && self.cached_layout_width != width.saturating_sub(1))
        {
        ...
        self.content_total()
            ...
            .saturating_sub(self.resolved_scroll)
    }
