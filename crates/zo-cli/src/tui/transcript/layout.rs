...
impl Transcript {
    ...
    fn text_block_height(&mut self, idx: usize, width: u16, theme: &Theme) -> Option<u16> {
        ...
        // Neither role carries a header row. The user turn is identified by its
        // `┃` rail and background band, and assistant prose by the `◆` bullet
        // riding its first body row — so no block spends a row on a label.
        // Kept as a named zero rather than deleted: every use below reads
        // `header_row`, so the invariant "measure adds exactly what paint draws"
        // stays visible in one place instead of becoming an implicit 0.
        let header_row: u16 = 0;
        ...
        let cached_body = match &self.rendered_cache[idx] {
            Some(RenderCache::Text {
                ...
                && *cached_done == done => Some(*body_rows),
            _ => None,
        };
    ...
    pub(super) fn cached_block_height(
        ...
        // Expanded reasoning uses a compact viewport index rather than a second
        // owned copy of the text. A cache fill performs the full wrap once;
        // later layout passes read height in O(1), and draw binary-searches the
        // row prefix to build only visible borrowed spans.
        if let RenderBlock::Reasoning { id, text, done, .. } = &self.blocks[idx] {
            ...
            if !expanded {
                return blocks::reasoning::estimate_rows(
                    text, *done, theme, focused, false, width, elapsed, id.0,
                );
            }
            if self.rendered_cache.len() <= idx {
                self.rendered_cache.resize_with(self.blocks.len(), || None);
            }
            let content_version = self.render_versions.get(idx).copied().unwrap_or_default();
            let theme_fp = self.render_cache_theme_fp;
            let cached_height = match &self.rendered_cache[idx] {
            ...
            self.render_cache_misses = self.render_cache_misses.saturating_add(1);
            let body_width = width.saturating_sub(blocks::reasoning::gutter_width()).max(1);
            let source_starts = blocks::reasoning::source_line_starts(text);
            let row_prefix = blocks::reasoning::expanded_row_prefix(
                text,
                &source_starts,
                *done,
                theme,
            ...
            debug_assert_eq!(
                row_prefix.len(),
                source_starts.len().saturating_add(2),
                "reasoning cache metadata must cover header plus source lines"
            );
            let body_rows =
                u16::try_from(row_prefix.last().copied().unwrap_or(0)).unwrap_or(u16::MAX);
            self.rendered_cache[idx] = Some(RenderCache::Reasoning {
                content_version,
                theme_fp,
                width,
                done: *done,
