...
impl InputWidget {
    ...
    /// Drop the whole image badge — the attachments have left the composer.
    ///
    /// The counter mirrors what is still attached *here*, so submitting has to
    /// clear it: the queued path re-stages the badge when the entry pops
    /// (`App::stage_queued_images_for_submit`), and without this nothing ever
    /// took it back down again.
    ...
    /// to `inner_width`. Includes the composer's chrome — the bottom border plus
    /// the blank rows of air that centre the text between the rules (see
    /// [`input_block`]; there is no top edge) — and is clamped to
    /// `INPUT_MIN_ROWS..=INPUT_MAX_ROWS`; content beyond the cap scrolls inside
    /// the box (see [`Self::draw`]). This asks for the full
    /// [`ComposerAir::Centered`] allowance; `draw` degrades to less air if the
    /// layout could not grant it. The caller feeds this into
    /// [`super::layout::LayoutRegions::compute_with_sidebar`] so the box
    /// auto-grows.
    #[must_use]
    pub fn desired_rows(&self, inner_width: u16, mode: AppMode) -> u16 {
        ...
        // 1 closing border + the centred air.
        let chrome = 1 + ComposerAir::Centered.rows();
    ...
    pub fn draw(
        ...
        self.draw_with_heat(frame, area, theme, mode, HeatState::Cold, None);
    }

    /// Render the widget with draw-time chrome temperature styling.
    ///
    /// The composer is wrapped in a full border whose colour encodes the
    /// current reasoning `effort` (see [`effort_border_style`]); text styling is
    /// unchanged. `heat_state` still tints the prompt glyph. `None` effort (no
    /// level set) renders the neutral off hue.
    ...
    pub fn draw_with_heat(
        ...
        // One closing rule whose colour encodes reasoning effort, plus a blank
        // row of air above the text (see [`input_block`]). Heat is a different
        // signal — effort is a setting, heat says whether a turn is running — and
        // it rides the prompt glyph below, which is why removing the left rail
        // costs no information.
        //
        // The air is the first thing sacrificed when the composer is squeezed —
        // see [`ComposerAir::for_height`].
        let block = input_block(ComposerAir::for_height(area.height))
            .border_style(effort_border_style(theme, effort));

        // Compute the inner area (content area between the rules).
...
/// The composer's frame: one horizontal rule *below* the text, no top edge, no
/// side borders, and one blank row of air above the text.
///
/// This is the shape Pi's editor uses; the prompt glyph, not a box, is what
/// marks where input begins. The colour is applied by the caller from the
/// current reasoning effort via [`effort_border_style`].
///
/// There is deliberately no top border. The composer sits directly beneath
/// `layout`'s `rule_top` row, which already paints a full-width hairline when
/// idle — a top border here put *two* full-width rules in adjacent rows with
/// nothing between them, which is the stacked-rule noise the Pi pass was meant to
/// remove. `rule_top` carries the effort colour on that hairline instead, so
/// dropping the edge costs no signal.
///
/// The blank row is not decoration. While a turn streams, `rule_top` is not a
/// rule at all — it is the live spinner (`+ Planning · model · 87% ctx …`), so
/// with the border gone its text landed on the row immediately above the input
/// text: two lines of live content touching, with nothing between them. A blank
/// row separates them for the same one row a second rule would have cost, and
/// reads as air rather than chrome.
///
/// The air is *symmetric* so the text reads as vertically centered between the
/// two rules bracketing it. With padding only above, the row was the second of
/// two inside the band and visibly sat low.
///
/// `air` is chosen by the caller from the height actually granted, so a squeezed
/// composer spends its rows on text rather than on whitespace: both rows when
/// there is room, one above when there is only room for one (never touching the
/// spinner is worth more than symmetry), and none at all below that. The left
/// edge stays open for the prompt, while the right rail joins the closing rule so
/// an ultra-wide composer has a visible endpoint.
fn input_block(air: ComposerAir) -> Block<'static> {
    Block::default()
        .borders(Borders::RIGHT | Borders::BOTTOM)
        .border_type(BorderType::Plain)
        .padding(match air {
            ComposerAir::None => Padding::ZERO,
            ComposerAir::Above => Padding::top(1),
            ComposerAir::Centered => Padding::vertical(1),
        })
}

/// Width available to composer text and to the rule/badges above it. Keeping
/// this in the input module makes measurement, paint, and the right rail agree
/// on the same one-cell reservation.
#[must_use]
pub(crate) const fn composer_content_width(outer_width: u16) -> u16 {
    outer_width.saturating_sub(1)
}

/// How much blank space the composer can spend around its text.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum ComposerAir {
    /// No room for any: every row goes to text.
    None,
    /// One row above, so the streaming spinner never touches the input.
    Above,
    /// One row above and one below — the text reads as centered.
    Centered,
}

impl ComposerAir {
    /// Rows of blank space this variant consumes.
    ...
    /// Pick the richest air a composer of `height` rows can afford. `height`
    /// includes the closing border, so the thresholds are one higher than the
    /// air itself: 4 rows = border + 2 air + 1 text.
...
/// Border style for the composer, keyed to the current reasoning `effort`.
///
/// Color groups effort into three glanceable driving modes: Low=Eco,
/// Medium=Normal, and High through Smart=Sport. Exact effort remains visible in
/// the HUD and picker, so hue is not the only signal. `None` and Off stay
/// neutral; [`Theme::no_color`] keeps the rules but drops their hue.
///
/// Shared with the frame renderer: the composer has no top border, so the
/// `rule_top` hairline directly above it is styled from here too and the pair of
/// lines bracketing the composer keep one colour.
pub(crate) fn effort_border_style(theme: &Theme, effort: Option<Effort>) -> Style {
    if theme.no_color {
        return Style::default();
    }
    let color = effort.map_or(theme.palette.thinking_off, |effort| {
        super::modals::effort_picker::effort_mode_color(effort, theme)
    });
...
fn mode_tag_for(mode: AppMode) -> &'static str {
    match mode {
        ...
        AppMode::ModalProviders => "/providers ",
...
mod tests {
    use super::{
        Effort, HeatState, InputWidget, effort_border_style, input_block,
        strip_sgr_mouse_sequences_from_text, wrap_segments,
    };
    ...
    /// The composer keeps its left edge for the prompt and spends exactly one
    /// column on the right rail. Its vertical air remains unchanged.
    #[test]
    fn the_bottom_rule_and_right_rail_reserve_one_column() {
        use ratatui::layout::Rect;
        use super::ComposerAir;

        let area = Rect::new(7, 11, 30, 6);
        // Centred: a row of air above and below, then the closing rule.
        assert_eq!(
            input_block(ComposerAir::Centered).inner(area),
            Rect::new(7, 12, 29, 3)
        );
        // Squeezed: air above only, so the spinner still cannot touch the text.
        assert_eq!(
            input_block(ComposerAir::Above).inner(area),
            Rect::new(7, 12, 29, 4)
        );
        // Squeezed harder: every row but the rule goes to text.
        assert_eq!(
            input_block(ComposerAir::None).inner(area),
            Rect::new(7, 11, 29, 5)
        );
    }

    /// The air degrades with the height actually granted, richest first.
    #[test]
    fn composer_air_degrades_with_height() {
        use super::ComposerAir;

        assert_eq!(ComposerAir::for_height(10), ComposerAir::Centered);
        assert_eq!(ComposerAir::for_height(4), ComposerAir::Centered);
        assert_eq!(ComposerAir::for_height(3), ComposerAir::Above);
        assert_eq!(ComposerAir::for_height(2), ComposerAir::None);
        assert_eq!(ComposerAir::for_height(0), ComposerAir::None);
    }
    ...
    fn prompt_and_mode_tag_follow_heat_state() {
        ...
        let cell_at = |heat_state, mode, x| {
            ...
            terminal
                ...
                .expect("input draws");
            // Row 1: no top border, but a blank row of air above the text.
            terminal.backend().buffer()[(x, 1)].clone()
        };

        // Column 0: with no side border the prompt starts at the very edge.
        let cold = cell_at(HeatState::Cold, AppMode::Normal, 0);
        let hot = cell_at(HeatState::Hot, AppMode::Normal, 0);
        ...
        let mode_tag = cell_at(HeatState::Hot, AppMode::ModalModel, 2);
    ...
    fn desired_rows_grows_with_soft_wrapped_long_line() {
        ...
        // Inner width 12 → text width after the 2-cell prompt gutter is 10, so 26
        // chars wrap onto 3 display rows → 3 + 1 bottom border + 2 rows of
        // centring air = 6.
    ...
    #[test]
    fn input_card_closes_the_right_edge_without_consuming_the_prompt_column() {
        use crate::tui::theme::Theme;
        use ratatui::Terminal;
        use ratatui::backend::TestBackend;
        use ratatui::layout::Rect;
        ...
        assert_eq!(buf[(0, 1)].symbol(), "❯", "the left edge stays open");
        for y in 0..3 {
            assert_eq!(
                buf[(29, y)].symbol(),
                "│",
                "the right rail must stay visible at row {y}"
            );
        }
        assert_eq!(
            buf[(29, 3)].symbol(),
            "┘",
            "the bottom rule must terminate in a right corner"
        );
        assert!(
            (0..29).all(|x| buf[(x, 3)].symbol() == "─"),
            "the closing rule must reach the right corner"
        );
    }

    #[test]
    fn border_colour_encodes_three_reasoning_modes() {
        use crate::tui::theme::Theme;

        let theme = Theme::zo();
        let eco = effort_border_style(&theme, Some(Effort::Low));
        let normal = effort_border_style(&theme, Some(Effort::Medium));
        let sport = effort_border_style(&theme, Some(Effort::High));
        assert_eq!(eco.fg, Some(theme.palette.thinking_low));
        assert_eq!(normal.fg, Some(theme.palette.thinking_medium));
        assert_eq!(sport.fg, Some(theme.palette.thinking_max));
        assert_ne!(eco.fg, normal.fg);
        assert_ne!(normal.fg, sport.fg);
        assert_ne!(eco.fg, sport.fg);
        ...
        for effort in [
            ...
            assert_eq!(
                effort_border_style(&theme, Some(effort)).fg,
                sport.fg,
                "{effort:?} must stay in Sport"
            );
        }
    }

    #[test]
    fn border_renders_structurally_but_without_hue_under_no_color() {
        use crate::tui::theme::Theme;
        use ratatui::Terminal;
        use ratatui::backend::TestBackend;
        use ratatui::layout::Rect;
        ...
        // No effort hue is applied when colour is disabled.
        assert_eq!(effort_border_style(&theme, Some(Effort::Max)).fg, None);

        // The border box still renders structurally (corners + rules present),
        // it simply carries no colour.
        let mut input = InputWidget::new();
        for c in "hi".chars() {
            input.insert_char(c);
        }
        ...
        // The rules still render structurally under NO_COLOR; only the hue is
        // dropped. `─` has no ASCII fallback obligation because it is the same
        // glyph a colour terminal draws — there is no box to degrade.
        assert!(dump.contains('\u{2500}'), "structural rules survive NO_COLOR ─:\n{dump}");
        assert_eq!(
            buf[(0, 0)].fg,
            ratatui::style::Color::Reset,
            "NO_COLOR carries no hue on the rule:\n{dump}"
        );
    }
