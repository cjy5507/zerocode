...
/// Corners joining a floating card's closing rules to its left anchor rail.
///
/// Light weight on purpose: they sit between [`HORIZONTAL_RULE`] and
/// [`VERTICAL_SEP`], which are both light, so the frame reads as one continuous
/// stroke. [`ANVIL_CORNER`] is heavy and would thicken exactly the two cells the
/// eye uses to judge whether the bracket is closed.
pub const CARD_CORNER_TOP: &str = "\u{250c}";
pub const CARD_CORNER_BOTTOM: &str = "\u{2514}";
pub const CARD_CORNER_NC: &str = "+";
