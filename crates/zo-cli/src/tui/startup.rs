...
/// Where the launchpad sits inside a region taller than its content.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Anchor {
    /// Near the top, with a small proportional pad — the full-screen look.
    Top,
    /// Flush against the bottom edge. Inline mode uses this: the launchpad is
    /// the newest thing on screen and a terminal fills upward from the prompt,
    /// so any slack belongs above it, not between it and the composer.
    Bottom,
}

/// Render the startup launchpad into `area`, anchored at the top.
///
...
pub fn draw(
    ...
    draw_anchored(frame, area, startup, theme, intro, Anchor::Top);
}

/// [`draw`] with an explicit [`Anchor`].
///
/// `Bottom` sinks by the launchpad's *rendered* line count rather than
/// [`preferred_height`], which is an upper estimate — anchoring on the estimate
/// left a stray blank row between the last hint line and the composer.
pub fn draw_anchored(
    ...
    // Sink against the ORIGINAL `area`, not `content_area`: the latter already
    // applied the Top layout's proportional pad and height cap, which are what
    // the bottom anchor is replacing. Everything above (line building, compact
    // decisions, session capacity) still reads `content_area`, so only the
    // placement changes.
    let used = u16::try_from(lines.len()).unwrap_or(u16::MAX);
    let content_area = match anchor {
        Anchor::Top => content_area,
        Anchor::Bottom => match area.height.checked_sub(used).filter(|gap| *gap > 0) {
            Some(gap) => Rect::new(
                content_area.x,
            ...
            None => Rect::new(content_area.x, area.y, content_area.width, area.height),
        },
    };
...
fn info_line(startup: &StartupScreen, theme: &Theme) -> Line<'static> {
    ...
    let model_label = compact_model_label(&startup.model);
...
fn compact_model_label(model: &str) -> String {
    ...
    if let Some(rest) = leaf.strip_prefix("claude-") {
        let mut parts = rest.split('-');
        let Some(family) = parts.next() else {
            return resolved;
        };
        let mut label = family.to_ascii_uppercase();
        label.get_mut(1..).map(str::make_ascii_lowercase);
        let release = parts
            .take(2)
        ...
        if !release.is_empty() {
            label.push(' ');
            label.push_str(&release.join("."));
        }
        return label;
    }
    if leaf.starts_with("gpt-") {
        return leaf.replacen("gpt", "GPT", 1);
    }
    if leaf.starts_with("gemini-") {
        return leaf.replacen("gemini", "Gemini", 1);
    }
    leaf.to_string()
}
