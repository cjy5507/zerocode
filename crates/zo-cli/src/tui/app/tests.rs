...
use crate::tui::{INLINE_VIEWPORT_MIN_HEIGHT, TerminalMode};
...
/// Lowest row inside `region` that the transcript actually painted. A washed
/// block's band is made of spaces carrying a background, so "painted" has to
/// mean symbol *or* background — the defect this guards is a wash band, not
/// text, sitting flush against the activity row.
fn last_painted_row(terminal: &Terminal<TestBackend>, region: Rect) -> Option<u16> {
    let buffer = terminal.backend().buffer();
    let content_right = region.x + region.width.saturating_sub(1);
    (region.y..region.y + region.height).rev().find(|&y| {
        (region.x..content_right).any(|x| {
            let cell = &buffer[(x, y)];
...
/// Screenshot defect: while a turn streamed, the last transcript block's wash
/// band sat on the transcript region's bottom edge with the `✦ Thinking…`
/// activity row directly beneath it — the two read as one glued band. A live
/// turn keeps one row of air; a settled turn is unchanged.
#[test]
fn live_turn_keeps_one_slack_row_above_the_activity_line() {
    let mut app = test_app_with_theme(Theme::default_dark());
    app.set_terminal_mode(TerminalMode::Inline);
    app.enable_input();
    app.push_block(RenderBlock::UserMessage {
        id: BlockId(1),
        text: "wash band probe".to_string(),
    });
    ...
    app.draw(&mut terminal).expect("live draw");

    let region = app.regions.expect("regions").transcript;
    let live_bottom = last_painted_row(&terminal, region).expect("live transcript content");
    assert_eq!(
        live_bottom + 3,
        region.y + region.height,
        "a live turn shows the sink's breathing row plus the live-tail slack row"
    );

    // Settling releases the live reservation but keeps one blank row of air:
    // a settled answer glued to the composer rule was the readability defect
    // the bottom-aligned sink now guards against.
    app.end_turn();
    app.draw(&mut terminal).expect("settled draw");
    let region = app.regions.expect("regions").transcript;
    let settled_bottom = last_painted_row(&terminal, region).expect("settled transcript content");
    assert_eq!(
        settled_bottom + 2,
        region.y + region.height,
        "a settled turn keeps one blank row between the conversation and the composer rule"
    );
}

/// Screenshot defect: a settled answer that overflowed the region (its head
/// shed to native scrollback) had its final text row glued to the composer
/// rule — no air between the conversation and the input line, unlike every
/// terminal-native peer. The settled tail must keep exactly one blank row
/// above the rule.
#[test]
fn settled_overflow_tail_keeps_one_blank_row_above_the_composer() {
    let mut app = test_app_with_theme(Theme::default_dark());
    app.set_terminal_mode(TerminalMode::Inline);
    app.enable_input();
    app.push_block(RenderBlock::UserMessage {
        id: BlockId(1),
        text: "gap probe".to_string(),
    });
    app.begin_turn_with_generation(0);
    let long_answer = (0..60)
        .map(|i| format!("line {i} of a long settled answer"))
        .collect::<Vec<_>>()
        .join("\n");
    ...
    .expect("inline test terminal");
    // Two draws: the overflow seal reads the previous frame's regions, so the
    // row-shed that makes the tail bottom-alignable happens on the second.
    app.draw(&mut terminal).expect("first draw");
    app.draw(&mut terminal).expect("second draw");

    let region = app.regions.expect("regions").transcript;
    let bottom =
        last_painted_row(&terminal, region).expect("settled overflow tail renders content");
    assert!(
        bottom + 2 <= region.y + region.height,
        "the settled tail keeps at least one blank row between the answer and the composer rule"
    );
}

/// The slack row is a live-turn reservation, so it must come out of the drawn
/// viewport the interaction paths clamp against — otherwise scroll math and
/// hit-testing run one row past what was painted.
#[test]
fn live_turn_slack_row_is_reflected_in_the_drawn_transcript_rect() {
    let mut app = test_app();
    app.set_terminal_mode(TerminalMode::Inline);
    app.enable_input();
    app.push_block(RenderBlock::UserMessage {
        id: BlockId(1),
        text: "viewport probe".to_string(),
    });
    ...
    app.draw(&mut terminal).expect("idle draw");
    let idle_rect = app.transcript_view_rect().expect("idle viewport");
    assert_eq!(idle_rect.height, app.regions.expect("regions").transcript.height);

    app.begin_turn_with_generation(0);
    app.draw(&mut terminal).expect("live draw");
    let live_rect = app.transcript_view_rect().expect("live viewport");
    assert_eq!(
        live_rect.height + 1,
        idle_rect.height,
        "the live viewport is one row shorter — the row the activity line is kept off of"
    );
}
...
fn inline_viewport_emits_settled_turn_and_keeps_live_composer() {
    ...
    let backend = TestBackend::new(72, INLINE_VIEWPORT_MIN_HEIGHT);
    let mut terminal = Terminal::with_options(
        backend,
        TerminalOptions {
            viewport: Viewport::Inline(INLINE_VIEWPORT_MIN_HEIGHT),
        },
    )
    .expect("inline test terminal");
    ...
    assert_eq!(
        ...
        0,
        "live inline frames keep history in the app"
    );
    wheel_inline_to_top(&mut app, &mut terminal);
    let oldest = dump_all(&terminal, 72, INLINE_VIEWPORT_MIN_HEIGHT);
    assert!(oldest.contains("inline prompt"), "{oldest}");

    wheel_inline_to_bottom(&mut app, &mut terminal);
    let newest = dump_all(&terminal, 72, INLINE_VIEWPORT_MIN_HEIGHT);
    assert!(
        newest.contains("END-OF-INLINE-ANSWER"),
        "the newest content stays directly above the composer:\n{newest}"
    );
...
#[test]
fn inline_settled_short_conversation_stays_visible_above_the_composer() {
    // The finance-session screenshot: after a short exchange settled, the old
    // model drained the whole transcript into scrollback, leaving a
    // full-screen blank band between the (scrolled-away) conversation and the
    // composer. A short conversation must stay live, bottom-aligned against
    // the input.
    let mut app = test_app();
    app.set_terminal_mode(TerminalMode::Inline);
    app.enable_input();
    ...
    app.push_block(RenderBlock::TextDelta {
        id: BlockId(2),
        text: "안녕하세요! 어떤 도움이 필요하신가요?".to_string(),
        done: true,
    });
    app.end_turn();
    ...
    .expect("inline test terminal");
    app.draw(&mut terminal).expect("draw1");
    app.draw(&mut terminal).expect("draw2");

    let regions = app.regions.expect("regions");
    let dump = dump_all(&terminal, width, height);
    let reply_row = (0..height)
        .find(|&y| buffer_row(&terminal, width, y).contains("안 녕 하 세 요"))
        .unwrap_or_else(|| panic!("the settled reply must stay in the live viewport:\n{dump}"));
    // Bottom-aligned: the reply sits within a few rows of the activity rule
    // above the composer, not at the top of a 54-row region.
    assert!(
        reply_row + 4 >= regions.rule_top.y,
        "settled content must sit against the composer (reply at {reply_row}, rule at {}):\n{dump}",
        regions.rule_top.y
    );
}

#[test]
fn inline_repeated_redraw_handles_a_nonzero_viewport_origin() {
    let mut app = test_app();
    app.set_terminal_mode(TerminalMode::Inline);
    app.enable_input();
    ...
    // A late report can cause another ordinary redraw before teardown. Live
    // frames remain paint-only at the viewport's absolute origin; a separate
    // shutdown test covers the one native-scrollback insertion.
...
fn inline_slash_hint_draw_stays_inside_a_nonzero_viewport() {
    ...
    assert_eq!(frame_area, Rect::new(0, 20, 110, INLINE_VIEWPORT_MIN_HEIGHT));
...
/// An anchored prompt that wants more rows than the inline viewport has still
/// renders, clipped.
///
/// It used to degrade to a "needs full-screen mode" notice, which was the wrong
/// trade for a *blocking* surface: the turn that opened it cannot proceed until
/// it is answered, so hiding it left the session with no reachable answer at
/// all. Clipping loses trailing fields; hiding loses the prompt.
#[test]
fn inline_oversized_anchored_surface_still_renders_clipped() {
    let mut app = test_app();
    app.set_terminal_mode(TerminalMode::Inline);
    app.enable_input();
    ...
    let viewport = dump_all(&terminal, 72, INLINE_VIEWPORT_MIN_HEIGHT);
    assert!(
        !viewport.contains("needs full-screen mode"),
        "blocking prompt must not be replaced by a notice: {viewport}"
    );
    assert!(
        viewport.contains("Connect custom provider"),
        "prompt title must survive the clip: {viewport}"
    );
}
...
#[test]
fn end_turn_lands_required_paced_tail_before_the_settled_frame() {
    let mut app = test_app();
    app.begin_turn_with_generation(0);

    let full = format!("REQUIRED-BEGIN\n{}\nREQUIRED-END", "streamed value ".repeat(40));
    let t0 = std::time::Instant::now();
    app.buffer_paced_at(t0, BlockId(9), full.clone(), true);

    assert!(app.stream_pending(), "precondition: a final tail is still paced");
    assert!(
        !visible_text_deltas(&app).contains("REQUIRED-END"),
        "precondition: the required final value has not reached the transcript yet"
    );
    ...
    assert_eq!(
        visible_text_deltas(&app),
        full,
        "the definitive completed frame must contain every received value"
    );
    assert!(
        !app.stream_pending(),
        "no hidden tail may remain after the turn is presented as complete"
    );
    assert!(
        app.transcript().blocks().iter().any(|block| matches!(
            block,
            RenderBlock::TextDelta { done: true, text, .. } if text.ends_with("REQUIRED-END")
        )),
        "the retained transcript block must be sealed and complete"
    );

    let width = 80;
    let height = 24;
    let mut terminal = Terminal::new(TestBackend::new(width, height)).expect("test terminal");
    app.draw(&mut terminal).expect("draw definitive settled frame");
    let settled_screen = (0..height)
        .map(|row| buffer_row(&terminal, width, row))
        .collect::<Vec<_>>()
        .join("\n");
    assert!(
        settled_screen.contains("REQUIRED-END"),
        "the final required value must be painted in the settled viewport: {settled_screen}"
    );
}
...
#[test]
fn paced_large_utf8_backlog_compacts_without_losing_or_reordering_text() {
    let mut app = test_app();
    app.begin_turn_with_generation(0);

    // Large enough to cross the 16 KiB consumed-prefix compaction threshold
    // several times, with mixed-width UTF-8 boundaries throughout.
    let source = "한국어 🌟 emoji 混在 テスト 🚀 end ".repeat(1_200);
    let t0 = std::time::Instant::now();
    app.buffer_paced_at(t0, BlockId(14), source.clone(), true);

    let frame = std::time::Duration::from_millis(33);
    for i in 1..=32 {
        app.drip_stream_at(t0 + frame * i, Some(frame));
        if !app.stream_pending() {
            break;
        }
    }

    assert!(!app.stream_pending(), "large UTF-8 backlog must terminate");
    assert_eq!(
        visible_text_deltas(&app),
        source,
        "cursor advancement and amortized compaction must preserve exact text"
    );
}
...
fn cooling_animation_request_stops_at_cold_boundary() {
    ...
    let buffer = terminal.backend().buffer();
    // No side borders and no top border, but one blank row of air above the
    // text: the prompt sits at the very first content column of the region's
    // *second* row, and the bottom row is the closing rule. With no effort level
    // set that rule carries the neutral `thinking_off` hue; the caret keeps its
    // Cold `bright` colour, which is the signal this test is actually about.
    let caret = &buffer[(input.x, input.y + 1)];
    let rule = &buffer[(input.x, input.y + input.height - 1)];
    assert_eq!(caret.symbol(), "❯");
    assert_eq!(caret.fg, app.theme.palette.bright);
    assert_eq!(rule.symbol(), "─");
    assert_eq!(rule.fg, app.theme.palette.thinking_off);
...
#[test]
fn provider_retry_notices_are_live_only_and_clear_when_the_turn_settles() {
    for notice in [
        "transient provider error; retrying in 4s (attempt 1)",
        "rate limited; retrying in 8s (attempt 2/5) — HTTP 429",
        "provider overloaded; retrying in 12s (attempt 3)",
    ] {
        let mut app = test_app();
        app.begin_turn_with_generation(0);
        app.push_block(RenderBlock::System {
        ...
        assert_eq!(
            app.turn_activity()
                .expect("retry is part of the active turn")
                .current_action(),
            notice,
            "the retry pause must remain visible as live activity"
        );
        assert!(
            app.transcript().blocks().iter().all(|block| !matches!(
                block,
                RenderBlock::System { text, .. } if text == notice
            )),
            "a transient retry must not become permanent transcript history"
        );
        ...
        assert!(
            ...
            "settling the turn must leave no retry notice behind"
        );
    }
}

#[test]
fn settled_frame_contains_no_provider_retry_notice_cells() {
    let notice = "transient provider error; retrying in 4s (attempt 1)";
    let mut app = test_app_with_theme(Theme::zo());
    app.begin_turn_with_generation(0);
    app.push_block(RenderBlock::System {
        id: BlockId(9),
    ...
    app.turn_activity
        .as_mut()
        .expect("retry is part of the active turn")
        .started_at -= std::time::Duration::from_millis(crate::tui::heat::IGNITION_MS + 1);

    let (width, height) = (140, 30);
    let mut terminal = Terminal::new(TestBackend::new(width, height)).expect("test terminal");
    app.draw(&mut terminal).expect("draw live retry frame");
    let live = buffer_text(&terminal, width, height);
    assert!(
        live.contains("transient provider error"),
        "the retry pause must be visible while the turn is live:\n{live}"
    );

    app.end_turn();
    app.enable_input();
    app.draw(&mut terminal).expect("draw settled frame");
    let settled = buffer_text(&terminal, width, height);
    assert!(
        !settled.contains("transient provider error"),
        "the settled frame must erase the transient retry notice:\n{settled}"
    );
}
...
// --- plan-step chapter notices ---------------------------------------------
// A long turn's transcript gives no answer to "which step was this work for?".
// The App answers it by pushing one `System`/`Info` chapter heading — prefixed
// with `PLAN_CHAPTER_PREFIX` — each time the active plan step changes.

/// Every chapter heading currently in the transcript.
fn plan_chapters(app: &App) -> Vec<String> {
    app.transcript()
        .blocks()
        .iter()
        .filter_map(|block| match block {
            RenderBlock::System { level, text, .. }
                if *level == SystemLevel::Info
                    && text.starts_with(
...
fn todo_step_result(id: u64, call: &str) -> RenderBlock {
    RenderBlock::ToolResult {
        ...
        body: ToolResultBody::Todos(vec![
            runtime::message_stream::TodoResultItem {
                ...
                active_form: "Implementing the parser".to_string(),
                status: TodoResultStatus::InProgress,
            },
            runtime::message_stream::TodoResultItem {
                content: "Verify the parser".to_string(),
                active_form: "Verifying the parser".to_string(),
...
#[test]
fn an_active_plan_step_pushes_one_chapter_notice() {
    let mut app = test_app();
    app.begin_turn_with_generation(0);
    app.push_block(todo_step_result(70, "todo-chapter-1"));

    let chapters = plan_chapters(&app);
    assert_eq!(chapters.len(), 1, "exactly one chapter: {chapters:?}");
    assert!(
        chapters[0].starts_with(crate::tui::blocks::system::PLAN_CHAPTER_PREFIX),
        "{chapters:?}"
    );
    assert!(
        chapters[0].contains("1/2"),
        "the chapter carries the step coordinate: {chapters:?}"
    );
    assert!(
        chapters[0].contains("Implementing the parser"),
        "the chapter uses the step's present-tense sentence: {chapters:?}"
    );
}
...
fn re_applying_the_same_active_step_pushes_no_second_chapter() {
    ...
    // list, a re-write of the same plan). Only a *change of active step* is a
    // new chapter; re-announcing would spam the transcript.
    let mut app = test_app();
    app.begin_turn_with_generation(0);
    app.push_block(todo_step_result(71, "todo-chapter-2"));
    assert_eq!(plan_chapters(&app).len(), 1);

    app.push_block(todo_step_result(72, "todo-chapter-3"));
    assert_eq!(
        plan_chapters(&app).len(),
        1,
        "the same active step must not re-announce itself"
    );
}

#[test]
fn a_restored_session_replay_pushes_no_chapter() {
    // No `turn_activity` means this is replay, not live work: the plan snapshot
    // is history and must not inject headings into a restored transcript.
    let mut app = test_app();
    app.push_block(todo_step_result(73, "todo-chapter-4"));
    assert!(
        plan_chapters(&app).is_empty(),
...
#[test]
fn moving_to_a_new_active_step_pushes_a_second_chapter() {
    let mut app = test_app();
    app.begin_turn_with_generation(0);
    app.push_block(todo_step_result(74, "todo-chapter-5"));
    assert_eq!(plan_chapters(&app).len(), 1);

    // Step 1 completes, step 2 becomes the active one — a new chapter.
    app.push_block(RenderBlock::ToolResult {
        id: BlockId(75),
        tool_call_id: ToolCallId("todo-chapter-6".to_string()),
        is_error: false,
        body: ToolResultBody::Todos(vec![
            runtime::message_stream::TodoResultItem {
                content: "Implement the parser".to_string(),
    ...
    let chapters = plan_chapters(&app);
    assert_eq!(chapters.len(), 2, "a step change is a new chapter: {chapters:?}");
    assert!(
        chapters[1].contains("2/2") && chapters[1].contains("Verifying the parser"),
        "the second chapter names the newly active step: {chapters:?}"
    );
}

#[test]
fn steps_sharing_content_still_chapter_separately() {
    // The dedupe key is position-qualified: two distinct steps that happen to
    // carry identical content must each get their own heading.
    let mut app = test_app();
    app.begin_turn_with_generation(0);
    let twin = |first: TodoResultStatus, second: TodoResultStatus| {
        ToolResultBody::Todos(vec![
            runtime::message_stream::TodoResultItem {
                content: "Deploy the service".to_string(),
                active_form: "Deploying the service".to_string(),
                status: first,
            },
            runtime::message_stream::TodoResultItem {
                content: "Deploy the service".to_string(),
    ...
    app.push_block(RenderBlock::ToolResult {
        id: BlockId(76),
        tool_call_id: ToolCallId("todo-twin-1".to_string()),
        is_error: false,
        body: twin(TodoResultStatus::InProgress, TodoResultStatus::Pending),
    });
    app.push_block(RenderBlock::ToolResult {
        id: BlockId(77),
        tool_call_id: ToolCallId("todo-twin-2".to_string()),
        is_error: false,
        body: twin(TodoResultStatus::Completed, TodoResultStatus::InProgress),
    });
    let chapters = plan_chapters(&app);
    assert_eq!(
        chapters.len(),
        2,
        "identical content in a different position is a different step: {chapters:?}"
    );
    assert!(chapters[0].contains("1/2") && chapters[1].contains("2/2"), "{chapters:?}");
}

#[test]
fn a_read_only_task_list_result_pushes_no_chapter_and_does_not_pin_the_plan() {
    // `TaskList` results arrive as the same typed `Todos` body a mutating
    // `TodoWrite` produces. Merely listing tasks must refresh the checklist
    // without announcing a chapter or pinning the live plan panel.
    let mut app = test_app();
    app.begin_turn_with_generation(0);
    app.push_block(RenderBlock::ToolCall {
        id: BlockId(78),
        tool_call_id: ToolCallId("task-list-1".to_string()),
    ...
    assert!(
        plan_chapters(&app).is_empty(),
        "a listing is not a plan change: {:?}",
        plan_chapters(&app)
    );
    assert!(
        !app.todo_touched_this_turn,
        "a listing must not pin the live plan panel"
    );
    assert_eq!(
        app.hud_state.todo_items.len(),
        1,
        "the checklist itself still refreshes from the listing"
    );
}
...
fn live_todo_panel_border_box_and_overlap_prevention() {
    ...
    let width = 80;
    // 24, not 20: the plan-chapter notice now adds one transcript block per
    // TodoWrite, and this test needs "short log" to stay on screen ABOVE the
    // panel — the pinned contract is overlap geometry, not window capacity.
    let height = 24;
    let backend = TestBackend::new(width, height);
    let mut terminal = Terminal::new(backend).expect("test terminal");
    app.draw(&mut terminal).expect("draw");
...
fn exact_workflow_ids_place_only_the_phase_agent_under_its_plan_step() {
    ...
    let owner_y = rows
        .iter()
        // The plan-chapter notice and the activity line repeat the sentence;
        // the panel row is the one with the in-progress checkbox.
...
fn exact_executor_owner_stays_visible_outside_the_default_plan_window() {
    ...
    assert!(
        // The activity line and chapter notice legitimately name the active
        // step; the pinned contract is about the plan window's checkbox rows.
...
fn height_constrained_plan_dock_keeps_a_plan_row_and_drops_executor_chrome_safely() {
    ...
    // This test is specifically about the height at which *no* executor body row
    // fits, so it tracks the composer's chrome: 11 rows while that chrome costs 2
    // (one closing rule plus a blank row of air). A height on the wrong side of
    // the boundary still asserts the chrome is gone but tests the opposite case —
    // a row that fits — so it must move whenever the composer's row count does.
...
fn mid_turn_slash_enter_queues_without_steering() {
    ...
    assert!(
        queued_hint_notes(&app)
            .iter()
            .any(|note| note.contains("runs after this turn")
                && note.contains("slash commands cannot steer")),
        "a deferred slash entry must say why it did not steer: {:?}",
        queued_hint_notes(&app)
    );

    // A second identical deferral right behind it must not restamp the hint.
    ...
    assert_eq!(
        queued_hint_notes(&app).len(),
        1,
        "back-to-back identical deferrals collapse to one hint: {:?}",
        queued_hint_notes(&app)
    );
}

/// System notes explaining a deferred queue entry, in transcript order.
fn queued_hint_notes(app: &App) -> Vec<String> {
    app.transcript
        .blocks()
        .iter()
        .filter_map(|block| match block {
            RenderBlock::System { text, .. } if text.starts_with("queued \u{00b7}") => {
                Some(text.clone())
            }
            _ => None,
        })
...
#[test]
fn mid_turn_image_entry_explains_that_it_cannot_steer() {
    // The steering channel is text-only, so an image-bearing entry waits for
    // its own turn — and must say so instead of going silent.
    let (mut app, mut cmd_rx) = test_app_with_cmd();
    app.begin_turn_with_generation(0);
    app.disable_input();
    app.push_clipboard_image("image/png".to_string(), "AAAA".to_string())
        .expect("stage clipboard image");
    for ch in "here is the mockup".chars() {
        app.handle_key(press(KeyCode::Char(ch))).expect("typed");
    }
    app.handle_key(press(KeyCode::Enter)).expect("enter");
    ...
    assert!(
        cmd_rx.try_recv().is_err(),
        "image entries must not ride the steering channel"
    );
    assert!(
        queued_hint_notes(&app)
            .iter()
            .any(|note| note.contains("messages with images cannot steer")),
        "an image entry must explain the deferral: {:?}",
        queued_hint_notes(&app)
    );
}

#[test]
fn mid_turn_head_of_line_guard_explains_the_deferral() {
    // Once an unsteered entry sits in the queue, later plain text stops
    // steering to preserve FIFO order. Without a hint the user only sees
    // typing silently stop cutting in.
    let (mut app, mut cmd_rx) = test_app_with_cmd();
    app.begin_turn_with_generation(0);
    app.disable_input();
    for ch in "/status".chars() {
    ...
    app.handle_key(press(KeyCode::Enter)).expect("enter");
    for ch in "also update the docs".chars() {
        app.handle_key(press(KeyCode::Char(ch))).expect("typed");
    }
    app.handle_key(press(KeyCode::Enter)).expect("enter");
    ...
    assert!(
        cmd_rx.try_recv().is_err(),
        "nothing may steer past an unsteered entry"
    );
    assert!(
        queued_hint_notes(&app)
            .iter()
            .any(|note| note.contains("keeps its place behind an earlier queued entry")),
        "the head-of-line guard must explain itself: {:?}",
        queued_hint_notes(&app)
    );
}

#[test]
fn steered_entry_gets_no_deferral_hint() {
    // Plain text with a clear queue steers; its receipt is the runtime's
    // `⤷ steering:` echo, so it must NOT also print a deferral hint.
    let (mut app, mut cmd_rx) = test_app_with_cmd();
    app.begin_turn_with_generation(0);
    app.disable_input();
    for ch in "focus on tests".chars() {
    ...
    assert!(
        matches!(cmd_rx.try_recv(), Ok(AgentCommand::Steer(text)) if text == "focus on tests"),
        "plain text with an all-steered queue must steer"
    );
    assert!(
        queued_hint_notes(&app).is_empty(),
        "a steered entry must not print a deferral hint: {:?}",
        queued_hint_notes(&app)
    );
}

#[test]
fn no_steer_reason_classifies_slash_images_and_head_of_line() {
    use super::queue::NoSteerReason;
    assert_eq!(
        NoSteerReason::classify(true, false, true),
        Some(NoSteerReason::SlashCommand)
    );
    assert_eq!(
        NoSteerReason::classify(false, true, true),
        Some(NoSteerReason::Images)
    );
    assert_eq!(
        NoSteerReason::classify(false, false, false),
        Some(NoSteerReason::BehindUnsteered)
    );
    assert_eq!(NoSteerReason::classify(false, false, true), None);
    for reason in [
        NoSteerReason::SlashCommand,
        NoSteerReason::Images,
        NoSteerReason::BehindUnsteered,
    ] {
        assert!(
            reason.hint().contains("runs after this turn"),
            "every hint must say WHEN the entry runs: {}",
            reason.hint()
        );
    }
}
...
/// 스트리밍 중 클릭: press 와 release 사이에 tail 이 자라 행이 밀려도 클릭은
/// press 가 겨눈 카드로 응답해야 한다. 릴리즈 셀을 다시 풀어 블록을 대조하던
/// 엄격 검사는 살아있는 턴에서 사실상 매번 어긋나 — 정확히 에이전트가 도는
/// 동안 — 클릭을 통째로 삼켰다.
#[test]
fn click_on_agent_card_survives_transcript_growth_between_press_and_release() {
    let mut app = test_app();
    app.push_block(RenderBlock::ToolCall {
        id: BlockId(7),
        tool_call_id: ToolCallId("call-bg".to_string()),
    ...
    let (col, row) = (transcript.x.saturating_add(1), transcript.y);
    let at_pointer = |column, row| MouseEvent {
    ...
    // 턴은 계속 스트리밍된다: 새 블록이 붙고 뷰가 tail 에 다시 고정되면서
    // 포인터가 놓인 화면 행이 다른 블록 위로 밀린다.
    for idx in 0..20u64 {
        app.push_block(RenderBlock::TextDelta {
            id: BlockId(100 + idx),
            text: format!("streamed row {idx}"),
            done: true,
        });
    }
    app.draw(&mut terminal).expect("redraw after tail growth");
    assert_ne!(
        app.block_id_at_mouse(at_pointer(col, row)),
        Some(BlockId(7)),
        "회귀의 전제: 릴리즈 셀은 이제 다른 블록을 가리킨다"
    );

    let release = app
        .handle_mouse(at_pointer(col, row))
        .expect("release handled");
    assert_eq!(
        release,
        AppAction::OpenWorkflowViewer,
        "클릭은 press 가 겨눈 에이전트 카드의 상세를 열어야 한다"
    );
}
...
/// The footer's anchor row — the one naming the model, cost, and context. It
/// sits directly under the location row, so tests address it by name instead of
/// each re-deriving the offset.
fn hud_anchor_row(terminal: &Terminal<TestBackend>, width: u16, hud: Rect) -> String {
    let y = if hud.height > 1 { hud.y + 1 } else { hud.y };
    buffer_row(terminal, width, y)
}

/// The footer's live-activity row — always the last one the layout granted.
fn hud_activity_row(terminal: &Terminal<TestBackend>, width: u16, hud: Rect) -> String {
    buffer_row(terminal, width, hud.y + hud.height.saturating_sub(1))
}
...
fn workflow_hud_height_is_stable_across_turn_boundaries() {
    ...
    assert_eq!(
        idle_hud_h,
        crate::tui::hud::desired_rows(&active.hud_state),
        "an active workflow keeps the activity row"
    );
}
...
fn live_activity_row_shows_model_and_workflow_context() {
    ...
    assert_eq!(
        regions.hud.height,
        crate::tui::hud::desired_rows(&app.hud_state),
        "an active workflow grants the activity row"
    );
    let workflow_row = hud_activity_row(&terminal, width, regions.hud);
    assert!(
...
fn live_activity_context_collapses_bottom_hud_duplicates() {
    ...
    let hud = hud_anchor_row(&terminal, width, regions.hud);
    ...
    assert!(
        // The rail no longer carries a second copy of the pressure reading —
        // the HUD gauge asserted just above is the single owner. What the rail
        // still owns is the *ceiling* that percent is measured against, so this
        // checks the two surfaces remain consistent about which window is in
        // play rather than both printing the same percent.
        screen.contains("compacts at") && !screen.contains("use "),
        "rail names the ceiling and leaves the percent to the HUD: {screen:?}"
    );
...
fn workflow_phase_survives_short_wide_terminals() {
    ...
    assert_eq!(
        regions.hud.height,
        crate::tui::hud::desired_rows(&app.hud_state),
        "short-but-wide terminals still get the activity row"
    );
    let workflow_row = buffer_row(&terminal, width, regions.hud.y + regions.hud.height - 1);
    assert!(
...
fn tiny_terminal_shows_truncated_workflow_badge_on_single_row_hud() {
    ...
    // An 8-row terminal is below the activity-row floor, so the footer keeps
    // its base height and the phase falls back onto the anchor row's badge.
    assert_eq!(
        regions.hud.height,
        crate::tui::layout::HUD_ROWS,
        "below the grant floor the footer stays at its base height"
    );
    let hud = buffer_row(&terminal, width, regions.hud.y + 1);
    assert!(
        hud.contains("phase 2/4"),
        "the anchor row carries a truncated phase badge: {hud:?}"
    );
}
...
#[test]
fn sidebar_owns_workflow_detail_so_footer_skips_the_activity_row() {
    ...
    assert_eq!(
        regions.hud.height,
        crate::tui::layout::HUD_ROWS,
        "sidebar owns the workflow detail — the footer stays at its base height \
         instead of adding the live-activity row"
    );
}
...
fn short_sidebar_cannot_carry_the_phase_so_hud_keeps_workflow_row() {
    ...
    let width = 160;
    // 11 rows, not 12: the composer gave a row back when its top border was
    // removed, and at 12 the rail's WORKFLOW section now *fits* — which is the
    // opposite of this test's premise. One row shorter restores the clipping
    // this regression is about, so the assertion still exercises the HUD
    // fallback rather than silently passing on the rail's own phase row.
    let height = 11;
    let backend = TestBackend::new(width, height);
    let mut terminal = Terminal::new(backend).expect("test terminal");
    app.draw(&mut terminal).expect("draw");
    ...
    assert!(regions.sidebar_width > 0, "sidebar renders at 160 cols");
    // What matters is that the phase stays on screen, not which surface carries
    // it. At this height the transcript outranks the footer's activity row, so
    // the phase lands on whichever surface is still live — the live activity
    // context, or the anchor row's inline badge when it is not.
    let screen = dump_all(&terminal, width, height);
    assert!(
        screen.contains("phase 2/4"),
        "workflow phase must never disappear from the screen: {screen}"
    );
}
...
fn wrapped_sidebar_session_lines_keep_the_hud_workflow_row() {
    let width = 160;
    // Rebuilt per probe: `App` is not `Clone`, and each probe needs a pristine
    // layout cache anyway.
    let build = || {
        let mut app = test_app();
        app.sidebar.visible = true;
    ...
    // The workflow phase remains visible on at least one compact viewport; the
    // responsive rail/footer policy may choose either owner.
    let height = (8u16..=60)
        .find(|height| {
            let mut app = build();
            let mut terminal = Terminal::new(TestBackend::new(width, *height))
                .expect("terminal");
            app.draw(&mut terminal).is_ok()
                && dump_all(&terminal, width, *height).contains("2/4")
        })
        .expect("some compact viewport must show the workflow phase");
    let mut app = build();
    let mut terminal = Terminal::new(TestBackend::new(width, height)).expect("terminal");
    app.draw(&mut terminal).expect("draw");
    assert!(dump_all(&terminal, width, height).contains("2/4"));
}
...
fn two_row_hud_masks_the_inline_workflow_badge() {
    ...
    // Live workflow → the third row appears under location and anchor.
    assert_eq!(
        regions.hud.height,
        crate::tui::hud::desired_rows(&app.hud_state),
        "a live workflow grants the activity row"
    );
    let session_row = buffer_row(&terminal, width, regions.hud.y + 1);
    let workflow_row = buffer_row(&terminal, width, regions.hud.y + 2);
    assert!(
        workflow_row.contains("phase 1/2"),
        "activity row carries the phase: {workflow_row:?}"
    );
    assert!(
        !session_row.contains("phase 1/2"),
        "anchor row must not duplicate the activity row: {session_row:?}"
    );
}
...
#[test]
fn ultrawide_composer_closes_after_the_effort_badge() {
    let mut app = test_app_with_theme(Theme::zo());
    app.sidebar.visible = false;
    app.hud_state.effort = Some(Effort::Xhigh);
    let width = 320;
    let mut terminal = Terminal::new(TestBackend::new(width, 24)).expect("test terminal");

    app.draw(&mut terminal).expect("ultrawide draw");
    let regions = app.regions.expect("layout regions after draw");
    let right = regions.input.x + regions.input.width.saturating_sub(1);
    let buffer = terminal.backend().buffer();

    assert_eq!(
        buffer[(right, regions.rule_top.y)].symbol(),
        "┐",
        "the idle top rule must close after xhigh"
    );
    for y in regions.input.y..regions.input.y + regions.input.height.saturating_sub(1) {
        assert_eq!(
            buffer[(right, y)].symbol(),
            "│",
            "the composer right rail must remain visible at row {y}"
        );
    }
    assert_eq!(
        buffer[(right, regions.input.y + regions.input.height.saturating_sub(1))].symbol(),
        "┘",
        "the bottom rule must close on the same column"
    );
    assert!(
        buffer_row(&terminal, width, regions.rule_top.y).contains("xhigh"),
        "closing the edge must not hide the effort badge"
    );
}
...
fn full_frame_keeps_sidebar_chat_and_activity_in_sync_for_agents() {
    ...
    assert!(
        // Context pressure is the HUD's alone now (see
...
/// A pasted image goes into the queue, and the queue re-attaches it to the
/// composer when the entry pops so the submit path can pick it up. Draining the
/// attachments has to take the badge down with them — otherwise the composer
/// keeps showing `[image: image/png]` with nothing behind it, and the next turn
/// looks like it is carrying an image it does not have.
#[test]
fn submitting_a_queued_image_leaves_no_badge_behind_in_the_composer() {
    let mut app = test_app();
    app.begin_turn_with_generation(0);
    app.disable_input();
    app.push_clipboard_image("image/png".to_string(), "AAAA".to_string())
    ...
    app.handle_key(press(KeyCode::Enter)).expect("enter");
    assert_eq!(app.input().image_count(), 0, "queueing empties the composer");

    // The turn ends and the queued entry pops: the host re-stages its images and
    // then drains them into the outbound message.
    let queued = app.pop_next_queued_message().expect("one queued entry");
    app.stage_queued_images_for_submit(queued.images);
    assert_eq!(app.input().image_count(), 1, "staged for this submit");

    let taken = app.take_pending_images();
    assert_eq!(taken.len(), 1, "the attachment reaches the submit path");
    assert_eq!(
        app.input().image_count(),
        0,
        "the badge left with the attachment"
    );
    assert!(
        !app.has_pending_images(),
        "nothing is staged after the drain"
    );
    assert!(app.input().is_empty(), "the composer is genuinely empty");
}

/// A local slash command clears the composer but never reaches the model, so it
/// does not consume a pasted image. The badge has to come back, or the image is
/// staged invisibly and rides whatever the user types next.
#[test]
fn a_slash_command_keeps_a_pasted_image_visible_instead_of_hiding_it() {
    let mut app = test_app();
    app.enable_input();
    app.push_clipboard_image("image/png".to_string(), "AAAA".to_string())
        .expect("stage clipboard image");
    ...
    let action = app.handle_key(press(KeyCode::Enter)).expect("enter");
    assert!(
        matches!(action, AppAction::Submit(ref text) if text == "/help"),
        "the idle path submits straight through, got {action:?}"
    );
    // Submitting emptied the composer, attachments included on screen...
    assert_eq!(app.input().image_count(), 0);
    // ...but the slash command never drained them, so the loop's return to the
    // prompt puts the badge back over the attachment that is still there.
    assert!(app.has_pending_images(), "the image is still staged");
    app.resync_image_badge();
    assert_eq!(
        app.input().image_count(),
        1,
        "a staged attachment must be visible in the composer"
    );
}
...
#[test]
fn paste_enters_user_question_freeform_answer() {
    let (responder, mut rx) = tokio::sync::oneshot::channel();
    let mut app = test_app();
    app.open_user_question_modal(UserQuestionPrompt {
        id: BlockId(3),
        question: "pick or explain".to_string(),
        header: None,
        options: vec![QuestionOption::plain("alpha")],
        multi_select: false,
        responder,
    });
    assert_eq!(app.mode(), AppMode::ModalQuestion);

    app.handle_paste("붙여넣은 답변");
    let action = app.handle_key(press(KeyCode::Enter)).unwrap();

    assert_eq!(action, AppAction::None);
    let answer = rx.try_recv().expect("a pasted answer was sent on the responder");
    assert_eq!(answer, vec!["붙여넣은 답변".to_string()]);
}
...
/// Every modal wears the Pi dialog chrome: a titled top rule, a bottom rule,
/// and **no side borders** — so the body owns the full width of the pane.
#[test]
fn a_drawn_modal_wears_the_pi_dialog_rules_and_no_side_borders() {
    let mut app = test_app_with_theme(Theme::pi());
    app.sidebar.visible = false;
    app.open_permission_picker_modal(runtime::PermissionMode::WorkspaceWrite);
    let mut terminal = Terminal::new(TestBackend::new(80, 24)).expect("test terminal");
    app.draw(&mut terminal).expect("draw the permission picker");

    let buffer = terminal.backend().buffer().clone();
    // Locate the top rule row: the first row holding a `─` in the border hue.
    let rule_row = (0..24u16)
        .find(|&y| {
            (0..80u16).any(|x| {
                buffer[(x, y)].symbol() == "\u{2500}"
                    && buffer[(x, y)].fg == app.theme.palette.border
            })
        })
        .expect("the modal draws a border-hued top rule");
    let row: String = (0..80u16)
        .map(|x| buffer[(x, rule_row)].symbol())
        .collect();
    assert!(
        row.contains("\u{2500} /permissions \u{2500}"),
        "the title rides the top rule: {row:?}"
    );

    // One row below the rule is body, never a `│` side border.
    let body: String = (0..80u16)
        .map(|x| buffer[(x, rule_row + 1)].symbol())
        .collect();
    assert!(
        !body.contains('\u{2502}'),
        "a Pi dialog has no side borders: {body:?}"
    );
}
...
/// Whether the backlog badge is painted anywhere in the frame.
///
/// Scans the whole buffer rather than guessing the transcript's bottom row. The
/// scrollbar paints a bare end-arrow with nothing after it, so a badge is only
/// an arrow carrying a count; callers assert the before/after transition, which
/// a static false match could not satisfy in both directions.
fn backlog_badge_showing(terminal: &ratatui::Terminal<TestBackend>) -> bool {
...
/// The backlog badge must reach the screen through the path a reader actually
/// uses — a wheel notch — not a hand-set flag. Follow-output is written at a
/// dozen sites and the transcript only learns the reader's intent through
/// `set_follow_output`, so a missed call site silently pins the badge off. The
/// widget-level test one layer down cannot catch that; this pins the wiring.
#[test]
fn backlog_badge_surfaces_through_the_real_wheel_scroll_path() {
    let (mut app, mut terminal) = app_with_overflowing_transcript();
    let transcript = app.regions.expect("regions after draw").transcript;

    assert!(
        !backlog_badge_showing(&terminal),
        "following the tail leaves nothing behind, so no badge"
    );
    ...
    .expect("wheel handled");
    app.draw(&mut terminal).expect("redraw after scrolling up");
    assert!(
        !app.transcript_view.follow_output,
        "precondition: the wheel notch parked the reader"
    );
    assert!(
        !backlog_badge_showing(&terminal),
        "reading back with nothing new arriving is not news, so still no badge"
    );

    for i in 0..8u64 {
        app.push_block(RenderBlock::TextDelta {
            id: BlockId(9_000 + i),
            text: format!("late arrival {i}"),
            done: true,
        });
    }
    app.draw(&mut terminal).expect("redraw after new output");
    assert!(
        backlog_badge_showing(&terminal),
        "output landing while parked must raise the badge through the real input path"
    );
}

/// Focus mode scrolls the transcript through its own key arms, so intent wired
/// only into the wheel path leaves follow-output armed here: the next block
/// snaps the reader back to the tail mid-read and the badge never fires. A
/// wheel-only test cannot catch that.
#[test]
fn backlog_badge_surfaces_through_focus_mode_keyboard_scroll() {
    let (mut app, mut terminal) = app_with_overflowing_transcript();
    ...
    app.draw(&mut terminal).expect("redraw after keyboard scroll");
    assert!(
        !app.transcript_view.follow_output,
        "PageUp in focus mode must park the reader"
    );
    let parked = app.transcript.scroll();
    ...
    assert_eq!(
        app.transcript.scroll(),
        parked,
        "output arriving after a keyboard park must not yank the reader to the tail"
    );
    app.draw(&mut terminal).expect("redraw after new output");
    assert!(
        backlog_badge_showing(&terminal),
        "output landing while parked must raise the badge through the keyboard path"
    );
}
...
/// Regression for the wheel-flood twin of the keystroke case above.
///
/// Both mouse paths went through `on_stream_update` but never re-stamped the
/// gate to draw-*completion*, so on a terminal whose paint outlasts the frame
/// interval every wheel event measured "elapsed ≥ interval" and repainted — one
/// full transcript redraw per event, which is precisely the "wheel lags / UI
/// tears on big context" symptom that arm was written to prevent. Routing both
/// mouse paths through `draw_gated` gives them the same re-stamp the key, paste
/// and stream arms already had.
#[tokio::test]
async fn scroll_burst_redraws_share_frame_budget() {
    use crate::tui::render_schedule::STREAM_FRAME_INTERVAL;
    ...
    // One inertial trackpad flick's worth of wheel events.
    for _ in 0..40 {
        event_tx
            .send(Ok(Event::Mouse(MouseEvent {
                kind: MouseEventKind::ScrollUp,
                column: 10,
                row: 5,
                modifiers: KeyModifiers::NONE,
    ...
    // Type and submit so the loop returns instead of idling.
    event_tx
        .send(Ok(Event::Key(press(KeyCode::Char('a')))))
        .expect("queue keystroke");
    event_tx
    ...
    .await
    .expect("scroll burst must not hang the event loop")
    .expect("run must succeed");
    assert!(
        matches!(action, AppAction::Submit(_)),
        "expected submit after the scroll burst, got {action:?}"
    );

    let draws = terminal.backend().draw_count;
    // Initial paint + the first wheel event + the trailing keystrokes and any
    // tick-recovered frame. Without the draw-completion re-stamp on the mouse
    // path the 20 ms paint makes every one of the 40 wheel events redraw.
    assert!(
        draws <= 8,
        "a wheel burst must coalesce to a handful of frame redraws on a slow-paint terminal, got {draws}"
    );
    assert!(draws >= 1, "the loop must perform its initial draw");
}
...
/// Send one captured wheel event through the same app path Zed uses, then
/// paint the resulting viewport. The event must be app-owned — handing it to
/// native scrollback would make click/drag and wheel address different history.
fn wheel_inline_transcript(
    app: &mut App,
    terminal: &mut Terminal<TestBackend>,
    kind: MouseEventKind,
) {
    let area = app.regions.expect("inline regions").transcript;
    let action = app
        .handle_mouse(MouseEvent {
            kind,
        ...
        .expect("wheel handled");
    assert_eq!(action, AppAction::None);
    app.draw(terminal).expect("wheel repaint");
}

fn wheel_inline_to_top(app: &mut App, terminal: &mut Terminal<TestBackend>) {
    for _ in 0..500 {
        let before = app.transcript.scroll();
        wheel_inline_transcript(app, terminal, MouseEventKind::ScrollUp);
        if app.transcript.scroll() == before {
            break;
        }
    }
}

fn wheel_inline_to_bottom(app: &mut App, terminal: &mut Terminal<TestBackend>) {
    for _ in 0..500 {
        let before = app.transcript.scroll();
        wheel_inline_transcript(app, terminal, MouseEventKind::ScrollDown);
        if app.transcript.scroll() == before {
            break;
        }
    }
...
/// Mouse capture must not split history ownership between Zo and the terminal.
///
/// Regression: live inline frames moved overflow blocks into Ratatui
/// `insert_before` history. Crossterm mouse capture then delivered wheel and
/// drag to Zo, but those old blocks no longer existed in Zo's transcript; wheel
/// repainted the current screen without moving and drag had no old text to
/// select. Keeping the bounded transcript app-owned makes wheel, highlight,
/// copy, and click-to-expand operate on one data source. Native scrollback is
/// populated only by the shutdown path.
#[test]
fn inline_wheel_and_drag_reach_history_without_native_handoff() {
    let mut app = test_app();
    app.set_terminal_mode(TerminalMode::Inline);
    app.enable_input();
    ...
    for i in 0..30u64 {
        app.push_block(RenderBlock::UserMessage {
            id: BlockId(i + 1),
            text: format!("HISTORY{i:02} selectable history row"),
        });
        app.draw(&mut terminal).expect("history draw");
    }

    assert_eq!(app.transcript.len(), 30, "live history stays app-owned");
    assert_eq!(
        terminal.backend().scrollback().area.height,
        0,
        "live frames must never mutate native scrollback"
    );
    assert!(visible_screen(&terminal, width, height).contains("HISTORY29"));

    // Input changes only the composer. Before this regression was fixed, every
    // key-triggered frame also attempted the live native-scrollback seal and
    // exposed a whole-band flash even though transcript content had not moved.
    for ch in "typing-stays-stable".chars() {
        let _ = app
            .handle_key(KeyEvent::new(KeyCode::Char(ch), KeyModifiers::NONE))
            .expect("typing handled");
        app.draw(&mut terminal).expect("typing repaint");
        assert_eq!(
            terminal.backend().scrollback().area.height,
            0,
            "typing frames must be paint-only"
        );
    }

    wheel_inline_to_top(&mut app, &mut terminal);
    assert_eq!(app.transcript.scroll(), 0, "wheel-up reaches the oldest row");
    let screen = visible_screen(&terminal, width, height);
    assert!(
        screen.contains("HISTORY00"),
        "oldest app-owned row must be wheel-reachable:\n{screen}"
    );

    let area = app.regions.expect("inline regions").transcript;
    let row = (area.y..area.bottom())
        .find(|&y| buffer_row(&terminal, width, y).contains("HISTORY00"))
        .expect("oldest history row is visible");
    let start = area.x;
    ...
    app.draw(&mut terminal).expect("selection highlight paint");
    assert!(
        app.transcript.has_char_selection(),
        "dragged history row must show the selection wash"
    );
    match app
        .handle_mouse(MouseEvent {
            kind: MouseEventKind::Up(MouseButton::Left),
        ...
        .expect("selection release")
    {
        AppAction::ClipboardCopyBlock(text) => assert!(
            text.contains("HISTORY00"),
            "drag copy must contain old app-owned history: {text:?}"
        ),
        other => panic!("history drag must copy on release, got {other:?}"),
    }

    wheel_inline_to_bottom(&mut app, &mut terminal);
    assert!(
        app.transcript.scroll() > 0,
        "wheel-down returns to the resolved tail offset"
    );
    assert!(visible_screen(&terminal, width, height).contains("HISTORY29"));
    assert_eq!(
        terminal.backend().scrollback().area.height,
        0,
        "wheel frames must remain paint-only and avoid the flash-producing handoff"
    );
}

/// A single large Markdown block remains wheel-reachable across reflow. This
/// covers the former row-level handoff path without writing any streaming or
/// wheel frame into native scrollback.
#[test]
fn inline_oversized_answer_reflows_without_losing_wheel_history() {
    let mut app = test_app();
    app.set_terminal_mode(TerminalMode::Inline);
    app.enable_input();

    let (initial_width, resized_width, height) = (80u16, 52u16, 18u16);
    let mut terminal = Terminal::with_options(
        TestBackend::new(initial_width, height),
        TerminalOptions {
            viewport: Viewport::Inline(height),
        },
    )
    .expect("inline test terminal");
    let answer = (1..=24)
        .map(|i| format!("{i}. ITEM-{i:02} required streamed value"))
        .collect::<Vec<_>>()
        .join("\n");
    app.push_block(RenderBlock::TextDelta {
        id: BlockId(78),
    ...
    wheel_inline_to_top(&mut app, &mut terminal);
    assert!(visible_screen(&terminal, initial_width, height).contains("ITEM-01"));

    terminal.backend_mut().resize(resized_width, height);
    app.draw(&mut terminal).expect("reflow after resize");
    wheel_inline_to_top(&mut app, &mut terminal);
    assert!(visible_screen(&terminal, resized_width, height).contains("ITEM-01"));
    wheel_inline_to_bottom(&mut app, &mut terminal);
    assert!(visible_screen(&terminal, resized_width, height).contains("ITEM-24"));
    assert_eq!(terminal.backend().scrollback().area.height, 0);
}

/// Screenshot defect: a hole of ~11 blank rows opened between "Compacting
/// conversation…" and "Compacted conversation · …".
///
/// The sequence below is the one `apply_auto_compaction_streaming` really
/// sends (`compaction.rs`): a `System` start notice, a
/// `CompactionProgress` heartbeat per streamed chunk of the summary, then a
/// `System` done notice. Each heartbeat renders as zero rows — but it used to
/// be *pushed into the transcript*, and a zero-row block still has edges, so
/// every one of them charged a block gap.
#[test]
fn compaction_heartbeats_leave_no_hole_between_the_start_and_done_notices() {
    let mut app = test_app();
    let (width, height) = (100u16, 30u16);
    let mut terminal = Terminal::new(TestBackend::new(width, height)).expect("test terminal");
    ...
    app.push_block(RenderBlock::System {
        id: BlockId(1),
        level: SystemLevel::Info,
        text: "Context trim · cleared 8 old tool result(s) (~8k tokens freed)".to_string(),
    });
    app.push_block(RenderBlock::System {
        id: BlockId(2),
        level: SystemLevel::Info,
        text: "Compacting conversation… (summarizing 512 messages)".to_string(),
    });
    let before = app.transcript.len();
    for chunk in 1..=12u64 {
        app.push_block(RenderBlock::CompactionProgress {
            streamed_chars: chunk * 400,
        });
    }
    assert_eq!(
        app.transcript.len(),
        before,
        "a live-ledger heartbeat must never become a transcript block — a zero-row \
         block still charges a block gap on each side"
    );
    app.push_block(RenderBlock::System {
        id: BlockId(3),
        level: SystemLevel::Info,
        text: "Compacted conversation · 495 messages summarized · 208.3k → 43.4k tokens"
            .to_string(),
    });

    app.draw(&mut terminal).expect("draw");
    // Only the transcript's own columns: the sidebar rail puts a glyph on every
    // row of the frame, so a whole-width row is never "blank".
    let transcript = app.regions.expect("regions after draw").transcript;
    let rows: Vec<String> = (0..height)
        .map(|y| {
            buffer_row(&terminal, width, y)
                .chars()
                .skip(usize::from(transcript.x))
                .take(usize::from(transcript.width))
                .collect()
        })
        .collect();
    let start = rows
        .iter()
        .position(|row| row.contains("Compacting conversation"))
        .expect("start notice on screen");
    let done = rows
        .iter()
        .position(|row| row.contains("Compacted conversation"))
        .expect("done notice on screen");
    assert!(done > start, "the done notice follows the start notice");
    let blank_between = rows[start + 1..done]
        .iter()
        .filter(|row| row.trim().is_empty())
        .count();
    assert!(
        blank_between <= 2,
        "the two notices must read as consecutive chrome lines, found {blank_between} blank rows \
         between them:\n{}",
        rows[start..=done].join("\n")
    );
}

/// A glyph no chrome, label, or hint in the TUI can produce, so finding one
/// inside a modal's rect proves the transcript under it was never covered.
const MODAL_LEAK_SENTINEL: &str = "\u{a7}";

/// Fill the transcript with sentinel glyphs, draw one frame, and return the
/// terminal plus the rect the fullscreen-placement modals are drawn into.
fn draw_over_sentinel_transcript(
    app: &mut App,
    width: u16,
    height: u16,
    ...
    for i in 0..40u64 {
        app.push_block(RenderBlock::UserMessage {
            id: BlockId(i + 1),
            text: MODAL_LEAK_SENTINEL.repeat(usize::from(width)),
        });
    }
    let mut terminal = Terminal::new(TestBackend::new(width, height)).expect("test terminal");
    app.draw(&mut terminal).expect("draw");
    let regions = app.regions.expect("regions after draw");
    let rect = crate::tui::app::modal_geometry::diff_modal_rect(
        &regions,
...
/// Assert not one cell of `rect` still shows the transcript underneath.
fn assert_no_sentinel_inside(terminal: &Terminal<TestBackend>, rect: Rect, what: &str) {
    let buffer = terminal.backend().buffer();
    for y in rect.y..rect.y + rect.height {
        for x in rect.x..rect.x + rect.width {
            assert_ne!(
                buffer[(x, y)].symbol(),
                MODAL_LEAK_SENTINEL,
                "{what} leaked the transcript at ({x}, {y}) — every cell inside a modal must be painted"
            );
        }
    }
}

/// Screenshot defect: a strip of transcript ran down the left edge of the
/// agents viewer. The Pi conversion took the side borders away, so a 96%-wide
/// centered rect left a margin of frosted transcript with no border to say the
/// dialog had ended — it read as content leaking into the modal.
#[test]
fn agents_viewer_covers_its_whole_rect_with_no_transcript_bleed() {
    let mut app = test_app_with_theme(Theme::pi());
    app.open_agents_viewer();
    let (terminal, rect) = draw_over_sentinel_transcript(&mut app, 120, 40);
    assert_no_sentinel_inside(&terminal, rect, "agents viewer");

    // The rect spans the transcript column edge to edge: with no side border,
    // any horizontal margin is a strip of visible transcript.
    let regions = app.regions.expect("regions");
    assert_eq!(rect.x, regions.transcript.x);
    assert_eq!(rect.width, regions.transcript.width);

    // And the interior is *filled*, not merely blanked: a row the body never
    // reaches still carries the dialog's elevation-2 surface, so a short list
    // cannot punch a hole through the glass.
    let surface = Theme::pi().surface2().expect("pi theme has an elevation-2 surface");
    let buffer = terminal.backend().buffer();
    let last_body_row = rect.y + rect.height - 3;
    assert_eq!(
        buffer[(rect.x + rect.width / 2, last_body_row)].bg,
        surface,
        "an empty body row must still be painted with the dialog surface"
    );
}

/// The same rect math backs every fullscreen-placement viewer, so the fix has
/// to hold for the ones that merely share it.
#[test]
fn workflow_and_diff_viewers_cover_their_whole_rect_too() {
    let mut app = test_app_with_theme(Theme::pi());
    app.open_diff_viewer(vec![runtime::message_stream::DiffView {
        old_path: Some("crates/zo-cli/src/tui/sidebar.rs".to_string()),
        new_path: Some("crates/zo-cli/src/tui/sidebar.rs".to_string()),
        language: Some("rust".to_string()),
        hunks: vec![runtime::message_stream::DiffHunk {
            old_start: 1,
            old_lines: 1,
            new_start: 1,
            new_lines: 1,
            lines: vec![
                runtime::message_stream::DiffLine {
                    kind: runtime::message_stream::DiffLineKind::Removed,
                    text: "old".to_string(),
                },
                runtime::message_stream::DiffLine {
                    kind: runtime::message_stream::DiffLineKind::Added,
                    text: "new".to_string(),
                },
            ],
        }],
    ...
    let (terminal, rect) = draw_over_sentinel_transcript(&mut app, 120, 40);
    assert_no_sentinel_inside(&terminal, rect, "diff viewer");

    let mut app = test_app_with_theme(Theme::pi());
    app.open_workflow_viewer(WorkflowViewerModal::new(WorkflowView::default()));
    let (terminal, rect) = draw_over_sentinel_transcript(&mut app, 120, 40);
    assert_no_sentinel_inside(&terminal, rect, "workflow viewer");
}
...
fn workflow_viewer_open_and_refresh_join_the_live_hud_plan_label() {
    ...
    assert!(opened.contains("FLEET") && opened.contains("Executor"), "{opened}");
    ...
    // A running phase heads its group with the Todo's *active* form, and the
    // join is live: the stale label is gone on the very next draw.
    assert!(refreshed.contains("Verifying workflow view"), "{refreshed}");
    assert!(!refreshed.contains("Building workflow viewer"), "{refreshed}");
}

/// 라우팅 회귀 핀 — `AppMode::ModalWorkflow` arm 은 `ScrollUp/ScrollDown` 만
/// 처리하고 `Down(Left)` 를 통째로 버렸다. 모달이 `handle_click` 을 갖추어도
/// 호스트가 이벤트를 넘기지 않으면 Ctrl+O 뷰어의 클릭은 100% 무시된다.
#[test]
fn workflow_viewer_routes_left_clicks_to_the_modal() {
    let mut view = workflow_view_for_plan_label_test();
    view.phases[0].agents.push(WorkflowAgentRow {
        id: "second-agent".to_string(),
        name: "second-builder".to_string(),
        status: "running".to_string(),
        ..WorkflowAgentRow::default()
    });
    view.phases[0].total = 2;
    let mut app = test_app();
    app.sidebar.visible = false;
    app.open_workflow_viewer(WorkflowViewerModal::new(view));

    let (width, height) = (140, 30);
    let backend = TestBackend::new(width, height);
    let mut terminal = Terminal::new(backend).expect("test terminal");
    app.draw(&mut terminal).expect("draw the viewer");
    assert_eq!(
        app.modals
            .workflow
            .as_ref()
            ...
            .selected_agent(),
        0,
        "precondition: the first executor is selected"
    );

    // Coordinates come from the painted frame, never from a guess.
    let (row, column) = {
        let buffer = terminal.backend().buffer();
        let mut found = None;
        for y in 0..height {
            let text: String = (0..width).map(|x| buffer[(x, y)].symbol()).collect();
            if let Some(idx) = text.find("second-builder") {
                found = Some((y, u16::try_from(text[..idx].chars().count()).unwrap_or(0)));
                break;
            }
        }
        found.expect("the second executor is on screen")
    };

    let action = app.handle_mouse(left_click(column, row)).expect("routed");
    assert_eq!(
        action,
        AppAction::Redraw,
        "a landed click must repaint the viewer"
    );
    assert_eq!(
        app.modals
            .workflow
            ...
            .selected_agent(),
        1,
        "clicking an executor row must select it"
    );
}
...
#[test]
fn tall_inline_viewport_sinks_a_short_conversation_onto_the_composer() {
    // Reported three separate times as "맨하단으로 내려오지 않아": on a tall
    // terminal a two-line exchange sat at the TOP of the viewport with a field
    // of blank rows between it and the composer. A terminal fills from the
    // bottom, so the conversation must sit against the composer and grow
    // upward. Mid-turn is the case that was screenshotted, so the assertion
    // runs with a turn open.
    let mut app = test_app();
    app.set_terminal_mode(TerminalMode::Inline);
    app.sidebar.visible = true;
    app.enable_input();
    ...
    let regions = app.regions.expect("regions after draw");
    // The composer and HUD keep their bottom slots; the transcript owns
    // everything above them.
    assert!(regions.input.height > 0, "composer must stay on screen");
    assert!(regions.hud.height > 0, "HUD must stay on screen");
    assert_eq!(
        regions.transcript.y.saturating_add(regions.transcript.height),
        regions.rule_top.y,
        "the transcript must reach down to the activity rule above the composer"
    );

    let dump = dump_all(&terminal, width, height);
    let ask_y = (0..height)
        .find(|&y| buffer_row(&terminal, width, y).contains("short ask"))
        .expect("the user message must render somewhere in the viewport");
    // Two content rows (label + text) plus the tail's single blank breathing
    // row directly above the activity rule — not parked at the top of a
    // 46-row transcript region.
    assert!(
        ask_y + 3 >= regions.rule_top.y,
        "a short conversation must sink onto the composer, not float at the top \
         (message at row {ask_y}, activity rule at row {}):\n{dump}",
        regions.rule_top.y
    );
}
...
#[test]
fn inline_launchpad_sits_against_the_composer_not_the_top_of_the_screen() {
    // On a tall terminal the launchpad anchored at the top of the transcript
    // region, leaving ~18 blank rows between it and the input box. Inline mode
    // is a terminal: the newest thing belongs next to the prompt.
    let mut app = test_app();
    app.set_terminal_mode(TerminalMode::Inline);
    app.enable_input();
    app.set_startup_screen(startup_screen_for_shortcut());
    ...
    assert_eq!(
        hint_row + 1,
        regions.rule_top.y,
        "launchpad must end flush against the composer (rule at row {}):\n{dump}",
        regions.rule_top.y
    );
    // And the masthead is pushed down with it rather than staying at row 0.
    let masthead_row = (0..height)
        .find(|&y| buffer_row(&terminal, width, y).contains("workspace zo"))
        .expect("the launchpad masthead must render");
    assert!(
        masthead_row > 0,
        "the whole launchpad moves down, not just its tail:\n{dump}"
    );
}

/// Draw-cost floor for the render path, kept runnable rather than remembered.
///
/// `#[ignore]` because it measures rather than asserts — run it with
/// `cargo test --release -p zo-cli --lib render_draw_cost -- --ignored --nocapture`.
/// Release matters: a debug build is several times slower and says nothing about
/// what ships.
///
/// What it establishes, and why it is worth keeping: per-frame cost does **not**
/// grow with conversation length. Measured on an M-series mac at 120x40 the cost
/// is flat at roughly 0.2ms/frame from 100 to 10,000 pushed blocks — it even
/// falls slightly as history grows, because `draw` renders the visible window and
/// the layout/render caches absorb the rest, making the frame O(visible rows)
/// rather than O(history). A machine 10x slower therefore still lands near 2ms,
/// well inside a 60fps budget, so the render path is not where a slow terminal
/// loses its responsiveness — blocking work awaited on the UI thread is (see
/// `turn_controller::pump_blocking_with_draw`).
///
/// It also pins that retention is bounded: the printed `retained` count stops
/// climbing once `MAX_TRANSCRIPT_BYTES` is reached (256KB under `cfg(test)`, 32MB
/// in a real binary), with blocks and their parallel render caches front-pruned
/// in lockstep. Inline mode is tighter still, sealing settled blocks into native
/// scrollback and retaining only what the viewport needs.
#[test]
#[ignore = "measurement, not an assertion; run with --release --ignored --nocapture"]
fn render_draw_cost_is_flat_in_conversation_length() {
    for blocks in [50usize, 500, 5000] {
        let mut app = test_app_with_theme(Theme::default_dark());
        app.set_terminal_mode(TerminalMode::Fullscreen);
        ...
        for i in 0..blocks {
            ...
            app.push_block(RenderBlock::UserMessage {
                id: BlockId(i * 2),
                text: format!("질문 {i}: 라우터 스코어링이 tier에 압사당하는 문제"),
            });
            app.push_block(RenderBlock::TextDelta {
                id: BlockId(i * 2 + 1),
                text: format!(
                    "답변 {i}: `crates/runtime/src/model_router`의 learned ±90이 \
                     tier 300 / capability 1000에 눌려 서열을 뒤집지 못합니다. \
                     프로브 결과를 밴딧에 넣어 온라인으로 갱신하는 편이 낫습니다."
                ),
                done: true,
            });
        }
        // Warm the layout/render caches so the loop below measures steady-state
        // frames, not first-paint cache fills.
        app.draw(&mut terminal).expect("warm draw");

        let iterations: u32 = 60;
        let started = std::time::Instant::now();
        for _ in 0..iterations {
            // Advance the animation clock so each frame is a real redraw rather
            // than a no-op diff against an unchanged buffer.
            app.advance_tick();
            app.draw(&mut terminal).expect("draw");
        }
        let per_frame_us = started.elapsed().as_micros() / u128::from(iterations);
        println!(
            "pushed={:5}  per_frame={per_frame_us:5}us  retained={}",
            blocks * 2,
...
/// Rows of the drawn frame's *transcript region only*, trailing blanks trimmed.
///
/// Scoped to the transcript on purpose: the surrounding chrome legitimately
/// differs between a streaming turn and an idle one (the `rule_top` row carries
/// the spinner instead of a rule, and the footer hides cost mid-turn), so
/// comparing whole frames would fail on differences that are the design.
fn transcript_rows(
    app: &App,
    ...
    let region = app
        .regions
        .expect("draw computes layout regions")
        .transcript;
    (region.y..region.y.saturating_add(region.height))
        .map(|y| visible_row(terminal, width, y).trim_end().to_string())
        .collect()
}

/// Draw until the reveal pacer has nothing left to show.
///
/// A streaming turn does not paint its text all at once: `stream_pace` reveals it
/// over frames (a typewriter effect), so a single `draw` on a freshly-pushed delta
/// legitimately shows only its first chunk. Any streaming assertion has to let the
/// pacer finish or it measures the animation instead of the render — which is
/// exactly the trap that made an earlier hand-run probe look like dropped content.
fn draw_until_settled(
    ...
    for _ in 0..400 {
        ...
        let rows = transcript_rows(app, terminal, width);
        // Every intermediate frame is itself a real frame the user sees, so the
        // width invariant is checked on all of them, not only the settled one.
        for row in &rows {
            assert!(
                crate::tui::text_metrics::display_width(row) <= usize::from(width),
                "a mid-stream frame overran the {width}-cell viewport: {row:?}"
            );
        }
        if rows == previous {
            stable += 1;
...
/// Streaming a document token-by-token must land on exactly the render it would
/// have had if the whole thing arrived at once.
///
/// This is the contract the markdown/code-card comments assert but nothing pinned:
/// the streaming interior and the settled render are "구성상 동일" (structurally
/// identical, differing only in colour) so that settling causes no reflow and no
/// scroll jump. A mismatch is the class of bug that reads as the screen breaking
/// mid-answer.
///
/// Each prefix is also checked for viewport overrun as it is revealed (see
/// `draw_until_settled`), so a frame that only exists mid-stream — an *unclosed*
/// fence, a half-built table — cannot silently paint outside the region.
#[test]
fn streamed_markdown_settles_to_the_same_render_as_one_shot() {
    // Shapes chosen because each has a distinct mid-stream state: a fence is
    // unclosed for many frames, a table has no width until its delimiter row
    // lands, a nested fence changes indent, and CJK is the case a char-based
    // width budget gets wrong.
    let corpus = [
        (
            "fence",
            "Here is the plan.\n\n```rust\nfn main() {\n    println!(\"hi\");\n}\n```\n\nDone.",
    ...
    for (name, full) in corpus {
        // One-shot: the whole document arrives settled.
        let mut oneshot = test_app_with_theme(Theme::default_dark());
        oneshot.set_terminal_mode(TerminalMode::Fullscreen);
        oneshot.enable_input();
        let mut oneshot_term = Terminal::new(TestBackend::new(w, h)).expect("terminal");
        oneshot.push_block(RenderBlock::TextDelta {
            id: BlockId(1),
            text: full.to_string(),
            done: true,
        });
        let expected = draw_until_settled(&mut oneshot, &mut oneshot_term, w);

        // Streamed: the same text as same-id increments, which `Transcript::push`
        // appends onto the tail block — the real wire path.
        let mut streamed = test_app_with_theme(Theme::default_dark());
        streamed.set_terminal_mode(TerminalMode::Fullscreen);
        streamed.begin_turn_with_generation(0);
        let mut streamed_term = Terminal::new(TestBackend::new(w, h)).expect("terminal");
        let chars: Vec<char> = full.chars().collect();
        let mut sent = 0usize;
        while sent < chars.len() {
            let take = 7.min(chars.len() - sent);
            let chunk: String = chars[sent..sent + take].iter().collect();
            sent += take;
            streamed.push_block(RenderBlock::TextDelta {
                id: BlockId(1),
                text: chunk,
                done: sent >= chars.len(),
            });
            // Reveal what has arrived so far, checking every frame's width.
            draw_until_settled(&mut streamed, &mut streamed_term, w);
        }
        let actual = draw_until_settled(&mut streamed, &mut streamed_term, w);

        assert_eq!(
            actual, expected,
            "`{name}`: a streamed document must settle to the same render as a \
             one-shot one, or settling reflows the transcript under the reader"
        );
    }
}

/// Read a row the way a REAL terminal shows it: a double-width glyph covers the
/// following cell, so whatever that cell still holds is invisible on screen.
/// `TestBackend::draw` only writes the cells ratatui's diff emits, and the diff
/// deliberately skips a wide glyph's continuation — so reading the buffer
/// verbatim reports letters no user can see, and a plain `dump_all` turns a
/// clean frame into apparent garbage. Every assertion about *user-visible*
/// content has to go through this.
fn visible_row(terminal: &Terminal<TestBackend>, width: u16, y: u16) -> String {
    let buffer = terminal.backend().buffer();
    visible_cells(width, |x| buffer[(x, y)].symbol())
}

fn visible_cells<'a>(width: u16, cell: impl Fn(u16) -> &'a str) -> String {
    ...
    for x in 0..width {
        ...
        let symbol = cell(x);
        out.push_str(symbol);
        skip = u16::try_from(crate::tui::text_metrics::display_width(symbol))
            .unwrap_or(1)
            .saturating_sub(1);
    }
    out
...
fn visible_screen(terminal: &Terminal<TestBackend>, width: u16, height: u16) -> String {
    (0..height).fold(String::new(), |mut out, y| {
        out.push_str(&visible_row(terminal, width, y));
        out.push('\n');
        out
    })
}

/// Stream pacing reveals a delta over several frames, so a seal test has to let
/// each chunk land before pushing the next one.
fn settle_stream(app: &mut App, terminal: &mut Terminal<TestBackend>) {
    for _ in 0..200 {
        app.advance_tick();
        app.draw(terminal).expect("draw");
    }
...
fn seal_section(i: usize) -> String {
    format!(
        "## 섹션 {i}\n\n한국어 문단 {i} 입니다. 이 줄은 충분히 길어서 줄바꿈이 필요합니다.\n\n- 항목 하나\n- 항목 둘\n\n"
    )
}

/// Section numbers reachable through captured wheel events. Each wheel step is
/// smaller than the viewport, so successive screens overlap and no heading can
/// fall between samples.
fn sections_reachable_by_wheel(
    app: &mut App,
    terminal: &mut Terminal<TestBackend>,
    width: u16,
    height: u16,
    ...
    wheel_inline_to_top(app, terminal);
    let mut seen = BTreeSet::new();
    for _ in 0..1_000 {
        let viewport = visible_screen(terminal, width, height);
        for line in viewport.lines() {
            if let Some(index) = line.find("## 섹션 ") {
                let rest = &line[index + "## 섹션 ".len()..];
...
fn seal_test_app(width: u16, height: u16) -> (App, Terminal<TestBackend>) {
    let mut app = test_app();
    app.set_terminal_mode(TerminalMode::Inline);
    app.enable_input();
    app.push_block(RenderBlock::UserMessage {
    ...
    let mut terminal = Terminal::with_options(
        TestBackend::new(width, height),
        TerminalOptions {
            viewport: Viewport::Inline(crate::tui::inline_viewport_height(height)),
        },
    )
    .expect("inline test terminal");
    app.draw(&mut terminal).expect("draw");
...
/// Streaming and settled Markdown stay in the bounded app transcript until
/// shutdown. Ending a turn must not silently move rows away from the wheel and
/// drag-selection handlers.
#[test]
fn active_and_settled_markdown_remain_wheel_reachable() {
    let (width, height) = (100u16, 30u16);
    let (mut app, mut terminal) = seal_test_app(width, height);
    ...
    for i in 0..22 {
        ...
        settle_stream(&mut app, &mut terminal);
    }
    ...
    assert_eq!(
        terminal.backend().scrollback().area.height,
        0,
        "turn settlement must remain paint-only"
    );
    assert_eq!(
        sections_reachable_by_wheel(&mut app, &mut terminal, width, height),
        (0..22).collect::<Vec<_>>()
    );
}

/// A long streamed answer must keep every section in one app-owned history.
/// This replaces the former native-scrollback/live-band seam with the exact
/// interaction the user performs: wheel from the oldest row back to the tail.
#[test]
fn a_long_streamed_answer_keeps_every_section_wheel_reachable() {
    let (width, height) = (100u16, 30u16);
    let (mut app, mut terminal) = seal_test_app(width, height);
    ...
    let seen = sections_reachable_by_wheel(&mut app, &mut terminal, width, height);
    assert_eq!(
        seen,
        (0..22).collect::<Vec<_>>(),
        "every section must be reachable in order through app-managed wheel history"
    );
    assert_eq!(terminal.backend().scrollback().area.height, 0);
}

/// A full-width modal may cover the live viewport, but it must neither stamp
/// modal cells into history nor discard transcript rows. Closing it restores
/// the same wheel-reachable document.
#[test]
fn streaming_under_an_open_diff_modal_keeps_wheel_history_whole() {
    let (width, height) = (100u16, 30u16);
    let (mut app, mut terminal) = seal_test_app(width, height);
    ...
    app.open_diff_viewer(diff_modal_fixture());
    app.draw(&mut terminal).expect("draw with modal");
    for i in 12..22 {
        app.push_block(RenderBlock::TextDelta {
            id: BlockId(2),
            text: seal_section(i),
    ...
    assert_eq!(
        terminal.backend().scrollback().area.height,
        0,
        "modal and streaming frames must not mutate native history"
    );
    ...
    assert_eq!(
        sections_reachable_by_wheel(&mut app, &mut terminal, width, height),
        (0..22).collect::<Vec<_>>(),
        "closing the modal must reveal one unbroken wheel history"
    );
}

fn diff_modal_fixture() -> Vec<runtime::message_stream::DiffView> {
    use runtime::message_stream::{DiffHunk, DiffLine, DiffLineKind, DiffView};
    vec![DiffView {
        old_path: Some("src/marker.rs".to_string()),
        new_path: Some("src/marker.rs".to_string()),
        language: Some("rust".to_string()),
        hunks: vec![DiffHunk {
            old_start: 1,
            old_lines: 3,
            ...
            lines: vec![
                DiffLine {
                    ...
                    text: "MODALMARKER-CTX".to_string(),
                },
                DiffLine {
                    kind: DiffLineKind::Removed,
                    text: "MODALMARKER-DEL".to_string(),
                },
                DiffLine {
                    kind: DiffLineKind::Added,
                    text: "MODALMARKER-ADD".to_string(),
                },
            ],
        }],
    }]
...
/// Distinct 2-cell graphemes, one per index.
///
/// The wrap defect keys on *display width*, not on any language, so the
/// fixtures below are runs of wide glyphs rather than prose. Distinct per index
/// so a dropped one can be named in the failure message.
fn wide_glyph(index: usize) -> char {
    // CJK Unified Ideographs: every codepoint in this block measures two cells.
    const BLOCK_START: u32 = 0x4E00;
    const BLOCK_LEN: u32 = 0x1000;
    let offset = u32::try_from(index).unwrap_or(0) % BLOCK_LEN;
    char::from_u32(BLOCK_START + offset).expect("CJK block codepoint is valid")
}

/// `word_count` space-separated words of `word_len` wide glyphs each, so the
/// wrapper has real break opportunities to get wrong.
fn wide_glyph_words(word_count: usize, word_len: usize) -> String {
    (0..word_count)
        .map(|w| {
            (0..word_len)
                .map(|c| wide_glyph(w * word_len + c))
                .collect::<String>()
        })
        .collect::<Vec<_>>()
        .join(" ")
...
/// The pager must show every glyph it was given, at every width.
///
/// It paints arbitrary text — file bodies, command output, model prose — and
/// its lines arrive unwrapped, so `.wrap(...)` used to do the folding. ratatui's
/// `WordWrapper` decides a row is full *before* adding the current glyph's
/// width, and a 2-cell grapheme landing on that break is discarded outright.
/// The row count stayed right, so the loss was invisible vertically and read as
/// characters vanishing mid-sentence.
#[test]
fn pager_never_drops_a_wide_glyph_at_any_width() {
    const WORDS: usize = 12;
    const WORD_LEN: usize = 4;
    let body = wide_glyph_words(WORDS, WORD_LEN);
    let expected: Vec<char> = body.chars().filter(|ch| !ch.is_whitespace()).collect();

    // Swept because the defect is not monotonic: whether a glyph straddles the
    // break depends on the exact column, so any single width can sit in a
    // lossless gap. From the narrowest usable frame (below it the app paints
    // its "terminal too small" notice instead of the pager) up to the width at
    // which the body stops wrapping — past that there is no break to get wrong.
    let widest = u16::try_from(crate::tui::text_metrics::display_width(&body))
        .unwrap_or(u16::MAX)
        .max(crate::tui::layout::MIN_USABLE_WIDTH);
    let height = crate::tui::layout::MIN_USABLE_HEIGHT * 2;
    for width in crate::tui::layout::MIN_USABLE_WIDTH..=widest {
        let mut app = test_app();
        app.open_pager(body.clone());
        let mut terminal =
            Terminal::new(TestBackend::new(width, height)).expect("pager test terminal");
        app.draw(&mut terminal).expect("pager draw");

        let painted: String = (0..height)
            .map(|y| visible_row(&terminal, width, y))
            .collect::<Vec<_>>()
            .join("\n");
        for ch in &expected {
            assert!(
                painted.contains(*ch),
                "width {width}: pager dropped `{ch}`"
            );
        }
    }
}

/// The pager's scroll extent must be measured in terminal cells, not `chars()`.
///
/// A 2-cell grapheme spends two columns per character, so a char-count estimate
/// reported roughly half the real wrapped height. `max_scroll` was derived from
/// that estimate, which clamped scrolling before the end — the tail of such a
/// document could not be reached at all. Asserted through the painted frame so
/// it holds against the renderer, not against a second copy of the formula.
#[test]
fn pager_can_scroll_to_the_last_row_of_a_wide_glyph_document() {
    // ASCII so the sentinel is unaffected by the very wrapping under test.
    const SENTINEL: &str = "END-OF-DOCUMENT";

    let width = crate::tui::layout::MIN_USABLE_WIDTH;
    let height = crate::tui::layout::MIN_USABLE_HEIGHT * 2;
    // Each line's *cells* exceed `width` while its *characters* do not, which is
    // exactly the gap the old estimate fell into: it called each line one row
    // when the renderer needs two, halving the document and stopping the
    // viewport short of the end.
    let per_line_glyphs = usize::from(width) * 3 / 4;
    let line_count = usize::from(height) * 4;
    let mut body = String::new();
    for i in 0..line_count {
        body.extend((0..per_line_glyphs).map(|c| wide_glyph(i + c)));
        body.push('\n');
    }
    ...
    let mut app = test_app();
    app.open_pager(body);
    // Far more than any plausible extent; the renderer clamps to `max_scroll`,
    // so this lands exactly at the bottom if the extent is measured correctly.
    app.pager_scroll_down(u16::MAX);

    let mut terminal = Terminal::new(TestBackend::new(width, height)).expect("pager terminal");
    app.draw(&mut terminal).expect("pager draw");

    // Read through `visible_row`, not raw cell concatenation: `TestBackend`
    // pads the cell *after* a wide glyph with a space, so a naive dump splits
    // every wide run and any substring search misses.
    let painted: String = (0..height)
        .map(|y| visible_row(&terminal, width, y))
        .collect::<Vec<_>>()
        .join("\n");
