...
use super::modals::{
    ...
    CustomProviderPrefill, CustomProviderWizardModal, DeepTierModal, DeepTierView,
    DiffViewerAction, DiffViewerModal,
    ...
    ProviderAccountRow, ProviderManagerAction, ProviderManagerModal, ProviderManagerRow,
...
struct TranscriptPress {
    ...
    /// Transcript geometry (`resolved_scroll`, block count) when the press
    /// landed. A live turn appends blocks and re-pins the tail between press
    /// and release, so the release cell often resolves to a *different* block
    /// than the one the user aimed at. The strict same-block check then
    /// swallowed the click outright — which is why clicking an agent card
    /// while it streamed did nothing. When the geometry moved, the press
    /// anchor is the honest target.
    geometry: (u16, usize),
}
...
pub struct App {
    ...
    /// Retained inline transcript awaiting shutdown-time native scrollback.
    ...
    /// Screen rect the right sidebar panel actually painted on the most recent
    /// draw (`None` when hidden). In inline mode the floating card hugs its
    /// content, so this is the *hugged* rect, not the full-height budget. A
    /// left-click inside it toggles the capped-row detail view — and, just as
    /// importantly, is consumed here so it never falls through to the
    /// transcript beneath the card as a stray text-selection press.
    sidebar_click_rect: Option<Rect>,
    ...
    /// Set when the `/providers` manager hands off to the add/edit wizard, so
    /// saving (or cancelling) the form lands back on the list the user came
    /// from instead of dropping them into an empty transcript.
    reopen_provider_manager: bool,
    ...
    /// Step keys already announced as chapters this turn, keyed by the step's
    /// position plus its stable `content` (not its present-tense `active_form`,
    /// which the model rewords freely between writes; position included so two
    /// steps that happen to share content still chapter separately). A plan
    /// that returns to an earlier step — a verify pass bouncing back to
    /// implement — must not re-push a heading the transcript already carries.
    plan_chapters_this_turn: std::collections::HashSet<String>,
    /// Tool-call ids of in-flight read-only `TaskList` calls. Their result body
    /// is the same typed `Todos` snapshot a mutating `TodoWrite` produces, so
    /// without this marker the mere listing of tasks would announce a plan
    /// chapter and pin the live plan panel.
    task_list_calls: std::collections::HashSet<String>,
...
impl App {
    ...
    pub fn new_with_spectator(
        ...
        Self {
            ...
            sidebar_click_rect: None,
            hovered_agent: None,
            workflow_viewer_empty_refreshes: 0,
            reopen_provider_manager: false,
            rx,
            ...
            plan_chapters_this_turn: std::collections::HashSet::new(),
            task_list_calls: std::collections::HashSet::new(),
    ...
    pub fn begin_turn_with_generation(&mut self, turn_generation: u64) {
        ...
        // Chapters are turn-scoped: the same plan step reached again in a later
        // turn is new work and gets its own heading. Stale TaskList markers
        // (a call whose result never landed) must not leak across turns either.
        self.plan_chapters_this_turn.clear();
        self.task_list_calls.clear();
    ...
    /// ("마지막 출력되면 화면으로 내려가게") after every received tail byte
    /// has reached the transcript. A reader who scrolled up mid-stream keeps
    /// their place instead: the earlier unconditional
    ...
    fn settle_turn(&mut self, cool: bool) {
        // Provider deltas stay paced while the turn is live, but the definitive
        // completed frame must contain every byte already received. Leaving a
        // hidden tail for later idle ticks made fullscreen answers look clipped
        // as soon as the spinner disappeared; inline mode also needs the whole
        // block before native scrollback takes ownership.
        self.finish_stream();
        self.flush_stream();
        self.clear_reasoning_activity_title_caches();
    ...
    pub fn set_terminal_mode(&mut self, mode: TerminalMode) {
        ...
        // The inline transcript region is the whole terminal, so a short
        // conversation has to sit against the composer rather than at the top of
        // an otherwise empty screen.
        self.transcript.set_bottom_aligned(mode.is_inline());
        if mode.is_inline() {
            // The app owns live history so wheel, click, and drag selection all
            // address the same bounded transcript. Native scrollback receives
            // that transcript only during shutdown.
    ...
    /// Seal every currently-live transcript block for native scrollback.
    ///
    /// Shutdown-only: while the session is live, mouse capture routes wheel,
    /// click, and drag events to the app, so history stays in the bounded
    /// transcript those handlers address. Once interaction ends, move that
    /// retained history into the terminal before the viewport is cleared.
    ...
    fn update_turn_activity(&mut self, block: &RenderBlock) {
        ...
        if let Some(activity) = self.turn_activity.as_mut() {
            ...
            match block {
                ...
                RenderBlock::System { text, .. } if is_provider_retry_notice(text) => {
                    activity.mark_event();
    ...
    fn update_live_tool_activity(&mut self, block: &RenderBlock) {
        ...
        } = block
        {
            ...
            // A read-only `TaskList` result arrives as the same typed `Todos`
            // body a mutating `TodoWrite` produces. Remember which ids are
            // listings so the result path refreshes the checklist without
            // announcing a chapter or pinning the plan for a mere read.
            if in_flight && name == "TaskList" {
                self.task_list_calls.insert(tool_call_id.0.clone());
            }
        }
        ...
        } = block
        {
            ...
            let from_task_list = self.task_list_calls.remove(&tool_call_id.0);
            self.apply_todo_tool_result(body, from_task_list);
    ...
    fn route_block_to_surface(&mut self, block: RenderBlock) {
        match block {
            ...
            // Retry backoff is turn-scoped live state. `update_turn_activity`
            // surfaced it above; committing it to history leaves a stale warning
            // after success, failure, or cancellation settles the activity line.
            RenderBlock::System { text, .. } if is_provider_retry_notice(&text) => {}
            // Live ledger only, the third of the trio. `update_turn_activity`
            // above already spent this heartbeat on the spinner's action line.
            //
            // It used to fall through to the transcript instead. It renders as
            // zero rows there (`layout::estimate_rows`), but a zero-row block
            // still has *edges*: `visual_block_gap` charged the base block gap on
            // both sides of every one of them, so a compaction that streamed a
            // dozen heartbeats opened a dozen blank rows between "Compacting
            // conversation…" and "Compacted conversation · …". Routing it here
            // makes the code match what `block_body_bytes` already documents —
            // Usage/CompactionProgress/RateLimit never enter `blocks`.
    ...
    /// the live todo panel.
    ///
    /// `from_task_list` marks a read-only `TaskList` result: the checklist
    /// still refreshes, but the listing neither pins the live plan panel nor
    /// announces a chapter — only a mutating `TodoWrite` does.
    fn apply_todo_tool_result(
        &mut self,
    ...
    /// Announce the plan step the agent just moved to as a one-line transcript
    /// "chapter", so scrolling back through a long turn shows *which step* each
    /// stretch of work belonged to. Reads the same
    /// [`crate::tui::hud::now_step`] sentence the activity line shows, so the two
    /// can never disagree.
    ///
    /// Called from the two `TodoWrite` result paths only — never from
    /// [`Self::apply_todo_items_to_hud`], which the ~330 ms live-snapshot poll
    /// also drives, and never without an active turn, so a restored session's
    /// replay stays silent.
    fn announce_plan_chapter(&mut self) {
        if self.turn_activity.is_none() {
            return;
        }
        let Some((key, text)) =
            crate::tui::hud::now_step(&self.hud_state.todo_items).map(|step| {
                (
                    // Position-qualified key (see the field doc): two steps
                    // sharing the same content must still chapter separately.
                    format!(
                        "{}:{}",
                        step.index,
        ...
        if !self.plan_chapters_this_turn.insert(key) {
            return;
        }
        self.push_diff_note(SystemLevel::Info, text);
    }
    ...
    pub fn handle_paste(&mut self, text: &str) {
        ...
        if matches!(self.mode, AppMode::ModalQuestion) {
            if let Some(modal) = self.active_modal_as::<UserQuestionModal>() {
                modal.paste_text(text);
            }
            return;
        }
    ...
    /// Wheel + click over the Ctrl+G agents viewer.
    fn handle_agents_viewer_mouse(&mut self, mouse: MouseEvent) {
        let modal_area = self
            .regions
            .as_ref()
            .map(|regions| diff_modal_rect(regions, regions.transcript));
        let (Some(modal), Some(area)) = (self.modals.agents.as_mut(), modal_area) else {
            return;
        };
        match mouse.kind {
    ...
    /// Wheel + click over the `/diff` viewer. Returns `true` when the frame
    /// needs a redraw.
    fn handle_diff_viewer_mouse(&mut self, mouse: MouseEvent) -> bool {
        let modal_area = self
            .regions
            .as_ref()
            .map(|regions| diff_modal_rect(regions, regions.transcript));
        let Some(modal) = self.modals.diff_viewer.as_mut() else {
            return false;
        };
        match mouse.kind {
            MouseEventKind::ScrollUp => modal.scroll_up(Self::MOUSE_SCROLL_ROWS),
            MouseEventKind::ScrollDown => modal.scroll_down(Self::MOUSE_SCROLL_ROWS),
            MouseEventKind::Down(MouseButton::Left) => {
                return modal_area.is_some_and(|area| {
                    modal.handle_click(mouse.column, mouse.row, area, &self.theme)
                });
            }
            _ => {}
    ...
    pub fn handle_mouse(&mut self, mouse: MouseEvent) -> Result<AppAction, TuiError> {
        match self.mode {
            ...
            // a long fan-out is navigable by mouse, not just PgUp/PgDn — and a
            // left-click picks a step/executor or folds a detail section. The
            // click arm was missing entirely, so every click over this modal was
            // dropped while its panes advertised themselves as pickable.
                    ...
                    match mouse.kind {
                        ...
                        MouseEventKind::Down(MouseButton::Left) => {
                            if let Some(area) = modal_area {
                                if modal.handle_click(mouse.column, mouse.row, area, &self.theme) {
                                    return Ok(AppAction::Redraw);
                                }
                            }
                        }
            ...
            AppMode::ModalAgents => self.handle_agents_viewer_mouse(mouse),
            // Diff viewer (`/diff`): this mode had no mouse arm at all, so the
            // wheel and every click over it were dropped — on the one modal
            // whose whole job is scrolling through a long patch. The wheel
            // scrolls the diff and a click on the file rail selects that file.
            AppMode::ModalDiff => {
                if self.handle_diff_viewer_mouse(mouse) {
                    return Ok(AppAction::Redraw);
                }
            }
    ...
    /// `true` when the pointer is inside the right sidebar panel painted by the
    /// most recent frame (the hugged card rect in inline mode).
    fn mouse_over_sidebar_panel(&self, mouse: MouseEvent) -> bool {
        self.sidebar_click_rect
            .is_some_and(|rect| Self::rect_contains(rect, mouse.column, mouse.row))
    }
    ...
    fn handle_left_press(&mut self, mouse: MouseEvent) -> Option<AppAction> {
        ...
        // The sidebar card is checked before every transcript affordance: in
        // inline mode it floats OVER the transcript (and its scrollbar column),
        // so without this a click on the card leaked through as a selection
        // press on whatever the card covers. Clicking the panel expands its
        // capped rows (`+N more`) in place.
        if self.mouse_over_sidebar_panel(mouse) {
            self.sidebar.toggle_detail();
            self.clear_hovered_copy_button();
            return Some(AppAction::Redraw);
        }
            ...
            .filter(|rect| Self::rect_contains(*rect, mouse.column, mouse.row))
        {
            ...
            // Read the geometry after `block_id_at_mouse`, which settles the
            // layout — so press and release compare the same resolved state.
            let geometry = self.transcript_geometry();
            self.clear_hovered_copy_button();
            self.transcript
                .begin_char_selection(mouse.column, mouse.row.saturating_sub(rect.y));
            self.transcript_view.transcript_press = Some(TranscriptPress { block, geometry });
        }
    ...
    fn finish_left_release(&mut self, mouse: MouseEvent) -> Option<AppAction> {
        ...
        let press = press?;
        let anchor = press.block?;
        // Enforce release-over-the-same-block only while the transcript held
        // still. Mid-turn it does not: the tail grows and the view re-pins to
        // it between press and release, so re-resolving the release cell names
        // a different block and the click was dropped on the floor.
        if press.geometry == self.transcript_geometry()
            && self.block_id_at_mouse(mouse) != Some(anchor)
        {
            return None;
        }
    ...
    /// Cheap fingerprint of where the transcript rows currently sit: the
    /// resolved scroll offset and the block count. Both change exactly when the
    /// rows shift under a stationary pointer, which is what a click gesture
    /// needs to know.
    fn transcript_geometry(&self) -> (u16, usize) {
        (
            self.transcript.resolved_scroll(),
    ...
    fn refresh_follow_output(&mut self) {
        let following = self.transcript_view_rect().is_none_or(|rect| {
            self.transcript
                .is_at_bottom(rect.height, &self.theme, rect.width, self.image_protocol)
        });
        self.set_follow_output(following);
    }

    /// Set the follow-output intent, mirroring it into the transcript.
    ///
    /// The transcript cannot derive this itself: an explicit `Rows` offset
    /// covers both "the reader scrolled up to read back" and "the view is
    /// top-anchored because content outgrew it", and only the first earns a
    /// backlog badge. Every site that changes the intent routes through here —
    /// a direct field write leaves the badge reading a stale intent, which is
    /// exactly how the badge came to never fire on the real scroll paths.
    fn set_follow_output(&mut self, follow: bool) {
        self.transcript_view.follow_output = follow;
        self.transcript.set_user_parked(!follow);
    }
}
...
fn is_provider_retry_notice(text: &str) -> bool {
    let Some((label, retry)) = text.split_once("; retrying in ") else {
        return false;
    };
    let Some((seconds, attempt)) = retry.split_once("s (attempt ") else {
        return false;
    };
    let Some((attempt, _)) = attempt.split_once(')') else {
        return false;
    };
    !label.is_empty()
        && !seconds.is_empty()
