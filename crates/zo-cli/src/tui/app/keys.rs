    ...
    fn dispatch_nav_key(&mut self, key: KeyEvent) -> Option<AppAction> {
        match self.mode {
            ...
            AppMode::Focus => match key.code {
                ...
                // Same plumbing as the Normal-mode scroll keys: normalize the
                // tail sentinel first, then record the reader's intent. Moving
                // the viewport without it left follow-output armed, so the next
                // block snapped the reader back to the tail mid-read and the
                // backlog badge never fired.
    ...
    fn handle_queued_input_key(&mut self, key: KeyEvent) -> Option<AppAction> {
        ...
        if !self.input_enabled && matches!(self.mode, AppMode::Normal | AppMode::ModalWorkflow) {
            ...
            if key.code == KeyCode::Enter && !enter_with_modifier {
                ...
                // One classifier for both the steering decision and the hint,
                // so the transcript can never claim a reason the queue did not
                // actually apply.
                let no_steer = NoSteerReason::classify(
                    trimmed.starts_with('/'),
                    !images.is_empty(),
                    earlier_all_steered,
                );
                let steer_text = if no_steer.is_none() {
                ...
                } else if let Some(reason) = no_steer {
                    // Steered entries get the runtime's `⤷ steering:` echo;
                    // deferred ones get this line so the silence is explained.
                    self.note_queued_without_steering(reason);
                }
                self.input.clear();
                return Some(AppAction::None);
            }
