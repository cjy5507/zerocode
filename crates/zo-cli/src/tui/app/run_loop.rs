...
impl App {
    /// Draw when the shared frame gate allows it, reporting whether the frame
    /// was painted (a caller's `dirty` flag is the inverse).
    ///
    /// Both stamps are load-bearing. `note_draw_cost` feeds the adaptive
    /// interval, so a terminal painting slower than the frame grid degrades
    /// frame rate instead of input latency. `note_stream_draw` re-stamps the
    /// budget at draw *completion*: `on_stream_update` marks draw-start, so
    /// without it a paint that itself outlasts the frame interval lets the very
    /// next event measure "elapsed ≥ interval" and repaint at once — collapsing
    /// coalescing back to one full redraw per event, which is the input-lag /
    /// wheel-flood case every caller below exists to avoid.
    fn draw_gated<B: Backend>(
        &mut self,
        terminal: &mut Terminal<B>,
        gate: &mut StreamFrameGate,
    ) -> Result<bool, TuiError>
    where
        B::Error: std::fmt::Display,
    {
        if !gate.on_stream_update(Instant::now()).draws_now() {
            return Ok(false);
        }
        let draw_started = Instant::now();
        self.draw(terminal)?;
        gate.note_draw_cost(draw_started.elapsed());
        gate.note_stream_draw(Instant::now());
        Ok(true)
    }
                ...
                maybe_event = events.next() => {
                    match maybe_event {
                        Some(Ok(Event::Key(key))) => {
                            match self.handle_key(key)? {
                                ...
                                AppAction::ManageProvider(action) => {
                                    return Ok(AppAction::ManageProvider(action));
                                }
                                ...
                                // `TerminalResized` is produced by the resize
                                // arm below, never by a keystroke.
                                AppAction::Redraw
                                | AppAction::TerminalResized
                                | AppAction::None => {}
                            }
                            // Coalesce keystroke repaints to the shared frame
                            // cadence: a full widget-tree redraw per keystroke
                            // floods slower terminals faster than they paint,
                            // which reads as input lag. The first keystroke still
                            // draws immediately (the gate starts ready); a burst
                            // within the frame interval defers to the next tick.
                            dirty = !self.draw_gated(terminal, &mut frame_gate)?;
                        }
                        Some(Ok(Event::Paste(text))) => {
                            self.handle_paste_owned(text);
                            // Same frame coalescing as keystrokes: a rapid
                            // paste/keystroke burst shares one frame budget.
                            dirty = !self.draw_gated(terminal, &mut frame_gate)?;
                        }
                        Some(Ok(Event::Mouse(mouse))) => {
                            ...
                            // A trackpad's inertial scroll fires dozens–hundreds
                            // of ScrollUp/Down events per flick; the offset is
                            // already accumulated in `handle_mouse`, so repaint
                            // at most once per frame budget and let the tick arm
                            // land the final frame via `dirty`. Raw move/click
                            // events fall through without painting.
                            if matches!(action, AppAction::Redraw) || is_scroll {
                                dirty = !self.draw_gated(terminal, &mut frame_gate)?;
                            }
                        }
                        Some(Ok(Event::Resize(..))) if self.terminal_mode.is_inline() => {
                            // An inline viewport keeps the height it was built
                            // with — ratatui offers no setter — so a taller
                            // window would leave the extra rows permanently
                            // unpainted and a shorter one would push the
                            // composer past the bottom edge. Only the outer
                            // loop can rebuild it (this one is generic over the
                            // backend), so yield and let it re-fit.
                            return Ok(AppAction::TerminalResized);
                        }
                        Some(Ok(_)) => {
                            // Focus changes, fullscreen resizes (ratatui
                            // autoresizes those itself), etc. — defer to the
                            // tick to avoid event-flood repaints.
                            dirty = true;
                        }
                ...
                maybe_block = self.rx.recv() => {
                    match maybe_block {
                        Some(block) => {
                            ...
                            // path too — gated so a fast burst can't redraw
                            // faster than the frame grid.
                            dirty = !self.draw_gated(terminal, &mut frame_gate)?;
                        }
                        None => {
