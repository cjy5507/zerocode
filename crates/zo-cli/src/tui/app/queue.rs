...
/// Why an Enter-queued entry could not ride the steering channel into the
/// running turn. Every variant is a *deferral*, never a rejection: the entry is
/// queued either way and auto-submits as its own turn in FIFO order — the
/// distinction the user needs is that it will not reach the model until then.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum NoSteerReason {
    /// Slash commands are dispatched by the host, not folded into a live turn.
    SlashCommand,
    /// The steering channel carries text only, so image-bearing entries wait.
    Images,
    /// Head-of-line guard: an earlier queued entry did not steer, so steering
    /// this one would deliver it out of order (`keys.rs` FIFO guard).
    BehindUnsteered,
}

impl NoSteerReason {
    /// Classify a composed entry the moment it is queued. `None` means the
    /// entry rode the steering channel and needs no explanation — the runtime's
    /// `⤷ steering:` echo is its receipt.
    pub(crate) const fn classify(
        starts_with_slash: bool,
        has_images: bool,
        earlier_all_steered: bool,
    ) -> Option<Self> {
        if starts_with_slash {
            Some(Self::SlashCommand)
        } else if has_images {
            Some(Self::Images)
        } else if earlier_all_steered {
            None
        } else {
            Some(Self::BehindUnsteered)
        }
    }

    /// The one-line transcript hint: WHY it did not steer and WHEN it runs.
    pub(crate) const fn hint(self) -> &'static str {
        match self {
            Self::SlashCommand => {
                "queued \u{00b7} runs after this turn (slash commands cannot steer a live turn)"
            }
            Self::Images => {
                "queued \u{00b7} runs after this turn (messages with images cannot steer a live turn)"
            }
            Self::BehindUnsteered => {
                "queued \u{00b7} runs after this turn (it keeps its place behind an earlier queued entry)"
            }
        }
    }
}
...
impl App {
    ...
    /// Explain why a just-queued entry did NOT steer the live turn. A steered
    /// entry announces itself with the runtime's `⤷ steering:` echo; without
    /// this note an unsteered entry looks identical from the composer's side,
    /// so the user cannot tell that their message is waiting for the next turn
    /// instead of cutting into this one. Same one-line transcript surface as
    /// [`Self::report_queue_limit_error`] — no new UI.
    ///
    /// Typing three slash commands in a row must not stamp the same sentence
    /// three times, so an identical hint sitting at the tail of the transcript
    /// suppresses this one; any intervening output re-arms it.
    pub(crate) fn note_queued_without_steering(&mut self, reason: NoSteerReason) {
    ...
    ///
    /// The composer badge goes with them. It counts what is still attached to
    /// the composer, and these have just left it — an image turn that went
    /// through the queue re-stages the badge when its entry pops
    /// ([`Self::stage_queued_images_for_submit`]), so a drain that left the count
    /// standing parked a phantom `[image: image/png]` in the composer for the
    /// rest of the session, with no attachment behind it.
    pub fn take_pending_images(&mut self) -> Vec<ImageAttachment> {
        let images = std::mem::take(&mut self.pending_images);
        self.input.clear_images();
        images
    }
    ...
    /// the app, so the two can drift: submitting clears the composer (badge to
    /// zero) but a *local slash command* never reaches the model and so never
    /// drains the attachments — leaving a pasted image staged and invisible,
    /// ready to ride an unrelated later turn. Called every time the loop returns
    /// to the prompt, which makes the badge self-healing rather than something
    /// every exit path has to remember.
    pub fn resync_image_badge(&mut self) {
        let staged = self.pending_images.len();
        self.input.clear_images();
        for _ in 0..staged {
            self.input.add_image();
        }
