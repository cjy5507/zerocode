...
use super::{
    ...
    CustomProviderPrefill, CustomProviderWizardModal, DeepTierModal, DeepTierView, DiffViewerModal,
    EffortPickerModal,
...
impl App {
    ...
    /// Open the `/providers` manager over the current registrations.
    pub fn open_provider_manager_modal(
        &mut self,
        ...
        self.set_active_modal(
            Box::new(ProviderManagerModal::new(accounts, rows, settings_path)),
            AppMode::ModalProviders,
        );
    }

    /// Open the `/providers` edit form on an already-registered provider.
    pub fn open_provider_edit_modal(&mut self, prefill: &CustomProviderPrefill) {
        self.set_active_modal(
            Box::new(CustomProviderWizardModal::editing(prefill)),
            AppMode::ModalCustomProvider,
        );
    }

    /// Re-list the open `/providers` manager after a mutation, so the tree
    /// reflects the write without the user reopening it. No-op when the
    /// manager is not the active modal (an edit form has taken the slot).
    pub fn refresh_provider_manager_modal(
        &mut self,
        accounts: Vec<ProviderAccountRow>,
        rows: Vec<ProviderManagerRow>,
    ) {
        if let Some(modal) = self.active_modal_as::<ProviderManagerModal>() {
            modal.refresh(accounts, rows);
        }
    }

    /// Remember that the add/edit wizard was reached from the manager, so the
    /// host can return there once the form resolves.
    pub fn mark_provider_manager_return(&mut self) {
        self.reopen_provider_manager = true;
    }

    /// Consume the return flag; `true` exactly once per hand-off.
    pub fn take_provider_manager_return(&mut self) -> bool {
        std::mem::take(&mut self.reopen_provider_manager)
    }
    ...
    pub(super) fn apply_modal_outcome(&mut self, result: Option<ModalResult>) -> AppAction {
        match result {
            ...
            Some(ModalResult::Cancelled) => {
                // Backing out of the add/edit form returns to the list it was
                // opened from — re-entered through the slash path, so the
                // manager is rebuilt by its one owner instead of being
                // reconstructed from stale rows here.
                let return_to_manager = matches!(self.mode, AppMode::ModalCustomProvider)
                    && self.take_provider_manager_return();
                self.exit_modal();
                if return_to_manager {
                    return AppAction::Submit("/providers".to_string());
                }
                AppAction::None
            }
    ...
    fn apply_modal_selection(&mut self, selection: ModalSelection) -> AppAction {
        match selection {
            ...
            // Add/Edit hand off to another surface, which replaces the manager
            // on the slot; the mutations keep the manager open so several
            // deletes can be made in one visit. Closing is decided per action by
            // the host handler, so nothing is closed here.
            ModalSelection::ProviderManage(action) => match action {
                // Connecting an account re-enters the text path, so the
                // provider's own `/login` handler stays the single authority on
                // OAuth instead of a second copy living in the manager.
                ProviderManagerAction::ConnectAccount { id } => {
                    self.mark_provider_manager_return();
                    self.exit_modal();
                    AppAction::Submit(format!("/login {id}"))
                }
                action => AppAction::ManageProvider(action),
            },
...
fn preset_models(models: &[&str]) -> Vec<String> {
