...
impl App {
    ...
    ///
    /// Inline mode zeroes [`App::synchronized_output`] when the terminal mode is
    /// chosen, but it still repaints a primary-screen band while Markdown
    /// streams. Read the terminal's real capability so supported hosts present
    /// that multi-write paint atomically instead of exposing intermediate rows.
    /// `ZO_SYNC_OUTPUT=0` still forces this off.
    fn frame_synchronized_output(&self) -> bool {
        sync_bracket_enabled(
            self.synchronized_output,
            self.terminal_mode.is_inline(),
            crate::tui::term::TermProfile::current().synchronized_output,
        )
    }
    ...
    /// inline shutdown, never while wheel/drag/click interaction is live.
    ///
    /// Callers outside this module must use [`Self::draw`].
    #[allow(clippy::too_many_lines)] // cohesive full-frame TUI render (layout + all panels)
    fn draw_prepared<B: Backend>(&mut self, terminal: &mut Terminal<B>) -> Result<(), TuiError>
    where
        B::Error: std::fmt::Display,
    {
        ...
        let mut rendered_sidebar_panel: Option<Rect> = None;
        ...
        terminal
            .draw(|frame| {
                ...
                // Soft-wrap the input against its *real* inner width so the box
                // grows to the right number of rows before the vertical layout
                // runs. Reserve the one-cell right rail through the same helper
                // the painter uses; otherwise an exactly fitting line would be
                // measured wider than it is drawn and the transcript would jump
                // while typing.
                let input_inner_w = crate::tui::input::composer_content_width(
                    LayoutRegions::content_width(area.width, sidebar_visible),
                );
                ...
                // The footer asks for its own height; a short terminal still
                // gets clamped by `compute_with_sidebar`. The sidebar already
                // showing the workflow phase is the one case where the third
                // row would only repeat what is on screen.
                let hud_rows = if sidebar_carries_phase || area.height < 10 {
                    layout::HUD_ROWS
                } else {
                    hud::desired_rows(&self.hud_state)
                };
                ...
                if regions.sidebar_width > 0 {
                    ...
                    rendered_sidebar_panel = sidebar::draw(
                        frame,
                        full_height_sidebar,
                        &self.sidebar,
                        &self.hud_state,
                ...
                // Inline mode never splits the transcript for the panel — doing
                // so would reflow the conversation every time it is toggled, and
                // the transcript here is scrollback the terminal owns. The panel
                // floats over the right edge instead, so closing it returns the
                // full width with no relayout and no reflow of settled output.
                //
                // Which means it has to be painted *after* the transcript. Drawn
                // before, every frame put the panel down and then painted the
                // transcript straight over it at full width — the panel was
                // erased on the same frame that drew it.
                let inline_sidebar_width = (self.terminal_mode.is_inline() && self.sidebar.visible)
                    .then(|| layout::resolved_sidebar_width(regions.transcript.width, true))
                    .filter(|width| *width > 0 && *width < regions.transcript.width);
                ...
                // Live-turn slack: keep one blank row between the transcript's
                // last rendered row and the activity line directly below it.
                // Inline mode sinks a short conversation onto the region's
                // bottom edge, which put a washed user block flush against the
                // spinner row — one glued band with no boundary between the
                // conversation and the machine's status. One row of air is the
                // boundary. Only while a turn is live: a settled frame keeps the
                // whole region, so idle rendering is untouched. The floor keeps
                // the reservation from eating into
                // `MIN_READABLE_TRANSCRIPT_ROWS` on a tiny viewport.
                if self.turn_activity.is_some()
                    && transcript_area.height > layout::MIN_READABLE_TRANSCRIPT_ROWS
                {
                ...
                if let Some(startup_screen) = self.startup.screen.as_ref() {
                    ...
                    if self.transcript.is_empty() {
                        // Nothing has been said yet, so the launchpad IS the
                        // conversation — and inline, a conversation grows up
                        // from the composer. Anchored at the top it left a
                        // field of blank rows between itself and the input,
                        // which is the same "content floats at the top"
                        // complaint the transcript's bottom alignment fixed.
                        let anchor = if self.terminal_mode.is_inline() {
                            startup::Anchor::Bottom
                        } else {
                            startup::Anchor::Top
                        };
                        startup::draw_anchored(
                            frame,
                            transcript_area,
                            startup_screen,
                            &self.theme,
                ...
                if regions.rule_top.height > 0 {
                    ...
                    let indicator_content = Rect::new(
                        indicator.x,
                        indicator.y,
                        crate::tui::input::composer_content_width(indicator.width),
                        indicator.height,
                    );
                    if let Some(activity) = self.turn_activity.as_ref() {
                        ...
                        let effort_badge = effort_rule_badge(&self.hud_state, &self.theme)
                            .filter(|(_, width)| *width > 0 && *width < indicator_content.width);
                        let rule_width = effort_badge.as_ref().map_or(
                            indicator_content.width,
                            |(_, width)| {
                                indicator_content
                                    .width
                        ...
                        if rule_width > 0 {
                            ...
                            if self.theme.no_color {
                                ...
                                // This hairline is now the *only* line above the
                                // composer, so it carries the reasoning-effort
                                // colour the composer's removed top border used
                                // to show. Together with the composer's bottom
                                // border it brackets the input in one hue.
                                let effort_style = crate::tui::input::effort_border_style(
                                    &self.theme,
                                    self.hud_state.effort,
                                );
                    ...
                    if indicator.width > 0 {
                        let closure = if self.turn_activity.is_some() {
                            "│"
                        } else {
                            "┐"
                        };
                        let closure_area = Rect::new(
                            indicator.x + indicator.width.saturating_sub(1),
                            indicator.y,
                            1,
                            1,
                        );
                        frame.render_widget(
                            Paragraph::new(Span::styled(
                                closure,
                                crate::tui::input::effort_border_style(
                                    &self.theme,
                                    self.hud_state.effort,
                                ),
                            )),
                ...
                if !self.queued_messages.is_empty() && regions.rule_top.height > 0 {
                    ...
                    let badge_host_width = crate::tui::input::composer_content_width(
                        regions.rule_top.width,
                    );
                    if badge_width < badge_host_width {
                        let badge_x =
                            regions.rule_top.x + badge_host_width.saturating_sub(badge_width);
                ...
                // Floating inline panel, painted last over the transcript and the
                // bottom overlay stack. `transcript_area` after the overlays took
                // their rows is the *budget* it may use, not its height: the card
                // measures its content and hugs it, clearing only the rows it
                // paints so the transcript keeps its right columns beside a short
                // panel.
                if let Some(width) = inline_sidebar_width {
                    // Inset one column from the right edge: the transcript's
                    // scrollbar lives in the last column, and a card flush
                    // against it clipped the track to the rows below the card —
                    // the dotted rail appeared to start at the card's bottom
                    // corner, reading as a broken border.
                    let panel = Rect::new(
                        transcript_area
                            .right()
                    ...
                    let st = perf_on.then(std::time::Instant::now);
                    rendered_sidebar_panel = sidebar::draw_floating(
                        frame,
                        panel,
                        &self.sidebar,
                        &self.hud_state,
                ...
                if regions.hud.height > 0 {
                    ...
                    // Narrower than `details_owned_elsewhere`: a *visible*
                    // sidebar may still be too short to carry the phase, and
                    // then the footer is the only surface that can show it.
                    let workflow_owned_elsewhere =
                        sidebar_carries_phase || self.turn_activity.is_some();
                    hud::draw_with_heat(
                        frame,
                        regions.hud,
                        &self.hud_state,
                        &self.theme,
                        hud::FooterOwnership {
                            ledger_visible: details_owned_elsewhere,
                            // The pinned agent panel already shows the fleet in
                            // detail — drop the redundant bottom-bar chip.
                            agent_panel_visible: rendered_agent_panel.is_some(),
                            workflow: workflow_owned_elsewhere,
                        },
                        heat_state,
                    );
                }
        ...
        self.sidebar_click_rect = rendered_sidebar_panel;
        // Reclaim the styled render cache for blocks the user has scrolled well
        // past. Post-draw on purpose: `resolved_scroll` and `cached_layout` now
        // describe the frame just painted, and nothing shed here can affect a
        // frame already on screen.
        //
        // No size threshold: the scan is integer comparisons over
        // `cached_layout` and returns immediately when it is empty, and the only
        // allocation work is dropping lines for blocks that just left the keep
        // margin — once per block, not once per frame. Heights come from
        // `body_rows`/`row_prefix`, so this cannot move anything, and the draw
        // arms treat empty lines as a cache miss rather than a blank row.
        if let Some(rect) = rendered_transcript_rect {
            self.transcript.shed_offscreen_styled_lines(rect.height);
        }
    ...
    /// Emit the retained inline transcript during shutdown without composing a
    /// viewport frame.
    ///
    /// Live draws never call `insert_before`: wheel, click, drag, and typing use
    /// one app-owned history. Shutdown calls this immediately before clearing
    /// the viewport so the completed conversation survives in native scrollback.
    ...
    /// A card thinner than this — a title, one option, and its borders — reads
    /// as a rendering glitch rather than a prompt.
    const MIN_INLINE_MODAL_ROWS: u16 = 5;

    /// Whether the active surface genuinely cannot be drawn in the inline
    /// viewport.
    ///
    /// Almost never, now. This list used to send document-scale viewers — the
    /// agent tree, diffs, the pager — to a `needs full-screen mode` notice,
    /// because the inline viewport was a fixed twelve rows and they had nowhere
    /// to go. The viewport is the terminal now, so those surfaces have exactly
    /// the room they had on the alternate screen and there is nothing left to
    /// degrade. Only a viewport too short to host a legible card still falls
    /// back, which needs a terminal of a handful of rows.
    fn inline_surface_requires_fullscreen(&self, area: Rect) -> bool {
        self.terminal_mode.is_inline() && area.height < Self::MIN_INLINE_MODAL_ROWS
    }
    ...
    fn draw_pager(&self, frame: &mut ratatui::Frame, area: Rect) {
        ...
        // Measured with the engine that paints, not a `chars().count()` fill
        // estimate. The estimate was wrong twice over: it counted *characters*
        // where the terminal spends *cells*, so a Hangul line reported half its
        // real height and `max_scroll` stopped short of the end — the tail of a
        // Korean document was simply unreachable; and it divided instead of
        // breaking on words, so it disagreed with the renderer on any line that
        // wraps mid-word. See `blocks::wrapped_rows`: never a second height
        // formula.
        // Wrapped once, then both measured and painted from that same result —
        // scroll extent and pixels cannot drift apart if there is only one wrap.
        //
        // Not `.wrap(...)`: ratatui's `WordWrapper` drops a 2-cell grapheme that
        // lands on the break (see `tui::text_metrics`), and the pager shows
        // arbitrary text — file bodies, command output, model prose — where CJK
        // is ordinary. The extent was separately wrong: it counted `chars()`
        // where the terminal spends cells, so a Hangul document reported half
        // its height and its tail was unreachable by scrolling.
        let body_lines: Vec<Line<'_>> = self
            .pager_lines
            .iter()
            .map(|line| Line::from(Span::styled(line.as_str(), self.theme.typography.body)))
            .collect();
        let body_rows = crate::tui::blocks::wrap_rows(&body_lines, body_rect.width);
        let total_lines = u16::try_from(body_rows.len()).unwrap_or(u16::MAX);
        ...
        let body_para = Paragraph::new(body_rows).scroll((scroll, 0));
...
/// Pure CSI ?2026 bracket decision for one frame.
///
/// Split out of [`App::frame_synchronized_output`] so the inline case is
/// testable without reaching the process-wide [`crate::tui::term::TermProfile`]
/// snapshot: `field` is what the terminal-mode switch left on the app (inline
/// zeroes it), `inline` is the current mode, `term_supports` the real capability.
const fn sync_bracket_enabled(field: bool, inline: bool, term_supports: bool) -> bool {
    field || (inline && term_supports)
}

#[cfg(test)]
mod sync_bracket_tests {
    use super::sync_bracket_enabled;

    /// Inline repaints a primary-screen band through multiple terminal writes
    /// while Markdown and the composer change. The terminal-mode switch zeroes
    /// the app field, so the capability fallback is what keeps those writes in
    /// one presented frame on supported hosts.
    #[test]
    fn inline_frames_bracket_when_the_terminal_supports_synchronized_output() {
        assert!(sync_bracket_enabled(false, true, true));
    }

    /// An unlisted terminal keeps today's unbracketed behavior, and
    /// `ZO_SYNC_OUTPUT=0` reaches this through `term_supports` as the escape
    /// hatch for a listed terminal that misbehaves.
    #[test]
    fn inline_frames_stay_unbracketed_without_terminal_support() {
        assert!(!sync_bracket_enabled(false, true, false));
    }

    /// The fullscreen path is unchanged: its own resolved flag still decides.
    #[test]
    fn fullscreen_bracket_still_follows_the_app_flag() {
        assert!(sync_bracket_enabled(true, false, false));
        assert!(!sync_bracket_enabled(false, false, true));
    }
}
...
#[cfg(test)]
mod composer_border_layout_tests {
    use super::Rect;
    use crate::tui::app::AppMode;
    use crate::tui::input::InputWidget;
    use crate::tui::layout::LayoutRegions;

    /// The composer's chrome consumes 3 rows of height, and only one of them is a
    /// border: `input::input_block` has no top edge (the `rule_top` row above
    /// already separates the composer from the transcript) but keeps a blank row of
    /// air above *and* below, so the text reads as centred between the two rules
    /// and the streaming spinner on `rule_top` never touches the input line. The
    /// surrounding layout must account for all three.
    /// `InputWidget::desired_rows` already includes them in its count, so wiring
    /// it through `LayoutRegions::compute` must leave the transcript 2 rows
    /// shorter than a hypothetical chrome-less composer of the same content.
    #[test]
    fn transcript_viewport_shrinks_by_the_composer_chrome_rows() {
        let area = Rect::new(0, 0, 80, 24);
        let input = InputWidget::new();

        // The composer reserves one cell for its right rail (mirrors the
        // measurement in `App::draw`).
        let inner_w = crate::tui::input::composer_content_width(
            LayoutRegions::content_width(area.width, false),
        );
        let bordered_rows = input.desired_rows(inner_w, AppMode::Normal);
        // An empty composer's content is a single row; the chrome adds 3.
        let borderless_rows = bordered_rows - 3;

        let bordered = LayoutRegions::compute(area, bordered_rows).expect("bordered layout");
        let borderless = LayoutRegions::compute(area, borderless_rows).expect("borderless layout");

        assert_eq!(
            borderless.transcript.height - bordered.transcript.height,
            3,
            "the chrome's 3 rows must be taken out of the transcript viewport"
        );
        // The reclaimed rows land on the input box, not lost chrome.
        assert_eq!(bordered.input.height - borderless.input.height, 3);
    }
}
