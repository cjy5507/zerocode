...
use crate::tui::modals::{CustomProviderDraft, ProviderManagerAction, SmartSettingsCommit};
...
pub enum AppMode {
    ...
    ModalCustomProvider,
    /// `/providers` register / edit / delete manager is open.
    ModalProviders,
...
impl std::fmt::Display for AppMode {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ModalProviders => f.write_str("ModalProviders"),
...
pub enum AppAction {
    ...
    /// A register / edit / delete request from the `/providers` manager.
    ManageProvider(ProviderManagerAction),
    ...
    /// The terminal changed size. An inline viewport's height is fixed at
    /// `Terminal` construction, so only the outer loop — which owns the concrete
    /// backend — can re-fit it; this loop is generic over `B: Backend` and
    /// cannot. Yielding lets that happen before the next paint, then the loop is
    /// re-entered unchanged.
    TerminalResized,
...
impl PartialEq for AppAction {
    ...
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            | (Self::TerminalResized, Self::TerminalResized)
            ...
            (Self::ManageProvider(left), Self::ManageProvider(right)) => left == right,
