...
pub(super) fn effort_modal_rect(regions: &LayoutRegions, area: Rect) -> Rect {
    ...
    // The ladder list is one row per effort level plus a blank + key hints:
    // 7 + 2 = 9 content rows, then 2 rules + 2 vertical padding = 13.
    let height = 13u16.min(host.height.saturating_sub(2).max(9));
...
///
/// Full host **width** on purpose. These viewers are Pi dialogs: two full-width
/// rules and no side borders. A 96%-wide centered rect left a 2-4 column margin
/// down each side where the frosted transcript still showed — and with no side
/// border to terminate the dialog, those muted glyph fragments read as content
/// leaking *inside* the modal rather than as the backdrop they are. Spanning the
/// host width lets the rules reach both edges, which is what closes the dialog
/// horizontally. The vertical margin stays: the top and bottom rules already
/// terminate the dialog there, so the backdrop above/below reads correctly.
pub(super) fn diff_modal_rect(regions: &LayoutRegions, area: Rect) -> Rect {
    ...
    let height = (host.height * 90 / 100)
        ...
        .min(host.height);
    let y = host.y + host.height.saturating_sub(height) / 2;
    Rect::new(host.x, y, host.width, height)
}
...
pub(super) fn anchored_modal_rect(app: &App, regions: &LayoutRegions, area: Rect) -> Rect {
    ...
    // Cap against the room that actually exists above the composer rather than
    // a fixed margin. In the short inline viewport the two differ, and the
    // fixed margin let a card extend past its own anchor and paint over the
    // input row it was supposed to sit above.
    let above_input = if input.height == 0 || input.y <= area.y {
        area.height.saturating_sub(4)
    } else {
        input.y.saturating_sub(area.y).saturating_sub(2)
    };
    let height = height.min(above_input).max(1);
