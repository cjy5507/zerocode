...
// (highlight on/off, marker sniffing, callout break), not a configuration
// record, so collapsing them into a struct adds no clarity.
...
impl<'t> Renderer<'t> {
    ...
    fn on_inline_code(&mut self, text: &str) {
        // Markdown delimiters change style, not content geometry. Preserving
        // exact source adjacency keeps mixed prose such as `1.26.3`로 and
        // x`code`y stable while the foreground color carries the distinction.
        let style = code_inline_style(self.theme);
...
/// 공유하는 단일 출처(roadmap ⑨). 상단 보더(`  ╭─ {lang} ───╮`), 각 행의
/// 좌우 레일(`  │ ` … `│`) + 코드 배경, 그리고 `with_bottom_border` 면 하단 보더
/// (`  ╰───╯`). 스트리밍은 펜스가 아직 열려 있어 하단 보더를 생략하고 light 행을
/// 넘기며, done 은 syntect 행 + 하단 보더를 넘긴다 — 레일/래핑/줄수는 양쪽이
/// **구성상** 동일(색만 다름)이라 settle 시 reflow/스크롤 점프가 없다. 입력은
/// `rows`(이미 색칠된 코드 행)라 호출부가 highlighter(syntect / light / diff)를
/// 고르고 이 fn 은 모드 무관하게 프레임만 입힌다.
pub(super) fn code_card_frame_lines(
    ...
    // Four cells lead each body row (`  │ `), one closes it (`│`), and the
    // remainder is code surface. Truncate explicitly before padding so a long
    // CJK/code row cannot push the closing rail past the viewport; the source
    // text remains untouched in the transcript model.
    let body_width = target.saturating_sub(1);
    for row in rows {
        let styled: Vec<Span<'static>> = row
            ...
            .collect();
        let mut body = crate::tui::text_metrics::truncate_line_to_cells(
            Line::from(styled),
            body_width,
        )
        .spans;
        ...
        framed.push(Span::styled("  │ ".to_string(), border_style));
