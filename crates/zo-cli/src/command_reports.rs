...
pub(crate) fn build_council_prompt(task: Option<&str>) -> String {
    ...
    format!(
        "Run a council comparison for this task:\n\n{task}\n\n\
         Steps:\n\
         1. Use `SpawnMultiAgent` to launch exactly three independent candidate agents for the same task. Keep each candidate focused on producing its best answer, not on judging the others, and tell each one to end its reply with a final line of exactly the form `VERDICT: <its conclusion in one short line>`, stating the decision itself rather than reasoning about it.\n\
         2. When the candidates finish, call `Council` with each candidate's answer text VERBATIM — including its final `VERDICT:` line — plus its completion status. There is no separate verdict field to fill in: the Council reads each verdict out of the answer that candidate wrote, so you cannot assert agreement on a candidate's behalf. Do not include model names, agent names, or any other source identity in the `Council` candidates. Agreement is judged on the verdicts only when every successful candidate declared one; otherwise it falls back to comparing whole answers, which independently written replies almost never match — the returned `voted_on` tells you which basis decided it.\n\
         ...
         5. Never call an LLM judge before `Council`, after `best_of`, or more than `llm_judge_call_limit` times. Nothing enforces that budget for you — it is your obligation, and exceeding it spends adjudication the comparison did not earn."
    )
}
