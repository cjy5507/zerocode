...
pub(crate) struct RouteHint {
    ...
    /// The turn's FAN-OUT difficulty: how hard this turn is *to split*, which
    /// decides how many agents a fan-out spawns
    /// (`fanout::fanout_width_for`) and — via
    /// [`Self::should_host_prespawn`] — whether the host may pre-spawn at all.
    ///
    /// NOT simply the router's `RouteTaskComplexity`. That table measures
    /// IMPLEMENTATION size, and by that measure a read-only investigation is
    /// `Small` no matter how much ground it covers: "코드베이스 전반을
    /// 분석해서 성능 병목 전수 조사해줘" classifies `Small` because it edits
    /// nothing. Keying fan-out on it made the gate structurally unreachable
    /// for exactly the turns fan-out exists for — measured, not assumed (a
    /// probe over six representative breadth prompts returned `Small` for
    /// five of them). The two vocabularies had drifted apart, too: the
    /// router's `Large` list wants the exact phrases "코드베이스 전체" /
    /// "레포 전체", while the host's [`MULTI_SCOPE_INTENT`] covers "전반",
    /// "여러", "각각", "프로젝트" — so the host could see breadth the router
    /// could not.
    ///
    /// See [`fanout_difficulty`] for how the two axes combine.
    complexity: runtime::RouteTaskComplexity,
    /// Whether the HOST consumes this turn by pre-spawning agents, decided
    /// once at construction (see [`resolve_host_prespawn`]) rather than
    /// re-derived from a spread of flags at every read.
    ///
    /// It used to be re-derived, from `explicit_parallel || ultracode`, and
    /// the "evidence must never pre-spawn" contract was encoded IMPLICITLY
    /// by forcing both of those off at that construction site. Adding
    /// difficulty as a third input silently broke that encoding — the flags
    /// were off but the gate read something else. Resolving once, where
    /// every input is in scope, means a new input cannot reopen a closed
    /// gate behind a caller's back.
    host_prespawn: bool,
}
...
impl RouteHint {
    ...
    /// The turn's difficulty verdict, for the host's resource allocation
    /// (fan-out width). Read from the hint rather than re-classified so the
    /// gate that opened the fan-out and the width it spawns can never
    /// disagree about how hard the turn is.
    pub(crate) fn complexity(&self) -> runtime::RouteTaskComplexity {
        self.complexity
    }
...
/// The turn's fan-out difficulty: the HARDER of the router's
/// implementation-size verdict and what the host's own breadth evidence
/// implies.
///
/// Derived from what each input MEANS, not from a tuned table:
/// - several named scopes (`multi_scope`) is direct evidence of independent
///   slices, which is precisely what a fan-out splits into — at least the
///   narrowest genuine split;
/// - several scopes AND enough accumulated context to strain one agent's
///   window (`large_ctx`) is the full-band case: there is both something to
///   split and a reason one context should not hold it all.
///
/// The escalation is one-directional (`max`): breadth can raise a
/// read-only investigation the size table calls `Small`, but it never lowers
/// a genuinely large implementation. That direction is the whole point —
/// the size table is right about edits and blind to reading.
/// What the host's own scan found about a turn's breadth — the two signals
/// [`fanout_difficulty`] reads, named so a caller cannot transpose them.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct BreadthEvidence {
    /// The ask names several directories/modules/perspectives.
    multi_scope: bool,
    /// Enough accumulated context that one agent's window is strained.
    large_ctx: bool,
}

fn fanout_difficulty(
    table: runtime::RouteTaskComplexity,
    scope: BreadthEvidence,
) -> runtime::RouteTaskComplexity {
    use runtime::RouteTaskComplexity as C;
    let breadth = match (scope.multi_scope, scope.large_ctx) {
        (true, true) => Some(C::Large),
        (true, false) => Some(C::Medium),
        (false, _) => None,
    };
    let Some(breadth) = breadth else {
        return table;
    };
    // `RouteTaskComplexity`'s derived order puts `Unknown` last, so rank
    // explicitly rather than comparing the enum.
    let rank = |value: C| match value {
        C::Trivial => 0,
        C::Small => 1,
        C::Unknown => 2,
        C::Medium => 3,
...
/// Resolve, once and where every input is in scope, whether the HOST consumes
/// this turn by pre-spawning agents.
///
/// Three ways the gate opens, in descending directness:
/// - `explicit_parallel` — the user asked for orchestration in words;
/// - `ultracode` — the standing opt-in;
/// - a CONFIGURED delegation effort plus a difficulty that fills the whole
///   fan-out band.
///
/// That last clause is what closed the "research always runs as one agent"
/// gap. The gate used to open only for the first two, precisely because a
/// breadth PHRASE ("분석해줘") was too weak to spend four agents on and there
/// was no second signal to lean on. Difficulty is that second signal: not an
/// independent opinion (both readings come from the same classification
/// pipeline), but a genuinely additional requirement, so a phrase table can
/// no longer consume a turn by itself. Below the bar the harness still nudges
/// the model toward `SpawnMultiAgent` and lets it decide, exactly as before.
///
/// The bar is DERIVED: the task must be hard enough to use the entire width
/// band (`fanout_width_for` returning the cap), because pre-spawning takes
/// the turn away from the model and a two-agent split is too marginal to be
/// worth that. A ladder change re-derives the bar instead of stranding a
/// hand-picked rank.
///
/// `model_led_only` is a hard veto for hints that may only NUDGE — the
/// evidence-driven fallthrough, whose whole job is to catch turns the keyword
/// tables missed. It fires broadly, so letting it consume turns is the
/// self-DoS risk the shape ladder was built to avoid.
///
/// Cheap by construction: a wrong "yes" degrades to the model-led turn rather
/// than a spawn storm, because the decomposition it triggers is itself
/// difficulty-vetoed and the caller requires two or more subtasks before any
/// agent runs.
fn resolve_host_prespawn(
    shape: RouteShape,
    complexity: runtime::RouteTaskComplexity,
    consent: PrespawnConsent,
) -> bool {
    if shape != RouteShape::FanoutParallel {
        return false;
    }
    match consent {
        PrespawnConsent::ExplicitDirective => true,
        PrespawnConsent::DifficultyGated {
            effort_opted_into_delegation,
        } => {
            effort_opted_into_delegation
                && tools::fanout_width_for(complexity) == Some(tools::MAX_FANOUT_SUBTASKS)
        }
        PrespawnConsent::ModelLedOnly => false,
    }
}

/// How much permission a hint carries to let the HOST consume the turn.
///
/// An enum rather than a row of booleans because the cases are mutually
/// exclusive by construction — and because writing them out revealed one of
/// the old boolean terms was DEAD: the gate read `explicit_parallel ||
/// ultracode`, but no `FanoutParallel` construction site ever set `ultracode`
/// without also setting `explicit_parallel` (the multi-scope path forces it
/// off by design; every other site is a shape the gate rejects anyway). A
/// boolean row hid that; three named cases cannot.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum PrespawnConsent {
    /// The user named an orchestration tool/API in the turn text — the most
    /// direct request there is, honored at any effort.
    ExplicitDirective,
    /// No directive: only a full-band difficulty opens the gate, and only for
    /// a user who CONFIGURED a delegation-capable effort (BUG-R2 — someone
    /// who never set an effort must not discover their harness spending four
    /// agents because a classifier called the turn large; not choosing is not
    /// consenting).
    DifficultyGated { effort_opted_into_delegation: bool },
    /// This hint may only NUDGE the model, never consume the turn.
    ModelLedOnly,
}
...
pub(crate) fn build_route_hint(
    ...
    // "The user configured delegation" — an explicit, delegation-capable
    // effort. Unset stays out (BUG-R2) even though it resolves to a working
    // default elsewhere: not choosing is not consenting.
    let effort_opted_into_delegation = effort.is_some() && !low_effort;
    // One difficulty verdict per turn, carried on the hint: the width
    // derivation and the pre-spawn gate must never read two different
    // answers for the same text. The breadth inputs are computed below and
    // folded in at each construction site via `fanout_difficulty`.
    let table_complexity = tools::assess_turn_complexity(user_text);
    ...
    if is_explicit_parallel_directive(&lower) {
        return RouteHint {
            ...
            canonical_override: None,
            complexity: table_complexity,
            host_prespawn: resolve_host_prespawn(
                RouteShape::FanoutParallel,
                table_complexity,
                PrespawnConsent::ExplicitDirective,
            ),
        };
    }
    ...
    if breadth {
        return breadth_route_hint(
            ultracode,
            BreadthEvidence { multi_scope, large_ctx },
            table_complexity,
            effort_opted_into_delegation,
        );
    }
    ...
    if let Some(hint) = evidence_driven_fallthrough(user_text, ultracode, table_complexity) {
        return hint;
    }
...
fn breadth_route_hint(
    ultracode: bool,
    scope: BreadthEvidence,
    table_complexity: runtime::RouteTaskComplexity,
    effort_opted_into_delegation: bool,
) -> RouteHint {
    let BreadthEvidence { multi_scope, large_ctx } = scope;
    // The breadth path is where the size table is most wrong: a read-only
    // investigation edits nothing, so it classifies `Small` however much
    // ground it covers. Fold the host's own scope evidence in here.
    let complexity = fanout_difficulty(table_complexity, scope);
    let mut confidence: f32 = 0.6;
    if ultracode {
        confidence += 0.1;
    }
    ...
    if multi_scope {
        ...
        return RouteHint {
            ...
            host_prespawn: resolve_host_prespawn(
                RouteShape::FanoutParallel,
                complexity,
                // A natural-language phrase is not an orchestration
                // directive, so difficulty is the only thing that can let
                // this path consume the turn.
                PrespawnConsent::DifficultyGated {
                    effort_opted_into_delegation,
                },
            ),
...
fn evidence_driven_fallthrough(
    user_text: &str,
    ultracode: bool,
    complexity: runtime::RouteTaskComplexity,
) -> Option<RouteHint> {
    ...
    Some(RouteHint {
        ...
        complexity,
        // The fallthrough nudges, never consumes: it fires on broad "agent
        // need" evidence, so it is exactly the path that must not spend the
        // turn on its own. Routed through the resolver with an explicit
        // veto rather than a bare `false`, so the reason is in the type.
        host_prespawn: resolve_host_prespawn(
            shape,
            complexity,
            PrespawnConsent::ModelLedOnly,
        ),
    })
}
...
mod tests {
    ...
    /// **"리서치가 늘 에이전트 1개" 회귀 핀** — 이 게이트가 열리기 전에는
    /// 조사 요청이 `explicit_parallel`/`ultracode` 없이는 절대 호스트 팬아웃을
    /// 못 받았고, 난이도 축은 조사를 전부 `Small`로 읽어 열릴 수도 없었다.
    #[test]
    fn multi_scope_breadth_host_prespawns_once_the_difficulty_fills_the_band() {
        let prompt = "코드베이스 전반을 분석해서 성능 병목 전수 조사해줘";

        // The size table alone reads this as small — it edits nothing. That
        // is exactly why the width must not key on it directly.
        assert_eq!(
            tools::assess_turn_complexity(prompt),
            runtime::RouteTaskComplexity::Small,
            "an investigation edits nothing, so the implementation-size table calls it Small"
        );

        // Breadth evidence raises it. Small context: several scopes but no
        // reason one agent cannot hold them → the narrowest split, model-led.
        let narrow = build_route_hint(prompt, Some(Effort::High), 1_000);
        assert_eq!(narrow.complexity, runtime::RouteTaskComplexity::Medium);
        assert_eq!(tools::fanout_width_for(narrow.complexity), Some(tools::MIN_FANOUT_SUBTASKS));
        assert!(!narrow.should_host_prespawn());

        // Large accumulated context: both "there is something to split" and
        // "one context should not hold it all" → the full band, host-led.
        let wide = build_route_hint(prompt, Some(Effort::High), 20_000);
        assert_eq!(wide.complexity, runtime::RouteTaskComplexity::Large);
        assert_eq!(tools::fanout_width_for(wide.complexity), Some(tools::MAX_FANOUT_SUBTASKS));
        assert!(
            wide.should_host_prespawn(),
            "a full-band research turn must not still run as one agent"
        );
    }

    /// The escalation is one-directional: breadth raises an
    /// investigation the size table under-reads, but never lowers a genuinely
    /// large implementation.
    #[test]
    fn breadth_evidence_raises_difficulty_and_never_lowers_it() {
        use runtime::RouteTaskComplexity as C;
        assert_eq!(fanout_difficulty(C::Small, BreadthEvidence { multi_scope: false, large_ctx: false }), C::Small);
        assert_eq!(fanout_difficulty(C::Small, BreadthEvidence { multi_scope: true, large_ctx: false }), C::Medium);
        assert_eq!(fanout_difficulty(C::Small, BreadthEvidence { multi_scope: true, large_ctx: true }), C::Large);
        // Never lowers.
        assert_eq!(fanout_difficulty(C::Large, BreadthEvidence { multi_scope: true, large_ctx: false }), C::Large);
        assert_eq!(fanout_difficulty(C::Medium, BreadthEvidence { multi_scope: true, large_ctx: false }), C::Medium);
        // No scope evidence changes nothing at all.
        assert_eq!(fanout_difficulty(C::Large, BreadthEvidence { multi_scope: false, large_ctx: true }), C::Large);
        assert_eq!(fanout_difficulty(C::Unknown, BreadthEvidence { multi_scope: false, large_ctx: true }), C::Unknown);
    }

    #[test]
    fn multi_scope_breadth_hints_model_led_fanout_and_never_host_prespawns() {
        let prompt = "runtime/tools/api 각각에서 timeout 취약점 찾아줘";
        // Multi-scope breadth in a SMALL context surfaces the fan-out shape as
        // a model-led nudge — the model judges task shape and spawns its own
        // agents (CC-style). A natural-language phrase alone still never makes
        // the host pre-consume the turn; the difficulty must fill the whole
        // fan-out band first (see
        // `multi_scope_breadth_host_prespawns_once_the_context_is_large`).
    ...
    fn canonical_override_is_used_for_the_audit_shape() {
        ...
        let with_override = RouteHint {
            ...
            complexity: runtime::RouteTaskComplexity::Medium,
            host_prespawn: false,
        };
    ...
    fn evidence_parallel_lanes_is_model_led_fanout_and_never_prespawns() {
        ...
        // Large complexity — i.e. the difficulty gate WOULD open for a
        // keyword-breadth hint — must still not let the broad fallthrough
        // consume the turn. The veto is explicit for exactly this reason.
    ...
    fn high_effort_pipeline_stays_model_led_without_phrase_table() {
        ...
        // Collaboration strategy is decided by model triage, not a phrase
        // table: no explicit-parallel directive means no host consumption.
        assert!(
        ...
        // The wording is not a hardcoded explicit-parallel token: it stays a
        // Pipeline and never consumes the turn.
        assert!(
    ...
    fn route_matrix_pins_pipeline_fanout_and_no_delegation_boundaries() {
        ...
        let cases = [
            ...
            // Multi-scope breadth with a large accumulated context fills the
            // fan-out band, so a configured delegation effort host-prespawns.
            (
                "프로젝트 전체 분석해줘",
                Some(Effort::Smart),
                20_000,
            ...
            // Same words, small context: the difficulty no longer fills the
            // band, so the shape is a nudge and the model owns the decision.
            (
                "프로젝트 전체 분석해줘",
                Some(Effort::Max),
                1_000,
    ...
    fn no_split_brain_every_fanout_is_host_prespawned() {
        ...
        // The inverse split: a breadth turn whose difficulty does NOT fill the
        // band (multi-scope, but a small context — nothing yet says one agent
        // cannot hold it) surfaces the shape as a model-led nudge and the host
        // stays out, so again exactly one side owns the spawning decision.
        let high = build_route_hint("analyze the whole project", Some(Effort::Max), 1_000);
        assert_eq!(high.shape, RouteShape::FanoutParallel);
        assert!(!high.should_host_prespawn());
        assert!(high.model_reminder().is_some());
        // And the ownership invariant holds on the OTHER side of that same
        // boundary: once the difficulty fills the band the host takes the
        // turn and the nudge disappears. Never both, never neither.
        let wide = build_route_hint("analyze the whole project", Some(Effort::Max), 20_000);
        assert!(wide.should_host_prespawn());
        assert!(wide.model_reminder().is_none());
    }
    ...
    fn broad_analysis_hints_fanout_but_the_host_never_spawns_the_swarm() {
        ...
        for effort in [Effort::Medium, Effort::High, Effort::Max] {
            ...
            assert_eq!(
                hint.shape,
                RouteShape::FanoutParallel,
                "{effort:?} surfaces the fan-out shape"
            );
            // A configured delegation effort + a full-band difficulty (this
            // prompt is multi-scope and the context is large) now DOES
            // pre-spawn: that combination is the whole point of the
            // difficulty gate, and a phrase alone still cannot reach it.
            assert!(
                hint.should_host_prespawn(),
                "{effort:?} + full-band difficulty must pre-spawn"
            );
            // Exactly one side owns the decision, still.
            assert!(hint.model_reminder().is_none());
        }
        // The SAME prompt in a small context stays model-led: without the
        // accumulated context there is no reason one agent cannot hold it.
        let narrow = build_route_hint(prompt, Some(Effort::High), 1_000);
        assert_eq!(narrow.shape, RouteShape::FanoutParallel);
        assert!(!narrow.should_host_prespawn());
        assert!(narrow.model_reminder().is_some());
        // Unset effort never host-prespawns however hard the turn looks —
        // not choosing is not consenting (BUG-R2 stays closed).
        ...
        // Ultracode pre-spawns as it always did, and still arms cheap LLM
        // triage as the gate that picks `decompose` vs `Diagnose`.
