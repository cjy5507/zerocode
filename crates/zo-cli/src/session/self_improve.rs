...
const GENERATION_PASSTHROUGH_ENV: &[&str] = &[
    ...
    "DASHSCOPE_API_KEY",
    "DEEPSEEK_API_KEY",
    // Not a credential: experiment integrity. This child is a full `zo -p`
    // session that exercises routing, replay, guidance and topology, so an
    // ablation the parent is measuring under must reach it — otherwise one
    // run mixes a control arm with a treatment arm and the measurement is
