...
/// `/fast [on|off]` — toggle a catalog-supported OpenAI priority tier. The
/// state lives on the resolved model id; the real logic, including its
/// orthogonality with reasoning effort and the `service_tier: "priority"`
...
pub(super) fn brief(ctx: &mut DispatchCtx) -> CommandOutput {
    ...
    .unwrap_or_default();
    // Off is the *default* style, not "markdown" — there is no such style, so
    // `output_style` rejected it before persisting and the toggle only ever went
    // one way: once a directory was pinned to `concise`, `/brief` could not
    // unpin it, and every later session there kept the concise instruction in
    // its system prompt.
    let next = if current == runtime::output_style::CONCISE_STYLE {
        runtime::output_style::DEFAULT_STYLE
    } else {
        runtime::output_style::CONCISE_STYLE
    };
