...
/// sidebar badge.
///
/// This handler does the parts that can fail cleanly while the session is still
/// fully alive — gate the request and resolve the re-exec — then records the
/// plan and returns [`CommandOutput::RestartRequested`]. It deliberately does
/// NOT persist: the session loop persists only after it has drawn the
/// `Restarting…` acknowledgement, so the save never precedes the visual
/// feedback and a save failure can abort the restart with the session intact.
/// `run_repl` runs the actual `exec` after the loop returns and the terminal is
/// restored (see [`crate::session::restart`]). Nothing here is teardown itself,
/// so a rejected gate or an unresolvable binary leaves the live session fully
/// intact.
/// Both failure modes above are *reported*, never propagated: each one leaves
/// the session intact, so they are command output rather than a `DispatchError`.
pub(super) fn restart(ctx: &mut DispatchCtx) -> CommandOutput {
    ...
    let plan = match RestartPlan::resolve(ctx.cli.session.path.clone(), ctx.cli.cwd.clone()) {
        Ok(plan) => plan,
        Err(error) => {
            return CommandOutput::error(format!("Restart\n  Not restarted    {error}"));
        }
    };

    // Record the resolved plan; `run_repl` execs it after the terminal is
    // restored. Persistence is DEFERRED to the session loop: it saves only after
    // it has drawn the `Restarting…` acknowledgement (see the loop's
    // `DispatchOutcome::Restart` handling), so the save never precedes the visual
    // feedback and a failed save can abort the restart with the session intact.
    // Recording the plan is in-memory only, so a later persist failure leaves
    // nothing to undo here.
    ctx.cli.pending_restart = Some(plan);
    CommandOutput::RestartRequested
}
