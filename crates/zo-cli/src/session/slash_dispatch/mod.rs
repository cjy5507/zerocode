...
pub(crate) use providers::{
    ...
    providers_text_report, settings_path_display,
};
...
/// Returns `true` when the command asks the loop to stop the idle prompt (a
/// plain exit, or a restart that also records a re-exec plan on the `LiveCli`).
...
fn dispatch(ctx: &mut DispatchCtx, command: SlashCommand) -> Result<CommandOutput, DispatchError> {
    ...
    Ok(match command {
        ...
        C::Providers => providers::providers(ctx),
        C::Login { provider } => auth::login(ctx, provider.as_deref()),
        C::Logout { scope } => auth::logout(ctx, scope.as_deref()),
...
mod effort_tests {
    ...
    fn render_effort_status_annotates_effective_tier_when_model_clamps() {
        ...
        // Luna has no internal ultra rung, so it annotates down to its own
        // ceiling — `max`, which is a real GPT-5.6 wire value.
        let luna_ultra = render_effort_status(Some(26_000), "gpt-5.6-luna");
        assert!(luna_ultra.contains("ultra → max"), "{luna_ultra}");
    }
    ...
    fn render_effort_status_shows_smart_dynamic_band_per_model() {
        ...
        assert!(
            sol.contains("smart → dynamic xhigh~max (escalates on heavy turns)"),
            "automatic escalation stops at max even though sol itself allows ultra:\n{sol}"
        );
...
mod allowed_tools_turn_scoping_tests {
    ...
    /// Names of the tools the wire request would advertise with the
    /// session-global `allowed_tools`. NOTE: this helper hard-codes that
    /// choice rather than calling the turn build site, so by itself it cannot
    /// catch a regression that re-threads `turn_allowed_tools` into the
    /// advertisement — the gate assertions below pin the production wiring
    /// (`arm_turn_allowed_tools` + `check_turn_allowed_tools`) instead.
    fn offered_tool_names(cli: &LiveCli) -> BTreeSet<String> {
        let allowed = cli.allowed_tools.clone();
    ...
    fn prompt_command_scopes_tools_for_one_turn_then_resets() {
        ...
        // (2) The wire advertisement is UNCHANGED by the one-turn override:
        // shrinking `tools` for a turn and restoring it the next would
        // cold-rewrite the whole cached prefix twice per scoped command
        // ("tool definitions changed" cache breaks). Enforcement moved to the
        // executor's shared slot instead.
        assert_eq!(
            offered_tool_names(&cli),
            full,
            "a scoped turn must NOT shrink the wire advertisement (prompt-cache stability)"
        );

        // (3) The PRODUCTION arm wiring installs the scope into the executor
        // slot at turn start, and the gate enforces it. `arm_turn_allowed_tools`
        // is the exact function the turn build site calls — deleting that call
        // chain turns the scope into a silent no-op, which this pins.
        super::super::turn_controller::arm_turn_allowed_tools(&cli);
        let slot = cli.runtime.tool_executor().shared_turn_allowed_tools();
        assert!(
            crate::cli_tool_executor::check_turn_allowed_tools(&slot, "bash").is_ok(),
            "a tool inside the scope must pass the turn gate"
        );
        let denied = crate::cli_tool_executor::check_turn_allowed_tools(&slot, "glob_search")
            .expect_err("a tool outside the scope must be rejected by the turn gate");
        assert!(
            denied.to_string().contains("bash"),
            "the rejection must name the permitted set: {denied}"
        );
        // Deferred-tool escape hatch: the advertisement no longer force-lists
        // a scoped deferred tool's schema, so `ToolSearch` (its only route to
        // the model) and `AskUserQuestion` (user interaction, answered inline
        // on the streaming path anyway) must pass the gate regardless of scope.
        for exempt in ["ToolSearch", "AskUserQuestion"] {
            assert!(
                crate::cli_tool_executor::check_turn_allowed_tools(&slot, exempt).is_ok(),
                "{exempt} must be exempt from the turn scope"
            );
        }

        // (4) Turn completion seam: clearing the override and re-arming (the
        // same production sequence as `run_live_turn_with_images`) restores
        // the full execution surface — the advertisement never changed.
        cli.turn_allowed_tools = None;
        super::super::turn_controller::arm_turn_allowed_tools(&cli);
        assert!(
            crate::cli_tool_executor::check_turn_allowed_tools(&slot, "glob_search").is_ok(),
            "after the turn completes every tool must execute again"
        );
        assert_eq!(offered_tool_names(&cli), full);
    }
    ...
    /// `/restart` must NOT save the session inside the handler: persistence has
    /// to wait until the session loop has drawn the `Restarting…`
    /// acknowledgement, so a save never precedes the visual feedback and a
    /// failed save can abort the restart with the session intact. Recording the
    /// plan is fine — that is in-memory only.
    #[test]
    fn restart_defers_persistence_until_the_loop_acknowledges() {
        let _env_lock = crate::test_env_lock();
        let temp_dir = std::env::temp_dir().join(format!(
            "zo-restart-defer-{}",
            SystemTime::now()
        ...
        // Start from a known-absent transcript so a write is observable.
        ...
        let output = {
            ...
            super::system::restart(&mut ctx)
        };

        // The handler returns the internal restart outcome, not a plain exit…
        assert!(
            matches!(output, super::output::CommandOutput::RestartRequested),
            "restart must return the deferred-persist outcome, got {output:?}"
        );
        // …the plan is recorded (in-memory) so the loop can exec after the ack…
        assert!(
            cli.pending_restart.is_some(),
            "the restart plan must be recorded for the loop to exec"
        );
        // …but the on-disk transcript must NOT have been written yet.
        assert!(
            !session_path.exists(),
            "persistence must be deferred until the loop draws `Restarting…`"
        );

        // Rendering the outcome asks the loop to stop the idle prompt (a restart
        // is a `true` that carries a `pending_restart`), and it draws the
        // acknowledgement — still before any persistence, which the loop does
        // only after this.
        let should_quit = super::output::render(&mut app, &ids, output);
        assert!(should_quit, "a restart must stop the idle prompt");
        assert!(
            !session_path.exists(),
            "rendering the acknowledgement must still not have persisted"
        );
    }
}
