...
pub(super) enum CommandOutput {
    ...
    Exit,
    /// The user asked to restart (`/restart`). The handler has recorded the
    /// re-exec plan but deliberately has NOT persisted the session: the loop
    /// must draw the `Restarting…` acknowledgement first, THEN persist and
    /// exec. Distinct from [`Self::Exit`] so persistence never precedes the
    /// visual feedback and a failed save can abort the restart with the session
    /// intact. See [`render`] and the loop's `RestartRequested` handling.
    RestartRequested,
...
///
/// Returns `true` when the command asks the session loop to stop running the
/// idle prompt — a plain exit OR a restart. A restart is a `true` that also
/// leaves a re-exec plan recorded on the `LiveCli` (`pending_restart`): the loop
/// draws the `Restarting…` acknowledgement pushed here, THEN persists and
/// execs, so the save never precedes the visual feedback. A plain exit records
/// no plan and tears down immediately.
pub(super) fn render(app: &mut App, ids: &BlockIdGen, output: CommandOutput) -> bool {
    match output {
        ...
        CommandOutput::RestartRequested => {
            // The acknowledgement the loop draws BEFORE it persists and execs.
            // Pushed here (not in the handler) so it rides the same transcript
            // funnel as every other command output.
            push_report(
                app,
                ids,
                SystemLevel::Info,
    ...
    match block {
        OutputBlock::Report(level, body) => ReportViewerBlock::Text {
            tone: match level {
                ...
                // The popup carries no success tint of its own; the transcript
                // notice's check glyph is where a success is signalled.
                SystemLevel::Info | SystemLevel::Success => ReportTone::Info,
            },
            body,
        },
