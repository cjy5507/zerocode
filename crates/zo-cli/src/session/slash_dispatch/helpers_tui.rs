...
fn seed_user_visibility(message: &runtime::ConversationMessage) -> SeedUserVisibility {
    ...
    // Marker constants come from the prompt builders themselves
    // (`runtime::conversation::deep_gate`) — matching free literals here let
    // a reworded marker silently orphan this filter, leaking internal leg
    // prompts into a reseeded transcript. `[[ZO-DEEP:VERIFY]]` is a retired
    // legacy marker that survives only inside old saved transcripts, so it
    // stays a literal: no builder produces it any more.
    if trimmed.starts_with(runtime::DEEP_VERIFY_MARKER)
        || trimmed.starts_with("[[ZO-DEEP:VERIFY]]")
    {
        return SeedUserVisibility::HideTurn;
    }
    if trimmed.starts_with(runtime::AUTO_RETRY_MARKER)
        || trimmed.starts_with(runtime::DEEP_PLAN_MARKER)
        || trimmed.starts_with(runtime::DEEP_EXEC_MARKER)
    {
    ...
    for message in session.messages.iter() {
        ...
        match message.role {
            ...
            runtime::MessageRole::System => {
                for block in &message.blocks {
                    if let runtime::ContentBlock::Text { text } = block {
                        // Persisted harness reminders (todo nags, recalled
                        // memory, …) are model-facing context, not transcript
                        // content — they were never rendered live, so keep
                        // them out of the resumed view too. Compaction
                        // summaries and other System text still show.
                        //
                        // LINEAGE, not the live tag: microcompact rewrites a
                        // superseded reminder to a placeholder that carries no
                        // tag (deliberately, so clearing stays idempotent), and
                        // reclaiming context must not turn a block that was
                        // never visible into a visible row.
                        if runtime::session::is_reminder_lineage_text(text) {
                            continue;
                        }
...
mod tests {
    ...
    /// Reclaiming a reminder must not make it visible. Microcompact rewrites a
    /// superseded reminder to a placeholder that deliberately carries no
    /// `<system-reminder>` tag — that is what keeps clearing idempotent — and
    /// the tag test used to be the only thing keeping reminders out of the
    /// resumed view. So every cleared copy came back as a grey System row on
    /// `/resume`, `/session switch|fork`, `/rename`, the boot seed, and the
    /// post-auto-compaction reseed: content that was invisible both live and on
    /// resume before, now rendered once per copy, and the better microcompact
    /// worked the noisier the transcript got. Both properties are asserted here
    /// because they are only in tension while one predicate answers both.
    #[test]
    fn a_microcompacted_reminder_stays_out_of_the_resumed_transcript() {
        use runtime::message_stream::{BlockIdGen, RenderBlock};
        use runtime::{ContentBlock, ConversationMessage, MessageRole, Session};
        use tokio::sync::mpsc;
        use zo_cli::tui::{App, Theme};

        let reminder = runtime::wrap_reminder(&"nag ".repeat(200));
        let reminder_message = || ConversationMessage {
        ...
        // Two byte-identical copies: the older is superseded, the newer stays.
        let mut session = Session::new();
        session.push_user_text("start").unwrap();
        session.push_message(reminder_message()).unwrap();
        session.push_user_text("work").unwrap();
        session.push_message(reminder_message()).unwrap();

        let event = runtime::microcompact_session(&mut session, 0, 1)
            .expect("the older copy must be superseded");
        assert_eq!(event.cleared_superseded_reminders, 1, "premise");

        let (_tx, rx) = mpsc::channel::<RenderBlock>(16);
        let (cmd_tx, _cmd_rx) = mpsc::channel(16);
        let mut app = App::new(Theme::no_color(), rx, cmd_tx);
        super::seed_transcript_from_session(&mut app, &BlockIdGen::default(), &session);

        let system_text: Vec<&str> = app
            .transcript()
            .blocks()
            .iter()
        ...
        assert!(
            ...
            "neither the live reminder nor the placeholder standing in for a \
             cleared one is transcript content, got {system_text:?}"
        );

        // …and the property that non-matching bought stays bought.
        assert!(
            runtime::microcompact_session(&mut session, 0, 1).is_none(),
            "a cleared reminder must still not be a clearing candidate again"
        );
    }
}
