...
/// A headless terminal backed by an in-memory buffer with a fixed viewport, so
/// the prelude/pump seams — which draw real frames through `draw_frame`, hence
/// require a `CrosstermBackend`, not a `TestBackend` — run without a TTY.
/// `Vec<u8>: Write` satisfies `draw_frame`'s bound, and a fixed viewport avoids
/// the `crossterm::terminal::size()` query that has no TTY to answer it under
/// `cargo test`.
fn headless_terminal() -> Terminal<ratatui::backend::CrosstermBackend<Vec<u8>>> {
    ...
    .expect("headless terminal builds with a fixed viewport")
}
...
impl SharedWriter {
    fn clear(&self) {
        self.bytes.lock().expect("shared terminal bytes").clear();
...
fn observed_headless_terminal(
    writer: SharedWriter,
) -> Terminal<ratatui::backend::CrosstermBackend<SharedWriter>> {
    Terminal::with_options(
    ...
    .expect("observed headless terminal builds with a fixed viewport")
}

/// The turn prelude must make `Thinking` live and disable input BEFORE any
/// preflight work (route probe / checkpoint settle), and it must hold for BOTH
/// the initial turn (generation 0) and a follow-up turn (generation 1) — the
/// two Enters the stall was reported on. Regression guard for the fix that
/// moved the begin ahead of the awaited preflight instead of leaving it inside
/// `drive_turn`, after the probe.
#[test]
fn turn_prelude_begins_thinking_and_disables_input_before_preflight() {
    for generation in [0_u64, 1] {
        ...
        assert!(
            ...
            "gen {generation}: no activity before the prelude"
        );
        super::begin_turn_prelude(&mut app, &mut terminal, generation)
            .expect("prelude draws the first frame");
        assert!(
            app.turn_activity().is_some(),
            "gen {generation}: Thinking must be live before any preflight runs"
        );
        assert!(
            !app.input_enabled(),
            "gen {generation}: the prelude must disable input for the turn"
        );
    }
}

/// A second `begin_turn_prelude` (the idempotent guard `drive_turn` runs after
/// the preflight) must NOT restart the activity: the spinner's start instant is
/// preserved so the elapsed clock is continuous across the prelude/drive
/// hand-off.
#[test]
fn turn_prelude_is_idempotent_and_preserves_the_activity_start() {
    let mut app = test_app();
    let mut terminal = headless_terminal();

    super::begin_turn_prelude(&mut app, &mut terminal, 0).expect("first prelude");
    let started = app
        .turn_activity()
        .expect("activity after first prelude")
        .started_at;

    super::begin_turn_prelude(&mut app, &mut terminal, 0).expect("second prelude");
    assert_eq!(
        app.turn_activity()
            .expect("activity after second prelude")
            .started_at,
        started,
        "the duplicate begin must not reset the turn activity"
    );
}

/// The generalized pump keeps the spinner animating AND returns the preflight
/// future's output, so `run_live_turn_with_images` can consume the route
/// assessment it awaited — instead of the old `()`-returning pump that forced
/// the await to happen (frozen) outside the animation loop.
#[tokio::test]
async fn pump_draw_until_preserves_the_preflight_output_while_the_turn_is_live() {
    let mut app = test_app();
    let mut terminal = headless_terminal();
    super::begin_turn_prelude(&mut app, &mut terminal, 0).expect("prelude");

    let mut events = futures_util::stream::pending::<Result<Event, io::Error>>();
    let mut clipboard_write = None;
    let assessment = super::pump_draw_until(
        &mut app,
        &mut terminal,
        &mut events,
        &mut clipboard_write,
        ...
        async {
            ...
            tools::assess_turn_deterministic("what does this flag do")
        },
    )
    .await
    .expect("pump resolves the preflight future");

    assert_eq!(assessment.intent, runtime::RouteTaskIntent::Other);
    assert!(
        app.turn_activity().is_some(),
        "the turn stays live across the pumped preflight"
    );
}

#[tokio::test]
async fn preflight_pump_paints_ascii_and_korean_input_before_future_finishes() {
    let mut app = test_app();
    let writer = SharedWriter::default();
    let mut terminal = observed_headless_terminal(writer.clone());
    super::begin_turn_prelude(&mut app, &mut terminal, 0).expect("prelude");
    writer.clear();
    ...
    let painted_before_completion = super::pump_draw_until(
        &mut app,
        &mut terminal,
        &mut events,
        &mut clipboard_write,
        ...
        async move {
            ...
            loop {
                if observed.contains(b"Z") && observed.contains("한".as_bytes()) {
                    return true;
                }
    ...
    .await
    .expect("preflight pump stays live");

    assert!(
        painted_before_completion,
        "ASCII and Korean input must paint while preflight is still pending"
    );
    assert_eq!(
        app.input().text(), "Z한",
        "pre-stream input must reach the queued composer"
    );
    assert!(
        app.turn_activity().is_some(),
        "input must not end the Thinking activity"
    );
}

#[tokio::test]
async fn preflight_pump_coalesces_key_burst_repaints() {
    let mut app = test_app();
    let writer = SharedWriter::default();
    let mut terminal = observed_headless_terminal(writer.clone());
    super::begin_turn_prelude(&mut app, &mut terminal, 0).expect("prelude");
    writer.clear();
    app.draw_frame(&mut terminal).expect("baseline frame");
    let flushes_per_frame = writer.flush_count().max(1);
    writer.clear();

    let key_count = 40;
    let mut events = futures_util::stream::iter((0..key_count).map(|_| {
        Ok::<_, io::Error>(Event::Key(KeyEvent::new(
            KeyCode::Char('x'),
            KeyModifiers::NONE,
        )))
    }));
    let mut clipboard_write = None;
    super::pump_draw_until(
        &mut app,
        &mut terminal,
        &mut events,
        &mut clipboard_write,
    ...
    .await
    .expect("preflight key burst finishes");

    assert_eq!(app.input().text().chars().count(), key_count);
    let burst_flushes = writer.flush_count();
    let max_flushes = flushes_per_frame.saturating_mul(4);
    assert!(
        burst_flushes <= max_flushes,
        "40 queued keys must fit within four frame equivalents: observed \
         {burst_flushes} flushes, baseline {flushes_per_frame} per frame"
    );
}

#[tokio::test(flavor = "current_thread")]
async fn blocking_skill_discovery_keeps_async_ticks_alive() {
    let mut app = test_app();
    let mut terminal = headless_terminal();
    super::begin_turn_prelude(&mut app, &mut terminal, 0).expect("prelude");
    ...
    let mut events = futures_util::stream::pending::<Result<Event, io::Error>>();
    let mut clipboard_write = None;
    let value = super::pump_blocking_with_draw(
        &mut app,
        &mut terminal,
        &mut events,
        &mut clipboard_write,
    ...
    .await
    .expect("blocking skill discovery finishes");
    ...
    assert!(
        observed_ticks >= 2,
        "the async runtime must keep ticking during skill discovery, observed {observed_ticks} ticks"
    );
}

/// Negative control for [`blocking_skill_discovery_keeps_async_ticks_alive`].
///
/// That test asserts the runtime keeps ticking through a blocking preflight step.
/// On its own it cannot tell you whether the assertion has any teeth — if the
/// heartbeat ticked regardless of how the work were run, it would pass even after
/// someone deleted the `spawn_blocking` seam it exists to protect.
///
/// So this pins the failure it is guarding against: awaiting nothing and blocking
/// the async thread *directly* — the exact anti-pattern that froze the spinner
/// three separate times (the post-turn checkpoint settle, skill discovery, and the
/// route probe / OAuth refresh) — starves the heartbeat on this `current_thread`
/// runtime. Both tests use `current_thread` deliberately: on a multi-thread
/// runtime another worker would keep the heartbeat alive and neither test would
/// measure anything.
///
/// If this test ever starts observing ticks, the positive test above has stopped
/// proving anything and the seam needs re-checking.
#[tokio::test(flavor = "current_thread")]
async fn blocking_the_async_thread_directly_starves_ticks() {
    let ticks = Arc::new(AtomicUsize::new(0));
    let tick_count = Arc::clone(&ticks);
    let heartbeat = tokio::spawn(async move {
        loop {
    ...
    // The anti-pattern: blocking work run on the async thread instead of through
    // `pump_blocking_with_draw`'s `spawn_blocking`.
    std::thread::sleep(std::time::Duration::from_millis(75));
    ...
    assert_eq!(
        observed_ticks, 0,
        "blocking the async thread must starve the runtime — otherwise \
         `blocking_skill_discovery_keeps_async_ticks_alive` proves nothing, since \
         it would pass with or without the spawn_blocking seam"
    );
}
...
fn routed_provider_client_pins_the_session_cache_scope() {
    ...
    let credentials_path = api::credentials_path().expect("resolve isolated credentials path");
    ...
    std::fs::write(
        credentials_path,
        format!(
            r#"{{"openai_oauth":{{"accessToken":"test-token","expiresAt":{expires_at},"accountId":"test-account","scopes":[]}}}}"#
        ),
    )
    .expect("write isolated ChatGPT OAuth credentials");

    let session_id = "session-cache-scope";
    let client =
        build_routed_provider_client("gpt-5.6-sol", api::AuthRoute::OAuth, session_id)
            .expect("construct routed ChatGPT client without a live provider");

    assert_eq!(client.pinned_cache_scope(), Some(session_id));

    drop(config_guard);
    std::fs::remove_dir_all(config_home).expect("remove isolated config home");
}
...
/// The two halves of the contract must overlap *once the swap is armed*.
/// `plan_first` fires from `Medium` up and `Always` arms every band, so an
/// opted-in Medium/Large turn both plans and hands off, while a Trivial/Small
/// turn hands off without paying for a plan round trip. Earlier defaults made
/// these bands disjoint (`plan_first` at `Large`, swap at `Trivial` only),
/// which is how the contract could be "on" and never once split plan from
/// implement.
///
/// Armed explicitly rather than through `default()`: the default is now
/// `Never`, because the swap's escalation net could not fire and every
/// implementation turn was being authored by a non-premium model regardless of
/// which model the user had chosen.
#[test]
fn plan_first_and_the_armed_swap_band_overlap_on_real_work() {
    use runtime::RouteTaskComplexity::{Large, Medium, Small, Trivial, Unknown};

    assert_eq!(
        tools::SmartExecSwap::default(),
        tools::SmartExecSwap::Never,
        "an unconfigured session keeps the user's model on the EXEC leg"
    );

    for complexity in [Medium, Large] {
        assert!(plan_first_for(complexity), "{complexity:?} must PLAN first");
        assert!(
            should_install_exec_implementer(tools::SmartExecSwap::Always, complexity),
            "{complexity:?} must also hand implementation to the implementer"
        );
    }
    for complexity in [Trivial, Small, Unknown] {
        assert!(
            !plan_first_for(complexity),
            "{complexity:?} must not pay for a plan round trip"
        );
        assert!(
            should_install_exec_implementer(tools::SmartExecSwap::Always, complexity),
            "{complexity:?} must still leave the reserved model out of the diff"
        );
    }
}
...
fn turn_escalation_carries_the_router_complexity_band() {
    // The assessment arrives resolved from the effort-independent caller;
    // `resolve_turn_escalation` stays a pure mapping over whatever verdict and
    // provenance it is handed.
    let easy = resolve_turn_escalation(
        0,
        Some(Effort::Smart),
        None,
        assessment(
            tools::assess_turn_complexity("fix the typo in the docs"),
    ...
    assert_eq!(hard.turn_effort, Some(api::EffortLevel::Max));
}
...
/// A trusted probe may lower the Smart floor. Design no longer carries a
/// separate floor override: it keeps the complexity-derived floor and only
/// contributes the independently measured non-Large ceiling.
#[test]
fn trusted_design_intent_keeps_the_complexity_floor_and_caps_the_ceiling() {
    let design = resolve_turn_escalation(
        0,
        Some(Effort::Smart),
        None,
        assessment(
            ...
            runtime::RouteAssessmentProvenance::TrustedProbe,
        ),
    );
    assert_eq!(design.turn_effort, Some(api::EffortLevel::Medium));
    assert_eq!(design.turn_effort_ceiling, Some(api::EffortLevel::Xhigh));

    // Explicit repo-scale evidence raises unconditionally; Design cannot lower
    // its floor.
    let large_design = resolve_turn_escalation(
        0,
        Some(Effort::Smart),
        None,
        assessment(
    ...
    assert_eq!(large_design.turn_effort, Some(api::EffortLevel::Max));
    assert_eq!(
        large_design.turn_effort_ceiling,
        Some(api::EffortLevel::Ultra)
    );

    // The same easy complexity without trusted-probe provenance cannot lower
    // the default Smart band.
    let neutral = resolve_turn_escalation(
        0,
        Some(Effort::Smart),
        None,
        assessment(
    ...
    assert_eq!(neutral.turn_effort, Some(api::EffortLevel::Xhigh));
    assert_eq!(neutral.turn_effort_ceiling, Some(api::EffortLevel::Max));
}

/// The Korean-generative-turn gap: `만들어줘` is in no verb table (and must not
/// be added to one), so `turn_has_write_intent` says "no" and the Architect
/// contract never arms. A resolved design intent is what re-opens it.
#[test]
fn contract_eligibility_falls_back_to_resolved_intent() {
    use runtime::RouteTaskIntent as I;

    let korean_generative = "랜딩페이지 하나 만들어줘";
    assert!(
        !tools::turn_has_write_intent(korean_generative),
        "guard: if the verb tables start matching this, the fallback is no longer under test"
    );
    assert!(!turn_is_contract_eligible(korean_generative, I::Other));
    assert!(!turn_is_contract_eligible(korean_generative, I::Analysis));
    assert!(turn_is_contract_eligible(korean_generative, I::Design));
    assert!(turn_is_contract_eligible(korean_generative, I::Implementation));

    // Keyword write intent stays authoritative on its own: the deterministic
    // intent is neutral, so this can arm only through the first disjunct.
    let verb_matched_implementation = "fix the retry path";
    assert!(tools::turn_has_write_intent(verb_matched_implementation));
    assert!(turn_is_contract_eligible(
        verb_matched_implementation,
        I::Other
    ));
}

/// The deep gate's VERIFY design lens rides the same kill switch as the
/// reminder and the effort-band cap. The runtime reads no env, so this host-side
/// seam is where `off` has to land: it must hand the runtime `Other`, which is
/// the pre-lens behavior byte-for-byte.
#[test]
fn verify_intent_applies_the_design_kill_switch_before_the_runtime_sees_it() {
    use runtime::RouteTaskIntent as I;

    // The crate-wide env lock, so this serializes against every other
    // env-mutating test rather than racing them.
    ...
    // Non-design reads are pass-through in both switch positions.
    for intent in [I::Other, I::Implementation, I::Analysis] {
        assert_eq!(verify_intent_for_turn(intent), intent);
    }

    for value in ["off", "0"] {
        std::env::set_var("ZO_DESIGN_GUIDANCE", value);
        assert_eq!(verify_intent_for_turn(I::Design), I::Other, "{value}");
        for intent in [I::Other, I::Implementation, I::Analysis] {
            assert_eq!(verify_intent_for_turn(intent), intent, "{value}");
        }
    }
...
fn only_a_small_low_risk_band_can_reach_a_single_lens() {
    ...
    use runtime::RouteTaskRisk as R;

    for risk in [R::Low, R::Medium] {
        assert_eq!(
            verify_leg_depth_for_band(C::Small, risk),
            VerifyLegDepth::SingleLens,
            "Small/{risk:?}"
        );
    }
    for risk in [R::High, R::Critical, R::Unknown] {
        assert_eq!(
            verify_leg_depth_for_band(C::Small, risk),
            VerifyLegDepth::Full,
            "Small/{risk:?}"
    ...
    for complexity in [C::Trivial, C::Medium, C::Large, C::Unknown] {
        for risk in [R::Low, R::Medium, R::High, R::Critical, R::Unknown] {
            assert_eq!(
                verify_leg_depth_for_band(complexity, risk),
                VerifyLegDepth::Full,
                "{complexity:?}/{risk:?}"
            );
        }
...
/// The reminder is armed by resolved design intent, is turn-scoped by
/// replace-by-prefix, and is killable by env.
#[test]
fn design_guidance_reminder_is_armed_only_by_design_intent() {
    use runtime::RouteTaskIntent as I;

    let cwd = unique_temp_dir("design-guidance");
    ...
    let armed = {
        ...
        let prior = std::env::var_os("ZO_DESIGN_GUIDANCE");
        std::env::remove_var("ZO_DESIGN_GUIDANCE");
        let armed = build_turn_design_reminder(&cwd, I::Design);
        // Every non-Design read leaves the turn exactly as it is today.
        for intent in [I::Other, I::Implementation, I::Analysis] {
            assert!(
                build_turn_design_reminder(&cwd, intent).is_none(),
                "{intent:?} must not inject design guidance"
            );
        }
        // Killswitch.
        std::env::set_var("ZO_DESIGN_GUIDANCE", "off");
        assert!(build_turn_design_reminder(&cwd, I::Design).is_none());
        match prior {
            Some(prior) => std::env::set_var("ZO_DESIGN_GUIDANCE", prior),
            None => std::env::remove_var("ZO_DESIGN_GUIDANCE"),
        }
    ...
    let armed = armed.expect("a Design turn must inject guidance");
    assert!(armed.starts_with(runtime::DESIGN_GUIDANCE_REMINDER_PREFIX));
    assert!(armed.contains("token plan FIRST"));

    let _ = std::fs::remove_dir_all(&cwd);
}

#[tokio::test]
async fn effort_independent_turn_assessment_keeps_a_skipped_probe_intent_neutral() {
    // Effort is intentionally absent from this seam. The sole cost gate lives
    // inside `assess_turn_probed`, and this common verb-matched Medium returns
    // before settings, inventory, or provider access.
    let ask = "implement the retry backoff in the client";
    let resolved = super::turn_assessment(ask, "claude-opus-5").await;
    assert_eq!(resolved.complexity, tools::assess_turn_complexity(ask));
    assert_eq!(resolved.intent, runtime::RouteTaskIntent::Other);
    assert_eq!(
        resolved.provenance,
        runtime::RouteAssessmentProvenance::Deterministic
    );
...
fn route_reminder_sets_for_model_led_and_clears_for_host_prespawn() {
    // Multi-scope breadth in a SMALL context: the difficulty does not fill
    // the fan-out band, so the host stays out and the model gets the nudge.
    // (The same words in a large context host-prespawn instead — see
    // `multi_scope_breadth_host_prespawns_once_the_difficulty_fills_the_band`.)
    let model_led = super::super::auto_fanout::build_route_hint(
        "analyze the whole project",
        Some(Effort::Medium),
        1_000,
    );
    assert!(!model_led.should_host_prespawn());
