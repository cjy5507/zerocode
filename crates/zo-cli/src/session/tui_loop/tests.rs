...
/// `init_terminal` arms the fatal-signal restore, and `tokio::signal` panics
/// when no runtime is present. The installer must detect that and no-op, or the
/// TUI boot path would panic for any synchronous caller.
#[test]
fn signal_handler_install_is_a_noop_without_a_tokio_runtime() {
    assert!(
        tokio::runtime::Handle::try_current().is_err(),
        "this test must run outside a tokio runtime to exercise the guard"
    );
    // The guard's whole job: return quietly instead of panicking.
...
fn inline_shutdown_flush_does_not_redraw_a_nonzero_viewport() {
    ...
    let mut terminal = Terminal::with_options(
        backend,
        TerminalOptions {
            viewport: Viewport::Inline(zo_cli::tui::INLINE_VIEWPORT_MIN_HEIGHT),
        },
    )
...
fn persistent_slash_resume_renders_report_inside_transcript() {
    ...
    // The seeded prompt renders as a real `UserMessage` block — the same widget a
    // live turn pushes, not the old `User …` system info row that broke
    // multi-line rendering after /resume. The proof is the block's author rail on
    // the prompt's own row (`|` under NO_COLOR, `┃` otherwise); it used to be the
    // `You` label, which no longer exists — the rail and background band attribute
    // the turn instead.
    assert!(
        rendered.contains("|  resume me"),
        "the seeded prompt must render as a railed UserMessage block: {rendered}"
    );
}
...
/// 하단 스택(스피너 상태줄·입력·HUD)의 리더 계약. 스피너는 `── ⣾ …`,
/// HUD 는 한 셀 heat marker를 가운데 둔 ` + mode …` 로 col 3 거터를 유지하고,
/// 입력은 좌측 레일을 버린 뒤 col 0 에서 시작한다(Pi 에디터는 측면 테두리가
/// 없다). 스피너 리더가 3셀 rule(`─── `)로 그려져 1칸 어긋나던 회귀를 고정한다.
#[test]
fn bottom_stack_leaders_keep_their_gutters() {
    let mut app = new_test_app();
    ...
    // The composer dropped its left rail (Pi's editor has no side borders), so
    // its prompt now starts at col0 and the text at col2 — while the spinner
    // and HUD still carry the legacy 3-cell gutter above. Bringing those two
    // onto col0 as well belongs to the footer rewrite, not here.
    assert!(
        input.starts_with("❯ workflow"),
        "composer starts at col0 with no left rail: {input:?}"
    );
...
/// EVERY stop notifies (CC parity). A user-cancelled background agent used to
/// be dropped here, which left the main model believing it was still running;
/// it now re-injects with the `was stopped by user` summary, and its marker is
/// still consumed so the id set never leaks.
#[test]
fn reinject_notifies_stopped_background_and_clears_marker() {
    let mut app = new_test_app();
    ...
    assert!(
        queued[0].text.contains("was stopped by user"),
        "{}",
        queued[0].text
    );
    assert!(queued[0].text.contains(id), "{}", queued[0].text);
}

/// Completed event but the store has no full result for this id (never
/// recorded / TTL-evicted). The status still reaches the model — silence was
/// the bug — with an explicit stand-in body instead of an empty turn.
#[test]
fn reinject_completed_background_notifies_when_result_unavailable() {
    let mut app = new_test_app();
    ...
    assert_eq!(queued.len(), 1);
    assert!(
        queued[0].text.contains(tools::AGENT_NOTIFICATION_EMPTY_RESULT),
        "{}",
        queued[0].text
    );
}
...
/// A completion that was never launched in background mode is not this host's
/// to deliver — identical gating to the follow-up-turn path (shared builder).
#[test]
...
/// AGENT→MAIN, both halves. A mid-run message from a still-RUNNING sub-agent
/// stages into the SAME inbox completions use when a turn is live, and queues
/// as its own follow-up turn when the prompt is idle — with byte-identical
/// framing, so the model cannot tell the two paths apart. Neither path may
/// treat it as a completion.
#[test]
fn agent_message_delivers_mid_turn_and_when_idle_with_identical_framing() {
    use crate::session::agent_notice::{deliver_agent_message_mid_turn, queue_agent_message};
    let mut app = new_test_app();
    let inbox = new_notification_inbox();
    let mut event = channel_completion("agent-7f31#message", tools::AGENT_MESSAGE_STATUS, None);
    event.result = Some("the auth module is already async".to_string());

    assert!(deliver_agent_message_mid_turn(&mut app, &inbox, &event));
    let staged = {
        let pending = inbox.lock().expect("inbox");
        assert_eq!(pending.len(), 1);
        assert!(matches!(
            pending[0].kind,
            runtime::AgentNotificationKind::Message { ref agent_id } if agent_id == "agent-7f31"
        ));
        pending[0].text.clone()
    };
    assert!(
        staged.starts_with("Agent \"explorer\" sent a message while running:"),
        "{staged}"
    );
    assert!(staged.contains("[SYSTEM NOTIFICATION - NOT USER INPUT]"), "{staged}");
    assert!(
        staged.contains("Reply with SendMessage(to: \"agent-7f31\")"),
        "{staged}"
    );
    // The mid-turn path must NOT also queue a turn — that would deliver twice.
    assert!(app.take_queued_messages().is_empty());

    assert!(queue_agent_message(&mut app, &event));
    let queued = app.take_queued_messages();
    assert_eq!(queued.len(), 1, "the idle half starts exactly one turn");
    assert_eq!(
        queued[0].text, staged,
        "both delivery paths must render identical text"
    );
    let meta = queued[0].agent_result.as_ref().expect("agent-result meta");
    assert_eq!(meta.label, "explorer · message");
}
...
fn persistent_slash_report_command_opens_popup_and_leaves_transcript_clean() {
    const REPORT_MARKER: &str = "Git SHA";
    ...
    let rendered = render_app_buffer(&mut app);
    // The bare version also appears in the persistent sidebar build row, so use
    // a field label unique to the version report as the popup residue marker.
    assert!(
        rendered.contains(REPORT_MARKER),
