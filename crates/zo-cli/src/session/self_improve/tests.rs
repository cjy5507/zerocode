...
fn generation_environment_projects_config_and_credentials_without_ambient_leaks() {
    ...
    let environment: std::collections::BTreeMap<_, _> = super::generation_environment_with(
        ...
        |name| match name {
            ...
            name if name == telemetry::ABLATION_ENV => Some("routing_probe".into()),
            _ => None,
        },
    )
    .unwrap()
    ...
    // The child is a full `zo -p` session, so an ablation the parent is
    // measuring under has to survive `env_clear()` — otherwise one run mixes
    // a control arm with a treatment arm.
    assert_eq!(
        environment.get(&std::ffi::OsString::from(telemetry::ABLATION_ENV)),
        Some(&std::ffi::OsString::from("routing_probe"))
    );
