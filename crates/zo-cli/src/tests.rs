...
/// `--model` makes the dump show the `Model family:` line a session on that
/// model would get, and it resolves aliases like every other model flag.
#[test]
fn parses_system_prompt_model_option() {
    let args = vec![
        "system-prompt".to_string(),
        "--model".to_string(),
        "opus".to_string(),
    ];
    let CliAction::PrintSystemPrompt { model, .. } =
        parse_args(&args).expect("args should parse")
    else {
        panic!("expected PrintSystemPrompt");
    };
...
fn filtered_tool_specs_defer_plugin_tools_until_activated() {
    ...
    let filtered = filter_tool_specs(&registry, None);
    let names = filtered
        .into_iter()
    ...
    let names = filter_tool_specs(&registry, None)
        .into_iter()
        .map(|definition| definition.name)
        .collect::<Vec<_>>();
    assert!(names.contains(&"plugin_echo".to_string()));
...
fn runtime_slash_prompts_queue_real_work() {
    ...
    // The marker the fanout parser reads. If the prompt stops naming it, every
    // candidate falls back to whole-text voting and the council goes quiet.
    assert!(council.contains("VERDICT: <its conclusion in one short line>"));
    assert!(council.contains("voted_on"));
