...
pub struct WorkerRegistry {
    ...
    /// `TaskRegistry`: per-worker field writes and append-only event
    /// pushes complete in memory with no fallible step in between.
...
const MAX_WORKER_EVENTS: usize = 512;
...
impl WorkerRegistry {
    ...
    pub fn observe(&self, worker_id: &str, screen_text: &str) -> Result<Worker, String> {
        ...
        if let Some(blocked) = observe_trust_gate(worker, &lowered) {
            return Ok(blocked);
        }

        if let Some(misdelivered) = observe_prompt_misdelivery(worker, screen_text, &lowered) {
...
/// Trust-gate phase of [`WorkerRegistry::observe`]: when a trust prompt is
/// detected, mark the worker `TrustRequired`, auto-resolve it when the repo is
/// allowlisted, or hand the blocked worker back to the caller. `Some` means the
/// caller should return that worker snapshot immediately.
fn observe_trust_gate(worker: &mut Worker, lowered: &str) -> Option<Worker> {
...
/// Prompt-misdelivery phase of [`WorkerRegistry::observe`]: when the worker's
/// prompt landed somewhere other than the coding agent, record the failure and
/// either arm a replay (auto-recover) or mark it failed. `Some` means the caller
/// should return that worker snapshot immediately.
fn observe_prompt_misdelivery(
    worker: &mut Worker,
    screen_text: &str,
    lowered: &str,
) -> Option<Worker> {
    let observation = prompt_misdelivery_is_relevant(worker)
        .then(|| {
...
fn push_event(
    ...
    let excess = worker.events.len().saturating_sub(MAX_WORKER_EVENTS);
    if excess > 0 {
        worker.events.drain(0..excess);
    }
}
...
mod tests {
    ...
    #[test]
    fn worker_events_keep_recent_tail_with_monotonic_sequence() {
        let registry = WorkerRegistry::new();
        let mut worker = registry.create("/tmp/repo-events", &[], true);

        for index in 0..(MAX_WORKER_EVENTS + 10) {
            push_event(
                &mut worker,
                WorkerEventKind::Running,
                WorkerStatus::Running,
        ...
        assert_eq!(worker.events.len(), MAX_WORKER_EVENTS);
        assert_eq!(worker.events.first().map(|event| event.seq), Some(12));
        assert_eq!(
            worker.events.last().map(|event| event.seq),
            Some((MAX_WORKER_EVENTS + 11) as u64)
        );
    }
