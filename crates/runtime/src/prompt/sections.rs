...
pub(super) fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
        ...
        "For UI or frontend changes, run the app and verify the result visually before reporting the task complete — a browser for web work, a real render for TUI work. Type checks and test suites verify code correctness, not visual correctness: a page that compiles can still be unreadable, misaligned, or blank. If visual verification is not possible in this environment, say so explicitly rather than claiming success.".to_string(),
...
pub(super) fn get_delegation_section() -> String {
    let items = prepend_bullets(vec![
        ...
        "When the user adds a requirement while work is already in flight, absorb it instead of queueing it behind everything you are doing: if it is separable from your current task, launch a named background `Agent` for it right away, keep working on what you already started, and synthesize the agent's result when its completion notification arrives. Serialize the new requirement into your own lane only when it modifies the very thing you are editing, and never stall the work in flight merely to acknowledge it.".to_string(),
