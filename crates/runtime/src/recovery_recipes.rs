...
///
/// Holds per-scenario attempt counts, a structured event log, and
/// optional simulation knobs for controlling step outcomes during tests.
...
pub struct RecoveryContext {
    ...
    /// When `true`, skip real execution and treat all steps as
    /// successful (unless `fail_at_step` triggers). Used by tests
    /// that only verify the orchestration logic.
    simulate: bool,
}

impl RecoveryContext {
    /// Create a new context in simulation mode (no real execution).
    /// Use [`with_real_execution`](Self::with_real_execution) to enable
    /// real step execution in production.
    #[must_use]
    pub fn new() -> Self {
        Self {
            simulate: true,
    ...
    /// Enable real execution of recovery steps instead of simulation.
    #[must_use]
    pub fn with_real_execution(mut self) -> Self {
        self.simulate = false;
        self
    }
...
/// policy, executes steps (controlled by the context), and emits
/// structured [`RecoveryEvent`]s for every attempt.
pub fn attempt_recovery(scenario: &FailureScenario, ctx: &mut RecoveryContext) -> RecoveryResult {
    ...
    *attempt_count += 1;
    ...
    // Execute steps, honoring the optional fail_at_step simulation knob
    // (for tests) and real execution for automatable steps.
    ...
    for (i, step) in recipe.steps.iter().enumerate() {
        ...
        if !ctx.simulate {
            if let Err(reason) = execute_step(step) {
                eprintln!("[recovery] step {i} ({step:?}) failed: {reason}");
                failed = true;
                break;
            }
        }
...
/// Execute a single recovery step. Returns `Ok(())` on success, or an
/// error string describing why the step could not be completed.
///
/// Steps that cannot be safely automated (e.g. trust prompts, prompt
/// redirection) log a "manual intervention required" message and return
/// `Ok(())` so the recipe can proceed.
fn execute_step(step: &RecoveryStep) -> Result<(), String> {
    match step {
        RecoveryStep::CleanBuild => {
            eprintln!("[recovery] executing: cargo clean");
            ...
            if status.success() {
                ...
                Err(format!("cargo clean exited with {status}"))
            }
        }
        RecoveryStep::RebaseBranch => {
            eprintln!("[recovery] rebase queued — requires manual review");
            Ok(())
        }
        RecoveryStep::RestartPlugin { name } => {
            eprintln!("[recovery] requesting plugin restart: {name}");
            // Plugin lifecycle restart is handled by the plugin_lifecycle
            // module at a higher layer. Log the intent and succeed so the
            // recipe runner can continue.
            eprintln!(
                "[recovery] plugin restart for '{name}' queued (lifecycle layer will execute)"
            );
            Ok(())
        }
        RecoveryStep::RetryMcpHandshake { timeout } => {
            eprintln!("[recovery] retrying MCP handshake (timeout={timeout}ms)");
            // MCP handshake retry is performed by the mcp_lifecycle layer.
            // We log the intent and succeed; the caller re-drives the
            // handshake through the existing mcp_lifecycle_hardened path.
            Ok(())
        }
        RecoveryStep::RestartWorker => {
            eprintln!("[recovery] restarting worker (manual intervention required)");
            Ok(())
        }
        RecoveryStep::AcceptTrustPrompt => {
            eprintln!("[recovery] trust prompt acceptance requires manual intervention");
            Ok(())
        }
        RecoveryStep::RedirectPromptToAgent => {
            eprintln!("[recovery] prompt redirection requires manual intervention");
            Ok(())
        }
        RecoveryStep::EscalateToHuman { reason } => {
            eprintln!("[recovery] escalating to human: {reason}");
            Ok(())
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn execute_step_manual_steps_succeed() {
        assert!(super::execute_step(&RecoveryStep::AcceptTrustPrompt).is_ok());
        assert!(super::execute_step(&RecoveryStep::RedirectPromptToAgent).is_ok());
        assert!(super::execute_step(&RecoveryStep::RestartWorker).is_ok());
        assert!(super::execute_step(&RecoveryStep::RestartPlugin {
            name: "test".into()
        })
        .is_ok());
        assert!(super::execute_step(&RecoveryStep::RetryMcpHandshake { timeout: 1000 }).is_ok());
        assert!(super::execute_step(&RecoveryStep::EscalateToHuman {
            reason: "test".into()
        })
        .is_ok());
    }
    ...
    #[test]
    fn real_execution_mode_runs_steps() {
        let mut ctx = RecoveryContext::new().with_real_execution();
        let scenario = FailureScenario::ProviderFailure;
        let result = attempt_recovery(&scenario, &mut ctx);
        assert!(
