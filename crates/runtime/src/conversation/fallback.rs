...
/// Current Opus family head used for a Fable/Mythos safety-classifier refusal.
///
/// The fallback resolves through the provider catalog on every arm rather than
/// embedding a release id here. Updating the catalog's `opus` row therefore
/// updates refusal recovery, dry pre-arm, and agent defaults together.
fn refusal_fallback_model() -> &'static str {
    api::latest_anthropic_family_model(api::ANTHROPIC_OPUS_MODEL_ALIAS)
        .expect("provider catalog must define an Opus family head")
}
...
/// can decline a benign request, so a one-time the current Opus family head fallback is warranted.
    ...
    T: ToolExecutor,
{
    ...
    /// arming the Opus override (which would then force the current Opus catalog
    /// head onto a non-Anthropic client). This is the consistency the two fallbacks need to
    /// coexist.
    ...
    pub(super) fn decide_refusal_fallback(&mut self) -> RefusalDecision {
        ...
        if is_fable_or_mythos_model(&model) {
            ...
            // A refusal on Opus/Sonnet/Haiku. This is the family Fable falls
            // back TO, so there is nowhere further to swap — but one identical
            // retry is not hopeless, it is the most likely fix: the classifier
            // samples, and a 700-message benign session that trips it once
            // usually passes the second time. Exactly one per public turn.
