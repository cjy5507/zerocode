...
pub fn convert_messages(messages: &[ConversationMessage]) -> Vec<InputMessage> {
    ...
    for (index, message) in messages.iter().enumerate() {
        ...
        let content = convert_blocks(message, rewrite);
        if content.is_empty() {
            continue;
        }
...
fn mark_breakpoints_with_ttl(messages: &mut [InputMessage], cache_control: &api::CacheControl) {
    ...
    let verify_boundary = messages
        ...
        .and_then(|verify_start| {
            (0..verify_start)
                .rev()
                .find(|&index| has_cacheable_block(&messages[index].content))
        });
    if let Some(index) = verify_boundary {
        if mark_last_cacheable_block(&mut messages[index].content, cache_control) {
            marked += 1;
        }
    }
