...
pub fn probe_prompt(description: &str, prompt: &str) -> String {
    ...
    format!(
        "You are a routing classifier. Assess the software task below and \
         ...
         \"intent\":\"design|implementation|analysis|other\"}}\n\
         ...
         confidence in this assessment. intent = what the task asks you to \
         PRODUCE, in any language (design: user-visible UI/UX or visual work \
         — a page, screen, component's look, layout, theme, design system; \
         implementation: non-visual code to write or change; analysis: \
         reading, explaining, investigating, reviewing; other: anything \
         else).\n\nTASK:\n{task}"
    )
}

/// Parse the probe's reply. Strict by design on the THREE routing fields: the
/// first `{{…}}` JSON object found is parsed, and an unknown
/// complexity/risk/confidence token rejects the whole probe (fail closed to
/// the deterministic verdict rather than guess what a malformed probe meant).
///
...
pub fn parse_probe_response(raw: &str) -> Option<ProbeAssessment> {
    ...
    let intent = match field("intent").as_deref() {
        Some("design") => RouteTaskIntent::Design,
        Some("implementation") => RouteTaskIntent::Implementation,
        Some("analysis") => RouteTaskIntent::Analysis,
        _ => RouteTaskIntent::Other,
    };
