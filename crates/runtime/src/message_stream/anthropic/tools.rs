...
fn summarize_json(value: &Value, limit: usize) -> String {
    // An argument-less call has nothing to summarize. Stringifying it produced
    // the literal `{}` (or `[]`, or `null`), which is what surfaced in the agent
    // detail pane as a row of bare braces: every tool invoked with no arguments
    // rendered its empty payload as if it were its argument. No arguments must
    // read as no arguments — the caller shows the tool name alone.
    if json_is_argumentless(value) {
        return String::new();
    }
...
mod tests {
    ...
    fn tools_with_arguments_keep_their_summary() {
        ...
        assert!(
            input_summary.contains("depth"),
            "a payload with content still summarizes: {input_summary}"
        );
    }
