...
const DEFERRED_TOOL_HOOKS: &[(&str, &str)] = &[(
    ...
    "session-persistent Python kernel; variables survive across calls and compaction; \
     ...
     for import; zo.history(contains=, role=, limit=) searches this session's own \
     transcript as data",
)];
...
impl GlobalToolRegistry {
    ...
    pub fn definitions(
        ...
        let builtin = self
            ...
            // verbatim; deferring there would advertise nothing) or the model
            // activated them through `ToolSearch`.
            .filter(|spec| {
        ...
        // MCP and plugin schemas stay deferred until `ToolSearch` activates
        // them, on every provider.
        //
        // OpenAI targets used to front-load every discovered schema instead, so
        // that a `ToolSearch` activation could not mutate the cached tool
        // prefix mid-session. The cure cost more than the disease: it put ~77
        // tools and ~55 KB of schema on every single request — including turns
        ...
        let runtime: Vec<api::ToolDefinition> = self
            ...
            .filter(|tool| match allowed_tools {
                ...
                None => activated.contains(tool.name.as_str()),
            })
        ...
        let plugin = self
            ...
            .filter(|tool| match allowed_tools {
                ...
                None => activated.contains(tool.definition().name.as_str()),
            })
            .map(|tool| api::ToolDefinition {
...
mod tests {
    ...
    /// Deferral applies to OpenAI targets too — the same as everywhere else.
    ///
    /// OpenAI used to front-load every discovered MCP and plugin schema so a
    /// `ToolSearch` activation could not mutate the cached tool prefix. That
    /// bought prefix stability at the price of ~55 KB of schema on every
    /// request, including turns that call no tool. Activation is session-
    /// sticky, so deferring costs at most one prefix change per session.
    #[test]
    fn openai_defers_mcp_and_plugin_schemas_until_activation() {
        let registry = GlobalToolRegistry::with_plugin_tools(vec![plugin_tool("plugin_demo")])
            .expect("plugin registry")
            .with_runtime_tools(vec![runtime_tool("mcp__demo__echo")])
            .expect("runtime tool");

        let before_activation = registry.definitions(None);
        let advertised = |defs: &[api::ToolDefinition], name: &str| {
            defs.iter().any(|definition| definition.name == name)
        };
        assert!(!advertised(&before_activation, "mcp__demo__echo"));
        assert!(!advertised(&before_activation, "plugin_demo"));
        ...
        let after_activation = registry.definitions(None);
        assert!(advertised(&after_activation, "mcp__demo__echo"));
        assert!(advertised(&after_activation, "plugin_demo"));
    }
