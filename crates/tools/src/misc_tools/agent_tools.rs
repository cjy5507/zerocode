...
/// Suppress a cross-family priority-tier fallback when the selected OpenAI
/// family has its own catalog-verified demotion ladder.
fn suppress_cross_family_premium_fast_fallback(selected_model: &str, candidate: &str) -> bool {
    let selected = normalized_openai_fallback_model(selected_model);
    let candidate = normalized_openai_fallback_model(candidate);
    api::is_openai_current_family_model(&selected)
        && api::openai_fast_tier_enabled(&candidate)
        && !api::is_openai_current_family_model(&candidate)
}
    ...
    F: FnOnce(AgentJob) -> Result<(), ToolError> + Send,
{
    ...
    let system_prompt =
        build_agent_system_prompt(&resolved_subagent_type, custom_agent.as_ref(), Some(&model))?;
    ...
    let seen_outputs = if input.sees.is_empty() {
        ...
        match topology::resolve_seen_outputs(&input.sees, input.parent_session_id.as_deref()) {
            Ok(seen) => {
