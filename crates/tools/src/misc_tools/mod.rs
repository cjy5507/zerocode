...
pub use smart_router::{
    ...
    turn_has_write_intent, AgentTaskAssessment, DeepTierModelsSetting, SmartExecSwap,
    SmartSettingDefaults, TurnOrchestrationHint, TurnProbeAssessment,
};
...
pub use agent_tools::{
    ...
    wait_for_agent_completions, AgentCompletion, AGENT_MESSAGE_STATUS, AGENT_STARVED_STATUS,
};
...
/// worse than an error.
fn send_message_with_attachments(input: &SendMessageInput) -> Result<String, ToolError> {
    if input.attach_results.is_empty() {
    ...
    match agent_tools::resolve_seen_outputs(&input.attach_results, input.session_id.as_deref()) {
        Ok(seen) => {
...
pub(crate) fn run_send_message(
    ...
    let Some(mut manifest) = lookup_agent_manifest(&input.to, input.session_id.as_deref()) else {
        return to_pretty_json(json!({
            "to": input.to,
            "delivered": false,
    ...
    let outgoing_message = send_message_with_attachments(input)?;
    ...
    if manifest.status == "running" {
        ...
        let delivered = agent_tools::steer_agent(
            &manifest.agent_id,
            format!("[message via SendMessage] {outgoing_message}"),
        );
        ...
        if let Some(settled) = lookup_agent_manifest(&manifest.agent_id, None) {
            manifest = settled;
        }
    }
...
pub fn send_agent_message(
    ...
    // The Ctrl+G viewer already lists only this session's agents, and the user
    // picks a concrete row, so the target is unambiguous: resolve workspace
    // wide (`None`) exactly as before.
    let Some(manifest) = lookup_agent_manifest(target, None) else {
        return AgentSendOutcome::NotFound;
    };
    let name = manifest
        .label
...
pub fn background_agent_notification_header(
    ...
    format!(
        "[task notification — background agent `{name}` (id: {agent_id}) {summary}. To follow \
         up without losing its context, use SendMessage with that id/name as `to` to continue \
         this agent]"
    )
}
