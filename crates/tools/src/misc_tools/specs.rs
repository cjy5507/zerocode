        ...
        ToolSpec {
            ...
            input_schema: json!({
                ...
                "properties": {
                    ...
                    "name": { "type": "string", "description": "Name for the spawned agent. Makes it addressable via SendMessage({to: name}) while it runs and after it finishes. Must start with a letter or digit and contain only letters, digits, underscores, or hyphens (max 64 chars); `main` is reserved — SendMessage routes that to the main conversation." },
                    "model": { "type": "string", "description": "Explicit model or semantic alias for this sub-agent. Prefer versionless aliases such as opus, anthropic-latest, openai-latest, google-latest, and xai-latest. A provider/model value is an intentional exact pin; never infer a concrete release id from stale model knowledge. Same provider family as the session by default; crossing families requires allow_cross_provider." },
                    ...
                    "sees": {
                        "type": "array",
                        "items": { "type": "string" },
                        "description": "Names/ids of FINISHED agents whose final results this agent should read. The harness injects each deliverable verbatim as a labeled shared-context section ahead of the prompt — so never re-type one agent's output into another's prompt; declare the edge instead (e.g. a reviewer with sees: [builder] reads the builder's result without you relaying it). Errors if a referenced agent is unknown or still running (await it first)."
                    }
                },
