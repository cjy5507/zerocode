...
impl TailFlusher {
    ...
    /// `…involved in both blockers.The easycare snapshot…`, two sentences fused
...
fn should_skip_non_stream_fallback(model: &str) -> bool {
    api::is_openai_spark_model(model)
}
...
mod tests {
    ...
    fn tail_flusher_breaks_turns_instead_of_fusing_sentences() {
        ...
        tail.mark_segment_break();
        tail.push("The easycare snapshot already has semicolons.");
        tail.flush();
