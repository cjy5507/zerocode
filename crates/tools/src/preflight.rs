//! Branch-freshness preflight for workspace test commands.
//!
//! When the model issues `cargo test --workspace` (or `cargo nextest run
//! --workspace`/`--all`), we surface a [`BashCommandOutput`] up front that
//! blocks the command if the current branch is stale or has diverged from
//! `main`. Detection is best-effort: any git failure returns `None` so the
//! command runs normally instead of being incorrectly blocked.

use std::process::Command;

use runtime::{
    check_freshness, BashCommandOutput, BranchFreshness, LaneEvent, LaneEventName, LaneEventStatus,
    LaneFailureClass,
};
use serde_json::json;

use crate::dispatch::epoch_seconds_now;

/// Probe whether `command` is a workspace test invocation. When it is, run a
/// branch-divergence check and return a synthetic [`BashCommandOutput`] that
/// blocks execution when the branch is behind/diverged. Returns `None` when
