...
pub struct RunSpec {
    ...
    /// Harness features held out of this run, as validated
    /// [`telemetry::HarnessFeature`] keys (build them with
    /// [`crate::ablation::holdout_keys`]).
    ///
    /// Empty means the FULL-feature arm — and the spawn path enforces that by
    /// CLEARING `ZO_ABLATE` rather than leaving it inherited. Without the
    /// clear, running the suite from a shell that exported a holdout would
    /// execute a suppressed child and still record it as the baseline: an arm
    /// label that contradicts what ran is the one failure the whole ablation
...
pub(crate) struct AgentOutput {
    ...
    /// Whether a process was actually started. `false` only when the budget was
    /// already spent, so nothing ran and this call's true cost is exactly zero.
    /// No other field distinguishes that from a process killed before it wrote:
    /// both are empty stdout with `timed_out` set.
...
struct CapturedCommand {
    ...
    /// Whether a process was actually started; see [`AgentOutput::spawned`].
    spawned: bool,
}
        ...
        Some(cfg) => {
            ...
            (
                AgentOutput {
                    ...
                    // The loop ran processes and has already folded their usage.
                    spawned: true,
                },
                Some(r.verdict),
                Some(r.artifacts),
...
fn run_fail_reasons(
    ...
    if unknown_denial_blocks_pass(
        parse_denials_unknown(&agent.stdout),
        test.status,
        hygiene,
        intended_provided,
        objective_gate,
    ) {
        reasons.push("permission_denials_unknown".to_string());
    }
    reasons
}

/// The run's warnings: the policy's own list, plus an unknown denial count when
/// it did not block.
///
/// A count nobody observed is a floor, not a fact, so it is surfaced either way —
/// as a failure reason where it blocked, and here where it could not have.
fn run_warnings(
    agent: &AgentOutput,
    denials: usize,
    test: TestStatus,
    ...
    let mut warns: Vec<String> =
        warnings_for_policy(denials, test, hygiene, intended_provided, objective_gate)
            .iter()
            .map(|s| (*s).to_string())
            .collect();
    let denials_unknown = parse_denials_unknown(&agent.stdout);
    if denials_unknown
        && !unknown_denial_blocks_pass(
            denials_unknown,
            test,
            hygiene,
            intended_provided,
    ...
    {
        warns.push("permission_denials_unknown".to_string());
    }
    warns
}

/// The run's verdict: the lane's objective rule, plus the two gates that can only
/// ever block it — a deep loop whose strict verifier never accepted, and a denial
/// count nobody observed standing where an observed one would have been fatal.
fn run_passed_overall(
    agent: &AgentOutput,
    test: TestStatus,
    hygiene: &DiffHygiene,
    denials: usize,
    ...
    let base = run_passed_for_policy(
        agent.exit_code,
        test,
        hygiene,
        denials,
    ...
    // A loop that ran out of attempts is not a pass even with a clean objective
    // diff; the strict verifier must have said "accept".
    let deep_accepted = deep_verdict.is_none_or(|v| v.outcome == "accept");
    base && deep_accepted
        && !unknown_denial_blocks_pass(
            parse_denials_unknown(&agent.stdout),
            test,
...
/// Refuse a holdout the run cannot actually apply, before anything is spawned.
///
/// Both branches guard the same invariant — what a run RECORDS as held out has
/// to be what it held out. A key the runtime does not know suppresses nothing,
/// and a non-`zo` binary has no harness features to suppress at all; either way
/// the result would be an ordinary full-feature run filed as a control arm.
fn validate_holdout(spec: &RunSpec) -> io::Result<()> {
    if spec.ablation.is_empty() {
        return Ok(());
    }
    if spec.runner_kind != "zo" {
        return Err(io::Error::other(format!(
            "runner_kind '{}' has no harness features to hold out, but this run declares {:?}",
            spec.runner_kind, spec.ablation
        )));
    }
    for key in &spec.ablation {
        if telemetry::HarnessFeature::from_key(key).is_none() {
            return Err(io::Error::other(format!(
                "'{key}' names no harness feature, so setting it would suppress nothing"
            )));
        }
    }
    Ok(())
...
fn apply_zo_benchmark_env(cmd: &mut Command, spec: &RunSpec) {
    // Forced, not defaulted: a benchmark's agent behavior must not depend on
    // whichever value the launching shell happened to export. The synchronous
    // `-p --output-format json` path this harness uses never installs the
    // reactive auto-verify gate, so pinning the opt-out is what keeps an
    // inherited `ZO_AUTO_VERIFY=1` from quietly giving one run a capability
    // every recorded run before it lacked.
    cmd.env("ZO_AUTO_VERIFY", "0");
    // Forced in BOTH directions for the same reason: an empty holdout actively
    // REMOVES the variable, so an ambient `ZO_ABLATE` cannot turn the baseline
    // arm into a second treatment arm while the contract still labels it
    // full-feature. Setting it only for the ablated arm would leave the
    // baseline at the mercy of the launching shell.
    if spec.ablation.is_empty() {
        cmd.env_remove(telemetry::ABLATION_ENV);
    } else {
        cmd.env(telemetry::ABLATION_ENV, spec.ablation.join(","));
    }
...
pub(crate) fn spawn_with_prompt(
    ...
    match spec.runner_kind.as_str() {
        "zo" => {
            ...
            // Deliberately still `json`, not `stream-json`, even though only the
            // streamed format reports usage before a turn ends. zo's streaming
            // path does not run the spec-literal autopatch that
            // `ConversationRuntime::run_turn` applies on the synchronous path
            // (`crates/runtime/src/conversation/mod.rs:1372`, its only call
            // site), so asking for the other format would change the diff a run
            // produces and its pass/fail — a methodology change wearing a
            // telemetry change's clothes, and one that breaks comparability with
            // every result recorded so far. A phase killed at its cap therefore
            // stays unmeasured; `DeepUsage` marks the run incomplete instead of
            // letting the surviving phases' subtotal pose as a total.
            cmd.args(&spec.args)
                .arg("--output-format")
                .arg("json")
                .arg("-p")
...
/// Every JSON object a captured stdout buffer holds, in arrival order.
///
/// A completed run prints one envelope, so the whole buffer parses and is the
/// only document. A streamed run prints one event per line, and a run whose
/// process group was killed mid-stream leaves a truncated final line. Reading
/// line by line keeps every event before that truncation instead of discarding
/// the buffer wholesale — that discard is why a timed-out phase contributed no
/// tokens at all, and why the suite then summed a partial figure and presented
/// it as a complete total.
pub(crate) fn json_documents(stdout: &str) -> Vec<Value> {
...
/// The newest document `pick` accepts. Streamed telemetry restates the same
/// counters as a turn progresses, so the last statement is the live one and
/// summing the events would count the same tokens many times over.
pub(crate) fn last_json_value<T>(stdout: &str, pick: impl Fn(&Value) -> Option<T>) -> Option<T> {
    json_documents(stdout).iter().rev().find_map(pick)
}

/// The billed token breakdown one JSON document carries, in either shape a
/// runner emits: the terminal `{"usage":{…}}` envelope, or a streamed
/// `{"type":"usage",…}` event. Only the streamed form survives a killed phase,
/// and the two disagree on the cache key names, so both are read here rather
/// than at each call site.
///
/// Claude Code's per-message `assistant.message.usage` is deliberately NOT read.
/// Measured against a two-tool-call turn, de-duplicated per-message sums do
/// reproduce the terminal envelope's `input_tokens` and both cache counters
/// exactly, but its `output_tokens` by only 9 of 146 — so reconstructing a
/// killed claude phase from them would replace "unknown" with a figure wrong by
/// an order of magnitude. A phase we cannot measure stays unmeasured, and
/// [`crate::summary::LedgerRow::has_observed_metrics`] keeps it counted as such.
pub(crate) fn usage_in_value(v: &Value) -> Option<Tokens> {
    let (usage, creation_key, read_key) = match v.get("usage") {
        Some(usage) if usage.is_object() => (
            usage,
            "cache_creation_input_tokens",
            "cache_read_input_tokens",
        ),
        _ if v.get("type").and_then(Value::as_str) == Some("usage") => {
            (v, "cache_creation_tokens", "cache_read_tokens")
        }
        _ => return None,
    };
    let input = usage.get("input_tokens").and_then(Value::as_u64)?;
    let output = usage.get("output_tokens").and_then(Value::as_u64)?;
    let cache_creation = usage.get(creation_key).and_then(Value::as_u64);
    let cache_read = usage.get(read_key).and_then(Value::as_u64);
    let total = input + output + cache_creation.unwrap_or(0) + cache_read.unwrap_or(0);
    // A turn opens with an all-zero usage snapshot, emitted before the provider
    // has reported anything. Counting it would assert the phase spent nothing,
    // when what is true is that nothing is known yet — the same false precision
    // this scanner exists to remove, just pointed the other way.
    if total == 0 {
        return None;
    }
...
/// Whether the denial count is UNKNOWN rather than zero.
///
/// An absent `permission_denials` field normally means zero — zo omits it when
/// there were none. The deep loop sets this flag when a phase died before
/// reporting anything at all, where that reading would be a fabrication.
fn parse_denials_unknown(stdout: &str) -> bool {
    last_json_value(stdout, |v| {
        v.get("permission_denials_unknown").and_then(Value::as_bool)
    })
    .unwrap_or(false)
}

/// Whether an unknown denial count has to block the run.
///
/// Charged exactly where an OBSERVED denial would have been fatal, by asking the
/// single definition of "blocking" with a hypothetical count of one, under the
/// same effective test the gate itself uses. That matters most for
/// `verifier_only`, which has no failing-test or no-op clause of its own: there
/// `permission_denial_fatal` is the only term standing between a run that was
/// actually blocked and a pass, so a fabricated zero would buy that pass. Where a
/// denial would merely have been recovered, unknown changes nothing.
fn unknown_denial_blocks_pass(
    denials_unknown: bool,
    test: TestStatus,
    hygiene: &DiffHygiene,
    intended_provided: bool,
    objective_gate: &str,
    ...
    permission_denial_fatal(1, effective_test, hygiene, intended_provided)
}
...
mod tests {
    ...
    /// A `RunSpec` whose only interesting field for these tests is the arm.
    fn spec_with_ablation(runner_kind: &str, ablation: &[&str]) -> RunSpec {
        RunSpec {
            runner: "zo".into(),
    ...
    /// What a built command does to `ZO_ABLATE`. All three states are
    /// meaningful here: leaving it alone is the defect, so `Untouched` must be
    /// distinguishable from an explicit removal.
    ...
    fn ablation_env_of(cmd: &Command) -> AblationEnv {
        for (key, value) in cmd.get_envs() {
            if key == std::ffi::OsStr::new(telemetry::ABLATION_ENV) {
                return value.map_or(AblationEnv::Removed, |raw| {
                    AblationEnv::Set(raw.to_string_lossy().into_owned())
                });
            }
        }
        AblationEnv::Untouched
    }

    #[test]
    fn the_baseline_arm_clears_an_ambient_holdout_instead_of_inheriting_it() {
        // The defect this guards: run the suite from a shell exporting
        // `ZO_ABLATE` and the child is suppressed while its contract still
        // says full-feature. Setting the variable only for the ablated arm
        // would leave that inversion in place, so the empty arm must actively
        // REMOVE it.
        let mut cmd = Command::new("true");
        apply_zo_benchmark_env(&mut cmd, &spec_with_ablation("zo", &[]));
        assert_eq!(
            ablation_env_of(&cmd),
            AblationEnv::Removed,
            "an empty holdout must record a removal, not simply leave the var alone"
        );

        let mut cmd = Command::new("true");
        apply_zo_benchmark_env(
            &mut cmd,
            &spec_with_ablation("zo", &["routing_probe", "info_topology"]),
        );
        assert_eq!(
            ablation_env_of(&cmd),
            AblationEnv::Set("routing_probe,info_topology".to_string())
        );
    }

    #[test]
    fn a_holdout_the_run_cannot_apply_is_refused_before_anything_spawns() {
        // Recording a holdout that was never applied is the failure the whole
        // ablation machinery exists to prevent, so both impossible cases stop
        // the run rather than producing a mislabeled arm.
        let unknown = validate_holdout(&spec_with_ablation("zo", &["desgin_guidance"]))
            .expect_err("a key the runtime does not know suppresses nothing");
        assert!(unknown.to_string().contains("desgin_guidance"), "{unknown}");

        let foreign = validate_holdout(&spec_with_ablation("claude", &["routing_probe"]))
            .expect_err("another runner has no harness features to hold out");
        assert!(foreign.to_string().contains("claude"), "{foreign}");

        validate_holdout(&spec_with_ablation("claude", &[])).expect("no holdout is always fine");
        validate_holdout(&spec_with_ablation("zo", &["routing_probe"])).expect("valid holdout");
    }
    ...
    /// An unknown denial count is charged exactly where an observed one would
    /// have been fatal — no wider, no narrower.
    ///
    /// `verifier_only` is where this matters: it has no failing-test and no no-op
    /// clause, so `permission_denial_fatal` is the only term that catches a run
    /// whose refusal blocked the work. Reading a lost count as zero hands that run
    /// a pass.
    #[test]
    fn unknown_denials_block_only_where_an_observed_denial_would_have_been_fatal() {
        // Nothing landed against a declared intent: a denial here would be fatal,
        // so an unknown one must be too.
        let (noop, noop_intended) = score_for_policy("", &["src/a.js"], "intended_paths_only");
        assert!(unknown_denial_blocks_pass(
            true,
            TestStatus::Skipped,
            &noop,
            noop_intended,
        ...
        // The work landed: an observed denial there would have been recovered, so
        // an unknown one changes nothing.
        let (landed, landed_intended) =
            score_for_policy(" M src/a.js", &["src/a.js"], "intended_paths_only");
        assert!(!unknown_denial_blocks_pass(
            true,
            TestStatus::Skipped,
            &landed,
        ...
        // A known count never routes through this path at all.
        assert!(!unknown_denial_blocks_pass(
            false,
            TestStatus::Skipped,
            &noop,
        ...
        let r = RunResult {
            ...
            deep: Some(crate::deep::DeepVerdict {
                ...
                diagnostics: crate::deep::DeepDiagnostics {
                    ...
                    // The verifier is what went wrong here, not the planner: this
                    // fixture used a live plan, so the aggregate timeout above came
                    // from a later phase.
                    plan_timed_out: Some(false),
                    plan_result_present: Some(true),
