...
pub struct SuiteRunner {
    ...
    pub version: String,
    /// Harness features this runner holds out, as validated
    /// `telemetry::HarnessFeature` keys (build them with
    /// [`crate::ablation::holdout_keys`]).
    ///
    /// One source of truth for the arm: this same value configures the child's
    /// `ZO_ABLATE` through [`crate::runner::RunSpec::ablation`] AND is what the
    /// run's fairness contract records. Deriving the two separately is exactly
    /// how a contract comes to claim a holdout the run never applied.
...
pub fn run_suite(cfg: &SuiteConfig) -> io::Result<()> {
    ...
    for task in &tasks {
        for runner in &cfg.runners {
            for rep in 0..repeat {
                let cell = run_cell(cfg, task, runner, &catalog, rep, repeat)?;
                let _ = writeln!(ledger_text, "{}", cell.line);
                ledger_rows.push(cell.row);
                arms.extend(cell.arm);
            }
        }
    }
    ...
    // Only an experiment writes `ablation.json`. An ordinary suite run has no
    // treatment arm, and emitting an empty report for it would leave every out
    // dir carrying a file that reads like an experiment which found nothing.
    let ablation = crate::ablation::pair_suite_runs(
        &arms
            .iter()
            .map(|arm| crate::ablation::Arm::new(&arm.contract, &arm.result, arm.rep))
            .collect::<Vec<_>>(),
    );
    if !ablation.is_empty() {
        fs::write(
            cfg.out_dir.join("ablation.json"),
...
/// One `(task, runner)` cell's outputs: the ledger row (for the rollup), its
/// JSON line (for the on-disk ledger), and — when the cell actually ran — the
/// contract and result the ablation pairing needs.
struct Cell {
    row: LedgerRow,
    line: String,
    /// `None` for a fixture rejected at admission: it never ran, so it has no
    /// arm to pair. Keeping it optional rather than synthesizing an empty
    /// contract is what stops a fixture that never executed from entering an
    /// experiment as a failed run.
    arm: Option<ArmRun>,
}

/// One run's own record, owned so the pairing can borrow across the whole
/// suite. Only built for cells that ran.
struct ArmRun {
...
fn run_cell(
    ...
    let spec = RunSpec {
        ...
        keep_failed: false,
        ablation: runner.ablation.clone(),
    ...
    let mut cell = cell_from_verdict(runner, m, verdict, Some(&metrics));
    cell.arm = Some(ArmRun {
        contract,
        result,
        rep,
    });
...
/// `retry_budget` becomes the attempt cap (at least one), and its `phase_budget`
/// decides how each attempt's wall clock is split across the phases.
fn deep_config(catalog: &LaneCatalog, m: &TaskManifest) -> Option<DeepConfig> {
    ...
    (policy.verifier_policy == "strict").then(|| DeepConfig {
        ...
        phase_budget: policy.phase_budget,
    })
}
...
fn build_fairness(
    ...
    let input = FairnessInput {
        runner: runner.name.clone(),
        // The arm's NAME is `runner`; the thing under test is `runner_kind`.
        // An ablation pair holds the kind constant and lets the names differ —
        // see `ablation::HELD_CONSTANT`.
        runner_kind: runner.kind.clone(),
        lane: m.lane.clone(),
        ...
        // The SAME value that configured this cell's child process (see the
        // `RunSpec` built above from this runner). Reading the arm from one
        // place is what makes "recorded holdout == applied holdout" true by
        // construction instead of by comment — and the spawn path clears
        // `ZO_ABLATE` when this is empty, so an ambient holdout cannot turn
        // the baseline into a treatment arm behind the contract's back.
        ablation: runner.ablation.clone(),
    };
...
/// Suffix marking a token figure that covers only the runs which reported usage.
/// Empty when every run reported one, so a genuine total reads unadorned.
fn partial_token_note(missing: usize) -> String {
    if missing == 0 {
        return String::new();
    }
    format!(" (partial — {missing} run(s) reported no usage)")
}
...
fn render_report_markdown(
    ...
    let probe_failures: usize = rows
        ...
        .sum();
    // A run whose process was killed can report no usage at all, so this sum
    // covers only the runs that reported one. Saying so inline is the difference
    // between a total and a floor, and reading the floor as a total is exactly
    // how a three-run partial once became a twelve-run "16x fewer tokens" claim.
    let missing_tokens = rows
        .iter()
        .filter(|r| r.token_total.is_none() && r.has_observed_metrics())
        .count();
    let _ = writeln!(out, "- Wall seconds total: {wall_total}");
    let _ = writeln!(out, "- Token total: {token_total}{}", partial_token_note(missing_tokens));
    ...
    for (key, cell) in summary {
        let _ = writeln!(
            ...
            partial_token_note(cell.unknown_token_usage as usize),
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn an_ordinary_suite_run_writes_no_ablation_report() {
        // A benchmark that was never an experiment must not leave behind a file
        // that reads like an experiment which found nothing.
        let root = TempDir::new().unwrap();
        let bin = write_mock_runner(root.path());
        fs::write(root.path().join("lanes.toml"), LANES).unwrap();
        write_task(
        ...
        run_suite(&cfg).unwrap();
        assert!(!cfg.out_dir.join("ablation.json").exists());
    }

    #[cfg(unix)]
    #[test]
    fn two_arms_over_one_binary_produce_a_paired_ablation_report() {
        // End to end over the path an operator actually uses: two suite runners
        // sharing one binary and one `kind`, differing only in the holdout. The
        // arms' NAMES necessarily differ (they own separate output dirs), which
        // is exactly the shape that used to reject every pair.
        let root = TempDir::new().unwrap();
        let bin = write_mock_runner(root.path());
        fs::write(root.path().join("lanes.toml"), LANES).unwrap();
        write_task(
        ...
        let mut cfg = suite_cfg(root.path(), bin.clone(), "lanes.toml");
        cfg.runners.push(SuiteRunner {
            name: "zo_noprobe".to_string(),
            kind: "zo".to_string(),
            bin,
            args: vec![],
            model_label: "opus".to_string(),
            ...
            permission_mode: "danger-full-access".to_string(),
            version: "mock 1.0".to_string(),
            ablation: vec!["routing_probe".to_string()],
        });
        run_suite(&cfg).unwrap();

        // The contract records the holdout that configured the child, and the
        // baseline's stays absent.
        let ablated_contract = read_json(
            &cfg.out_dir
                .join("zo_noprobe")
                .join("fast-smoke")
                .join("fairness_contract.json"),
        );
        assert_eq!(ablated_contract["ablation"][0], "routing_probe");
        assert_eq!(ablated_contract["runner_kind"], "zo");
        let full_contract = read_json(
            &cfg.out_dir
                .join("zo")
                .join("fast-smoke")
                .join("fairness_contract.json"),
        );
        assert!(full_contract.get("ablation").is_none(), "{full_contract}");

        let ablation = read_json(&cfg.out_dir.join("ablation.json"));
        assert_eq!(
            ablation["rejected"].as_array().unwrap().len(),
            0,
            "the arms differ only in the holdout: {ablation}"
        );
        assert_eq!(ablation["unpaired"].as_array().unwrap().len(), 0);
        let pair = &ablation["pairs"][0];
        assert_eq!(pair["runner_full"], "zo");
        assert_eq!(pair["runner_ablated"], "zo_noprobe");
        assert_eq!(pair["held_out"][0], "routing_probe");
        assert_eq!(pair["outcome"], "both_passed");
        let rollup = &ablation["aggregates"][0];
        assert_eq!(rollup["pairs"], 1);
        assert_eq!(rollup["lane"], "fast");
        assert!(
            rollup["discordant_two_sided_p"].is_null(),
            "no pair disagreed, so no test was run: {rollup}"
        );
    }
