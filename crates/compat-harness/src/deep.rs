...
use crate::manifest::PhaseBudgetPolicy;
use crate::runner::filter_scratch;
use crate::runner::git_porcelain;
use crate::runner::json_documents;
...
use crate::runner::usage_in_value;
...
fn plan_phase_budget(phases: &PhaseBudgetPolicy, budget: &RunBudget) -> RunBudget {
    budget.reserving_capped(phases.objective_validation_reserve(), phases.plan_cap())
}

/// The exec turn's budget: smart-first tasks get the longer cap, everything else
/// the plain one.
///
/// A third branch used to sit between these two, picking a separate "complex"
/// cap via a predicate defined as exactly `needs_smart_first` — so the early
/// return above always won and that branch never ran. It is deleted rather than
/// carried into the catalog, because a lane key that cannot change anything is
/// worse than no key at all.
fn exec_phase_budget(
    phases: &PhaseBudgetPolicy,
    spec: &RunSpec,
    budget: &RunBudget,
) -> RunBudget {
    if needs_smart_first(spec) {
        return budget.reserving_capped(
            phases.smart_first_validation_reserve(),
            phases.smart_first_exec_cap(),
        );
    }
    budget.reserving_capped(phases.objective_validation_reserve(), phases.exec_cap())
}
...
fn verify_phase_budget(phases: &PhaseBudgetPolicy, budget: &RunBudget) -> RunBudget {
    budget.reserving_capped(phases.final_result_test_reserve(), phases.verify_cap())
}

fn verify_retry_phase_budget(phases: &PhaseBudgetPolicy, budget: &RunBudget) -> RunBudget {
    budget.reserving_capped(phases.verify_retry_reserve(), phases.verify_retry_cap())
}

/// Whether a compact verifier retry has enough time left to be worth a turn.
///
/// Named rather than left inline in the loop so the spawn decision itself is
/// reachable from a test, instead of only the floor it consults.
fn retry_is_worth_spawning(phases: &PhaseBudgetPolicy, retry: &RunBudget) -> bool {
    retry.remaining().unwrap_or_default() > phases.verify_retry_min_spawn()
}

/// How many execute→verify attempts the loop may use before giving up, and how it
/// splits the task's wall clock across each attempt's phases.
#[derive(Debug, Clone)]
pub struct DeepConfig {
    pub max_attempts: u32,
    /// Phase caps and reserves, from the lane's `lanes.toml` entry.
    pub phase_budget: PhaseBudgetPolicy,
}
...
pub struct DeepDiagnostics {
    ...
    /// Whether the PLANNER specifically ran out of time, or `None` when the
    /// deterministic plan was taken up front and no planner ran. `phase_timed_out`
    /// above is an aggregate over plan, exec, test, verify and retry, so it cannot
    /// answer that on its own — and without the answer, a plan-cap change is
    /// unfalsifiable: a run that still reports no tokens looks the same whether
    /// the planner died again or something else did.
    pub plan_timed_out: Option<bool>,
    /// Whether the planner returned any result text at all, or `None` when no
    /// planner ran. Separates "it ran out of time" from "it answered with
    /// nothing", which `plan_recovered` alone conflates because the fallback
    /// fires for both. `false` would have reinstated that conflation from the
    /// other side: the deterministic path has no planner, so reporting it as
    /// "did not answer" reads exactly like a planner that ran and returned
    /// nothing.
    pub plan_result_present: Option<bool>,
...
/// What a live planner turn reported about itself.
///
/// Held separately from the plan text so the loop can move that text on without
/// a copy, and kept as its own value so the reading can be exercised without
/// spawning a planner — a diagnostic flag whose own correctness is untested is
/// worse than no flag, because the next plan-cap decision will trust it.
...
impl PlanTurn {
    /// Read a finished planner turn's signals off the result text already
    /// extracted from its output.
    fn observe(timed_out: bool, plan_text: &str) -> Self {
        Self {
            timed_out,
            result_present: !plan_text.trim().is_empty(),
        }
    }
}
...
#[allow(
    clippy::struct_excessive_bools,
    reason = "independent completeness flags, not a state machine: a call can report tokens without turns, or turns without a cache breakdown, and collapsing them would lose exactly the distinctions this accounting exists to keep"
)]
struct DeepUsage {
    input: u64,
    output: u64,
    ...
    usage_seen: bool,
    cache_all: bool,
    /// Set when any call in the loop reported no usage at all. Tracked
    /// separately from `usage_seen`, because the dangerous state is the mix: one
    /// measured phase plus one killed phase yields a real number that is not the
    /// run's cost, and a bare `usage` object gives a reader no way to tell.
    usage_missing: bool,
    /// Set when a spawned call reported no turn count. Kept apart from
    /// `usage_missing` because a runner may report one and not the other.
    iterations_missing: bool,
    /// Set when a spawned call produced nothing readable at all, so whether it
    /// hit a permission denial is unknown rather than zero. An absent
    /// `permission_denials` field means zero for a call that reported *something*
    /// — zo omits the field when there were none — so only a call that reported
    /// nothing leaves the count genuinely unknown.
    denials_missing: bool,
}
...
impl DeepUsage {
    fn new() -> Self {
        Self {
            ...
            cache_all: true,
            usage_missing: false,
            iterations_missing: false,
            denials_missing: false,
        }
    }

    /// Fold one agent call's stdout into the running totals. Mirrors the
    ...
    ///
    /// Read through [`json_documents`] so a phase killed mid-stream still
    /// contributes the tokens it had already reported, instead of vanishing from
    /// the loop total. Streamed counters are cumulative over the single process
    /// one call spawns, so the newest event wins *within* a call and only the
    /// resulting per-call totals are summed across the loop — summing the events
    /// themselves would multiply the same tokens by however often the runner
    /// restated them.
    fn accumulate(&mut self, stdout: &str, spawned: bool) {
        if !spawned {
            // The budget was already spent, so no process ran and this call's true
            // cost is exactly zero — nothing to count, and nothing missing either.
            // Flagging it would void a run whose other phases were measured fine.
            return;
        }
        let documents = json_documents(stdout);
        let iterations = documents.iter().rev().find_map(|v| {
            v.get("iterations")
                .and_then(Value::as_u64)
                .or_else(|| v.get("num_turns").and_then(Value::as_u64))
        });
        let denials = documents.iter().rev().find_map(|v| {
            v.get("permission_denials")
                .and_then(|d| d.as_array().map(Vec::len))
        });
        let tokens = documents.iter().rev().find_map(usage_in_value);
        match iterations {
            Some(count) => self.iterations += count,
            // A killed phase never prints the envelope carrying its turn count, so
            // adding zero would report fewer turns than the loop actually took.
            None => self.iterations_missing = true,
        }
        self.denials += denials.unwrap_or(0);
        // An absent `permission_denials` field means zero for a call that reported
        // anything at all — zo omits it when there were none. It means UNKNOWN only
        // for a call that reported nothing, i.e. one killed before its envelope.
        // That distinction decides runs: the `verifier_only` gate has no
        // failing-test or no-op clause, so a fabricated zero here is the only thing
        // between a run that was actually blocked and a pass.
        if denials.is_none() && iterations.is_none() && tokens.is_none() {
            self.denials_missing = true;
        }
        let Some(tokens) = tokens else {
            // This call billed tokens nobody can count: a phase killed at its cap
            // never prints the envelope carrying them. Recording the gap is the
            // point of this whole change — without it the surviving phases' sum
            // leaves here indistinguishable from a complete measurement.
        ...
        match (tokens.cache_creation, tokens.cache_read) {
            (Some(cc), Some(cr)) => {
    ...
    fn synthesize(&self, last_rc: i32) -> String {
        ...
        if let Some(map) = obj.as_object_mut() {
            // A killed phase never reports its turn count, so summing only the
            // phases that did report understates the loop. Leaving the key out
            // makes `parse_iterations` yield `None`, which `RunResult.iterations`
            // can represent — a smaller number would just look like a shorter run.
            if !self.iterations_missing {
                map.insert("iterations".to_string(), json!(self.iterations));
            }
            // Say the count is unknown rather than letting the empty array above
            // read as "observed, and there were none".
            if self.denials_missing {
                map.insert("permission_denials_unknown".to_string(), json!(true));
            }
        }
        // A subtotal is not a total. If any call went unmeasured the loop reports
        // no usage at all, so `parse_tokens` yields `None`, the ledger row counts
        // as unmeasured, and the report says so — instead of publishing a partial
        // figure that reads exactly like a complete one.
        if self.usage_seen && !self.usage_missing {
...
pub(crate) fn run_deep_loop(
    ...
    // Stays `None` on the deterministic path: there is no planner turn to
    // describe, which is a different finding from a planner that answered
    // nothing and must not serialize as one.
    let mut plan_turn: Option<PlanTurn> = None;
    let (mut plan_md, mut plan_verdict, mut plan_recovered) =
        if needs_smart_first(spec) && !spec.intended.is_empty() && deterministic_verdict.valid {
            ...
            let plan_budget = plan_phase_budget(&cfg.phase_budget, budget);
            ...
            usage.accumulate(&plan_out.stdout, plan_out.spawned);
            let plan_text = extract_result(&plan_out.stdout);
            // A phase the budget never let start is not a planner turn. Its
            // output is byte-identical to a planner killed mid-write — empty,
            // `timed_out` set — but calling it one would claim the PLANNER ran
            // out of its own time, and that is the single question these fields
            // exist to answer. A cap change cannot help a phase that never ran.
            plan_turn = plan_out
                .spawned
                .then(|| PlanTurn::observe(plan_out.timed_out, &plan_text));
            let plan_verdict = validate_plan(&plan_text);
            (plan_text, plan_verdict, false)
        };
    ...
    while attempt <= cfg.max_attempts {
        ...
        // Every task gets to SEE the files it was asked to change. That used to be
        // gated on the same keyword predicate that picks the exec budget, so one
        // guess about a prompt's subject decided both how long a task got and
        // whether it could read the code — and the fixtures those keywords missed
        // went 0/6 while the ones they caught went 6/6. Withholding context is not
        // a lane policy, it is a defect: nothing justifies making
        // `schema-propagation`, which names four intended files, edit them blind.
        //
        // The strategy preamble stays gated, because it dictates the agent's first
        // action, and the budget stays gated too. So this moves exactly one
        // variable, and the next run can say whether context or wall-clock was the
        // binding constraint.
        ...
        let (verifier, verify_text) = if objective_ok {
            ...
            if probe_report.is_clean() {
                let verify_budget = verify_phase_budget(&cfg.phase_budget, budget);
                ...
                usage.accumulate(&verify_out.stdout, verify_out.spawned);
                ...
                if verifier_needs_compact_retry(&verifier) {
                    let retry_budget = verify_retry_phase_budget(&cfg.phase_budget, budget);
                    if retry_is_worth_spawning(&cfg.phase_budget, &retry_budget) {
                        ...
                        usage.accumulate(&retry_out.stdout, retry_out.spawned);
...
///
/// A streamed buffer holds many events, and tool-result events carry a `result`
/// field of their own, so the terminal `{"type":"result",…}` envelope is
/// preferred over whichever event merely happens to be last. The looser search
/// stays as the fallback for converted codex output, which has no such tag.
fn extract_result(stdout: &str) -> String {
    ...
    documents
        ...
        .find_map(text_of)
        .or_else(|| documents.iter().rev().find_map(text_of))
        .unwrap_or_default()
}
...
/// Source/test context for the exec phase.
///
/// `smart_first` gates only the strategy preamble, never the context itself. That
/// preamble orders the agent to edit before it inspects anything and to stop after
/// one edit — instructions that change what it DOES, so handing them to every task
/// would be a methodology change, the same objection that keeps this harness on
/// `--output-format json`. Being able to read the files it was asked to change is
/// not in that category, so every task gets that part.
fn exec_context_pack(
    work: &Path,
    intended: &[String],
    baseline: &str,
    task: &str,
