...
use compat_harness::{
    ...
    FairnessStatus, LaneCatalog, ObjectiveGate, PhaseBudgetPolicy, RunSpec, TaskManifest,
    VerifierDecision,
};
...
const USAGE: &str = "usage:
  ...
  deep-eval fairness-contract --runner R [--runner-kind K] --lane L --fixture-id ID --prompt-file F \\
  ...
  deep-eval suite [--fixtures DIR] [--lanes TOML] --out DIR [--runners CSV] \\
      [--suite-version V] [--repeat N]   (per-runner config from <NAME>_BIN/_KIND/_MODEL_LABEL/_EFFORT/_ARGS/_ABLATE env;
      set <NAME>_ARGS=\"--permission-mode acceptEdits\" or another explicit permission mode for fairness-valid runs;
      set <NAME>_ABLATE=routing_probe,info_topology (or `all`) to make that runner the ablated arm of an
      experiment — the harness sets ZO_ABLATE for its children and records the same holdout in the contract.
      An experiment is TWO runners over one binary, e.g. --runners zo_full,zo_noprobe with
      ZO_FULL_BIN=$ZO_BIN, ZO_FULL_KIND=zo, ZO_NOPROBE_BIN=$ZO_BIN, ZO_NOPROBE_KIND=zo and
      ZO_NOPROBE_ABLATE=routing_probe: the arms pair on <NAME>_KIND, so an ablated arm
      that leaves _KIND unset pairs with nothing. Do NOT name the control arm `zo`: it derives
      ZO_ABLATE, which is the variable the harness reads for its own ablation, so an ambient value
      there would be read as that runner's holdout and ablate the control (the suite refuses it). Pairs and per-lane rollups land in ablation.json;
...
fn run_run(args: &[String]) -> ExitCode {
    ...
    let spec = RunSpec {
        ...
        keep_failed,
        ablation: Vec::new(),
        deep: deep_attempts.map(|n| DeepConfig {
            max_attempts: n,
            // This path takes no lane catalog, so the defaults stand — the same
            // numbers `run_suite` resolves for a catalog that omits the table.
...
fn run_suite_cmd(args: &[String]) -> ExitCode {
    ...
    let runners = match build_runners(&runners_csv) {
        ...
        // Each problem carries its own remedy: the list mixes unset binaries
        // with unreadable holdouts, so a single binary-specific instruction
        // here would send a reader to fix the wrong variable.
        Err(problems) => {
            eprintln!(
                "deep-eval suite: runner configuration error(s):\n  {}\n\
                 fix each, or drop the runner from --runners.\n{USAGE}",
                problems.join("\n  ")
            );
            return ExitCode::from(2);
        }
...
/// always exactly the one that ran (reproducible).
///
/// Every misconfiguration is collected into the `Err` list and **never silently
/// dropped**: the suite compares exactly the runners asked for, or says which
/// variable to fix, consistent with the harness's no-silent-degradation rule.
/// Each entry states its own remedy because the list mixes kinds — an unset
/// binary and an unreadable `<NAME>_ABLATE` need different fixes.
/// Whether a runner's derived `<NAME>_ABLATE` key is one whose value cannot be
/// attributed to that runner.
///
/// True only when the key collides with the harness's own ablation variable AND
/// something set it: a collision alone is harmless (the runner is simply the
/// full-feature arm), and an ambient value under a non-colliding key belongs to
/// no runner at all. Taken as a separate predicate so the rule is testable
/// without mutating the process environment.
fn ablate_key_is_ambiguous(ablate_key: &str, is_set: bool) -> bool {
    is_set && ablate_key == telemetry::ABLATION_ENV
}
    ...
    for name in csv.split(',').map(str::trim).filter(|n| !n.is_empty()) {
        ...
        let Ok(bin) = std::env::var(format!("{up}_BIN")) else {
            problems.push(format!(
                "{up}_BIN (for runner '{name}') is unset — set it to the runner's \
                 executable; there is no PATH fallback, so the binary recorded in the \
                 fairness contract is always the one that ran"
            ));
            continue;
        };
        let bin = fs::canonicalize(&bin).unwrap_or_else(|_| PathBuf::from(&bin));
        ...
        // A runner named `zo` derives `ZO_ABLATE` — byte-identical to the
        // variable the harness reads for its OWN ablation. One variable cannot
        // answer both "what did the operator hold out from this runner" and
        // "what is this process holding out", so an ambient value is read as
        // the colliding runner's declaration and silently makes the CONTROL arm
        // ablated. The suite then reports `0 pairs / 2 unpaired` with a reason
        // that never names the variable responsible.
        let ablate_key = format!("{up}_ABLATE");
        if ablate_key_is_ambiguous(&ablate_key, std::env::var_os(&ablate_key).is_some()) {
            problems.push(format!(
                "{ablate_key} (for runner '{name}') is also the variable the harness reads \
                 for its own ablation, and it is set — its value cannot be told apart from \
                 an ambient holdout, which would silently ablate this arm. Rename the runner \
                 (e.g. 'zo_full') or unset {ablate_key}"
            ));
            continue;
        }
        // `<NAME>_ABLATE` declares this runner's ablation arm, validated through
        // the runtime's own feature vocabulary. A key the runtime does not know
        // would set an environment variable that suppresses nothing, so it is
        // collected as a config error rather than accepted — the same
        // no-silent-degradation rule `<NAME>_BIN` follows above.
        let ablation = match std::env::var(&ablate_key) {
            Ok(spec) => match compat_harness::holdout_keys(&spec) {
                Ok(keys) => keys,
                Err(error) => {
                    problems.push(format!(
                        "{up}_ABLATE (for runner '{name}'): {error} — fix the key or unset \
                         the variable to run this runner as the full-feature arm"
                    ));
                    continue;
                }
            },
            Err(std::env::VarError::NotPresent) => Vec::new(),
            // Unset and unreadable are NOT the same answer. Folding a non-UTF-8
            // value into "no holdout" would file a run the operator configured
            // as an experiment arm as the baseline instead.
            Err(std::env::VarError::NotUnicode(_)) => {
                problems.push(format!(
                    "{up}_ABLATE (for runner '{name}') is not valid UTF-8, so the holdout it \
...
fn run_fairness_contract(args: &[String]) -> ExitCode {
    ...
    while i < args.len() {
        ...
        match arg {
            ...
            "--runner-kind" => input.runner_kind = value.unwrap_or_default(),
...
mod tests {
    ...
    #[test]
    fn suite_usage_documents_the_ablation_arm_env() {
        // An experiment switch nobody can find in --help is a switch nobody
        // uses; the env list is where a reader looks for per-runner config.
        assert!(USAGE.contains("_ARGS/_ABLATE env"));
        assert!(USAGE.contains("<NAME>_ABLATE=routing_probe,info_topology"));
        // The step that is invisible from the switch itself: arms pair on
        // `<NAME>_KIND`, so an ablated arm that leaves it unset produces a run
        // and no comparison — a silent non-experiment.
        assert!(USAGE.contains("ZO_NOPROBE_KIND=zo"));
        assert!(USAGE.contains("ablation.json"));
        // The example must not be the one configuration the suite refuses. A
        // control arm named `zo` derives ZO_ABLATE, so anyone following the
        // printed example from a shell that exports it got `0 pairs / 2
        // unpaired` — from the documentation, on the documented path.
        assert!(
            !USAGE.contains("--runners zo,"),
            "the printed example must not name the control arm `zo`"
        );
        assert!(USAGE.contains("Do NOT name the control arm `zo`"));
    }

    #[test]
    fn a_runner_whose_ablate_key_is_the_harness_ablation_env_is_refused_when_set() {
        // The collision is real: uppercasing runner `zo` yields exactly the
        // variable the harness reads for its own ablation.
        assert_eq!(format!("{}_ABLATE", "zo".to_uppercase()), telemetry::ABLATION_ENV);

        assert!(super::ablate_key_is_ambiguous(telemetry::ABLATION_ENV, true));
        // Unset is not ambiguous — that runner is simply the full-feature arm.
        assert!(!super::ablate_key_is_ambiguous(telemetry::ABLATION_ENV, false));
        // A non-colliding key is a per-runner declaration however it was set.
        assert!(!super::ablate_key_is_ambiguous("ZO_NOPROBE_ABLATE", true));
    }
