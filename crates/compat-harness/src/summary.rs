...
impl LedgerRow {
    pub(crate) fn has_observed_metrics(&self) -> bool {
...
pub struct LaneRunnerSummary {
    ...
    /// Observed runs with no `token_output`. Tracked apart from
    /// `unknown_token_usage` because a ledger may carry a complete total-token
    /// column beside a partial output column, so each throughput has to be
    /// withheld on its own evidence rather than the other's.
    pub unknown_output_token_usage: u32,
    ...
    /// Observed runs that reported no cost. Non-zero withholds both figures
    /// below, because accepted-per-dollar divides the FULL accepted count by this
    /// total: an accepted run whose cost went unrecorded stays in the numerator
    /// while its dollars never reach the denominator.
    pub unknown_cost_usage: u32,
...
pub struct RepeatCellSummary {
    ...
    /// Repeats here that reported no usage. Non-zero voids both token statistics
    /// above, because `repeats` keeps claiming the full count: a mean over the
    /// survivors would be read as describing all N runs, and the runs that die at
    /// a wall-clock cap are the expensive ones.
    pub unknown_token_usage: u32,
    #[serde(skip)]
    wall_samples: Vec<u64>,
    #[serde(skip)]
...
pub fn summarize_repeats(rows: &[LedgerRow]) -> BTreeMap<String, RepeatCellSummary> {
    ...
    for cell in groups.values_mut() {
        ...
        // Wall time is observed for every repeat, so its statistics always stand.
        // Tokens are not: a cell whose usage is missing anywhere reports no token
        // figure rather than the survivors' one, which a reader would pair with
        // `repeats` and take for the whole cell.
        if cell.unknown_token_usage == 0 {
...
pub fn summarize_ledger(rows: &[LedgerRow]) -> BTreeMap<String, LaneRunnerSummary> {
    ...
    for row in rows {
        ...
        if let Some(tokens) = row.token_output {
            ...
            entry.unknown_output_token_usage += 1;
        }
        ...
        if let Some(cost) = row.cost_usd {
            ...
            entry.unknown_cost_usage += 1;
        }
    }
    for summary in groups.values_mut() {
        finalize_lane_summary(summary);
    }
    groups
}

/// Derive one lane×runner cell's central tendencies and rates from its samples.
fn finalize_lane_summary(summary: &mut LaneRunnerSummary) {
        ...
        // A rate needs numerator and denominator measured over the SAME runs. With
        // any run's usage missing, the tokens cover a subset while the wall time
        // covers every run, so the figure reads low by exactly however much went
        // unobserved — a measurement gap wearing the appearance of efficiency.
        //
        // Each rate is therefore gated on its OWN numerator: a ledger can carry
        // every run's total tokens while missing some run's output tokens, and one
        // column's completeness is no evidence for the other's.
        if wall_sum > 0 {
            if summary.unknown_token_usage == 0 {
                summary.aggregate_total_tokens_per_second =
                    Some(ratio_u64(summary.total_token_usage, wall_sum));
            }
            if summary.unknown_output_token_usage == 0 {
        ...
        // Withhold the spend itself, not just the ratio. A bare `total_cost_usd`
        // reads as this runner's cost, and accepted-per-dollar would divide the
        // full accepted count by a sum missing whatever the unrecorded runs spent
        // — the cheaper the gap, the better the score.
        if summary.unknown_cost_usage > 0 {
...
pub struct RunnerScore {
    pub runner: String,
    /// Fixtures this runner actually ran in the lane. Previously the lane's entire
    /// fixture count, which claimed full coverage even for a runner that ran one.
    pub fixtures_scored: u32,
    /// Fixtures that contributed a token ratio — the real base of `token_score`.
    /// Below `fixtures_scored` means the token axis saw only part of the lane.
    pub token_scored_fixtures: u32,
    pub accepted: u32,
...
struct CellAgg {
    ...
    accepted: u32,
    total: u32,
    /// Runs in this cell that reported no usage. A cell missing any run cannot
    /// be compared on tokens: its median would describe the runs that survived,
    /// and the runs that die are exactly the expensive ones, so the gap reads as
    /// thrift and wins the efficiency score it should have forfeited.
    token_missing: u32,
}

/// This cell's comparable medians, `(tokens, wall)`.
///
/// The token median is `None` for a cell with an unmeasured run, which the ratio
/// walk already treats as "does not compete". That exclusion is the point: the
/// runs that die at a wall-clock cap are the expensive ones, so a median over
/// only the survivors reads as thrift and would win an efficiency score the cell
/// forfeited by not being measurable. Wall time is unaffected — it is observed
/// for every run, killed or not.
fn cell_medians(agg: &CellAgg) -> (Option<f64>, Option<f64>) {
    let mut wall = agg.wall_samples.clone();
    let wall_median = median_u64(&mut wall);
    if agg.token_missing > 0 {
        return (None, wall_median);
    }
    let mut tok = agg.token_samples.clone();
    (median_plain_u64(&mut tok), wall_median)
}

/// One lane's cells: fixture -> runner -> aggregate.
type FixtureCells = BTreeMap<String, BTreeMap<String, CellAgg>>;

/// What one runner accumulated across a lane's fixtures.
#[derive(Default)]
struct RunnerTally {
    ...
    total: u32,
    fixtures_ran: u32,
    /// Any fixture this runner ran whose token data was incomplete.
    token_incomplete: bool,
}

/// Smallest strictly-positive value, or `None` when nothing qualifies — the
/// per-fixture best that a ratio normalizes against.
fn best_positive(values: impl Iterator<Item = f64>) -> Option<f64> {
    values
...
/// Fold each fixture's per-runner medians into per-runner ratio tallies.
fn lane_tallies(fixtures: &FixtureCells) -> BTreeMap<&str, RunnerTally> {
    let mut tallies: BTreeMap<&str, RunnerTally> = BTreeMap::new();
    for runners in fixtures.values() {
        let medians: BTreeMap<&str, (Option<f64>, Option<f64>)> = runners
            .iter()
            .map(|(runner, agg)| (runner.as_str(), cell_medians(agg)))
            .collect();
        let best_token = best_positive(medians.values().filter_map(|(tok, _)| *tok));
        let best_wall = best_positive(medians.values().filter_map(|(_, wall)| *wall));
        for (runner, agg) in runners {
            let (tok, wall) = medians
                .get(runner.as_str())
                .copied()
                .unwrap_or((None, None));
            let tally = tallies.entry(runner.as_str()).or_default();
            tally.accepted += agg.accepted;
            tally.total += agg.total;
            tally.fixtures_ran += 1;
            tally.token_incomplete |= agg.token_missing > 0;
            // Only a strictly positive ratio is one `geomean` will use, and
            // `token_scored_fixtures` reports this vector's length — pushing a zero
            // would claim a scored fixture the geomean then silently discards.
            if let (Some(tok), Some(best)) = (tok, best_token) {
                let ratio = tok / best;
                if ratio > 0.0 {
                    tally.token_ratios.push(ratio);
                }
            }
            if let (Some(wall), Some(best)) = (wall, best_wall) {
                let ratio = wall / best;
                if ratio > 0.0 {
                    tally.wall_ratios.push(ratio);
                }
            }
        }
    }
...
/// One runner's standing from its lane tally.
fn runner_score(runner: &str, tally: &RunnerTally) -> RunnerScore {
    let strict = rate(tally.accepted, tally.total);
    // A runner with an unmeasured run does not compete on tokens at all. Merely
    // leaving that fixture out of the geomean — which excluding its median does —
    // scores the runner on the fixtures that survived, and the runs that die at a
    // wall-clock cap are the expensive ones. Survivorship would read as thrift and
    // collect the efficiency weight the runner forfeited by being unmeasurable.
    let token_geomean = if tally.token_incomplete {
        None
    } else {
        geomean(&tally.token_ratios)
    };
    let wall_geomean = geomean(&tally.wall_ratios);
    ...
    RunnerScore {
        ...
        fixtures_scored: tally.fixtures_ran,
        token_scored_fixtures: u32::try_from(tally.token_ratios.len()).unwrap_or(u32::MAX),
...
/// Rank one lane's runners, highest composite first. Ties break by accuracy then
/// runner name so the ordering is deterministic (every input is finite, so NaN
/// cannot appear).
fn lane_scores(fixtures: &FixtureCells) -> Vec<RunnerScore> {
    let mut scores: Vec<RunnerScore> = lane_tallies(fixtures)
        .iter()
        .map(|(runner, tally)| runner_score(runner, tally))
        .collect();
    scores.sort_by(|a, b| {
        b.composite
...
pub fn build_leaderboard(rows: &[LedgerRow]) -> BTreeMap<String, Vec<RunnerScore>> {
    let mut cells: BTreeMap<String, FixtureCells> = BTreeMap::new();
    for row in rows {
        let cell = cells
        ...
        if let Some(tokens) = row.token_total {
            ...
            cell.token_missing += 1;
        }
    }

    cells
        .iter()
        .map(|(lane, fixtures)| (lane.clone(), lane_scores(fixtures)))
        .collect()
}
...
"#,
        ...
        // Row `c` is an observed run that reported no cost, so this runner's spend
        // is incomplete and both cost figures are withheld. This assertion used to
        // expect 5.0 and 0.2 — i.e. the full accepted count divided by a sum
        // missing whatever `c` spent, which reads cheaper than the work was.
        assert_eq!(deep.unknown_cost_usage, 1);
        assert!(deep.total_cost_usd.is_none());
        assert!(deep.cost_normalized_score.is_none());
    }
...
"#,
    ...
    #[test]
    fn leaderboard_withholds_the_token_axis_from_a_runner_with_an_unmeasured_run() {
        // The survivorship trap: `zo` is measured only on the cheap fixture and
        // dies unmeasured on the expensive one, while `claude` is measured on both.
        // Scoring the geomean over whichever fixtures survived would hand `zo` a
        // perfect 100 for the one cheap task it finished and let it beat the runner
        // that was measurable everywhere.
        let rows = parse_ledger(
            r#"
{"runner":"zo","lane":"deep","fixture":"cheap","final_decision":"accepted","leaderboard_eligible":true,"wall_seconds":10,"token_total":100}
{"runner":"zo","lane":"deep","fixture":"pricey","final_decision":"rejected","leaderboard_eligible":true,"wall_seconds":240,"timeout":true}
{"runner":"claude","lane":"deep","fixture":"cheap","final_decision":"accepted","leaderboard_eligible":true,"wall_seconds":20,"token_total":200}
{"runner":"claude","lane":"deep","fixture":"pricey","final_decision":"accepted","leaderboard_eligible":true,"wall_seconds":100,"token_total":900}
"#,
        );
        let board = build_leaderboard(&rows);
        let deep = &board["deep"];
        let zo = deep.iter().find(|s| s.runner == "zo").expect("zo scored");
        let claude = deep
            .iter()
            .find(|s| s.runner == "claude")
            .expect("claude scored");

        // `zo` forfeits the token axis rather than winning it on one survivor.
        assert!(zo.geomean_token_ratio.is_none());
        assert!(zo.token_score.abs() < 1e-9);
        // And the coverage it actually had is visible instead of implied.
        assert_eq!(zo.fixtures_scored, 2);
        assert_eq!(zo.token_scored_fixtures, 1);

        // `claude` still competes: ratios 2.0 on cheap and 1.0 on pricey.
        assert_eq!(claude.token_scored_fixtures, 2);
        let expected = 100.0 / 2.0_f64.sqrt();
        assert!((claude.token_score - expected).abs() < 1e-9);
        assert!(claude.token_score > zo.token_score);

        // Wall time is observed even for the killed run, so speed still scores.
        assert!(zo.geomean_wall_ratio.is_some());
    }

    #[test]
    fn repeats_withhold_token_statistics_when_a_repeat_went_unmeasured() {
        // `repeats` counts all three, so a mean over the two that reported usage
        // would be read as the cost of three runs.
        let rows = parse_ledger(
            r#"
{"runner":"zo","lane":"deep","fixture":"a","final_decision":"accepted","leaderboard_eligible":true,"wall_seconds":10,"token_total":100}
{"runner":"zo","lane":"deep","fixture":"a","final_decision":"accepted","leaderboard_eligible":true,"wall_seconds":20,"token_total":200}
{"runner":"zo","lane":"deep","fixture":"a","final_decision":"rejected","leaderboard_eligible":true,"wall_seconds":240,"timeout":true}
"#,
        );
        let cell = &summarize_repeats(&rows)["deep/zo/a"];

        assert_eq!(cell.repeats, 3);
        assert_eq!(cell.unknown_token_usage, 1);
        assert!(cell.mean_token_total.is_none());
        assert!(cell.median_token_total.is_none());

        // Wall time is observed for every repeat, killed or not, so it stands.
        assert!((cell.median_wall_seconds.unwrap() - 20.0).abs() < 1e-9);
        assert!((cell.mean_wall_seconds.unwrap() - 90.0).abs() < 1e-9);
    }

    #[test]
    fn each_rate_and_the_cost_score_need_their_own_column_complete() {
        // Total tokens are complete for both runs, output tokens are not, costs are.
        // One column's completeness is no evidence for another's.
        let rows = parse_ledger(
            r#"
{"runner":"zo","lane":"deep","fixture":"a","final_decision":"accepted","leaderboard_eligible":true,"wall_seconds":10,"token_total":100,"token_output":10,"cost_usd":1.0}
{"runner":"zo","lane":"deep","fixture":"b","final_decision":"accepted","leaderboard_eligible":true,"wall_seconds":10,"token_total":200,"cost_usd":3.0}
"#,
        );
        let deep = &summarize_ledger(&rows)["deep/zo"];

        assert_eq!(deep.unknown_token_usage, 0);
        assert_eq!(deep.unknown_output_token_usage, 1);
        assert_eq!(deep.unknown_cost_usage, 0);

        // The complete column still publishes a rate; the partial one does not.
        assert!(deep.aggregate_total_tokens_per_second.is_some());
        assert!(deep.aggregate_output_tokens_per_second.is_none());

        // Costs are complete, so accepted-per-dollar stands: 2 accepted over $4.
        assert!((deep.total_cost_usd.unwrap() - 4.0).abs() < 1e-9);
        assert!((deep.cost_normalized_score.unwrap() - 0.5).abs() < 1e-9);
    }

    #[test]
    fn token_scored_fixtures_counts_only_ratios_the_geomean_uses() {
        // `zo` reports zero tokens, a ratio `geomean` discards as non-positive. The
        // count must not advertise a scored fixture the score never used, or the
        // field contradicts its own documentation.
        let rows = parse_ledger(
            r#"
{"runner":"zo","lane":"deep","fixture":"a","final_decision":"accepted","leaderboard_eligible":true,"wall_seconds":10,"token_total":0}
{"runner":"claude","lane":"deep","fixture":"a","final_decision":"accepted","leaderboard_eligible":true,"wall_seconds":20,"token_total":200}
"#,
        );
        let board = build_leaderboard(&rows);
        let zo = board["deep"]
        ...
        assert_eq!(zo.fixtures_scored, 1);
        assert_eq!(zo.token_scored_fixtures, 0);
        assert!(zo.geomean_token_ratio.is_none());
        assert!(zo.token_score.abs() < 1e-9);
    }
}
