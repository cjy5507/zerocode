...
pub struct LanePolicy {
    ...
    /// Deep-loop phase budgets. A lane that omits the table keeps the compiled
    /// defaults, so its results stay comparable with every run recorded before
    /// the table existed; `deep` overrides only its planning cap.
    #[serde(default)]
...
/// How a lane splits its per-task timeout across the deep loop's phases, in
/// seconds (a `[lanes.<name>.phase_budget]` table).
///
/// These were the last benchmark numbers that still needed a recompile to retune,
/// which contradicted this catalog's own promise that retuning a lane is a data
/// change. Every unspecified field keeps the value the constants held, so a lane
/// omitting the table runs exactly as it did before the table existed.
///
/// Unknown keys are rejected rather than ignored: a typo that silently kept the
/// default would mean a benchmark that quietly did not use the settings its
/// operator wrote down.
#[derive(Debug, Clone, Copy, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct PhaseBudgetPolicy {
    /// Cap on the planning turn.
    #[serde(default = "default_plan_cap")]
    pub plan_cap_seconds: u64,
    /// Cap on an implementation turn.
    #[serde(default = "default_exec_cap")]
    pub exec_cap_seconds: u64,
    /// Cap on an implementation turn for a task routed smart-first.
    #[serde(default = "default_smart_first_exec_cap")]
    pub smart_first_exec_cap_seconds: u64,
    /// Cap on the verifier turn.
    #[serde(default = "default_verify_cap")]
    pub verify_cap_seconds: u64,
    /// Cap on the verifier's compact retry turn.
    #[serde(default = "default_verify_retry_cap")]
    pub verify_retry_cap_seconds: u64,
    /// Held back from a plan or exec turn so the objective gate can still run.
    #[serde(default = "default_objective_validation_reserve")]
    pub objective_validation_reserve_seconds: u64,
    /// The same reserve on the smart-first exec path.
    #[serde(default = "default_smart_first_validation_reserve")]
    pub smart_first_validation_reserve_seconds: u64,
    /// Held back from the verifier turn so the final result test can still run.
    #[serde(default = "default_final_result_test_reserve")]
    pub final_result_test_reserve_seconds: u64,
    /// Held back from the verifier's retry turn.
    #[serde(default = "default_verify_retry_reserve")]
    pub verify_retry_reserve_seconds: u64,
}

const fn default_plan_cap() -> u64 {
    25
}
const fn default_exec_cap() -> u64 {
    150
}
const fn default_smart_first_exec_cap() -> u64 {
    240
}
const fn default_verify_cap() -> u64 {
    30
}
const fn default_verify_retry_cap() -> u64 {
    15
}
const fn default_objective_validation_reserve() -> u64 {
    60
}
const fn default_smart_first_validation_reserve() -> u64 {
    60
}
const fn default_final_result_test_reserve() -> u64 {
    60
}
const fn default_verify_retry_reserve() -> u64 {
    5
}

/// The spawn floor the deep loop required before these budgets became data. It is
/// kept as the ceiling on the derived floor, so no cap value can gate a retry that
/// a different cap would have spawned.
const LEGACY_VERIFY_RETRY_MIN_SPAWN_SECONDS: u64 = 5;

impl Default for PhaseBudgetPolicy {
    fn default() -> Self {
        Self {
            plan_cap_seconds: default_plan_cap(),
            exec_cap_seconds: default_exec_cap(),
            smart_first_exec_cap_seconds: default_smart_first_exec_cap(),
            verify_cap_seconds: default_verify_cap(),
            verify_retry_cap_seconds: default_verify_retry_cap(),
            objective_validation_reserve_seconds: default_objective_validation_reserve(),
            smart_first_validation_reserve_seconds: default_smart_first_validation_reserve(),
            final_result_test_reserve_seconds: default_final_result_test_reserve(),
            verify_retry_reserve_seconds: default_verify_retry_reserve(),
        }
    }
}

impl PhaseBudgetPolicy {
    /// Each field as a [`Duration`]. The loop works in `Duration`; the catalog is
    /// written in seconds. Converting here keeps that boundary in one place
    /// instead of at every budget call site.
    #[must_use]
    pub const fn plan_cap(&self) -> Duration {
        Duration::from_secs(self.plan_cap_seconds)
    }
    #[must_use]
    pub const fn exec_cap(&self) -> Duration {
        Duration::from_secs(self.exec_cap_seconds)
    }
    #[must_use]
    pub const fn smart_first_exec_cap(&self) -> Duration {
        Duration::from_secs(self.smart_first_exec_cap_seconds)
    }
    #[must_use]
    pub const fn verify_cap(&self) -> Duration {
        Duration::from_secs(self.verify_cap_seconds)
    }
    #[must_use]
    pub const fn verify_retry_cap(&self) -> Duration {
        Duration::from_secs(self.verify_retry_cap_seconds)
    }
    #[must_use]
    pub const fn objective_validation_reserve(&self) -> Duration {
        Duration::from_secs(self.objective_validation_reserve_seconds)
    }
    #[must_use]
    pub const fn smart_first_validation_reserve(&self) -> Duration {
        Duration::from_secs(self.smart_first_validation_reserve_seconds)
    }
    #[must_use]
    pub const fn final_result_test_reserve(&self) -> Duration {
        Duration::from_secs(self.final_result_test_reserve_seconds)
    }
    #[must_use]
    pub const fn verify_retry_reserve(&self) -> Duration {
        Duration::from_secs(self.verify_retry_reserve_seconds)
    }

    /// The smallest retry budget worth spawning a turn for: a third of the cap, and
    /// never more than [`LEGACY_VERIFY_RETRY_MIN_SPAWN_SECONDS`].
    ///
    /// Both bounds exist to stop a cap from disabling the retry it configures, and
    /// they guard opposite directions. Scaling with the cap keeps a small cap
    /// usable: a fixed five-second floor made `verify_retry_cap_seconds = 5` a
    /// permanent no-op while the catalog still advertised a cap. Clamping at the
    /// legacy five keeps a large cap from *raising* the bar: at `cap = 60` an
    /// unclamped third would demand twenty seconds and skip a nineteen-second
    /// budget this loop accepted for as long as the floor was hard-coded.
    ///
    /// At the default fifteen both bounds agree on five, so the shipped catalog's
    /// behaviour is the behaviour it always had.
    #[must_use]
    pub const fn verify_retry_min_spawn(&self) -> Duration {
        let scaled = self.verify_retry_cap_seconds / 3;
        // `Ord::min` is not a `const fn`, so the clamp stays an if-expression.
        let seconds = if scaled < LEGACY_VERIFY_RETRY_MIN_SPAWN_SECONDS {
            scaled
        } else {
            LEGACY_VERIFY_RETRY_MIN_SPAWN_SECONDS
        };
