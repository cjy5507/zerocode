...
pub struct FairnessInput {
    pub runner: String,
    /// What KIND of runner this was (`zo`, `claude`, `codex`), as distinct from
    /// the arm label in `runner`.
    ///
    /// The two are separate because an experiment needs them to be: an ablation
    /// pair is two arms of the same runner, so their names MUST differ (they
    /// own different output directories and different `<NAME>_ABLATE` values)
    /// while the thing under test must be identical. Holding the *name*
    /// constant would reject every pair the suite can actually produce.
    ...
    /// Harness features held out of this run, as `telemetry::HarnessFeature`
    /// keys. Empty for an ordinary full-feature run. Recorded rather than
    /// judged: a holdout does not make a run less valid, but a comparison that
    /// does not know about it is not controlled.
...
pub struct FairnessContract {
    ...
    pub runner: String,
    /// See [`FairnessInput::runner_kind`]. Omitted when the producer did not
    /// declare one, so a contract written by a caller that predates the field
    /// stays byte-identical — and an ablation pair whose arms both omit it is
    /// refused rather than certified on two blanks.
    ...
    pub finished_at: String,
    /// Harness features held out of this run, as `telemetry::HarnessFeature`
    /// keys, sorted. Omitted entirely when empty, so an ordinary full-feature
    /// run's contract stays byte-identical to one written before ablation
    /// existed and only an experiment arm carries the field.
...
pub fn build_contract(input: &FairnessInput) -> FairnessContract {
    ...
    // Sorted and deduped so two arms declaring the same holdout in a different
    // order compare equal — the pair comparison rejects on inequality, and an
    // ordering artifact must never read as a different experiment.
    let mut ablation = input.ablation.clone();
    ablation.sort();
    ablation.dedup();
    ...
    FairnessContract {
        ...
        runner_kind: input.runner_kind.clone(),
