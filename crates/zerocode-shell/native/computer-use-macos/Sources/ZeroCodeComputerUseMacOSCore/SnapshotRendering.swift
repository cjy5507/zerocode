...
public enum SnapshotRenderHeuristics {
    ...
    public static let secureTextProbeRoleWords = ["text", "field", "password", "search", "combo"]
    public static let secureTextWords = ["secure", "password", "passcode", "verification code", "one-time code"]

    public static func shouldProbeSecureTextMetadata(role: String) -> Bool {
        let normalized = role.lowercased()
        return secureTextProbeRoleWords.contains { normalized.contains($0) }
    }

    /// Whether a text field's role, subrole, title, label or placeholder says
    /// it holds a secret: its value is redacted, and keys into it are watched.
    public static func looksLikeSecureText(_ haystack: String) -> Bool {
        let normalized = haystack.lowercased()
        return secureTextWords.contains { normalized.contains($0) }
    }
