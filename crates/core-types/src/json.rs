...
impl JsonValue {
    ...
    fn render_into(&self, rendered: &mut String) {
        match self {
            ...
            Self::Bool(value) => rendered.push_str(if *value { "true" } else { "false" }),
            Self::Number(value) => rendered.push_str(&value.to_string()),
            Self::String(value) => render_string_into(rendered, value),
            Self::Array(values) => {
                rendered.push('[');
...
fn render_string_into(rendered: &mut String, value: &str) {
    rendered.push('"');
    for ch in value.chars() {
        match ch {
            ...
            control if control.is_control() => push_unicode_escape(rendered, control),
...
fn render_string(value: &str) -> String {
    ...
    render_string_into(&mut rendered, value);
    rendered
}
...
mod tests {
    ...
    #[test]
    fn renders_nested_values_without_extra_separators() {
        ...
        assert_eq!(value.render(), r#"[1,{"message":"ok"}]"#);
    }
