    ...
    if message.role == "assistant" {
            ...
            match block {
                ...
                InputContentBlock::ToolUse { .. } => {
                    ...
                    if let Some(items) = reasoning_replay_for_call(message, id, session_id) {
                        if !telemetry::attest_ablated(
                            telemetry::HarnessFeature::ReasoningReplayCall,
                        ) {
        ...
        if !text.is_empty() {
            ...
            if let Some(items) = turn_final_reasoning_replay(message) {
                if !telemetry::attest_ablated(
                    telemetry::HarnessFeature::ReasoningReplayTurnFinal,
                ) {
    ...
    for block in &message.content {
        match block {
            ...
            InputContentBlock::ToolResult { .. } => {
                ...
                // Re-attach them as a following user item, which is the only
                // shape the Responses input accepts images in.
