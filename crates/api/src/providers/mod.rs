...
pub fn max_request_bytes_for_model(model: &str) -> Option<u64> {
    ...
    if let Ok(raw) = std::env::var(MODEL_CONTEXT_WINDOWS_ENV) {
        if !raw.trim().is_empty() {
            if let Ok(catalog) = serde_json::from_str::<ModelContextCatalog>(&raw) {
                if let Some(bytes) = catalog.max_request_bytes_for(model, &canonical_lower) {
                    return Some(bytes);
                }
            }
        }
    }
    builtin_model_context_catalog().max_request_bytes_for(model, &canonical_lower)
}
...
const OPENAI_CURRENT_MODEL_FAMILIES: &[&str] =
...
/// Base/priority pair for an OpenAI model family with a verified fast tier.
///
/// Release-specific service-tier spellings live beside the capability table so
/// CLI and router callers never copy them.
#[must_use]
pub fn openai_fast_variant_pair(model: &str) -> Option<(String, String)> {
    let family = openai_gpt_model_family(model)?;
    if family == "gpt-5.5" {
...
#[must_use]
pub fn is_openai_current_family_model(model: &str) -> bool {
    openai_gpt_model_family(model)
        .is_some_and(|family| OPENAI_CURRENT_MODEL_FAMILIES.contains(&family))
}
...
#[must_use]
pub fn is_openai_terra_model(model: &str) -> bool {
    openai_gpt_model_family(model).is_some_and(|family| family == "gpt-5.6-terra")
}

/// Whether `model` names the registered spark release, by alias or by id.
///
/// Deliberately not `resolve_model_alias`: that resolver is gated on the OpenAI
/// provider being configured, so a setup without OpenAI credentials read a real
/// spark id as an unrelated model and re-armed the non-stream fallback spark
/// cannot serve. Model identity is a capability fact, not a routing one.
#[must_use]
pub fn is_openai_spark_model(model: &str) -> bool {
    ...
    openai_gpt_model_family(canonical).is_some_and(|family| family == "gpt-5.3-codex-spark")
}
...
fn anthropic_family(model: &str) -> Option<&'static str> {
    let lower = model.trim().to_ascii_lowercase();
    let family = |token: &str| match token {
        "fable" => Some("fable"),
        "opus" => Some("opus"),
        "sonnet" => Some("sonnet"),
        "haiku" => Some("haiku"),
    ...
    if let Some(claude_tokens) = lower.strip_prefix("claude-") {
        claude_tokens.split(['-', '[', '@']).find_map(family)
    } else {
        lower.split(['-', '[', '@']).next().and_then(family)
    }
}

/// Canonical id of the current catalog head for `model`'s Anthropic family.
///
...
fn latest_google_family_model(model: &str) -> Option<&'static str> {
    let lower = model.trim().to_ascii_lowercase();
    if !lower.starts_with("gemini-") {
        return None;
    }
    let alias = if lower.contains("flash-lite") {
        "gemini-flash-lite"
...
/// Catalog-owned one-step fallback for a provider model starved by rate limits.
///
/// Exact release ids live only in this capability seam. Callers receive the
/// current registered target and never infer a release from naming convention
/// or stale model knowledge.
#[must_use]
pub fn starvation_demotion_model(model: &str) -> Option<&'static str> {
    ...
    if let Some(family) = openai_gpt_model_family(&lower) {
        if family == "gpt-5.5" {
            return Some(if openai_fast_tier_enabled(&lower) {
                "gpt-5.6-terra[fast]"
            } else {
                "gpt-5.6-terra"
            });
        }
        if OPENAI_CURRENT_MODEL_FAMILIES.contains(&family) {
            let current_rank = effort_rank(max_supported_effort(family));
            return OPENAI_CURRENT_MODEL_FAMILIES
                .iter()
                .copied()
                .filter(|candidate| *candidate != family)
                .filter(|candidate| effort_rank(max_supported_effort(candidate)) < current_rank)
                .max_by_key(|candidate| effort_rank(max_supported_effort(candidate)));
        }
    }
...
pub fn resolve_registered_model_alias(model: &str) -> String {
    ...
    if let Some(head) = latest_anthropic_family_model(&resolved) {
...
pub fn effective_effort_for_model(
    ...
    if is_openai {
        ...
        if level == EffortLevel::Ultra && !gpt_model_accepts_ultra(&lower) {
            // Fall to the model's *own* ceiling, not a fixed `Xhigh`: on a
            // max-capable family (Luna) that is `Max`, and pinning `Xhigh` here
            // made this projection disagree with `gpt_for_model`, which sends
            // `max` for the same input.
            return if gpt_model_accepts_max(&lower) {
                EffortLevel::Max
            } else {
                EffortLevel::Xhigh
            };
...
pub fn known_effort_ceiling(model: &str) -> Option<crate::types::EffortLevel> {
    ...
    if is_openai {
        ...
        if gpt_model_accepts_xhigh(&lower) {
            return Some(EffortLevel::Xhigh);
        }
        return Some(EffortLevel::High);
    }
...
mod tests {
    use super::{
        ...
        non_claude_adapters_enabled, openai_fast_tier_enabled, openai_gpt_model_family,
    ...
    fn starvation_demotion_uses_current_catalog_targets() {
        assert_eq!(starvation_demotion_model("opus"), Some("sonnet"));
        assert_eq!(
            starvation_demotion_model("gpt-5.5-fast"),
            Some("gpt-5.6-terra[fast]")
        );
        assert_eq!(
            starvation_demotion_model("gpt-5.6-sol"),
            Some("gpt-5.6-luna")
        );
        assert_eq!(starvation_demotion_model("gpt-5.6-luna"), None);
        assert_eq!(
            starvation_demotion_model("gemini-pro"),
