...
impl EffortLevel {
    ...
    /// `xhigh`, which is why the projection asks
    /// [`gpt_model_accepts_max`] rather than sending `max` blindly — that would
    /// 400 on GPT-5.5.
    ///
    ...
    pub fn gpt_for_model(self, model: &str) -> &'static str {
        match self {
            ...
            Self::Max | Self::Ultra if gpt_model_accepts_max(model) => "max",
    ...
    pub fn anthropic_for_model(self, model: &str) -> EffortLevel {
        match self {
            EffortLevel::Ultra if anthropic_model_accepts_max(model) => EffortLevel::Max,
...
///
/// Per Anthropic's effort docs, `xhigh` is available on Fable 5, Mythos 5,
/// Opus 5, Opus 4.8, Opus 4.7 — **and Sonnet 5**. Sonnet 4.6 and earlier reject
/// it (`This model does not support effort level 'xhigh'`, supported:
/// `low|medium|high|max`), and Haiku has no effort scale at all. So the split is
/// per *generation* within Sonnet, not per family: the blanket
/// `!contains("sonnet")` this replaced silently demoted every Sonnet 5 request
/// to `high`.
///
...
pub fn anthropic_model_accepts_xhigh(model: &str) -> bool {
    ...
    if lower.contains("sonnet") {
        return SONNET_XHIGH_FAMILIES
            .iter()
            .any(|family| model_id_matches_family(&lower, family));
    }
    true
}

/// Sonnet ids that reached the `xhigh` tier. Bare `sonnet` is the catalog alias
/// for the current frontier Sonnet, which is why it is listed alongside the
/// explicit id.
const SONNET_XHIGH_FAMILIES: &[&str] = &["sonnet", "claude-sonnet-5"];

/// Whether an adaptive Anthropic `model` id accepts the `max` reasoning-effort
/// tier — every effort-capable model except Haiku, which exposes no effort
/// scale. Anthropic has no tier above `max`, so this is also where a
/// provider-neutral [`EffortLevel::Ultra`] lands.
#[must_use]
pub fn anthropic_model_accepts_max(model: &str) -> bool {
...
mod tests {
    ...
    fn anthropic_for_model_clamps_xhigh_below_sonnet_5_and_on_haiku() {
        ...
        assert!(!anthropic_model_accepts_xhigh("claude-sonnet-4-6"));
        assert!(!anthropic_model_accepts_xhigh("haiku"));
        // `max` reaches everything with an effort scale; Haiku has none.
        assert!(crate::types::anthropic_model_accepts_max("claude-sonnet-4-6"));
        assert!(!crate::types::anthropic_model_accepts_max("claude-haiku-4-5-20251001"));
    }
