...
async fn send_message_posts_json_and_parses_response() {
    ...
    assert!(
        request.path.starts_with("/v1/messages"),
        "path: {}",
        request.path
    );
    assert_eq!(
    ...
    let beta_header = request.headers.get("anthropic-beta").map(String::as_str);
    assert!(
        beta_header.is_some(),
        "anthropic-beta header should be present"
    );
    assert!(
        beta_header.unwrap().contains("claude-code-20250219"),
        "should contain agentic beta"
    );
    ...
    // betas are sent via anthropic-beta header, not in request body
...
async fn send_message_applies_request_profile_and_records_telemetry() {
    ...
    let client = AnthropicClient::new("test-key")
        ...
        .with_extra_body_param("metadata", json!({"source": "zo"}))
    ...
    let captured = state.lock().await;
    let request = captured.first().expect("server should capture request");
    let beta = request.headers.get("anthropic-beta").expect("beta header");
    assert!(
        beta.contains("tools-2026-04-01"),
        "should contain custom beta: {beta}"
    );
    assert!(
        beta.contains("claude-code-20250219"),
        "should contain agentic beta: {beta}"
    );
    let ua = request
        .headers
        .get("user-agent")
        .expect("user-agent header");
    assert!(ua.contains("9.9.9"), "should contain version: {ua}");
    let body: serde_json::Value =
        serde_json::from_str(&request.body).expect("request body should be json");
    assert_eq!(body["metadata"]["source"], json!("zo"));
    // betas are sent via anthropic-beta header, not in request body
    assert!(body.get("betas").is_none());
...
async fn stream_message_parses_sse_events_with_tool_use() {
    ...
    // The prompt cache must write under ZO_CONFIG_HOME, not the real ~/.zo:
    // if production ignored the env var the cache would leak into the developer's
    // home and `temp_root` would never be created, panicking the cleanup below.
    assert!(
        temp_root.is_dir(),
        "prompt cache must materialize under ZO_CONFIG_HOME, got missing {}",
        temp_root.display()
    );
...
async fn send_message_reuses_recent_completion_cache_entries() {
    ...
    let cache = PromptCache::new("integration-session");
    let completion_dir = cache.paths().completion_dir;
    let client = AnthropicClient::new("test-key")
        .with_base_url(server.base_url())
    ...
    // The completion entry must be written under ZO_CONFIG_HOME (= temp_root),
    // proving the cache honors the env var instead of leaking into the real
    // ~/.zo and that temp_root exists for the cleanup below.
    assert!(
        completion_dir.starts_with(&temp_root),
        "cache completion dir must live under ZO_CONFIG_HOME, got {}",
        completion_dir.display()
    );
    assert!(
        completion_dir.is_dir(),
        "prompt cache must materialize the completion dir under ZO_CONFIG_HOME, got missing {}",
        completion_dir.display()
    );
...
async fn send_message_tracks_unexpected_prompt_cache_breaks() {
    ...
    let stats_path = cache.paths().stats_path;
    let client = AnthropicClient::new("test-key")
        .with_base_url(server.base_url())
        .with_prompt_cache(cache);
    ...
    // The persisted stats file must live under ZO_CONFIG_HOME (= temp_root),
    // proving the cache honors the env var rather than polluting the real
    // ~/.zo and that temp_root exists for the cleanup below.
    assert!(
        stats_path.starts_with(&temp_root),
        "cache stats file must live under ZO_CONFIG_HOME, got {}",
        stats_path.display()
    );
    assert!(
        stats_path.is_file(),
        "prompt cache must persist stats under ZO_CONFIG_HOME, got missing {}",
        stats_path.display()
    );
...
// Single-threaded test; the env lock across `.await` only serialises env access.
#[allow(clippy::await_holding_lock)]
async fn stream_message_from_env_uses_local_base_url_smoke() {
    let _guard = env_lock();
    let state = Arc::new(Mutex::new(Vec::<CapturedRequest>::new()));
    let sse = concat!(
        "event: message_start\n",
        "data: {\"type\":\"message_start\",\"message\":{\"id\":\"msg_env_stream\",\"type\":\"message\",\"role\":\"assistant\",\"content\":[],\"model\":\"claude-3-7-sonnet-latest\",\"stop_reason\":null,\"stop_sequence\":null,\"usage\":{\"input_tokens\":3,\"output_tokens\":0}}}\n\n",
        "event: content_block_start\n",
        "data: {\"type\":\"content_block_start\",\"index\":0,\"content_block\":{\"type\":\"text\",\"text\":\"hello from rust\"}}\n\n",
        "event: content_block_stop\n",
        "data: {\"type\":\"content_block_stop\",\"index\":0}\n\n",
        "event: message_delta\n",
    ...
    let _base_url = EnvVarGuard::set("ANTHROPIC_BASE_URL", Some(&server.base_url()));

    let client = ApiClient::from_env().expect("env-backed client should construct");
    let mut stream = client
        ...
        .await
        .expect("env-backed local stream should start");

    assert_eq!(stream.request_id(), Some("req_env_stream_789"));

    let mut events = Vec::new();
    while let Some(event) = stream
        .next_event()
        .await
        .expect("stream should yield deterministic local events")
    {
        events.push(event);
    }
