...
#[test]
fn provider_client_rejects_non_claude_models_when_adapter_gate_is_off() {
    let _lock = env_lock();
    let _experimental_gate = EnvVarGuard::set(EXPERIMENTAL_PROVIDERS_ENV, None);
    let _provider_gate = EnvVarGuard::set(NON_CLAUDE_ADAPTERS_ENV, None);

    let error = ProviderClient::from_model("grok-3")
        .expect_err("grok should be blocked without the adapter gate");

    match error {
        ApiError::UnsupportedProvider { provider, gate_env } => {
            assert_eq!(provider, "xAI");
            assert_eq!(gate_env, NON_CLAUDE_ADAPTERS_ENV);
        }
