...
# 실배포 위치. deploy 는 여기에 설치한 뒤 PATH에서 먼저 잡히는 다른 zo 사본
# (stale shadow, 예: /opt/homebrew/bin/zo)도 같은 빌드로 갱신한다.
# 재정의: `ZO_DEPLOY_DIR=/some/bin just deploy`
deploy_dir := env_var_or_default('ZO_DEPLOY_DIR', env_var('HOME') / '.local' / 'bin')
...
deploy: build
    ...
    # PATH에서 deploy_dir 보다 먼저 잡히는 옛 zo 사본(stale shadow)을 같은
    # 빌드로 갱신한다. 격리 테스트는 ZO_DEPLOY_NO_SHADOW_REFRESH=1 로 끈다.
    if [ -z "${ZO_DEPLOY_NO_SHADOW_REFRESH:-}" ]; then
        while IFS= read -r other; do
            if [ -n "$other" ] && [ -e "$other" ] && ! [ "$other" -ef "{{deploy_dir}}/zo" ]; then
                install -m 755 target/release/zo "$other"
                echo "  ↺ stale shadow 갱신: $other"
            fi
