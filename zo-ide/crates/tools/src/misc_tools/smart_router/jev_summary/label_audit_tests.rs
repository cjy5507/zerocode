...
use zerocode_core::jev::{
    Band, JevUse, AGENT_TOOL, COMPACTION, JEV_USES, MENTION_RERANK, NOTIFY, PATCH_REVIEW, PLACEMENT,
    PLACEMENT_SEEN_DWELL_MS, RECALL, ROUTING, SKILLS, STALL, SUMMON, ZO_STEP_EFFORT,
};
...
/// a ledger of its own.
const ZO_WRITES: [&JevUse; 8] =
    [&ROUTING, &RECALL, &SKILLS, &ZO_STEP_EFFORT, &COMPACTION, &AGENT_TOOL, &MENTION_RERANK, &PATCH_REVIEW];
