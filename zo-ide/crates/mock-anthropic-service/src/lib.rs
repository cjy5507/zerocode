...
impl Scenario {
    fn parse(value: &str) -> Option<Self> {
        match value.trim() {
            ...
            "goal_rate_limit_resume" => Some(Self::GoalRateLimitResume),
            "goal_gate_exact" => Some(Self::GoalGateExact),
            _ => None,
        }
    }
    ...
    fn name(self) -> &'static str {
        match self {
            ...
            Self::GoalRateLimitResume => "goal_rate_limit_resume",
            Self::GoalGateExact => "goal_gate_exact",
        }
    }
}

/// What an obedient model does in a `/goal` turn (`goal_gate_exact`): in a
/// gate turn it runs the exact command the controller asked for, once, and
/// reports; any other turn — the action, a repair — it answers in words and
/// edits nothing. The gate's verdict is then the command's alone.
enum GoalStep {
    Run(String),
    Say(&'static str),
}

fn goal_step(request: &MessageRequest) -> GoalStep {
    let mut gate: Option<String> = None;
    let mut ran_since = false;
    for message in &request.messages {
        for block in &message.content {
            match block {
                InputContentBlock::Text { text, .. } if text.contains("[zo:goal-gate]") => {
                    // The gate prompt's third paragraph is the command.
                    gate = text.split("\n\n").nth(2).map(|command| command.trim().to_string());
                    ran_since = false;
                }
                InputContentBlock::Text { text, .. } if text.contains("[zo:goal-") => {
                    gate = None;
                }
                InputContentBlock::ToolResult { .. } if gate.is_some() => ran_since = true,
                _ => {}
            }
        }
    }
    match gate {
        Some(command) if !ran_since => GoalStep::Run(command),
        Some(_) => GoalStep::Say("gate ran once, as asked"),
        None => GoalStep::Say("done what the turn asked; nothing edited"),
    }
}
...
async fn handle_connection(
    ...
    let scenario_attempt = captured
        .iter()
        .filter(|request| request.scenario == scenario.name())
        .count()
        .saturating_add(1);
    ...
    let response = if scenario == Scenario::GoalRateLimitResume && scenario_attempt <= 6 {
        http_response(
            "429 Too Many Requests",
            "application/json",
            r#"{"error":{"type":"rate_limit_error","message":"scheduled reset"},"type":"error"}"#,
            &[("retry-after", "0")],
        )
    } else {
...
#[allow(clippy::too_many_lines)] // flat scenario→SSE stream body mapping (mock)
fn build_stream_body(request: &MessageRequest, scenario: Scenario) -> String {
    match scenario {
        ...
        Scenario::GoalGateExact => match goal_step(request) {
            GoalStep::Run(command) => tool_use_sse(
                "toolu_goal_gate",
                "bash",
                &[&json!({ "command": command }).to_string()],
            ),
            GoalStep::Say(text) => final_text_sse(text),
        },
        Scenario::GoalRateLimitResume => {
            let serialized = serde_json::to_string(request).unwrap_or_default();
            if serialized.contains("[zo:goal-condition]") {
                match latest_tool_result(request) {
                    Some(_) => final_text_sse("goal condition observed"),
                    None => tool_use_sse(
                        "toolu_goal_condition",
                        "bash",
                        &[r#"{"command":"true"}"#],
                    ),
                }
            } else {
                final_text_sse("provider reset elapsed; action resumed")
            }
        }
    }
}

#[allow(clippy::too_many_lines)] // flat scenario→message response mapping (mock)
fn build_message_response(request: &MessageRequest, scenario: Scenario) -> MessageResponse {
    match scenario {
        ...
        Scenario::GoalRateLimitResume => text_message_response(
            "msg_goal_rate_limit_resume",
            "provider reset elapsed",
        ),
        Scenario::GoalGateExact => match goal_step(request) {
            GoalStep::Run(command) => tool_message_response(
                "msg_goal_gate_tool",
                "toolu_goal_gate",
                "bash",
                json!({ "command": command }),
            ),
            GoalStep::Say(text) => text_message_response("msg_goal_gate_text", text),
        },
    }
}
...
fn request_id_for(scenario: Scenario) -> &'static str {
    match scenario {
        ...
        Scenario::GoalRateLimitResume => "req_goal_rate_limit_resume",
        Scenario::GoalGateExact => "req_goal_gate_exact",
    }
}
