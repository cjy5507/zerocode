...
impl RuntimeLspState {
    pub(crate) fn new(
        ...
        let detect = std::time::Instant::now();
        let configs = lsp_startup_configs(cwd, runtime_config);
        crate::runtime_support::log_boot_stage(
            &format!("lsp/detect({} servers)", configs.len()),
            detect,
        );
        Self::new_with_startup_configs(cwd, configs)
    }
...
fn rust_analyzer_candidate_usable(path: &Path) -> bool {
    ...
    let stamp = probe_stamp(path);
    if let Some(cached) = read_probe_cache(stamp.as_deref()) {
    ...
    write_probe_cache(stamp.as_deref(), usable);
    RUST_ANALYZER_USABILITY_CACHE
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner)
        .insert(key, usable);
...
/// `<path>\0<mtime-nanos>\0<len>` — what makes a cached probe answer still
/// true. A reinstalled or `rustup component add`-ed binary changes at least one
/// of the three, so the answer is re-earned rather than inherited.
///
/// `None` when the file cannot be stat'ed; the caller then probes and does not
/// cache, because there is nothing to invalidate against.
fn probe_stamp(path: &Path) -> Option<String> {
...
/// Where the cross-process answer lives. The in-process `Mutex<HashMap>` above
/// only ever helped a single `zo` run, and the answer it caches costs a
/// subprocess spawn: on a machine whose `rust-analyzer` is rustup's proxy with
/// no component installed, `--version` takes **190ms to fail**, and every zo
/// start paid it again to re-learn the same "no".
fn probe_cache_path() -> Option<std::path::PathBuf> {
    let home = runtime::zo_global_config_roots().into_iter().next()?;
    Some(home.join("cache").join("lsp-probe.txt"))
}

fn read_probe_cache(stamp: Option<&str>) -> Option<bool> {
    read_probe_cache_at(&probe_cache_path()?, stamp?)
}

fn read_probe_cache_at(cache: &Path, stamp: &str) -> Option<bool> {
    let body = std::fs::read_to_string(cache).ok()?;
    body.lines().find_map(|line| {
        let (verdict, recorded) = line.split_once(' ')?;
        (recorded == stamp).then(|| verdict == "usable")
    })
}

/// Best-effort, and single-line: the file holds one entry, the last binary
/// probed. A machine switches `rust-analyzer` rarely, and a cache that grows
/// per path would need pruning it does not deserve.
fn write_probe_cache(stamp: Option<&str>, usable: bool) {
    let (Some(stamp), Some(cache)) = (stamp, probe_cache_path()) else {
        return;
    };
    write_probe_cache_at(&cache, stamp, usable);
}

fn write_probe_cache_at(cache: &Path, stamp: &str, usable: bool) {
    if let Some(parent) = cache.parent() {
        let _ = std::fs::create_dir_all(parent);
    }
    let verdict = if usable { "usable" } else { "unusable" };
    let _ = std::fs::write(cache, format!("{verdict} {stamp}\n"));
}
...
mod tests {
    ...
    use super::{
        ...
        detect_project_languages, probe_stamp, read_probe_cache_at, write_probe_cache_at,
        LspStartupConfig, RuntimeLspState,
    };

    /// The probe answer survives the process that earned it.
    ///
    /// `rust-analyzer` on PATH is often rustup's proxy with no component
    /// installed, and `--version` takes **190ms to fail**. The verdict was
    /// cached in a `Mutex<HashMap>`, which only ever helped a single `zo` run —
    /// so every start spawned that subprocess again to re-learn the same "no".
    /// Measured after this: `lsp/detect` 195ms → 0.8ms from the second start on.
    #[test]
    fn a_probe_verdict_outlives_the_process_that_earned_it() {
        let root = crate::support::temp_dir("lsp-probe-cache");
        let cache = root.join("cache").join("lsp-probe.txt");

        write_probe_cache_at(&cache, "stamp-a", false);
        assert_eq!(read_probe_cache_at(&cache, "stamp-a"), Some(false));
        write_probe_cache_at(&cache, "stamp-a", true);
        assert_eq!(read_probe_cache_at(&cache, "stamp-a"), Some(true));
    }

    /// A different binary is a different answer.
    ///
    /// The whole point of the stamp: `rustup component add rust-analyzer`
    /// replaces the file, and the cached "unusable" must not outlive it — the
    /// user would install the server and zo would keep ignoring it.
    #[test]
    fn a_changed_binary_does_not_inherit_the_old_verdict() {
        let root = crate::support::temp_dir("lsp-probe-stamp");
        let cache = root.join("cache").join("lsp-probe.txt");
        write_probe_cache_at(&cache, "stamp-before", false);
        assert_eq!(
            read_probe_cache_at(&cache, "stamp-after"),
            None,
            "a stamp that does not match must miss, not fall through to the old verdict"
        );
    }

    /// The stamp is built from what actually changes when a binary is replaced,
    /// and is `None` for a path that cannot be stat'ed (nothing to invalidate
    /// against, so the caller probes and does not cache).
    #[test]
    fn the_stamp_tracks_the_file_not_just_its_name() {
        let root = crate::support::temp_dir("lsp-probe-file");
        let binary = root.join("rust-analyzer");
        fs::write(&binary, b"one").expect("write");
        let first = probe_stamp(&binary).expect("a real file stamps");
        assert!(first.contains(&binary.display().to_string()), "{first}");

        fs::write(&binary, b"one-longer").expect("rewrite");
        let second = probe_stamp(&binary).expect("still stamps");
        assert_ne!(first, second, "a changed file must change its stamp");

        assert_eq!(probe_stamp(&root.join("absent")), None);
    }
    ...
    fn mark_test_script_executable(path: &std::path::Path) {
        ...
        #[cfg(windows)]
        {
            // Windows has no executable permission bits. Retain a concrete
            // regular-file assertion rather than silently compiling setup out.
            assert!(fs::metadata(path).expect("metadata").is_file());
        }
    }

    fn write_init_only_lsp_server() -> PathBuf {
        let root = crate::support::temp_dir("server");
    ...
    async fn build_runtime_lsp_state_does_not_panic_inside_ambient_runtime() {
        ...
        let root = crate::support::temp_dir("ambient");
    ...
    async fn lsp_state_shutdown_inside_ambient_runtime_does_not_panic() {
        ...
        let root = crate::support::temp_dir("ambient-shutdown");
        let cwd = root.join("project");
        let home = root.join("home").join(".zo");
        fs::create_dir_all(cwd.join(".zo")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
    ...
    fn project_language_detection_skips_root_and_nested_zo_operational_state() {
        let root = crate::support::temp_dir("private-zo-state");
    ...
    fn autodetect_skips_unusable_rust_analyzer_proxy() {
        let root = crate::support::temp_dir("broken-proxy");
...
")
        ...
        mark_test_script_executable(&server);
    ...
    fn autodetected_lsp_timeout_registers_error_server() {
        let root = crate::support::temp_dir("auto-timeout");
        let cwd = root.join("project");
        let bin = root.join("bin");
...
").expect("write fake hanging server");
    ...
    fn detects_language_server_from_supplied_path() {
        let root = crate::support::temp_dir("path");
        let bin = root.join("bin");
        fs::create_dir_all(&bin).expect("bin dir");
