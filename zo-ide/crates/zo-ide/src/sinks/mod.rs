...
pub(crate) mod serializable {
    use runtime::message_stream::{
        AgentResultStatus, BlockId, NotificationRoad, PermissionChoice, PermissionDecision,
        PermissionPrompt, ProjectedRenderBlock, RenderBlock, SystemLevel, ToolCallId,
        ToolCallStatus, ToolPreview, ToolResultBody, project_render_block,
    };
    ...
    pub enum SerializableRenderBlock {
        ...
        System {
            /// Stable block id.
            id: u64,
            /// Severity tag (`info`, `success`, `warn`, `error`,
            /// `housekeeping`, or `user` for a folded user message).
            level: String,
            /// Display text.
            text: String,
        },
        /// A `PushNotification` the surface delivered — the frame the window
        /// turns into an OS notification (t-2943).
        Notify {
            /// Stable block id.
            id: u64,
            /// Which zo is calling.
            ...
            /// The one-line body.
            body: String,
            /// Severity tag (`info` today).
            level: String,
            /// The road word: `window` or `terminal` (a skip is never sent).
            road: String,
        },
        ...
        AgentResult {
            ...
            status: String,
            /// What the run cost (`12 tool uses · 45.3k tokens · 1m 20s`).
            /// Absent on a frame from a peer that predates the field, and on a
            /// run nothing was measured for.
        ...
        /// Where the model request stands: `request_sent` (nothing back yet)
        /// or `retrying` (a retryable failure, next attempt after `delay_secs`).
        StreamPhase {
            /// `request_sent` | `retrying`.
            phase: String,
            /// 1-based attempt number.
            attempt: u32,
            /// Seconds until the next attempt; only with `retrying`.
            delay_secs: Option<u64>,
        },
        /// Which model the request goes out on and why (`session`,
        /// `refusal_fallback`, `quota_fallback`, …): the footer's model field.
        WireModel {
            /// The model id on the wire.
            model: String,
            /// The reason tag (`WireModelSource::as_tag`).
            source: String,
        },
    }
    ...
    impl SerializableRenderBlock {
        ...
        pub fn from_block(block: &RenderBlock) -> Self {
            ...
            match block {
                ...
                RenderBlock::WireModel(wire) => Self::WireModel {
                    model: wire.model.clone(),
                    source: wire.source.as_tag().to_string(),
                },
                ...
                RenderBlock::StreamPhase(phase) => match phase {
                    runtime::message_stream::types::StreamPhase::RequestSent { attempt } => {
                        Self::StreamPhase {
                    ...
                    runtime::message_stream::types::StreamPhase::Retrying {
                        attempt,
                        delay_secs,
                    } => Self::StreamPhase {
                    ...
                    runtime::message_stream::types::StreamPhase::QuietReasoning { since_secs } => {
                        Self::StreamPhase {
                            phase: "quiet_reasoning".to_string(),
                            attempt: 0,
                            delay_secs: Some(*since_secs),
                        }
                    }
                },
            }
    ...
    const fn level_tag(level: SystemLevel) -> &'static str {
        match level {
            ...
            SystemLevel::Success => "success",
            SystemLevel::Warn => "warn",
            SystemLevel::Error => "error",
            SystemLevel::Housekeeping => "housekeeping",
        }
    }
    ...
    pub fn to_render_block(frame: &SerializableRenderBlock) -> RenderBlock {
        match frame {
            ...
            SerializableRenderBlock::Notify {
                id,
                title,
                body,
                level,
                ...
                body: body.clone(),
                level: level_from_tag(level),
                // A road word this build does not know is read as the
                // honest "nothing went out": a client must never ring for a
                // frame it cannot place.
                road: NotificationRoad::parse(road).unwrap_or(NotificationRoad::SkippedNowhere),
            },
            ...
            SerializableRenderBlock::StreamPhase {
                phase,
                attempt,
                delay_secs,
            } => RenderBlock::StreamPhase(match (phase.as_str(), delay_secs) {
                ("retrying", Some(delay_secs)) => {
                    runtime::message_stream::types::StreamPhase::Retrying {
                        attempt: *attempt,
                        delay_secs: *delay_secs,
                    }
                }
                ("quiet_reasoning", Some(since_secs)) => {
                    runtime::message_stream::types::StreamPhase::QuietReasoning {
                        since_secs: *since_secs,
                    }
                }
                _ => runtime::message_stream::types::StreamPhase::RequestSent { attempt: *attempt },
            }),
            SerializableRenderBlock::WireModel { model, source } => {
                RenderBlock::WireModel(runtime::message_stream::WireModel {
                    model: model.clone(),
                    // An unknown tag from a newer writer reads as the session
                    // model: the footer then shows the model without a reason
                    // rather than inventing one.
                    source: runtime::message_stream::WireModelSource::from_tag(source)
                        .unwrap_or(runtime::message_stream::WireModelSource::Session),
                })
            }
    ...
    fn level_from_tag(tag: &str) -> SystemLevel {
        match tag {
            ...
            "housekeeping" => SystemLevel::Housekeeping,
            // Unknown tags — including a level minted by a newer peer — read as
            // a plain notice instead of dropping the line.
            _ => SystemLevel::Info,
        }
    }
...
mod agent_result_wire_tests {
    ...
    use runtime::message_stream::{
        AgentResultStatus, BlockId, NotificationRoad, RenderBlock, SystemLevel,
    };

    /// A housekeeping notice keeps its level over the wire, so a remote
    /// client can keep it off its transcript the way the local one does;
    /// the older tags are untouched.
    #[test]
    fn a_housekeeping_notice_keeps_its_level_over_the_wire() {
        let block = RenderBlock::System {
            id: BlockId(7),
            level: SystemLevel::Housekeeping,
            text: "Context trim · cleared 3 old tool result(s) (~12k tokens freed)".to_string(),
        };
        let json = serde_json::to_value(SerializableRenderBlock::from_block(&block))
            .expect("serialize");
        assert_eq!(json["level"], "housekeeping");
        for (level, tag) in [
            (SystemLevel::Info, "info"),
            (SystemLevel::Success, "success"),
            (SystemLevel::Warn, "warn"),
            ...
            let older = RenderBlock::System {
                id: BlockId(8),
                level,
                text: "older".to_string(),
            };
            let older = serde_json::to_value(SerializableRenderBlock::from_block(&older))
                .expect("serialize");
            assert_eq!(older["level"], tag, "an older tag moved");
        }
        match render_block_from_value(&json).expect("deserialize") {
            RenderBlock::System { level, text, .. } => {
                assert_eq!(level, SystemLevel::Housekeeping);
                assert!(text.starts_with("Context trim"));
            }
            other => panic!("not a System block: {other:?}"),
        }
    }
    ...
    fn agent_result_round_trips_over_the_wire() {
        let block = RenderBlock::AgentResult {
            ...
            summary: Some("2 tool uses · 1.2k tokens · 8s".to_string()),
            body: "line one\nline two".to_string(),
        };
        ...
        match restored {
            RenderBlock::AgentResult {
                ...
                assert_eq!(summary.as_deref(), Some("2 tool uses · 1.2k tokens · 8s"));
    ...
    /// A delivered `PushNotification` crosses the wire as a `notify` frame and
    /// comes back as the same block — title, body, level and road — so a
    /// window (or a `zo attach` client) reads exactly what the surface judged.
    #[test]
    fn a_notification_round_trips_over_the_wire_as_a_notify_frame() {
        ...
        match render_block_from_value(&json).expect("deserialize") {
            RenderBlock::Notification {
                ...
                assert_eq!(body, "Build is green");
                assert_eq!(level, SystemLevel::Info);
                assert_eq!(road, NotificationRoad::Window);
            }
            other => panic!("a notify frame must come back as a Notification, got {other:?}"),
        }
        // A road this build cannot place reads as nothing sent.
        let mut unknown = json.clone();
        unknown["road"] = serde_json::Value::from("phone");
        assert!(matches!(
            render_block_from_value(&unknown).expect("deserialize"),
            RenderBlock::Notification {
                road: NotificationRoad::SkippedNowhere,
                ..
            }
        ));
    }

    /// A success notice must keep its level across the wire. The producer
    /// (`level_tag`) and the consumer (`level_from_tag`) have to stay in step:
    /// when only one of them knew about `success`, a `zo attach` client silently
    /// re-rendered the green check as a plain info notice.
    #[test]
    fn success_level_round_trips_over_the_wire() {
        let block = RenderBlock::System {
            id: BlockId(11),
            level: SystemLevel::Success,
            text: "claude OAuth login successful!".to_string(),
        };
        let frame = SerializableRenderBlock::from_block(&block);
        match &frame {
            SerializableRenderBlock::System { level, .. } => assert_eq!(level, "success"),
            other => panic!("a System block must serialize as a System frame, got {other:?}"),
        }
        let json = serde_json::to_value(&frame).expect("serialize");
        match render_block_from_value(&json).expect("deserialize") {
            RenderBlock::System { id, level, text } => {
                assert_eq!(id, BlockId(11));
                assert_eq!(
                    level,
                    SystemLevel::Success,
                    "the wire must not downgrade Success to Info"
                );
