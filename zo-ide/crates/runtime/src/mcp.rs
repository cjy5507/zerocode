...
pub fn mcp_server_signature(config: &McpServerConfig) -> Option<String> {
    match config {
        McpServerConfig::Stdio(config) => Some(format!(
            "stdio:{}",
            render_command_signature(
...
pub fn scoped_mcp_config_hash(config: &ScopedMcpServerConfig) -> String {
    let rendered = match &config.config {
        McpServerConfig::Stdio(stdio) => format!(
            "stdio|{}|{}|{}|{}",
            stdio.command,
            render_command_signature(stdio.args.iter().map(String::as_str)),
...
fn render_command_signature<'a>(command: impl IntoIterator<Item = &'a str>) -> String {
...
mod tests {
    ...
    use super::{
        mcp_server_signature, mcp_tool_name, normalize_name_for_mcp, render_command_signature,
    ...
    #[test]
    fn command_signature_escapes_separator_characters_without_temp_join_buffers() {
        assert_eq!(
            render_command_signature(["uvx", "a|b\\c"].iter().copied()),
            "[uvx|a\\|b\\\\c]"
        );
    }
}
